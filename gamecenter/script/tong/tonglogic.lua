...
function Tong:CreateTong(anKinId, szTongName, nCamp, nCreateTime)
  ...
  --设置普通称号
  cTong.SetTitleMan("男帮众")
  cTong.SetTitleWoman("女帮众")
  cTong.SetTitleRetire("荣誉帮众")
  cTong.SetTitleExcellent("精英")
  --长老称号
  cTong.SetCaptainTitle(self.CAPTAIN_NORMAL, "长老")
  cTong.SetCaptainTitle(self.CAPTAIN_MASTER, "帮主")
  cTong.SetCaptainTitle(self.CAPTAIN_VICEMASTER, "副帮主")
  cTong.SetCaptainTitle(self.CAPTAIN_ASSISTANT2, "战争长老")
  cTong.SetCaptainTitle(self.CAPTAIN_ASSISTANT3, "战争长老")
  cTong.SetCaptainTitle(self.CAPTAIN_ASSISTANT4, "内政长老")
  cTong.SetCaptainTitle(self.CAPTAIN_ASSISTANT5, "外交长老")
  cTong.SetCaptainTitle(self.CAPTAIN_ASSISTANT6, "执事长老")
  --设定帮主长老的权限（所有）
...
function Tong:_AddKin2Tong(nTongId, cTong, nKinId, nCreateTime, i, nStockPercent)
  ...
  if nKinFund > 0 then
    ...
    if nStockPercent and nStockPercent ~= 1 then
      local szLog = string.format("家族[%s]在创建过程中家族成员所持有的帮会股份是正常情况下的[%d]百分比,(帮会建设资金上限问题)", cKin.GetName(), math.floor(nStockPercent * 100))
  ...
  while pMember do
    ...
    if nPercent and nPercent ~= 1 then
      local szLog = string.format("家族[%s]玩家[%s]加入帮会个人股份获取量是正常情况下的[%d]百分比,以前股份为[%d],(帮会建设资金上限问题)", cKin.GetName(), KGCPlayer.GetPlayerName(pMember.GetPlayerId()), math.floor(nPercent * 100), nPersonalStock)
      Dbg:WriteLog("Tong", szLog)
    end
...
function Tong:PresidentConfirm(nTongId, tbResult, bSetPosition) -- tbResult默认nil
  ...
  if not tbResult then
    ...
    if bSetPosition ~= 0 then
      ...
      if (tbResult[1].nKinId ~= nCandidateKin or tbResult[1].nMemberId ~= nCandidateMember or tbResult[1].nKinId == 0 or tbResult[1].nMemberId == 0) and self:CheckPresidentInTong(nTongId) == 1 then
  ...
  for i = 1, math.min(self.DIRECTORATE_MEMBERS + 1, #tbResult) do
    ...
    if pKin then
      ...
      if i == 1 then -- 第一个是首领
        ...
        bRecord = 1
      elseif (nPresidenKin ~= tbResult[i].nKinId or nPresidenMember ~= tbResult[i].nMemberId) and (nCount < self.DIRECTORATE_MEMBERS) then -- 其他是股东会成员
        if bSetPosition ~= 0 then
...
function Tong:CanAppointOfficial(nTongId, nKinId, nMemberId)
  ...
  if pMember.GetStockFigure() == Tong.DIRECTORATE or pMember.GetStockFigure() == Tong.PRESIDENT_CANDIDATE or pMember.GetStockFigure() == Tong.PRESIDENT then
    return 1
  end
  return 0
end
...
function Tong:CanCostedBuildFund(nTongId, nKinId, nMemberId, nMoney, nCheckPow)
  ...
  if nCheckPow ~= 0 then
    -- 判断权限
    if Tong:CheckSelfRight(nTongId, nKinId, nMemberId, Tong.POW_FUN) ~= 1 and Tong:CheckPresidentRight(nTongId, nKinId, nMemberId) ~= 1 then
      return 0
    end
  end
