...
function Tong:CreateTong_GC(anKinId, szTongName, nCamp)
  ...
  local nTongTransferCount = 0
  for _, nKinId in ipairs(anKinId) do
    Kin:JoinTong_GC(nKinId, szTongName, nTongId, nCamp)
    --创建帮会时若家族有人在全局服，帮会跨服计数更正，但不把帮会信息同步给全局服
    --todo：此时在全局服的家族玩家无法在帮会频道聊天
    nTongTransferCount = nTongTransferCount + KGCPlayer.GetPlayerGlobalLoginKinCount_Normal(nKinId)
  end
  KGCPlayer.AddPlayerGlobalLoginTongCount_Normal(nTongId, nTongTransferCount) --设置帮会跨服人数数量
  --Log 记录
...
function Tong:KinAdd_GC(nTongId, nKinId)
  ...
  local nKinTransferCount = KGCPlayer.GetPlayerGlobalLoginKinCount_Normal(nKinId)
  KGCPlayer.AddPlayerGlobalLoginTongCount_Normal(nTongId, nKinTransferCount) --增加帮会跨服人数数量
  if nKinTransferCount > 0 then
    --当前该家族有人在全局服，同步给全局服，能在帮会频道说话
    GC_AllExcute({ "Player:UpdatePlayerKinInTong_Global_GC", GetGatewayName(), cTong.GetName(), cKin.GetName() })
  end
end
...
function Tong:FireKin_GC(nTongId, nSelfKinId, nSelfMemberId, nTagetKinId)
  ...
  tbData.tbApplyRecord.nTimerId = Timer:Register(self.FIREKIN_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nTongId, self.REQUEST_KICK_KIN)
...
function Tong:KinDel_GC(nTongId, nKinId, nMethod) -- nMethod为0表示离开，1表示开除
  ...
  local nKinTransferCount = KGCPlayer.GetPlayerGlobalLoginKinCount_Normal(nKinId)
  KGCPlayer.DelPlayerGlobalLoginTongCount_Normal(nTongId, nKinTransferCount) --减少帮会跨服人数数量
  if nKinTransferCount > 0 then
    --当前该家族有人在全局服，同步给全局服，不能再在帮会频道说话
    GC_AllExcute({ "Player:UpdatePlayerKinInTong_Global_GC", GetGatewayName(), "_notong", cKin.GetName() })
  end
...
function Tong:ApplyTakeFund_GC(nTongId, nSelfKinId, nSelfMemberId, nPlayerId, nMoney)
  ...
  if nMoney >= self.TAKEFUND_APPLY then
    ...
    tbData.tbApplyRecord.nTimerId = Timer:Register(self.TAKEFUND_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nTongId, self.REQUEST_TAKE_FUND)
    return GlobalExcute({ "Tong:ApplyTakeFund_GS2", nTongId, nSelfKinId, nPlayerId, nMoney })
  end
  -- 回gs查找player是否还存在
...
function Tong:ApplyDispenseFund_GC(nTongId, nSelfKinId, nSelfMemberId, nType, nMoney)
  ...
  if nMoney >= self.DISPENSE_FUND_APPLY then -- 人均发资金量达到需要申请的界限
    ...
    tbData.tbAccept = {} --表态家族表
    -- 状态持10分钟
    tbData.tbApplyRecord.nTimerId = Timer:Register(self.DISPENSE_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nTongId, self.REQUEST_DISPENSE_FUND)

    return GlobalExcute({ "Tong:ApplyDispense_GS2", nTongId, nSelfKinId, nPlayerId, nType, nMoney, self.REQUEST_DISPENSE_FUND })
  end
  -- 没超界限，直发
...
function Tong:AcceptExclusiveEvent_GC(nTongId, nSelfKinId, nSelfMemberId, nKey, nAccept)
  ...
  GlobalExcute({ "Tong:AcceptExclusiveEvent_GS2", nTongId, nSelfKinId, cMember.GetPlayerId(), nKey, nAccept })
  -- 判断是否通过了申请，执行操作
  if nKey == self.REQUEST_DISPENSE_FUND then
    if tbData.nCount < self.DISPENSE_AGREE_COUNT then
      return 0
    end
    KTong.DispenseFun(nTongId, tbData.tbApplyRecord.nPlayerId, tbData.tbApplyRecord.nType, tbData.tbApplyRecord.nAmount)
  elseif nKey == self.REQUEST_TAKE_FUND then
    if tbData.nCount < self.TAKEFUND_AGREE_COUNT then
      return 0
    end
    return self:FindPlayer_GC(nTongId, tbData.tbApplyRecord.nAmount, tbData.tbApplyRecord.nPlayerId)
  elseif nKey == self.REQUEST_DISPENSE_OFFER then
    if tbData.nCount < self.DISPENSE_AGREE_COUNT then
      return 0
    end
    return KTong.DispenseOffer(nTongId, tbData.tbApplyRecord.nPlayerId, tbData.tbApplyRecord.nType, tbData.tbApplyRecord.nAmount)
...
function Tong:ApplyDispenseOffer_GC(nTongId, nSelfKinId, nSelfMemberId, nType, nAmount)
  ...
  if nAmount > self.DISPENSE_OFFER_APPLY then
    ...
    tbData.tbAccept = {} --表态家族表
    -- 状态持10分钟
    tbData.tbApplyRecord.nTimerId = Timer:Register(self.DISPENSE_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nTongId, self.REQUEST_DISPENSE_OFFER)
    return GlobalExcute({ "Tong:ApplyDispense_GS2", nTongId, nSelfKinId, nPlayerId, nType, nAmount, self.REQUEST_DISPENSE_OFFER })
  end
  -- 直发
...
function Tong:TakeStock_GC(nTongId, nKinId, nMemberId)
  ...
  GlobalExcute({
    ...
    nKinId,
    nMemberId,
    nTotalFund - nTakeMoney,
    nTakeMoney,
    nTotalStock - nTakeStock,
    nPersonalStock - nTakeStock,
  })
  -- Add TongLog 领取分红
...
function Tong:CheckAndSpiltOrComposeStock(nTongId)
  ...
  elseif nBuildFund / nTongStock < 2 then -- 股价太低，合并股份
    local nComposeNum = math.min(math.ceil(self.DEFAULT_STOCKPRICE / (nBuildFund / nTongStock)), math.ceil(nTongStock / 100))
...
function Tong:PresidentCandidateConfirm_GC(nTongId)
  ...
  if tbResult[1].nKinId == pTong.GetPresidentKin() and tbResult[1].nMemberId == pTong.GetPresidentMember() then
...
function Tong:ConsumeBuildFund_GC(nTongId, nMoney)
  ...
  elseif nCurFund / nCurTotalStock < 1 then -- 需要合并股了
    local nComposeNum = math.min(math.ceil(self.DEFAULT_STOCKPRICE / (nCurFund / nCurTotalStock)), math.ceil(nCurTotalStock / 100))
    self:ComposeStock_GC(nTongId, nComposeNum)
  end
...
function Tong:_AddTongBuildFund(nKinId, nMemberId, nFund, nTranceRate)
  ...
  if not pTong then
    ...
    return GlobalExcute({ "Tong:_AddTongBuildFund_GS2", nTongId, nKinId, nMemberId, 0, 0, nPersonalStock + nFund, bClear })
  end
  local nBuildFund = pTong.GetBuildFund()
  ...
  if nFund >= 200000 then
    ...
    Dbg:WriteLog("Tong", "Name:" .. szName, "Fund:" .. nFund, "TotalFund:" .. (nBuildFund + nFund), "TotalStock:" .. (nTotalStock + nStock), "PersonStock:" .. (nPersonalStock + nStock), "StockPrice" .. nStockPrice)
  end

  return GlobalExcute({ "Tong:_AddTongBuildFund_GS2", nTongId, nKinId, nMemberId, nBuildFund + nFund, nTotalStock + nStock, nPersonalStock + nStock, bClear })
end
...
function Tong:OfficialMaintain_GC(nTongId, nKinId, nMemberId)
  ...
  -- 扣个人资产和股份
  local nStockAmount = self:CalculateStockCost(nTongId, nKinId, nMemberId, tonumber(Tong.OFFICIAL_CHARGE[nPersonalLevel]))
    ...
    if tbRet[i] then
      ...
      if tbRet[i] then
        ...
        if bNoMail ~= 1 then
          ...
          Dbg:WriteLog("GreatMemberConfirm", "优秀成员确定", "玩家名:" .. szSelfName, "帮会ID:" .. nTongId, "家族ID:" .. tbRet[i].nKinId, "成员ID:" .. tbRet[i].nMemberId, "流水号:" .. nMemberVoteNo)
        end
      end
    end
  end
...
function Tong:RegisterTongAnnounce_GC(nTongId, nKinId, nMemberId, nTimes, nDistance, szMsg)
  ...
  if Tong:CheckSelfRight(nTongId, nKinId, nMemberId, 0) ~= 1 or nTimes < self.TONGANNOUNCE_MIN_TIMES or nTimes > self.TONGANNOUNCE_MAX_TIMES or nDistance < self.TONGANNOUNCE_MIN_DISTANCE or nDistance > self.TONGANNOUNCE_MAX_DISTANCE then
    return 0
  end
...
function Tong:ApplyStorageFundToKin_GC(nTongId, nSelfKinId, nSelfMemberId, nPlayerId, nMoney, nTargetKinId)
  ...
  tbData.nCount = 0
  tbData.tbAccept = {} --表态家族表
  tbData.tbApplyRecord.nTimerId = Timer:Register(self.TAKEFUND_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nTongId, self.REQUEST_STORAGE_FUND_TO_KIN)
  return GlobalExcute({ "Tong:ApplyStorageFundToKin_GS2", nTongId, nSelfKinId, nPlayerId, nMoney, nTargetKinId })
end
