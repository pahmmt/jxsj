...
function Relation:AfterAddRelation_GC(nAppId, nDstId, nType)
  if not nAppId or not nDstId or not nType or nAppId <= 0 or nDstId <= 0 then
    return
  end
...
function Relation:ProcessAfterAddRelation_GC(nType, nRole, nAppId, nDstId)
  ...
  if nType == Player.emKPLAYERRELATION_TYPE_TMPFRIEND and KRelation.HasRelation(nDstId, nAppId, 2) == 1 then
