...
if MODULE_GC_SERVER then
  ...
  tbTraining.MIN_TEACHERLEVEL = 60 -- 师傅的最顶等级80级
  ...
  function tbTraining:__CheckTeacher(nTeacherId)
    ...
    if nCurMonth == nMonthCode and nMonthCount >= self.MAX_STUDENTCOUNT_PERMONTH then
      Relation:SetInfoMsg(string.format("你们不能成为师徒，师傅每个月最多只能收%s个徒弟。", self.MAX_STUDENTCOUNT_PERMONTH))
      return 0
    end

    local nLevle = KGCPlayer.OptGetTask(nTeacherId, KGCPlayer.LEVEL)
    if nLevle < self.MIN_TEACHERLEVEL then
      Relation:SetInfoMsg(string.format("你们不能成为师徒，师傅至少要达到%s级才能收徒。", self.MIN_TEACHERLEVEL))
      return 0
    end
    ...
    if nCurStudentCount >= self.MAX_STUDENTCOUNT then
      Relation:SetInfoMsg(string.format("你们不能成为师徒，每个师傅当前的未出师弟子最多只能有%s个。", self.MAX_STUDENTCOUNT))
      return 0
    end

    if KRelation.GetOneRelationCount(nTeacherId, Player.emKPLAYERRELATION_TYPE_TRAINING, 0) ~= 0 then
      Relation:SetInfoMsg("你们不能成为师徒，师傅不能是其他的未出师弟子。")
      return 0
    end
  ...
  function tbTraining:__CheckStudent(nStudentId)
    ...
    if nLevle < self.MIN_STUDENTLEVEL or nLevle >= self.MAX_STUDENTLEVEL then
      Relation:SetInfoMsg(string.format("你们不能成为师徒，只有在%s级至%s级之间的玩家才可以拜师。", self.MIN_STUDENTLEVEL, self.MAX_STUDENTLEVEL - 1))
      return 0
    end

    if KRelation.GetOneRelationCount(nStudentId, Player.emKPLAYERRELATION_TYPE_TRAINING, 0) ~= 0 then
      Relation:SetInfoMsg("你们不能成为师徒，玩家不能在已经有师傅的情况下再拜其他人为师。")
      return 0
    end
...
if MODULE_GAMESERVER then
  ...
  function tbTraining:AddShituTitle_GS(tbParam)
    ...
    if pTeacher then
      local szCurTitle, szPlayerName = Relation:FindTitleAndName("的师傅", pTeacher)
      if szCurTitle and szPlayerName then
        pTeacher.RemoveSpeTitle(szCurTitle)
      end
      local szTeacherTitle = szStudentName .. "的师傅"
  ...
  function tbTraining:__TrainingResponse_Refuse(nTOrSFlag, szPlayerName)
    ...
    if 1 == nTOrSFlag then
      szMsg = string.format("%s拒绝成为你的师父，你们可以再聊聊或者通过师徒面板寻找其他合适的师父", me.szName)
    elseif 0 == nTOrSFlag then
      szMsg = string.format("%s拒绝成为你的徒弟，你们可以再聊聊或者通过师徒面板寻找其他合适的徒弟", me.szName)
    end
...
Relation:Register(Player.emKPLAYERRELATION_TYPE_TRAINING, tbTraining)
