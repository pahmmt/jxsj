...
function Relation:CheckRelationType(nType)
  if nType < Player.emKPLAYERRELATION_TYPE_TMPFRIEND or nType > Player.emKPLAYERRELATION_TYPE_BUDDY then
    return 0
  end
  return 1
end
...
function Relation:CanCreateRelation(nType, nRole, nAppId, nDstId)
  ...
  if
    nType == Player.emKPLAYERRELATION_TYPE_BLACKLIST -- 黑名单
    or nType == Player.emKPLAYERRELATION_TYPE_ENEMEY
  then -- 仇人
    if not bReachMaxCount_App or bReachMaxCount_App == 0 then
      self:SetInfoMsg("添加失败，你或对方的该人际关系数量已经达到上限。")
      return 0
    end
  else
    -- 只有双向关系的需要这个判断
    if not bReachMaxCount_App or not bReachMaxCount_Dst or bReachMaxCount_App == 0 or bReachMaxCount_Dst == 0 then
      self:SetInfoMsg("添加失败，你或对方的该人际关系数量已经达到上限。")
      return 0
    end
  end
  ...
  if not bHasRelation or bHasRelation == 1 then
    self:SetInfoMsg("你们已经存在着这种人际关系了，不用再次添加。")
    return 0
  end
...
function Relation:__CheckReachMaxCount(nType, nPlayerId, nRole)
  ...
  local nCurCount = 0
  if Player.emKPLAYERRELATION_TYPE_TMPFRIEND == nType or Player.emKPLAYERRELATION_TYPE_BIDFRIEND == nType then
    nCurCount = KRelation.GetOneRelationCount(nPlayerId, Player.emKPLAYERRELATION_TYPE_TMPFRIEND, 1) + KRelation.GetOneRelationCount(nPlayerId, Player.emKPLAYERRELATION_TYPE_BIDFRIEND, 1)
  else
...
-- 判断是否可以删除人际关系
function Relation:CanDelRelation(nType, nRole, nAppId, nDstId) end

--============================================

-- 判断亲密度上限
function Relation:CheckFavorLimit(nPlayerId) end
...
function Relation:WriteRelationLog(nOpt, nType, nAppId, nDstId)
  ...
  local pAppPlayer = KPlayer.GetPlayerObjById(nAppId)
...
function Relation:IsCloseRelationGroup()
  local nFlag = KGblTask.SCGetDbTaskInt(DBTASK_CLOSE_RELATIONGROUP) or 0
  return nFlag
end
