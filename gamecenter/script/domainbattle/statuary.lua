...
function tbStatuary:DelStatuaryCompetence(szName, nEventType, nSaveFlag)
  ...
  for nId, tbInfo in pairs(self.tbStatuData) do
    -- 表示还没插入过
    if tbInfo.tbPlayerInfo and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] == szName and tbInfo.tbPlayerInfo[self.INFOID_EVENTTYPE] == nEventType then
      tbInfo.tbPlayerInfo = {}
      -- tbInfo.tbPlayerInfo[self.INFOID_STATUARYINDEX] = nId;
...
function tbStatuary:AddStatuary(szName, nEventType, nFaction, nSex, szMsg)
  ...
  for nId, tbInfo in pairs(self.tbStatuData) do
    -- 表示还没插入过
    if tbInfo.tbPlayerInfo and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] == szName and tbInfo.tbNpcInfo and tbInfo.tbNpcInfo.nMapId > 0 and tbInfo.tbNpcInfo.nX > 0 and tbInfo.tbNpcInfo.nY > 0 and tbInfo.tbNpcInfo.nEventType == nEventType then
...
function tbStatuary:RefreshStatuary(szName, nEventType)
  ...
  for nId, tbInfo in pairs(self.tbStatuData) do
    ...
    if tbPlayerInfo[self.INFOID_PLAYERNAME] and tbPlayerInfo[self.INFOID_PLAYERNAME] == szName and (tbPlayerInfo[self.INFOID_EVENTTYPE] or 0) == nEventType then
      tbResInfo = tbInfo
      nIndex = nId
      break
    end
...
function tbStatuary:AlterRoleName(szOldName, szNewName)
  local bFind = 0
  for nId, tbInfo in pairs(self.tbStatuData) do
    if tbInfo.tbPlayerInfo and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] == szOldName then
      tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] = szNewName

      bFind = 1
    end
  end

  if MODULE_GC_SERVER and bFind == 1 then
    self:SaveStatuaryInfo()
    GlobalExcute({ "Domain.tbStatuary:AlterRoleName", szOldName, szNewName })
  end
end
...
function tbStatuary:GetStatuaryComInfoByName(szName, nEventType)
  ...
  for _, tbInfo in pairs(self.tbStatuData) do
    if tbInfo.tbPlayerInfo then
      if tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] == szName and tbInfo.tbPlayerInfo[self.INFOID_EVENTTYPE] == nEventType then
        return tbInfo
      end
    end
  end
...
function tbStatuary:GetStatuaryInfoByName(szName, nEventType)
  ...
  for _, tbInfo in pairs(self.tbStatuData) do
    if tbInfo.tbPlayerInfo then
      if tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] and tbInfo.tbPlayerInfo[self.INFOID_PLAYERNAME] == szName and tbInfo.tbPlayerInfo[self.INFOID_NPCID] > 0 and tbInfo.tbPlayerInfo[self.INFOID_EVENTTYPE] == nEventType then
        return tbInfo
      end
    end
  end
...
function tbStatuary:AddHelpNews_Result(tbTongInfo)
  ...
  for i, tbInfo in ipairs(tbTongInfo) do
    local szRank = string.format("第 %d 名", i)
