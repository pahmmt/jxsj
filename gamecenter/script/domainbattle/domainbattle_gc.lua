...
function Domain:StartAllGame_GC()
  ...
  if self.bOpenWritePlayerInfo == 1 and (GetTime() - nServerStart) <= self.nCheckPlayerTimeLimit then
    Timer:Register(self.nTimeJianju1, self.__QueryGlobalSta, self)
  end
  self:CalculateReact()
  ...
  for nTongId, tbPlayerSet in pairs(self.tbPlayer) do
    ...
    if pTong then
      ...
      for i = 1, math.min(10, #tbSortScore) do
        ...
        szResult = szResult .. Lib:StrFillL("第" .. i .. "名 ", 8) .. Lib:StrFillL(szName, 18) .. tbSortScore[i].nKey .. "\n"
      end
...
function Domain:AddDomainNews()
  ...
  while (nCount <= 10 or nSort <= 10) and (#tbResult >= nCount) do
    szMsg = szMsg .. "\n" .. Lib:StrFillL("第" .. nSort .. "名 ", 8) .. "<color=yellow>" .. Lib:StrFillL(tbResult[nCount].szName, 14) .. "<color>占领了<color=green>" .. tbResult[nCount].nKey .. "<color>块领土"
...
function Domain:SetTongAward_GC(nTongId, nSelfKinId, nSelfMemberId, nLevel, nPlayerId)
  ...
  if cTong and nLevel > 0 and nWastage > 0 and nTongBuildFund >= 0 and cTong.GetDomainAttendNo() == nGblBattleNo and nDomainAwardAmount > 0 and nDomainAwardAmount <= self.MAX_DOMAINAWARD_AWARD then
...
function Domain:UpdateBaZhuZhiYin_GC(szName, nTongId, nCurCount, nAddCount)
  if not szName or "" == szName or not nCurCount or nCurCount < 0 or not nAddCount or nAddCount < 0 or not nTongId or nTongId <= 0 then
    return 0
  end
  local cTong = KTong.GetTong(nTongId)
  if not cTong then
...
function Domain:IsGetTongAward()
  ...
  table.sort(tbTongAmount, function(a, b)
...
function Domain:GetTongAward_GC(szReason)
  ...
  table.sort(tbTongAmount_All, function(a, b)
    return (a.nAmount > b.nAmount)
  end)
...
function Domain:OnRecConnectEvent(nConnectId)
  ...
  if self.nBattleState == self.PRE_BATTLE_STATE then
    GlobalExcute({ "Domain:ResumePreBattle", Domain:UpdateDataVer(), self.tbDeclareDomainTong, self.tbTongDeclare })
...
function Domain:WritePlayerInfo__GC(szAccount, szPlayerName, szIp)
  Globalstat:Collect("DomainBattleCheck", "PlayGame", szAccount, szPlayerName, { szIp })
end

function Domain:__QueryGlobalSta()
  Globalstat:Query(1, "DomainBattleCheck", "ArrestPlayer")
  self.nChecakPlayerIndex = self.nChecakPlayerIndex + 1
  if self.nChecakPlayerIndex >= self.nEndTimes then
    Globalstat:Remove(0, "DomainBattleCheck", "PlayGame", "", "")
    return 0
  end
  return self.nTimeJianju2
end

function Domain:__ArrestPlayer(tbDate)
...
Globalstat:RegisterGlobalStatEventCallBack("DomainBattleCheck", "ArrestPlayer", Domain.__ArrestPlayer, Domain)
