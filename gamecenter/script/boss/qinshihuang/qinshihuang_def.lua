...
tbQinshihuang.TASK_USE_MEIYU = 11 -- 每天使用蓝田美玉个数
...
tbQinshihuang.tbTranList = {
  ...
  [4] = { [0] = "秦始皇陵五层", { "秦始皇陵五层（王座）", { 1540, 1711, 3429 } } },
}
...
function tbQinshihuang:GetNpcLevel()
  local nOpenDay = math.floor((GetTime() - KGblTask.SCGetDbTaskInt(DBTASD_SERVER_STARTTIME)) / (60 * 60 * 24))
  local nLevel = 120 + math.floor((nOpenDay - 139) * (150 - 120) / (365 - 139))
  return math.max(120, math.min(nLevel, 150))
end

if EventManager.IVER_bOpenTiFu ~= 1 then
  tbQinshihuang.MAX_DAILY_TIME = 7200 -- 每天2小时
else
  tbQinshihuang.MAX_DAILY_TIME = 12 * 7200 -- 不限制
end
