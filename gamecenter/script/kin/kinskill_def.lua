...
function Kin:LoadKinSkill()
  ...
  for nId, tbParam in ipairs(tbFileEx) do
    if nId >= 1 then
      ...
      self.tbKinSkill.tbSkillInfo[nGenreId][nDetailId][nSkillId] = {
        szSkillName = szSkillName,
        szSkillSpr = szSkillSpr,
        szSkillEffectSpr = szSkillEffectSpr,
        szSkillInfo = szSkillInfo,
        nTaskId = nTaskId,
        nPassive = nPassive,
        szLevelTable = szLevelTable,
        tbCondition = {},
      }
      for i = 1, 10 do
