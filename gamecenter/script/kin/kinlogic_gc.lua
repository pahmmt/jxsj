...
function Kin:CreateKin_GC(anPlayerId, anStoredRepute, szKinName, nCamp, bGoldLogo)
  ...
  local tbStock = {}
  local nChallengeTimes = HomeLand:GetDailyChallengeTimes()
  ...
  print("kingc:create", anPlayerId, anStoredRepute, szKinName, nCamp, nCreateTime, tbStock, nChallengeTimes, bGoldLogo)
  return GlobalExcute({ "Kin:CreateKin_GS2", anPlayerId, anStoredRepute, szKinName, nCamp, nCreateTime, tbStock, nChallengeTimes, bGoldLogo })
end
...
function Kin:MemberAdd_GC(nKinId, nExcutorId, nPlayerId, bCanJoinKinImmediately)
  ...
  if pTong then
    ...
    if nBuildFund > Tong.MAX_BUILD_FUND then
      ...
      -- log
      local szLog = string.format("玩家[%s] 加入家族[%s]个人股份在原有基础上降低了[%d]百分比,当下的股份[%d]", KGCPlayer.GetPlayerName(nPlayerId), cKin.GetName(), nBefore, math.floor(nPersonalStock * Tong.JOIN_TONG_STOCK / nPrice))
  ...
  if Player:GetGlobalLoginState_Normal(nPlayerId) == 1 then
    Player:AddGlobalPlayerKinTong_Normal_GC(nTongId, nKinId)
    --todo:暂不把他有家族的信息同步给全局服
  end
...
function Kin:MemberDel_GC(nKinId, nMemberId, nMethod)
  ...
  --如果该玩家当前在全局服，则家族和帮会的跨服数量减1
  if Player:GetGlobalLoginState_Normal(nPlayerId) == 1 then
    Player:DelGlobalPlayerKinTong_Normal_GC(nTongId, nKinId)
    --todo:暂不把他有家族的信息同步给全局服
  end
...
function Kin:ChangeCaptain_GC(nKinId, nExcutorId, nMemberId)
  ...
  if bSys == 1 then
    ...
    --传位成族长
    local szExcutorMsg = string.format("[%s] 任命为族长, [%s] 变成普通成员", KGCPlayer.GetPlayerName(cMember.GetPlayerId()), KGCPlayer.GetPlayerName(cExcutor.GetPlayerId()))
...
function Kin:ApplyTakeFund_GC(nKinId, nMemberId, nPlayerId, nMoney)
  ...
  if self:CheckSelfRight(nKinId, nMemberId, self.FIGURE_CAPTAIN) ~= 1 then --非族长取钱需要族长同意
    ...
    tbData.tbApplyRecord.nTimerId = Timer:Register(self.TAKE_FUND_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nKinId, self.KIN_EVENT_TAKE_FUND)
    return GlobalExcute({ "Kin:ApplyTakeFund_GS2", 0, nKinId, nMemberId, nPlayerId, nMoney, self.FIGURE_CAPTAIN })
  end
  if self:CheckSelfRight(nKinId, nMemberId, self.FIGURE_CAPTAIN) == 1 then --族长直接取钱,需要两名正式成员同意
    ...
    tbData.tbApplyRecord.nTimerId = Timer:Register(self.TAKE_FUND_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nKinId, self.KIN_EVENT_TAKE_FUND)
    return GlobalExcute({ "Kin:ApplyTakeFund_GS2", 1, nKinId, nMemberId, nPlayerId, nMoney })
  end
...
function Kin:AcceptExclusiveEvent_GC(nKey, nPlayerId, nKinId, nMemberId, nAccept, nAppleyMemberId)
  ...
  GlobalExcute({ "Kin:AcceptExclusiveEvent_GS2", nKey, nPlayerId, nKinId, nMemberId, nAccept })
  -- 判断是否通过了申请，执行操作
...
function Kin:ApplySendSalary_GC(nPlayerId, nKinId, nMemberId, tbMember, nAttendanceAward)
  ...
  tbData.tbAccept = {}
  tbData.nAgreeCount = 2
  tbData.tbApplyRecord.nTimerId = Timer:Register(self.TAKE_FUND_APPLY_LAST, self.CancelExclusiveEvent_GC, self, nKinId, self.KIN_EVENT_SALARY)
...
function Kin:SendSalary_GC(nKinId)
  ...
  for nMemberId, tbMemberData in pairs(tbSalary) do
    ...
    Dbg:WriteLog(szNameTo, "工资邮件", "工资金额：" .. nSalary, "出勤次数：" .. tbMemberData.nAttendance, "单次出勤奖励：" .. nAttendanceAward)
    --end
  end
