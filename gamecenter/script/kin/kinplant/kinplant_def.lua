...
KinPlant.tbTypeName = { "<color=yellow>【粮食订单】<color>", "<color=yellow>【水果订单】<color>", "<color=yellow>【鲜花订单】<color>" }
...
KinPlant.tbHealthTitile = { { "健康的", "white" }, { "饱满的", "green" }, { "优秀的", "blue" }, { "高产的", "yellow" }, { "丰硕的", "pink" } }
...
function KinPlant:LoadPlantInfo()
  ...
  for nId, tbParam in ipairs(tbFile) do
    if nId > 1 then
      ...
      self.tbPlantNpcInfo[nId] = {
        szItem = szItem,
        szFruitItem = szFruitItem,
        nType = nType,
        tbGrade = { nGrade1, nGrade2, nGrade3 },
        tbExp = { nExp1, nExp2, nExp3 },
        nKinExp = nKinExp,
        nMaxAwardCount = nMaxAwardCount,
        nMaxGetOther = nMaxGetOther,
        nPerGetOther = nPerGetOther,
        tbWeather = { nWeather1, nWeather2, nWeather3 },
        nDredging = nDredging,
        tbTempNpcId = tbTempNpcId,
        tbTime = tbTime,
      }
    end
  end
