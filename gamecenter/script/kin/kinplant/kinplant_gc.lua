...
function KinPlant:LoadBuff_GC()
  ...
  --删除太老的家族数据
  for nKinId, tbInfo in pairs(self.tbPlantInfo or {}) do
    local cKin = KKin.GetKin(nKinId)
    if not cKin or cKin.GetLastHomeLandRank() > 1000 then
      self.tbPlantInfo[nKinId] = nil
    else
      for i = 1, #self.tbNpcPoint do
        if not tbInfo[i] then
          self.tbPlantInfo[nKinId][i] = { "", 0, 0, 0, 0, 0, 0 }
          --发现有空洞，进行修复
          print("stack trackback", "script\\kin\\KinPlant\\kinplant_gc.lua", "PlantInfoErrorNULL", nKinId, i)
        end
      end
    end
  end
...
function KinPlant:SaveBuff_GC()
  ...
  SetGblIntBuf(GBLINTBUF_KIN_PLANT_DAILY, 0, 0, self.tbPlantInfo)
end
...
function KinPlant:Msg2World()
  local szWorldMsg = "家族领地种植活动火热进行中，赶紧去家园中看看吧！"
