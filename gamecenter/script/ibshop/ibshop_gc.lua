...
function IbShop:ServerendEvent()
  ...
  --服务器关闭时候进行一次列表同步
  if self.tbFavoriteBindCoinGoods and self.tbFavoriteCoinGoods and #self.tbFavoriteBindCoinGoods ~= 0 and #self.tbFavoriteCoinGoods ~= 0 then
...
function IbShop:SortFavoriteGoodsList()
  if self.tbFavoriteBindCoinGoods and self.tbFavoriteCoinGoods and #self.tbFavoriteBindCoinGoods ~= 0 and #self.tbFavoriteCoinGoods ~= 0 then
    local sortFunc = function(tb1, tb2)
      return tb1.nTimes > tb2.nTimes
    end
