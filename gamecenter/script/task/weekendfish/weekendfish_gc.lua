...
function WeekendFish:RefreshTask_GC(nSeg) -- 每星期六随一次
  ...
  if
    (nWeekDay == 6 and nTaskWeek ~= nWeek) -- 正常的星期六刷新
    or (nWeekDay == 6 and nTaskWeek == 0 and nWeek == 0) -- 新服第一次钓鱼刚好在新年第一周
    or (nWeekDay == 0 and nTaskWeek ~= nWeek and nDate ~= 101)
  then -- 意外情况星期六没有刷新，星期天刷新，剔除星期天刚好是1月1日的情况
...
function WeekendFish:GetLuckFishAward_GC(nPlayerId, nType)
  ...
  for i = 1, self.MAX_LUCKFISH_AWARD_RANK do
    if self.tbLuckFishRank[nType] and self.tbLuckFishRank[nType][i] and self.tbLuckFishRank[nType][i][1] and self.tbLuckFishRank[nType][i][1] == szPlayerName and self.tbLuckFishRank[nType][i][4] ~= 1 then
  ...
  for i = 1, self.MAX_LUCKFISH_AWARD_RANK do
    if self.tbLuckFishRank_Ex[nType] and self.tbLuckFishRank_Ex[nType][i] and self.tbLuckFishRank_Ex[nType][i][1] and self.tbLuckFishRank_Ex[nType][i][1] == szPlayerName and self.tbLuckFishRank_Ex[nType][i][4] ~= 1 then
...
function WeekendFish:RefreshFish_GC(nSeg, nForce)
  ...
  Dialog:GlobalNewsMsg_GC("钓鱼活动已经开始了，世界各大水域已经出现了鱼儿，请侠客们前往秦洼处接取任务开始钓鱼吧。")
  Dialog:GlobalMsg2SubWorld_GC("钓鱼活动已经开始了，世界各大水域已经出现了鱼儿，请侠客们前往秦洼处接取任务开始钓鱼吧。")
...
function WeekendFish:NoticeTimer()
  ...
  Dialog:GlobalNewsMsg_GC("钓鱼活动正在火热进行中，请侠客们在新手村秦洼处接取任务后到世界各大水域进行钓鱼。")
  Dialog:GlobalMsg2SubWorld_GC("钓鱼活动正在火热进行中，请侠客们在新手村秦洼处接取任务后到世界各大水域进行钓鱼。")
  return
end
...
function WeekendFish:ClearFishNotice(nSeg)
  ...
  if nWeekDay == 6 then
    if nSeg == 1 then
      Dialog:GlobalNewsMsg_GC("鱼儿们消失了，下次鱼儿出现的时间为今天的16：00—20：00，敬请侠客们留意。")
      Dialog:GlobalMsg2SubWorld_GC("鱼儿们消失了，下次鱼儿出现的时间为今天的16：00—20：00，敬请侠客们留意。")
    elseif nSeg == 2 then
      Dialog:GlobalNewsMsg_GC("鱼儿们消失了，下次鱼儿出现的时间为明天的10：00—14：00，请侠客们及时到秦洼处兑换今日钓鱼的奖励。")
      Dialog:GlobalMsg2SubWorld_GC("鱼儿们消失了，下次鱼儿出现的时间为明天的10：00—14：00，请侠客们及时到秦洼处兑换今日钓鱼的奖励。")
    end
  elseif nWeekDay == 0 then
    if nSeg == 1 then
      ...
      Dialog:GlobalMsg2SubWorld_GC("鱼儿们消失了，下次鱼儿出现的时间为今天的16：00—20：00，敬请侠客们留意。")
    elseif nSeg == 2 then
      Dialog:GlobalNewsMsg_GC("本周的钓鱼活动已经结束，请侠客们及时到秦洼处兑换今日的钓鱼奖励。")
      Dialog:GlobalMsg2SubWorld_GC("本周的钓鱼活动已经结束，请侠客们及时到秦洼处兑换今日的钓鱼奖励。")
    end
  end
end
