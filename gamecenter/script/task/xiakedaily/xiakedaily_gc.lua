...
function XiakeDaily:RefreshTask(nSeg, nDiff)
  ...
  if nSeg == 2 then
    local nTodayTask1 = KGblTask.SCGetDbTaskInt(DBTASK_XIAKEDAILY_TASK_ID1)
    local nTodayTask2 = KGblTask.SCGetDbTaskInt(DBTASK_XIAKEDAILY_TASK_ID2)
    local nTomrrowTask1 = KGblTask.SCGetDbTaskInt(DBTASK_XIAKEDAILY_TOMORROW_TASK_ID1)
    local nTomrrowTask2 = KGblTask.SCGetDbTaskInt(DBTASK_XIAKEDAILY_TOMORROW_TASK_ID2)
    if tbOff[nTodayTask1] or tbOff[nTodayTask2] then
      bToday = 1
    end
    if tbOff[nTomrrowTask1] or tbOff[nTomrrowTask2] then
      bTomorrow = 1
    end
  end
  ...
  if bToday == 1 then --今天有问题的话就随机今天的
    local nWeek = tonumber(GetLocalDate("%w"))
    local nTask1, nTask2 = self:RandomTask(nWeek)
    KGblTask.SCSetDbTaskInt(DBTASK_XIAKEDAILY_TASK_ID1, nTask1)
    KGblTask.SCSetDbTaskInt(DBTASK_XIAKEDAILY_TASK_ID2, nTask2)
  end
  if bTomorrow == 1 then --明天的有问题的话随机明天的
    local nWeek = math.mod(GetLocalDate("%w") + 1 + 7, 7)
    local nTask1, nTask2 = self:RandomTask(nWeek)
    KGblTask.SCSetDbTaskInt(DBTASK_XIAKEDAILY_TOMORROW_TASK_ID1, nTask1)
    KGblTask.SCSetDbTaskInt(DBTASK_XIAKEDAILY_TOMORROW_TASK_ID2, nTask2)
  end
...
function XiakeDaily:CheckTask()
  ...
  if nTaskDay >= nNowDate then -- 任务日期大于等于当前日期说明当前任务就是今日日常
    self:RefreshTask(2) --处理侠客任务改变，藏宝图重整相关。更新后下一版本可删掉
  else
