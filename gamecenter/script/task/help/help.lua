...
function tbHelp:_SetOrgLevelNews()
  ...
  local szMsg = KGblTask.SCGetDbTaskStr(DBTASK_HELPNEWS_MSG1) .. KGblTask.SCGetDbTaskStr(DBTASK_HELPNEWS_MSG2) .. KGblTask.SCGetDbTaskStr(DBTASK_HELPNEWS_MSG3)
...
function tbHelp:OnLevelUp(nLevel)
  ...
  if nLevel == 69 and self:GetDNewsTime(self.NEWSKEYID.NEWS_LEVELUP) == 0 then
    ...
    local szMsg = string.format("最先达到69级的武林高手诞生了！\n姓名：<color=gold>%s<color>\n性别：<color=gold>%s<color>\n门派路线：<color=gold>%s<color>\n累计在线：<color=gold>%.1f<color>小时", me.szName, Player.SEX[me.nSex], Player:GetFactionRouteName(me.nFaction, me.nRouteId), me.nOnlineTime / 3600)
    local nNowTime = GetTime()
...
if MODULE_GAMESERVER then -- GS专用
  ...
  PlayerEvent:RegisterGlobal("OnLoginOnly", tbHelp.OnLogin, tbHelp)
end
