...
-- 临时，todo:改成传入condition callback
function tbReadFile:CountRateWithMap(szCol)
  local nRow = self:GetRow()
  local nRandom = 0
  local nAdd = 0
  local i = 0
  local tbServerMap = GetLocalServerMapInfo()
  local nServerId = GetServerId()
  for i = 1, nRow do
    local nMapId = self:GetCellInt("MapId", i)
    if tbServerMap[nMapId] == nServerId then
      nAdd = nAdd + self:GetCellInt(szCol, i)
    end
  end
