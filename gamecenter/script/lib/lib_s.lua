...
function tbCalcAward:RandomAward(nGrade, nStep, nMulti, nExcept, nReduce, tbRate)
  ...
  local nStep = nStep or self.AWARD_STEP
  ...
  for szType, tbInfo in pairs(tbA) do
    ...
    table.sort(tbAwardList[szType], function(a, b)
      return a[1] < b[1]
    end)
  end
...
--给予奖励
function tbCalcAward:RandomItem(pPlayer, tbAward)
  if not pPlayer then
    return 0, "玩家不存在。"
  end
...
---------------------------------------------------------------------------------------------
tbCalcAward.tbFrameDay = { { 201112, 726 }, { 201203, 363 }, { 202010, 311 } }
