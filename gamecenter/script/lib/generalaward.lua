...
function GeneralAward:GiveAward(tbAward)
  ...
  elseif szType == "linktask_cancel" then
    ...
    me.Msg("您得到了一次取消任务的机会，您目前的取消任务机会是：<color=yellow>" .. nCancel .. "<color>")
  ...
  elseif szType == "bindmoney" then
    ...
    if nBindMoneyEx > 0 then
      me.Msg("您得到了 <color=yellow>" .. nBindMoneyEx .. "<color> 绑定银两！")
    end
    if szStatLogName then
  ...
  elseif szType == "linktask_repute" then
    ...
    me.Msg("您在义军中的声望提升了 <color=green>" .. varValue[3] .. "<color> 点！")
  ...
  elseif szType == "item" then
    ...
    Item:InsertXJRecordMemory(Item.emITEM_XJRECORD_TASK, Item.LINKTASK_XJLOG_ID, nXJLevel, (bBind == 1) and nXJCount or 0, (bBind == 0) and nXJCount or 0)

    me.Msg(string.format("您得到了%s个物品奖励！", nNum))
