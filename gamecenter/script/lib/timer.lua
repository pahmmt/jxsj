...
function Timer:OnActive(uCurrentTime)
  ...
  for nIdx, nRegisterId in pairs(tbTime) do
    if not self.tbToBeClose[nRegisterId] then -- 没有打算关闭此Timer
      ...
      if MODULE_GAMESERVER then
        local pPlayer = KPlayer.GetPlayerObjById(tbGRoleArg.nPlayerId)
        local pNpc = KNpc.GetById(tbGRoleArg.nNpcId)
  ...
  for nRegisterId in pairs(self.tbToBeClose) do
    local tbEvent = self.tbRegister[nRegisterId]
    if MODULE_GAMESERVER then
      local pPlayer = KPlayer.GetPlayerObjById(tbEvent.tbGRoleArg.nPlayerId)
      local pNpc = KNpc.GetById(tbEvent.tbGRoleArg.nNpcId)
      Setting:SetGlobalObj(pPlayer, pNpc)
    end
...
function Timer:RegisterEx(tbEvent)
  ...
  tbEvent.tbGRoleArg = { nPlayerId = nPlayerId, nNpcId = nNpcId }
