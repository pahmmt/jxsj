...
if MODULE_GAMECLIENT then
  ...
  function tbDownloadInfo:OnLoginEnd()
    ...
    if MINI_CLIENT then
      self.nSyncTimerId = Timer:Register(self.TIMER_SEND_INTERVAL * Env.GAME_FPS, self.OnTimer_SendMiniDownloadInfo, self)
    end
  end
...
if MODULE_GAMESERVER then
  ...
  function tbDownloadInfo:ApplyLogDownloaderBlockInfo(dwHostIp, pszFile, nState)
    StatLog:WriteStatLog("stat_info", "mini_client", "download_fail", me.nId, string.format("%s,%s,%s,%d", Lib:IntIpToStrIp(dwHostIp), Lib:IntIpToStrIp(me.dwIp), pszFile, nState))
  end
...
if MODULE_GC_SERVER then
  ...
  tbDownloadInfo.nStatisticTimerId = Timer:Register(tbDownloadInfo.TIMER_BASE_TIME * Env.GAME_FPS, tbDownloadInfo.OnTimer_Statistic, tbDownloadInfo)
