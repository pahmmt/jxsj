...
function tbDragonBoatFestival2012:FinishFire(dwNpcId)
  ...
  GlobalExcute({
    "Dialog:GlobalMsg2SubWorld_GS",
    string.format("<color=yellow>%s<color>家族成员在<color=green>%s<color>煮出了一锅美味的团圆粽，特邀各位侠士前来品尝！", cKin.GetName(), GetMapNameFormId(pNpc.nMapId)),
  })
...
function tbDragonBoatFestival2012:BuyItem(nFlag, nNum, bBuy)
  ...
  if not bBuy then
    Dialog:Say(string.format("您确定花费<color=yellow>%s金币<color>购买%s个<color=green>莲子粽<color>？", 30 * nNum, nNum), { { "确认", self.BuyItem, self, 1, nNum, 1 }, { "我再想想" } })
    return
  end
