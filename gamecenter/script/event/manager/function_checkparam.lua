...
tbFun.tbLimitParamFun = {
  ...
  CheckTimeDateEx = "CheckTimeDateEx", --检查开服多少天
...
function tbFun:CheckTimeDateEx(szParam, tbGParam)
  local tbParam = self:SplitStr(szParam)
  local nCount = tonumber(tbParam[1]) or 0
  local nType = tonumber(tbParam[2]) or 0
  local szReturnMsg = tbParam[3] or "你的条件不满足。"
  ...
  if nType == 1 and math.floor((nNowTime - nSec) / 24 / 3600) >= nCount then
    return 0
  elseif nType == 2 and math.floor((nNowTime - nSec) / 24 / 3600) <= nCount then
    return 0
  end
  return 1, szReturnMsg
end
