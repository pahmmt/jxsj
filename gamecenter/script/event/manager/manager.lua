...
function EventManager:GotoEventPartTable(nEventId, nEventPartId, nType, nCheck, nIsStopNext, nMsgType, nKind, nDelayCheck)
  ...
  if nCheck == 0 and #tbSelect > 0 then
    ...
    local tbOpt = {
      {
        EventManager.tbFun:StrVal(tbParam[2] or "确定领取"),
        self.GotoEventPartTable,
        self,
        nEventId,
        nEventPartId,
        nType,
  ...
  if nDelayCheck == 0 then
    ...
    if nDelay > 0 then
      ...
      GeneralProcess:StartProcess("请等待...", nDelay * Env.GAME_FPS, { self.GotoEventPartTable, self, nEventId, nEventPartId, nType, nCheck, nIsStopNext, nMsgType, nKind, 1 }, nil, tbEvent)
      return 0
    end
  end
