...
function tbFun:_GetItem(pPlayer, tbItem)
  ...
  if nSkillId > 0 and nSkillLevel > 0 and nSkillTime > 0 then
    pPlayer.AddSkillState(nSkillId, nSkillLevel, 2, nSkillTime * 60 * Env.GAME_FPS, 1, 0, 1)
  ...
  if not tbItem.szAnnouceContent and tbItem.szName and tbItem.szDesc then
    ...
    if nFriendMsg == 1 then
      pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]" .. tbItem.szDesc .. "获得了<color=yellow>" .. tbItem.szName .. "<color>。")
    end
  ...
  if tbItem.szAnnouceContent and tbItem.szAnnouceContent ~= "" then
    ...
    if nFriendMsg == 1 then
      pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>" .. tbItem.szAnnouceContent)
    end
...
function tbFun:ExeAddEquit(szParam)
  ...
  local tbAward = {}
  if not EventManager.tbOther.tbEquitList or not EventManager.tbOther.tbEquitList[nFaction] or not EventManager.tbOther.tbEquitList[nFaction][nRouteId] or not EventManager.tbOther.tbEquitList[nFaction][nRouteId][nSex] then
    print("【活动系统出错】装备表不存在。")
    return 1
  end
...
function tbFun:ExeAddSpecialEquit(szParam)
  ...
  local tbAward = {}
  if not EventManager.tbOther.tbSpecialEquitList or not EventManager.tbOther.tbSpecialEquitList[nType] or not EventManager.tbOther.tbSpecialEquitList[nType][nTypeEx] or not EventManager.tbOther.tbSpecialEquitList[nType][nTypeEx][nSeries] or not EventManager.tbOther.tbSpecialEquitList[nType][nTypeEx][nSeries][nSex] then
    print("【活动系统出错】装备表不存在。")
    return 1
  end
