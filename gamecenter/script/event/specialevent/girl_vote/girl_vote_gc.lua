  ...
  if tbBuf then
    for szName, tbInBuf in pairs(tbBuf) do
      ...
      if tbInfo then
        ...
        local szOut = string.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", szGateway, tbInfo.szAccount or "", szName or "", nHonor or 0, nRank or 0, Player:GetFactionRouteName(tbInfo.nFaction) or "", Player:GetFactionRouteName(tbInfo.nFaction, tbInfo.nRoute) or "", tbInfo.nLevel or 0, szKin or "", szTong or "", szFansName or "", nFansTickets or 0, szFansSex or "")
    ...
    if szGateway == GetGatewayName() and szAccount ~= "" and szRoleName ~= "" then
      local szScript = string.format(
        [=[
				...
				me.AddSkillState(2763, 1, 2, 15 * 24 * 3600 * 18, 1,0,1);
  ...
  if tbBuf and tbBuf.tPList then
    for szGateWayId, tbInBuf in pairs(tbBuf.tPList) do
      for szRoleName, tbPInfor in pairs(tbInBuf) do
        ...
        local szOut = string.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", szGateWayId, szAccount, szRoleName, nTicket, 0, szFaction, szRoute, nLevel, szKin, szTong, tbFans[1][1], tbFans[1][2], tbFans[1][3], tbFans[2][1], tbFans[2][2], tbFans[2][3], tbFans[3][1], tbFans[3][2], tbFans[3][3], tbFans[4][1], tbFans[4][2], tbFans[4][3], tbFans[5][1], tbFans[5][2], tbFans[5][3])
        KFile.AppendFile(szOutFile, szOut)
      end
    end
  end
