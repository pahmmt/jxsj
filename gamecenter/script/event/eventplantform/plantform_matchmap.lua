...
function tbMapPk:OnLeave()
  ...
  if EPlatForm:GetMacthState() == EPlatForm.DEF_STATE_MATCH_1 then
    ...
    szLeagueName = League:GetMemberLeague(EPlatForm.LGTYPE, me.szName)
    if nReadyId <= 0 or not EPlatForm.MissionList or not EPlatForm.MissionList[nReadyId] or not EPlatForm.MissionList[nReadyId][nDynId] or EPlatForm.MissionList[nReadyId][nDynId]:IsOpen() ~= 1 then
      return 0
    end
  end
  ...
  if EPlatForm.MissionList[nReadyId] and EPlatForm.MissionList[nReadyId][nDynId] and EPlatForm.MissionList[nReadyId][nDynId]:IsOpen() == 1 and EPlatForm.MissionList[nReadyId][nDynId]:GetPlayerGroupId(me) >= 0 then
