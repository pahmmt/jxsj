...
function HuiHuangZhiGuo:MakeCitySeed(nCityIndex, nNowTime)
  ...
  if nGrowPhase == 0 then
    _G.GlobalExcute({
      ...
      0,
      Env.NEWSMSG_NORMAL,
      "<辉煌之种>已在各新手村外出现，5分钟后将结出果实。请各位大侠速去采摘。具体情况请到礼官处查询。",
    })
  end
  _G.GlobalExcute({
    "HuiHuangZhiGuo:GreatSeedExecute",
    tbConfig[nCityIndex]["nMapId"],
    tbConfig[nCityIndex]["nSeedLevel"],
    tbConfig[nCityIndex]["nCount"],
    nGrowPhase,
    nWetherMakeGoldFruitTime,
  })
end
