...
function NewLottery:AlterRoleName(szOldName, szNewName)
  -- award, lottery数据是在申请改名的时候就已经检查过了
  -- 这里主要是检查***相关的数据
  for nDate, szName in pairs(self.tbGoldPlayerNameYear or {}) do
    if szName == szOldName then
      self.tbGoldPlayerNameYear[nDate] = szNewName
    end
  end

  if self.tbNameScores and self.tbNameScores[szOldName] then
    self.tbNameScores[szNewName] = self.tbNameScores[szOldName]
    self.tbNameScores[szOldName] = nil
  end

  for nDate, szName in pairs(self.tbGoldPlayerName or {}) do
    if szName == szOldName then
      self.tbGoldPlayerName[nDate] = szNewName
    end
  end

  if self.tbGoldPlayerNameYear_CoSub and self.tbGoldPlayerNameYear_CoSub[szOldName] then
    self.tbGoldPlayerNameYear_CoSub[szNewName] = self.tbGoldPlayerNameYear_CoSub[szOldName]
    self.tbGoldPlayerNameYear_CoSub[szOldName] = nil
  end

  if self.tbStudioRoleList and self.tbStudioRoleList[szOldName] then
    self.tbStudioRoleList[szNewName] = self.tbStudioRoleList[szOldName]
    self.tbStudioRoleList[szOldName] = nil
  end
end

function NewLottery:CheckDataForAlter(nPlayerId, szName)
  local nRes = 0
  ...
  local nCurDate = tonumber(GetLocalDate("%Y%m%d"))
  local nSec = Lib:GetDate2Time(nLastDate)
  local nEndDate = tonumber(os.date("%Y%m%d", nSec))
  if nCurDate < nFirstDate or nCurDate > nEndDate then
    nRes = 0
  elseif self.tbLottery[szName] then
    nRes = 1
  end

  GlobalExcute({ "Player:OnLotteryDataCheckRet", nPlayerId, nRes })
end
...
function SpecialEvent:PayAwardScheduleTask()
  ...
  local szTitle = string.format("%s月感恩充值回馈", Lib:Transfer4LenDigit2CnNum(nMonth))
  local szContent = string.format(
    [[尊敬的各位武林侠客：
    《剑侠世界》<color=red>尊敬的各位侠客：
    双旦盛宴，《剑侠世界》年末真情放送：<color=red>充值票号<color>首现江湖，福利精活<color=red>双倍购买<color>、积分区最低<color=red>5折兑换、终极神兽<color>雪魂再现江湖、<color=red>金牛坐骑/绝世雪羽<color>史无前例。<color=yellow>六大特色礼包<color>幸福/极致/尊贵/荣耀/尊享/无价至尊大礼包囊括丰厚的福利回馈、方便实用的便利道具，更有超值的游戏增值货币（五行魂石/游龙古币等）以及高端珍品的购买资格。
    游戏内点击主界面右上角小地图下方按钮<color=yellow>福利特权<color>-><color=yellow>充值优惠奖励<color>即可进入 <link=openwnd:全新充值奖励领取界面,UI_PAYAWARD,1>，可领取豪华大礼，活动详情请点击%s月<link=url:充值专题,充值促销网页,http://jxsj.xoyo.com/zt/monthpromotion/%s/%s/index.html>。
