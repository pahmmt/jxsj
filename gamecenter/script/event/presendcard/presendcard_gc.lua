...
function PresendCard:QueryGlbBuf()
  ...
  for nType, tbDataInfo in pairs(tbInfo) do
    ...
    szMsg = szMsg .. (string.format("%s\t%d\t%s\t%s\t%s\t%s\t%s\n", szState, nType, tbDataInfo[self.INDEX_NAME], tbDataInfo[self.INDEX_STARTTIME], tbDataInfo[self.INDEX_ENDTIME], tbDataInfo[self.INDEX_CDKEYFLAG], szIndex))
  end
  return szMsg
end
...
function PresendCard:CompareKey(tbSrc, tbDes)
  if not tbSrc[self.INDEX_CDKEYFLAG] or not tbSrc[self.INDEX_KEYINDEX] or not tbDes[self.INDEX_CDKEYFLAG] or not tbDes[self.INDEX_KEYINDEX] then
    return 0
  end
