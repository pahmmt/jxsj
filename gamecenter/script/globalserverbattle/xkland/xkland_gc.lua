...
function Xkland:OnCompetitiveBidding_GA(szPlayerName, nCompetitive, szGateway, szTongName, nPlayerSex, nRight)
  ...
  -- 重新排序
  table.sort(self.tbCompetitiveBuffer, function(a, b)
    return a.nCompetitive > b.nCompetitive
  end)
...
function Xkland:WritePlayerListLog()
  ...
  local szChengZhuInfo = string.format("城主：%s\t网关：%s\t帮会：%s\t奖励箱子：%s\t英雄令牌：%s\t城主令牌：%s\n", self.tbCastleBuffer.szPlayerName, self.tbCastleBuffer.szGateway, self.tbCastleBuffer.szTongName, self.tbCastleBuffer.nCastleBox, self.tbCastleBuffer.nLingPai, self.tbCastleBuffer.nChengZhuLingPai)
