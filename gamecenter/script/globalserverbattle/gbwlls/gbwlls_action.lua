...
function GbWlls:OnPrayPlayer(nFlag)
  ...
  if not nFlag or nFlag ~= 1 then
    Dialog:Say(
      string.format(
        [[    当今武林英雄辈出，我服务器中有一批侠士也参加了跨服联赛的比试。
    跨服联赛预赛期间%s月7日-%s月27日，为他们加油，你将会得到<color=yellow>跨服联赛幸运卡<color>，幸运卡要在<color=yellow>当日22点后到次日15点前<color>打开，将随机抽取到一名报名跨服联赛的选手，只要该选手在今日比赛中获得过一场胜利则你会获得跨服联赛幸运宝箱，奖励丰厚。]],
        tbTime.month,
        tbTime.month
      ),
      {
  ...
  local szTime = string.format("%02d/%02d/%02d/%02d/%02d/%02d", tbTime.year, tbTime.month, tbTime.day + 1, 15, 0, 0)
