...
function SuperBattle:StartSignup_GA()
  ...
  -- 大区公告
  self:GlobalAnnounce_GA("万众瞩目的跨服战场：<color=green>梦回采石矶<color>已开启，武侠江湖再次风起云涌！")
...
function SuperBattle:StopSignup_GA()
  ...
  -- 大区公告
  self:GlobalAnnounce_GA("跨服战场：<color=green>梦回采石矶<color>已经停止报名了！")
...
function SuperBattle:Deamon_GA()
  ...
  if math.mod(self._nAnn, 60) == 1 then
    self:GlobalAnnounce_GA("跨服战场：<color=green>梦回采石矶<color>已开启，请各位玩家前往跨服战场接引人处报名参加！")
  end

  -- 过了报名期
  if self:CheckIsSignup() ~= 1 then
    for _, tbInfo in pairs(self.tbPlayerQueue_GA) do
      self:SendMessage_GA(tbInfo.szName, self.MSG_BOTTOM, "对不起，已经过了报名期，请下次再来吧。")
    end
    return 0
  end
...
-- 玩家改名
function SuperBattle:AlterRoleName(szOldName, szNewName)
  if self.IS_GLOBAL == 1 then
    self:LoadBuffer_GC()
    local bFind = 0
    for i, tbInfo in pairs(self.tbGlobalBuffer) do
      if tbInfo[1] == szOldName then
        tbInfo[1] = szNewName
        self.tbGlobalBuffer[i] = tbInfo
        bFind = 1
        break
      end
    end
    if bFind == 1 and MODULE_GC_SERVER then
      self:SaveBuffer_GC()
    end
  end
end
