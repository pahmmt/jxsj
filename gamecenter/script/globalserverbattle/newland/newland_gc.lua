...
function Newland:StartSignup_GA()
  ...
  -- 大区公告
  self:GlobalAnnounce_GA("<color=green>【跨服城战】开始报名（周四0点至周六19:30）<color><color=gold>请帮会首领找各大城市的铁浮城远征大将，代表本帮申请参战，请本帮成员及时登记！<color>")
...
function Newland:InitGame_GA()
  ...
  if #self.tbGroupBuffer < self.MIN_GROUP or #self.tbGroupBuffer > self.MAX_GROUP then
    self:GlobalAnnounce_GA("<color=green>【跨服城战】截止报名，由于粮缺将寡，本届城战无法进行。<color>")
    return 0
  end
  ...
  -- 大区公告
  self:GlobalAnnounce_GA(string.format("<color=green>【跨服城战】截止报名！<color=gold>全区共有%s个帮会争夺铁浮城！<color>", #self.tbGroupBuffer))
end
...
function Newland:EndGame_GA()
  ...
  for nGroupIndex, tbInfo in pairs(self.tbWarBuffer) do
    ...
    table.sort(self.tbSortPlayer[nGroupIndex], function(a, b)
  ...
  -- 大区公告
  local szMsg = string.format("<color=green>【跨服城战】结束！<color><color=gold>%s的<color=green>[%s]<color>帮会力压群雄，其首领<color=green>[%s]<color>成为新一代铁浮城城主！", ServerEvent:GetServerNameByGateway(tbWinGroup.szGateway), tbWinGroup.szTongName, tbWinGroup.szCaptainName)
...
function Newland:GetCastleAward_GA(szPlayerName)
  if szPlayerName == self.tbCastleBuffer.szCaptainName and self.tbCastleBuffer.nAward == 1 then
    ...
    self:SaveBuffer_GC(GBLINTBUF_NL_CASTLE)
    self:GlobalAnnounce_GA(string.format("<color=gold>铁浮城城主<color=green>[%s]<color>领取了城主奖励，辉煌战功箱：<color=green>%s个<color>，城主令牌：<color=green>%s个<color>，侍卫令牌：<color=green>%s个<color>", szPlayerName, self.CASTLE_BOX, self.CASTLE_PAD, self.NORMAL_PAD))
  else
...
function Newland:TimerAnnounce_GA()
  ...
  if nDay == 4 or nDay == 5 then
    if nTime > 2000 and nTime < 2400 then
      local szMsg = "<color=green>【跨服城战】开始报名（周四0点至周六19:30）<color><color=gold>请帮会首领找各大城市的铁浮城远征大将，代表本帮申请参战，请本帮成员及时登记！<color>"
      self:GlobalAnnounce_GA(szMsg)
    end
  elseif nDay == 6 then
    if nTime > 1500 and nTime < 1930 then
      local szMsg = "<color=green>【跨服城战】报名将于今日19:30截止！<color><color=gold>请帮会首领找各大城市的铁浮城远征大将，代表本帮申请参战。请本帮成员及时登记！<color>"
      self:GlobalAnnounce_GA(szMsg)
    elseif nTime > 1930 and nTime < 2000 then
      local szMsg = "<color=green>【跨服城战】擂鼓（19:30~19:59）<color><color=gold>请符合条件的帮中豪杰，从各大城市的铁浮城远征大将处进入英雄岛，找铁浮城接引人进入铁浮城。<color>"
      self:GlobalAnnounce_GA(szMsg)
    elseif nTime > 2000 and nTime < 2100 then
      local szMsg = "<color=green>【跨服城战】激战（20:00~21:00）<color><color=gold>各帮各派已经开始在铁浮城中激烈厮杀！<color>"
      self:GlobalAnnounce_GA(szMsg)
    end
  end
end
...
function Newland:WritePlayerListLog()
  ...
  local szChengZhuInfo = string.format("城主：%s\t网关：%s\t帮会：%s\t城主箱子：%s\t城主令牌：%s\t侍卫令牌：%s\n", self.tbCastleBuffer.szCaptainName, self.tbCastleBuffer.szGateway, self.tbCastleBuffer.szTongName, self.CASTLE_BOX, self.CASTLE_PAD, self.NORMAL_PAD)
