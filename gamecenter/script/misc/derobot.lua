...
function DeRobot:gc_NewServerKick(szHardWareId, szName)
  ...
  if nPlayerServer > 0 and GCNewServerMuiOpen(szName) == 1 then
    ...
    for _, tbInfo in ipairs(tbMuiInfo) do
      szLog = szLog .. string.format("{角色名:%s, 账号:%s, 登陆时间:%s}, ", tbInfo.szName, tbInfo.szAccount, os.date("%y-%m-%d %H:%M:%S", tbInfo.nLoginTime))
    end
    print(szLog)
  ...
  for _, pPlayer in pairs(tbPlayer) do
    ...
    if nType == 3 and nCount > 5 then
      ...
      if nDiff ~= 0 then
        ...
        pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_ANTIBOT_PROCESS, "使用外挂")
      end
    end
  end
end
...
if MODULE_GAMESERVER then
  ...
  PlayerEvent:RegisterGlobal("OnLoginOnly", "DeRobot:OnLoginDo")
end
