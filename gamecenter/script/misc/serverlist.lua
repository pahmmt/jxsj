...
ServerEvent.tbDefGlobalAreaName = {
  [1] = { "电信一号战区", DBTASK_GLOBAL_NEWLAND_CITYER1, "gate0100" },
  ...
  [8] = { "电信六号战区", DBTASK_GLOBAL_NEWLAND_CITYER8, "gate1200" },
  [9] = { "网通三号战区", DBTASK_GLOBAL_NEWLAND_CITYER9, "gate1300" },
}
  ...
  for szZone, tbServer in pairs(tbNameList) do
    ...
    for szServer, szGateWay in pairs(tbServer) do
      ...
      if tbGateList[szGateWay] then
        tbGateList[szGateWay].nTransferId = 14 --默认14
      end
    end
  end
...
--设置战区标志，以此来做奖励。
function ServerEvent:SetGlobalTaskAreaId(nAreaId)
  if not GLOBAL_AGENT then
    return 0
  end
  local nOrgAreaId = GetGlobalSportTask(6, 11)
  local nFlag = SetGlobalSportTask(6, 11, nAreaId)
  return "执行：" .. nFlag .. ", 原战区：" .. nOrgAreaId .. ", 现战区：" .. nAreaId
end

--获得战区标志，以此来做奖励标志。
function ServerEvent:GetGlobalTaskAreaId()
  return GetGlobalSportTask(6, 11) or 0
end
