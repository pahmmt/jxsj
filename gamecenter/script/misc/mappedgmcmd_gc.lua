...
GmCmd.tbEyesKey2GcFun = {
  ...
  ["CancelJbReturn"] = { "szPlayerName", "nIsPass" }, --取消内部优惠
  ...
  ["ReadOldPlayerIniviteList"] = { "szPath" }, --读取邀请老玩家关联数据
}
...
function GmCmd:QueryServerInfo()
  ...
  local szMsg = string.format("%s,%s,%s,%s,%s,%s,%s,%s", GetGatewayName(), GetMaxPlayerId(), os.date("%Y-%m-%d", KGblTask.SCGetDbTaskInt(16)), os.date("%Y-%m-%d", nOpen150), Wlls:GetMacthSession(), KGblTask.SCGetDbTaskInt(DBTASK_DOMAIN_BATTLE_NO), Xkland:GetSession(), (tbXklandPreiod[nXLandPreiod] or "未知"))
...
function GmCmd:BatchArrest(tbParam)
  local tbData = Lib:LoadTabFile(tbParam.szDataPath)
  if not tbData then
    return "文件读取错误"
  end
  local nLoadCount = 0
  for _, tbRow in ipairs(tbData) do
    if tbRow.RoleName and tbRow.JailTime and GetGatewayName() == tbRow.GatewayID then
      local nPlayerId = KGCPlayer.GetPlayerIdByName(tbRow.RoleName)
      if nPlayerId then
        if tonumber(tbParam.bArrest) == 1 then
          GM:AddOnLine("", "", tbRow.RoleName, 0, 0, string.format([[Player:Arrest("%s", %s)]], tbRow.RoleName, tonumber(tbRow.JailTime)))
        else
          GM:AddOnLine("", "", tbRow.RoleName, 0, 0, string.format([[Player:SetFree("%s")]], tbRow.RoleName))
        end
...
-- 取消内部优惠（申请取消或角色转账号为正常玩家）
function GmCmd:CancelJbReturn(tbParam)
  local szPlayerName = tbParam.szPlayerName
  local nIsPass = tonumber(tbParam.nIsPass)
  local szScript = [[me.SetTask(2056, 27, 1)]]
  if nIsPass == 0 then
    szScript = [[Account:ApplySetIntValue(me.szAccount, "Account.ApplyCancelTime", 0, 0); me.Msg("你的请求不符合要求，申请失败。")]]
  end
  return GM:AddOnLine("", "", szPlayerName, 0, 0, szScript)
end
...
function GmCmd:QueryIbshopBuffInfo()
  ...
  for _, tbSubCmd in pairs(tbIbShopCmd) do
    ...
    if IbShop.tbPreloadWareInfo[nWareId] then
      ...
      local szOut = string.format("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", szState, szCurrencyType, nWareId, nDiscount, nDiscountStart, nDiscountClose, szTimeSaleStart or "", szTimeSaleClose or "", szWareName)
...
function GmCmd:AddNewBatchMail(tbParam)
  return Mail.BatchMail:AddIntoGblBuf(tbParam.szTitle, tbParam.szContent, tonumber(tbParam.nEndTime))
end
...
function GmCmd:GetEventCompensate()
  ...
  local szMsg = string.format("\n已有活动:%s\n开始时间:%s\n结束时间:%s\n描述信息:%s", tbBuf[24].szName or "无", tbBuf[24].nStartDate, tbBuf[24].nEndDate, tbBuf[24].szDesc or "无")
  return szMsg
end
...
function GmCmd:ReadOldPlayerIniviteList(tbParam)
  local szPath = tbParam.szPath or ""
  SpecialEvent.tbZhouNianQing2012:ReadInVitePlayerList(szPath)
end
