...
GCEvent.OUTPUTSWITCH = {
  ...
  ["AutoTeam:RemovePlayer"] = 1,
  ["AutoTeam:SyncTeamDataToOneClient"] = 1,
  ["SetPlayerHonor"] = 1,
  ["Relation:AddFriendFavor_GC"] = 1,
  ["Relation:ApplyPlayerRelationGroup"] = 1,
  ["Player.tbGlobalFriends:gc_OnPlayerLogin"] = 1,
}
...
function GCEvent:OnCoZoneStepSubZone(szSubGcDb)
  ...
  if not GLOBAL_AGENT then
    ...
    self.tbCoZoneKeyimenKinBuf = GetGblIntBuf(GBLINTBUF_KEYIMEN_KIN, 0)
    self.tbCoZoneKeyimenTongBuf = GetGblIntBuf(GBLINTBUF_KEYIMEN_TONG, 0)
  end
...
function GCEvent:CoZoneUpdateKeyimenBuf()
  Keyimen:CoZoneUpdateKeyimenBuf(self.tbCoZoneKeyimenKinBuf or {}, self.tbCoZoneKeyimenTongBuf or {})
end
...
--注册全局数据查询回调（这里的都不要直接用，请使用script\golbalstat\golbalstat.lua）
--szPrimeKey	大类
--szSudKey	小类
--fnStartFun	回调函数
function GCEvent:RegisterGlobalStatEventCallBack(szPrimeKey, szSudKey, fnStartFun, ...)
  if not self.tbGlobalStatFun then
    self.tbGlobalStatFun = {}
  end
  self.tbGlobalStatFun[szPrimeKey] = self.tbGlobalStatFun[szPrimeKey] or {}
  self.tbGlobalStatFun[szPrimeKey][szSudKey] = { fun = fnStartFun, tbParam = arg }
end

--跨区全局统计查询回调接口
function GCEvent:OnGlobalStatQueryResult(nId, tblData)
  if not self.tbGlobalStatQueryList then
    self.tbGlobalStatQueryList = {}
  end
  if not self.tbGlobalStatQueryListEx then
    self.tbGlobalStatQueryListEx = {}
  end
  --没有数据就return掉
  if not tblData or #tblData <= 0 then
    return
  end

  self.tbGlobalStatQueryList[nId] = tblData
  if type(tblData) ~= "table" then
    return
  end
  for i, tb in ipairs(tblData) do
    self.tbGlobalStatQueryListEx[tb.szPrimCategory] = self.tbGlobalStatQueryListEx[tb.szPrimCategory] or {}
    self.tbGlobalStatQueryListEx[tb.szPrimCategory][tb.szSecCategory] = nId
  end
  local szPrimeKey = tblData[1].szPrimCategory
  local szSudKey = tblData[1].szSecCategory
  if self.tbGlobalStatFun[szPrimeKey] and self.tbGlobalStatFun[szPrimeKey][szSudKey] then
    local tbCallBack = { self.tbGlobalStatFun[szPrimeKey][szSudKey].fun, unpack(self.tbGlobalStatFun[szPrimeKey][szSudKey].tbParam) }
    table.insert(tbCallBack, tblData)
    Lib:CallBack(tbCallBack)
  end
end

--通过id查寻全局统计数据
function GCEvent:OnGlobalStatGetDate4Id(nId)
  if not nId then
    return
  end
  if self.tbGlobalStatQueryList then
    return self.tbGlobalStatQueryList[nId]
  end
  return
end

--通过key查寻全局统计数据
function GCEvent:OnGlobalStatGetDate4Key(szPrimeKey, szSubKey)
  if not szPrimeKey or not szSubKey then
    return
  end
  if self.tbGlobalStatQueryList and self.tbGlobalStatQueryListEx and self.tbGlobalStatQueryListEx[szPrimeKey] and self.tbGlobalStatQueryListEx[szPrimeKey][szSubKey] then
    return self.tbGlobalStatQueryList[self.tbGlobalStatQueryListEx[szPrimeKey][szSubKey]]
  end
  return
end
