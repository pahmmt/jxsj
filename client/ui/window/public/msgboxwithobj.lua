...
local TXT_COIN_WARMING = "txtCoinWarming"
...
----------------------------------------------------------------------------------------
--                MsgBox通关攻略
-- 调用方式： UiManager:OpenWindow(Ui.UI_MSGBOXWITHOBJ, tbMsg, ...);
-- tbMsg参数详解：
-- tbMsg.szMsg		必填	显示的消息
-- tbMsg.tgObj		必填	显示的obj
-- tbMsg.szObjName必填	显示的obj名字
-- tbMsg.szObjEdt必填
-- tbMsg.bObjEdt必填
...
-- 2010/11/2 17:52:56 xuantao添加
-- tbMsg.szWarmingTxt 可选  显示一条警告消息，会遮挡住szObjEdtTxt和编辑框
----------------------------------------------------------------------------------------
...
function tbMsgBoxWithObj:OnOpen(tbMsg, ...)
  ...
  if tbMsg.szObjEdt ~= nil and tbMsg.szObjEdt ~= "" then
    Edt_SetTxt(self.UIGROUP, OBJ_EDIT, tbMsg.szObjEdt) -- 设置标题文字
  end
  if tbMsg.bObjEdt == 1 then
    ...
    if UiVersion == Ui.Version001 then
      ...
      Img_SetImage(self.UIGROUP, "Main", 1, "\\image\\ui\\002a\\msgboxwithobj\\bg_msgboxwithobj_01.spr")
    end
  else
    ...
    if UiVersion == Ui.Version001 then
      ...
      Img_SetImage(self.UIGROUP, "Main", 1, "\\image\\ui\\002a\\msgboxwithobj\\bg_msgboxwithobj_02.spr")
    end
  end

  if tbMsg.szObjEdtTxt ~= nil and tbMsg.szObjEdtTxt ~= "" then
    Txt_SetTxt(self.UIGROUP, "TxtObjEdt", tbMsg.szObjEdtTxt) -- 设置标题文字
  end

  if tbMsg.szObjName ~= "" and tbMsg.szObjName ~= nil then
    Txt_SetTxt(self.UIGROUP, OBJ_NAME, tbMsg.szObjName) -- 设置标题文字
  end

  if tbMsg.szWarmingTxt and tbMsg.szWarmingTxt ~= "" then
    Txt_SetTxt(self.UIGROUP, TXT_COIN_WARMING, tbMsg.szWarmingTxt) -- 设置警告文字
    if UiVersion == Ui.Version001 then
      Img_SetImage(self.UIGROUP, "Main", 1, "\\image\\ui\\001a\\auction\\comput_dlg.spr")
    else
      Img_SetImage(self.UIGROUP, "Main", 1, "\\image\\ui\\002a\\msgboxwithobj\\bg_msgboxwithobj_02.spr")
    end
  end
