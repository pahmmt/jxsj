...
function uiTaskPanel:OnOutLookItemSelected(szWndName, nGroupIndex, nItemIndex)
  if nGroupIndex == 0 then
    ...
    if me.nLevel >= 90 then
      --local nTimes = me.GetTask(2043,1);
      --local nWeekGutTask = me.GetTask(1024,52);
      --local nWeekDailyTask = me.GetTask(1024,51);
      --szInfo = szInfo.."本日已经进入副本次数：<color=Gold>"..nTimes.."<color=White>/4\n"
      --szInfo = szInfo.."本周完成的剧情任务：<color=Gold>"..nWeekGutTask.."<color=White>/4\n"
      --szInfo = szInfo.."本周完成的日常任务：<color=Gold>"..nWeekDailyTask.."<color=White>/28\n\n"
      szInfo = szInfo .. "<color=Gold>获取军营声望的途径：<color=White>\n	读兵法，参与剧情副本。\n<color=Gold>获取机关学造诣和机关耐久度的途径：<color=White>\n	读机关书，参与日常副本。\n\n<color=White>"

      local tbCampList = Task:GetMaxLevelCampTaskInfo(me)
      if tbCampList and #tbCampList > 0 then
        szInfo = szInfo .. "<color=Gold>当前可接的军营任务：<color=White>\n"
      end
    ...
    if tbRangeDesc then
      szInfo = szInfo .. "<color=Gold>等级段描述：<color=White>\n"
    ...
    if tbInfoList and #tbInfoList > 0 then
      szInfo = szInfo .. "<color=Gold>当前可接的主线剧情：<color=White>\n"
    ...
    if tbTaskListInfo and #tbTaskListInfo > 0 then
      szInfo = szInfo .. "<color=Gold>当前可接的世界任务：<color=White>\n"
    end
...
function uiTaskPanel:InitData()
  ...
  AddOutLookItem(self.UIGROUP, self.OUTLOOK_TASK_LIST, 0, { "任务向导" })
