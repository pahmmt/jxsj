...
uiTeamPortrait.PORTRAIT_WND = -- 控件句柄表
  {
    { "ImgMember1", "ImgPortrait1", "ImgPartBlood1", "ImgPartPower1", "TxtName1", "ImgFaction1", "TxtLevel1", "BtnInteract1", "BtnGo1", "Img_KinBadge1" },
    { "ImgMember2", "ImgPortrait2", "ImgPartBlood2", "ImgPartPower2", "TxtName2", "ImgFaction2", "TxtLevel2", "BtnInteract2", "BtnGo2", "Img_KinBadge2" },
    { "ImgMember3", "ImgPortrait3", "ImgPartBlood3", "ImgPartPower3", "TxtName3", "ImgFaction3", "TxtLevel3", "BtnInteract3", "BtnGo3", "Img_KinBadge3" },
    { "ImgMember4", "ImgPortrait4", "ImgPartBlood4", "ImgPartPower4", "TxtName4", "ImgFaction4", "TxtLevel4", "BtnInteract4", "BtnGo4", "Img_KinBadge4" },
    { "ImgMember5", "ImgPortrait5", "ImgPartBlood5", "ImgPartPower5", "TxtName5", "ImgFaction5", "TxtLevel5", "BtnInteract5", "BtnGo5", "Img_KinBadge5" },
  } -- [1]为PlayerID
...
uiTeamPortrait.FACTION_FLAG = {
  [Env.FACTION_ID_NOFACTION] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\wumenpai.spr",
  [Env.FACTION_ID_SHAOLIN] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\shaolin.spr",
  [Env.FACTION_ID_TIANWANG] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\tianwang.spr",
  [Env.FACTION_ID_TANGMEN] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\tangmen.spr",
  [Env.FACTION_ID_WUDU] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\wudu.spr",
  [Env.FACTION_ID_EMEI] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\ermei.spr",
  [Env.FACTION_ID_CUIYAN] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\cuiyan.spr",
  [Env.FACTION_ID_GAIBANG] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\gaibang.spr",
  [Env.FACTION_ID_TIANREN] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\tianren.spr",
  [Env.FACTION_ID_WUDANG] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\wudang.spr",
  [Env.FACTION_ID_KUNLUN] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\kunlun.spr",
  [Env.FACTION_ID_MINGJIAO] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\mingjiao.spr",
  [Env.FACTION_ID_DALIDUANSHI] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\duanshi.spr",
  [Env.FACTION_ID_GUMU] = "\\image\\ui\\002a\\teamportrait\\imgportrait2\\gumu.spr",
}
...
uiTeamPortrait.IMG_PORTRAIT = {
  [Env.SEX_MALE] = "image\\ui\\002a\\teamportrait\\icon_male.spr",
  [Env.SEX_FEMALE] = "image\\ui\\002a\\teamportrait\\icon_female.spr",
}

uiTeamPortrait.tbQiankunfuSet = {
  { 18, 1, 85, 1 },
  { 18, 1, 91, 1 },
}
  ...
  if szWndName == self.CLOSE_BUTTON then
    ...
    for i = 1, #self.PORTRAIT_WND do
      if self.PORTRAIT_WND[i][8] == szWndName then
        ...
        return
      elseif self.PORTRAIT_WND[i][9] == szWndName then
        local tbMember = me.GetTeamMemberInfo()
        if tbMember[i] and tbMember[i].nPlayerID then
          me.CallServerScript({ "UseQiankunfuTrans", tbMember[i].nPlayerID })
        end
      end
    end
  end
...
function uiTeamPortrait:GetMemberBaseData()
  ...
  for i = 1, #self.PORTRAIT_WND do
    if tbMember[i] and tbMember[i].szName and tbMember[i].nPlayerID then
      ...
      if tbMember[i].nKinBadge and tbMember[i].nKinBadge > 0 and Kin.tbKinBadge[tbMember[i].nKinBadge] then
        Img_SetImage(self.UIGROUP, self.PORTRAIT_WND[i][10], 1, Kin.tbKinBadge[tbMember[i].nKinBadge].OtherBadge)
      end
    else
      ...
      if UiVersion ~= Ui.Version001 then
        ...
        Wnd_Hide(self.UIGROUP, self.PORTRAIT_WND[i][10])
      end
...
function uiTeamPortrait:CheckHaveQiankunfu()
  local tbFind = {}
...
function uiTeamPortrait:OnMouseEnter(szWnd)
  if szWnd == self.ZHENFA_WND[1] then
    ...
    for i = 1, #self.PORTRAIT_WND do
      if szWnd == self.PORTRAIT_WND[i][9] then
        local tbQianKunFuSet = {
          { 18, 1, 85, 1 },
          { 18, 1, 91, 1 },
        }
        local tbFind = {}
        for i = 1, #tbQianKunFuSet do
          local tbTempFind = me.FindItemInBags(unpack(tbQianKunFuSet[i]))
          Lib:MergeTable(tbFind, tbTempFind)
        end
        local szMsg = ""
        if not tbFind or #tbFind <= 0 then
          szMsg = "<color=red>您当前无乾坤符暂不能使用队友传送功能。\n<color><color=green>当月累计充值50元可免费领取乾坤符（10次）一个。<color>"
        else
          szMsg = "瞬间传到队友身边，一个乾坤符可传送10次。"
        end
...
-- Bổ sung 2 helper functions (giống như các file khác)
