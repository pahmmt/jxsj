...
uiExpTask.NUM_PERPAGE_EVR = 6 --每页显示多少个
...
function uiExpTask:UpdateTask()
  ...
  --显示物品
  for i = 1, self.NUM_PERPAGE_EVR do
    ...
    if self.FLAG_VIEW == 1 then
      if self.nCurCmbIndex == 0 then
        if not self.tbTask[nIndex] then
          ...
          Txt_SetTxt(self.UIGROUP, self.PAGE_SET[i][self.Txt_TaskInfo], string.format("%s%s%s%s", Lib:StrFillR(self.tbTask[nIndex][3] .. "个", 9), Lib:StrFillR((self.tbTask[nIndex][4] / 2) .. "星", 16), Lib:StrFillR(self:CalculateAword(self.tbTask[nIndex][1], self.tbTask[nIndex][3], self.tbTask[nIndex][4]) .. IVER_g_szBindCoinName_ST, 16), Lib:StrFillR(self:CalculateTime(self.tbTask[nIndex][5]), 16)))
        end
      else
        if not self.tbOtherTask[self.nCurCmbIndex][nIndex] then
          ...
          Txt_SetTxt(self.UIGROUP, self.PAGE_SET[i][self.Txt_TaskInfo], string.format("%s%s%s%s", Lib:StrFillR(tbTask_One[1] .. "个", 9), Lib:StrFillR((tbTask_One[2] / 2) .. "星", 16), Lib:StrFillR(self:CalculateAword(self.nCurCmbIndex, tbTask_One[1], tbTask_One[2]) .. IVER_g_szBindCoinName_ST, 16), Lib:StrFillR(self:CalculateTime(tbTask_One[3]), 16)))
        end
      end
      ...
      if not self.tbMyTask[nIndex] then
        ...
        Txt_SetTxt(self.UIGROUP, self.PAGE_SET[i][self.Txt_TaskInfo], string.format("%s%s%s%s", Lib:StrFillR(self.tbMyTask[nIndex][3] .. "个", 9), Lib:StrFillR((self.tbMyTask[nIndex][4] / 2) .. "星", 16), Lib:StrFillR(self:CalculateAword(self.tbMyTask[nIndex][1], self.tbMyTask[nIndex][3], self.tbMyTask[nIndex][4]) .. IVER_g_szBindCoinName_ST, 16), Lib:StrFillR(self:CalculateTime(self.tbMyTask[nIndex][5]), 16)))
      end
  ...
  local nCurCoin = 0
  local szMsg = string.format("剩余发布点数：<color=yellow>%s<color>  您目前拥有的%s数：<color=yellow>%s<color>  您发布的任务数：<color=yellow>%s<color>", me.GetTask(2130, 4), IVER_g_szCoinName, me.nCoin, #self.tbMyTask)
  if IVER_g_nSdoVersion == 1 then
    szMsg = string.format("剩余发布点数：<color=yellow>%s<color>  您发布的任务数：<color=yellow>%s<color>", me.GetTask(2130, 4), #self.tbMyTask)
  end
...
function uiExpTask:OnButtonClick(szWnd, nParam)
  ...
  elseif szWnd == self.BtnGBListCountSort then
    ...
    if self.nCountSort == 0 then
      Img_SetImage(self.UIGROUP, self.BtnGBListCountSort, 1, "\\image\\ui\\002a\\common\\btn_combo.spr")
    else
      Img_SetImage(self.UIGROUP, self.BtnGBListCountSort, 1, "\\image\\ui\\002a\\common\\btn_combo_up.spr")
    end
  ...
  elseif szWnd == self.BtnGBListXingSort then
    ...
    if self.nXingSort == 0 then
      Img_SetImage(self.UIGROUP, self.BtnGBListXingSort, 1, "\\image\\ui\\002a\\common\\btn_combo.spr")
    else
      Img_SetImage(self.UIGROUP, self.BtnGBListXingSort, 1, "\\image\\ui\\002a\\common\\btn_combo_up.spr")
    end
  ...
  elseif szWnd == self.BtnGBListBonusSort then
    ...
    if self.nCoinSort == 0 then
      Img_SetImage(self.UIGROUP, self.BtnGBListBonusSort, 1, "\\image\\ui\\002a\\common\\btn_combo.spr")
    else
      Img_SetImage(self.UIGROUP, self.BtnGBListBonusSort, 1, "\\image\\ui\\002a\\common\\btn_combo_up.spr")
    end
