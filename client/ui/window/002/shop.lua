...
uiShop.TXT_CURRENCY = "TxtCurrency"
...
uiShop.BUY_WIDTH = 5
uiShop.BUY_HEIGHT = 8
uiShop.DETECT_INTERVAL = 18 -- 检测货币
...
function tbBuyCont:SwitchObj(nX, nY)
  ...
  elseif UiManager:GetUiState(UiManager.UIS_TRADE_REPAIR_EX) == 1 then
    ...
    if nShiftKey >= 0 then
      Tutorial:ShopObjClick(self.szUiGroup, self.szObjGrid, nX, nY)
...
function uiShop:OnOpen()
  ...
  --新手任务指引
  Tutorial:CheckSepcialEvent("shop", { tbShopItems })
  self.nUpdateCurrencyTimer = Timer:Register(self.DETECT_INTERVAL, self.Timer_UpdateCurrency, self)
end

function uiShop:OnClose()
  if self.nUpdateCurrencyTimer then
    Timer:Close(self.nUpdateCurrencyTimer)
  end
  local nShopId = me.GetShopId()
...
function uiShop:UpdateCurrency(nGoodsId)
  if not nGoodsId or self.nPageRecycle == 1 then
    Wnd_Hide(self.UIGROUP, self.TXT_CURRENCY)
    return
  end
  local szUnitName, nCount = Shop:GetCurrencyInfo(nGoodsId)
  if not szUnitName then
    Wnd_Hide(self.UIGROUP, self.TXT_CURRENCY)
  else
    Wnd_Show(self.UIGROUP, self.TXT_CURRENCY)
    Txt_SetTxt(self.UIGROUP, self.TXT_CURRENCY, szUnitName .. "：" .. Item:FormatMoney(nCount))
  end
end

function uiShop:Timer_UpdateCurrency()
  if UiManager:WindowVisible(Ui.UI_SHOP) ~= 1 then
    return 0
  end
  if self.nPageRecycle == 1 then
    return
  end
  local tbShopItems = me.GetShopItemList()
  local nItemCount = #tbShopItems
  if nItemCount > 0 then
    self:UpdateCurrency(tbShopItems[1].nId)
  else
    self:UpdateCurrency()
  end
end
