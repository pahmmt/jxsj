  ...
  if tbClass.CalcEnhanceAttrib then -- TODO: zyh: 很龊的方法……暂时先这样做
    ...
    if nUnLockIntervalTimes ~= 0 and it.GetLockIntervale() ~= 0 then
      if nUnLockIntervalTimes == -1 then
        szTip = szTip .. "\n\n<color=green>交易锁定状态，不能交易"
      elseif nUnLockIntervalTimes - nNowTime > 0 then
        szTip = szTip .. "\n\n<color=green>交易锁定状态，不能交易\n已申请解锁，将于" .. os.date("%Y/%m/%d %H:%M:%S", nUnLockIntervalTimes) .. "自动解锁<color>"
      end
    end
...
function Item:Tip_GetRefineLevel(nState)
  ...
  elseif nRefineLevel > 0 then
    return szTip .. "炼化<color=gold>" .. nRefineLevel .. "<color>级"
  else
    return szTip .. "不可炼化"
  end
end
...
function Item:Tip_GetCastInfo(nState)
  ...
  if it.IsExEquip() == 1 then
    ...
    if nCastLevel == 0 then
      szTip = it.nEquipCategory == 0 and " 不可精铸" or " 未精铸"
  ...
  elseif nState == Item.TIPS_GOODS and nGoodsId and (KItem.IsGetBindType(nGoodsId) == 1 or nCurrencyType == 7) then
    ...
    elseif (nState ~= Item.TIPS_PREVIEW) and (1 == it.IsBind()) then -- 属性预览状态时只按照nBindType显示
      ...
      if it.nGenre == 24 then
        szTip = szTip .. "（<color=greenyellow>可解绑<color>）"
      end
...
function Item:Tip_FightPower(nState) -- 获得Tip字符串：战斗力
  ...
  if nFightPower then
    ...
    szTip = string.format("\n<color=blue>战斗力 +%g<color>", nFightPower)
    if it.szClass == "zhenyuan" then
      local varRank = Item.tbZhenYuan:GetRank(it)
      if varRank == 0 then
        varRank = "未上榜"
      end
      szTip = szTip .. string.format("<color=blue>   排名：%s<color>", varRank)
    end
  ...
  if nState == Item.TIPS_SHOP then -- NPC交易状态显示物品价格
    ...
    if pGoodsId then
      ...
      if nBuyPlayerLevel > 0 then
        ...
        szTip = szTip .. string.format("\n<color=%s>玩家角色等级到达%d级<color>", szColor, nBuyPlayerLevel)
      end
      ...
      if nFightPowerRankNeed > 0 then
        ...
        szTip = szTip .. string.format("\n<color=%s>战斗力总排名达到前%d名<color>", szColor, nFightPowerRankNeed)
      end
    else
      ...
      if tbWareInfo and tbWareInfo.nDiscount > 0 then
        ...
        if tbWareInfo.nDiscountType == 0 then
          ...
          elseif tbWareInfo.nCurrencyType == 3 then
            szTemp = tbWareInfo.nDiscount .. "消费积分"
          else
            assert(false)
          end
        end
...
function Item:Tip_Timeout() -- 超时时间
  ...
  if tbAbsTime then
    local strTimeout = string.format("<color=yellow>此物品的使用时间至：%d年%d月%d日%d时%d分。<color>", tbAbsTime[1], tbAbsTime[2], tbAbsTime[3], tbAbsTime[4], tbAbsTime[5])
    szTip = "\n\n" .. szTip .. strTimeout
  else
    local tbRelTime = me.GetItemRelTimeout(it)
    if tbRelTime then
      local strTimeout = string.format("<color=yellow>此物品剩余的使用时间为：%d天%d时%d分。<color>", tbRelTime[1], tbRelTime[2], tbRelTime[3])
      szTip = "\n\n" .. szTip .. strTimeout
    end
  end
...
function Item:ChangeEquipToFac(varEquip, nFaction)
  ...
  if pEquip.nGenre == tbGDPL[1] and pEquip.nDetail == tbGDPL[2] and pEquip.nParticular == tbGDPL[3] and pEquip.nLevel == tbGDPL[4] then
    return 1
  end
  return pEquip.Regenerate(tbGDPL[1], tbGDPL[2], tbGDPL[3], tbGDPL[4], pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, pEquip.GetGenInfo(), 0, pEquip.dwRandSeed, pEquip.nStrengthen)
end
...
function Item:CheckCanChangable(pEquip)
  ...
  if not tbSetting.tbChange or not tbSetting.tbChange[nId] then
    return
  end
