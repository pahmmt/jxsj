...
Item.STRENGTHEN_TYPE_DIS = {
  -- 武器
  [1] = { Item.EQUIP_MELEE_WEAPON, Item.EQUIP_RANGE_WEAPON },
  -- 防具
  [2] = { Item.EQUIP_ARMOR, Item.EQUIP_BOOTS, Item.EQUIP_BELT, Item.EQUIP_HELM, Item.EQUIP_CUFF },
  -- 饰品
  [3] = { Item.EQUIP_RING, Item.EQUIP_NECKLACE, Item.EQUIP_AMULET, Item.EQUIP_PENDANT },
}

-- by zhangjinpin@kingsoft
Item.TASK_PEEL_APPLY_GID = 2085 -- 高强化装备剥离申请任务变量
Item.TASK_PEEL_APPLY_TIME = 1 -- 申请高强化剥离的时间
Item.VALID_PEEL_TIME = 3 * 60 * 60 -- 有效的剥离时间
Item.MAX_PEEL_TIME = 11 * 60 * 60 -- 完整的申请到消失时间
...
function Item:ApplyPeelHighEquipSure()
  -- SkillId : 1358
  if 1 == me.AddSkillState(1358, 1, 2, self.MAX_PEEL_TIME * Env.GAME_FPS, 1, 0, 1) then
...
function Item:Enhance(pEquip, tbEnhItem, nMoneyType, nParam) -- 程序接口：服务端执行装备强化
  ...
  if pEquip.nEnhTimes >= 11 then
    ...
    szMsg = "您的好友[" .. me.szName .. "]以" .. nProb .. "%的成功率将" .. szTypeName
  end
  ...
  if MathRandom(100) > nProb then
    ...
    if pEquip.nEnhTimes >= 11 then
      me.SendMsgToFriend(szMsg .. "强化到+" .. pEquip.nEnhTimes + 1 .. "时遗憾失败了。")
      Player:SendMsgToKinOrTong(me, "以" .. nProb .. "%的成功率将" .. szTypeName .. "强化到+" .. pEquip.nEnhTimes + 1 .. "时遗憾失败了。", 0)
    end
  ...
  if 1 ~= nRet then
    ...
    if pEquip.nEnhTimes >= 11 then
      me.SendMsgToFriend(szMsg .. "强化到+" .. pEquip.nEnhTimes + 1 .. "时遗憾失败了。")
      Player:SendMsgToKinOrTong(me, "在将" .. szTypeName .. "强化到+" .. pEquip.nEnhTimes + 1 .. "时遗憾失败了。", 0)
      self:FinishEnhanceAchievement(pEquip.nEnhTimes + 1, nProb, 0)
      --失败数据埋点
      if pEquip.nEnhTimes >= 4 then
        local szLevel = tostring(pEquip.nLevel)
  ...
  if pEquip.nEnhTimes >= 12 then
    me.SendMsgToFriend("您的好友[" .. me.szName .. "]以" .. nProb .. "%的成功率将" .. szTypeName .. "成功强化到+" .. pEquip.nEnhTimes .. "。")
    Player:SendMsgToKinOrTong(me, "以" .. nProb .. "%的成功率将" .. szTypeName .. "成功强化到+" .. pEquip.nEnhTimes .. "。", 0)
  end
  --新手强化任务
...
function Item:Peel(pEquip, nParam) -- 程序接口：服务端执行玄晶剥离
  ...
  for nLevel, nNum in pairs(tbPeelItem) do
    for i = 1, nNum do
      local pItem = me.AddItemEx(PEEL_ITEM.nGenre, PEEL_ITEM.nDetail, PEEL_ITEM.nParticular, nLevel, { bForceBind = 1 }, Player.emKITEMLOG_TYPE_UNENHANCE) -- 获得玄晶
    end
  end
...
function Item:BreakupXuanjing(pItem, nParam)
  ...
  if pItem.szClass ~= Item.STRENGTHEN_STUFF_CLASS or pItem.nLevel < NEEDLEVEL_MIN or pItem.nLevel > NEEDLEVEL_MAX or pItem.IsBind() ~= 1 then
  ...
  if pPlayer.CountFreeBagCell() < nNum then
    pPlayer.Msg(string.format("您的背包空间不足，您需要%s个空间格子。", nNum))
    return 0
  end
