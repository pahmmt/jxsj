...
Item.tbExchangeMatchTable = { --particular	声望大类，小类
...
function Item:ExChangeLongHun_CheckItemValid(pItem)
  ...
  local nMatchParticular = pItem.nParticular
  if pItem.szClass ~= "longhun_piece" or not self.tbExchangeMatchTable[nMatchParticular] then
    me.Msg("只能放入龙魂声望物品。")
    return 0
  end
...
function Item:ExChangeLongHun_OnOK(tbItemObj, nStep)
  ...
  if nStep == 0 then
    ...
    local tbOpt = {
      { "确定", self.ExChangeLongHun_OnOK, self, tbItemList, 1 },
      { "取消" },
    }
