...
function Item:CostRecastCurrency(nValue, nType)
  ...
  if nType == self.emEQUIP_RECAST_CURRENCY_MONEY then
    if jbreturn:IsPermitIp(me) == 1 and jbreturn:GetMonLimit(me) > 0 then
...
function Item:EquipRecast(tbRecastItem, nMoneyType, nIndex)
  ...
  if pOldEquip.IsEquip() == 1 and (pOldEquip.nDetail < Item.MIN_COMMON_EQUIP or pOldEquip.nDetail > Item.MAX_COMMON_EQUIP) then
