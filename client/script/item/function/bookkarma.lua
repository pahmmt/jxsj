  ...
  for _, nSkill in ipairs(tbSkill) do
    if nSkill > 0 then
      ...
      if nNewLevel ~= nOldLevel then
        pPlayer.Msg(string.format("恭喜你的技能<color=green>[%s]<color>等级提升至<color=green>%d<color>级", KFightSkill.GetSkillName(nSkill), nNewLevel))
        if nNewLevel == KFightSkill.GetSkillMaxLevel(nSkill) then
          pPlayer.Msg(string.format("恭喜您的坐骑技能<color=green>[%s]<color>已经升至最高级<color=green>%d<color>级，之后修炼无法继续升级", KFightSkill.GetSkillName(nSkill), nNewLevel))
        end
      end
    end
  end
