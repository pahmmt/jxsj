...
tbItem.NORMAL_ZHONGJIZHENFA_LIST = {
  { "中级阵法图：八卦阵·离", 1, 15, 2, 1 },
  { "中级阵法图：八卦阵·兑", 1, 15, 3, 1 },
  { "中级阵法图：八卦阵·艮", 1, 15, 4, 1 },
  { "中级阵法图：八卦阵·坎", 1, 15, 5, 1 },
  { "中级阵法图：八卦阵·巽", 1, 15, 6, 1 },
  { "中级阵法图：八卦阵·乾", 1, 15, 7, 1 },
  { "中级阵法图：青龙阵", 1, 15, 8, 1 },
  { "中级阵法图：玄武阵", 1, 15, 9, 1 },
  { "中级阵法图：白虎阵", 1, 15, 10, 1 },
  { "中级阵法图：朱雀阵", 1, 15, 11, 1 },
...
tbItem.NORMAL_GAOJIZHENFA_LIST = {
  { "高级阵法图：八卦阵·离", 1, 15, 2, 3 },
  { "高级阵法图：八卦阵·兑", 1, 15, 3, 3 },
  { "高级阵法图：八卦阵·艮", 1, 15, 4, 3 },
  { "高级阵法图：八卦阵·坎", 1, 15, 5, 3 },
  { "高级阵法图：八卦阵·巽", 1, 15, 6, 3 },
  { "高级阵法图：八卦阵·乾", 1, 15, 7, 3 },
  { "高级阵法图：青龙阵", 1, 15, 8, 3 },
  { "高级阵法图：玄武阵", 1, 15, 9, 3 },
  { "高级阵法图：白虎阵", 1, 15, 10, 3 },
  { "高级阵法图：朱雀阵", 1, 15, 11, 3 },
...
tbItem.HIGH_GAOJIZHENFA_LIST = {
  { "高级阵法图：月迷津渡阵", 1, 15, 12, 3 },
  { "高级阵法图：雾失楼台阵", 1, 15, 13, 3 },
  { "高级阵法图：贪狼阵·阳", 1, 15, 14, 3 },
  { "高级阵法图：贪狼阵·阴", 1, 15, 15, 3 },
  { "高级阵法图：八卦阵·震", 1, 15, 16, 3 },
  { "高级阵法图：八卦阵·坤", 1, 15, 17, 3 },
  { "高级阵法图：破军阵", 1, 15, 18, 3 },
  { "高级阵法图：七杀阵", 1, 15, 19, 3 },
}
...
function tbItem:OnUse()
  ...
  local tbOpt = {}
  tbOpt[#tbOpt + 1] = { "更换阵法图", self.ReplaceBook, self, me.nId, it.dwId }
  tbOpt[#tbOpt + 1] = { "取出阵法图", self.SelectBook, self, me.nId, it.dwId }
  tbOpt[#tbOpt + 1] = { "放回阵法图", self.DeleteBook, self, me.nId, it.dwId }
  tbOpt[#tbOpt + 1] = { "取消" }
  Dialog:Say(szMsg, tbOpt)
end

function tbItem:SelectBook(nPlayerId, nBoxId)
  local szMsg = "请选择您要取出的阵法图种类:"
  local tbOpt = {}
  ...
  if self:IsHighLevelBook(pItem.nGenre, pItem.nDetail, pItem.nParticular, pItem.nLevel) == 1 then
    tbOpt[#tbOpt + 1] = { "高级阵法图", self.HandleGiveZhenFaKind, self, self.HIGH_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "中级阵法图", self.HandleGiveZhenFaKind, self, self.NORMAL_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "初级阵法图", self.HandleGiveZhenFaKind, self, self.PRIMER_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
  else
    tbOpt[#tbOpt + 1] = { "中级阵法图", self.HandleGiveZhenFaKind, self, self.NORMAL_ZHONGJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "初级阵法图", self.HandleGiveZhenFaKind, self, self.PRIMER_ZHONGJIZHENFA_LIST, nPlayerId, nBoxId }
  end
  tbOpt[#tbOpt + 1] = { "取消" }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbItem:HandleGiveZhenFaKind(tbZhenFaList, nPlayerId, nBoxId)
  ...
  for i = 1, #tbZhenFaList do
    local szTip = "<item=" .. tbZhenFaList[i][2] .. "," .. tbZhenFaList[i][3] .. "," .. tbZhenFaList[i][4] .. "," .. tbZhenFaList[i][5] .. ">"
...
function tbItem:ReplaceBook(nPlayerId, nBoxId)
  ...
  if self:IsHighLevelBook(pItem.nGenre, pItem.nDetail, pItem.nParticular, pItem.nLevel) == 1 then
    tbOpt[#tbOpt + 1] = { "高级阵法图", self.HandleReplaceZhenFaKind, self, self.HIGH_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "中级阵法图", self.HandleReplaceZhenFaKind, self, self.NORMAL_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "初级阵法图", self.HandleReplaceZhenFaKind, self, self.PRIMER_GAOJIZHENFA_LIST, nPlayerId, nBoxId }
  else
    tbOpt[#tbOpt + 1] = { "中级阵法图", self.HandleReplaceZhenFaKind, self, self.NORMAL_ZHONGJIZHENFA_LIST, nPlayerId, nBoxId }
    tbOpt[#tbOpt + 1] = { "初级阵法图", self.HandleReplaceZhenFaKind, self, self.PRIMER_ZHONGJIZHENFA_LIST, nPlayerId, nBoxId }
  end
  tbOpt[#tbOpt + 1] = { "取消" }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbItem:HandleReplaceZhenFaKind(tbZhenFaList, nPlayerId, nBoxId)
  local szMsg = "请选择您要更换的阵法图，选择后将您当前装备的阵法图更换为新的阵法图:"
  local tbOpt = {}
  if not tbZhenFaList then
    return
  end
...
function tbItem:ChangeBook(nPlayerId, nBookId, nBoxId, tbZhenFaList)
  ...
  if not pOldItem or self:CheckItem(pOldItem.nGenre, pOldItem.nDetail, pOldItem.nParticular, pOldItem.nLevel, nIsHighBook) ~= 0 then
    ...
    local nResult = pOldItem.Regenerate(tbZhenFaList[nBookId][2], tbZhenFaList[nBookId][3], tbZhenFaList[nBookId][4], tbZhenFaList[nBookId][5], pOldItem.nSeries, pOldItem.nEnhTimes, pOldItem.nLucky, pOldItem.GetGenInfo(), pOldItem.nVersion, pOldItem.dwRandSeed, pOldItem.nStrengthen)
    if nResult ~= 1 then
      me.Msg("更换阵法图失败!")
      return
    end
    if tbTimeOut then
      local szTime = string.format("%02d/%02d/%02d/%02d/%02d/10", tbTimeOut[1], tbTimeOut[2], tbTimeOut[3], tbTimeOut[4], tbTimeOut[5])
...
function tbItem:DeleteBook(nPlayerId, nBoxId)
  ...
  Dialog:OpenGift("请放入要放回的阵法图", nil, { self.OnOpenGiftOk, self, nIsHighBook })
end
