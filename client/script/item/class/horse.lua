...
function tbHores:CalcEnhanceAttrib(nState)
  local pTemp = KItem.CreateTempItem(it.nGenre, it.nDetail, it.nParticular, it.nLevel, it.nSeries, it.nEnhTimes + 1, it.nLucky, it.GetGenInfo(), 0, it.dwRandSeed, it.nIndex)
...
function tbHores:GetTip(nState, tbEnhRandMASS, tbEnhEnhMASS)
  ...
  if szBaseAttrib and szBaseAttrib ~= "" then
    szTip = szTip .. "\n\n<color=blue>装备获得属性：<color>" .. szBaseAttrib
  end
