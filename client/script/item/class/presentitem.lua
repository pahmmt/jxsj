...
function tbItem:CheckItem(pItem, pPlayer, tbitem)
  if tbitem.nBindMoney ~= 0 and pPlayer.GetBindMoney() + tbitem.nBindMoney > pPlayer.GetMaxCarryMoney() then
    return 0, "您身上的绑定银两将达上限，请先整理身上的绑定银两。"
  end

  if tbitem.nMoney ~= 0 and pPlayer.nCashMoney + tbitem.nMoney > pPlayer.GetMaxCarryMoney() then
