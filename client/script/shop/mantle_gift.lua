...
function tbGift:OnSwitch(pPickItem, pDropItem, nX, nY)
  if pDropItem then
    ...
    if tbMantle == nil or type(tbMantle) ~= "table" or tbMantle.nStore <= 0 or tbMantle.nTimeOut <= 0 or tbMantle.nGenre ~= 1 or tbMantle.nDetail ~= 17 then
...
function tbGift:OnOK()
  ...
  if me.CountFreeBagCell() < nNeedBag then
    me.Msg(string.format("您的背包空间不足，您需要%s个空间格子。", nNeedBag))
    return 0
  end
