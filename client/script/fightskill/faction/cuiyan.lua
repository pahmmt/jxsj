...
local tb = {
  ...
  bingxinqianying = { --冰心倩影_20
    ...
    state_knock_resisttime = { { { 1, 26 }, { 20, 260 }, { 21, 286 } } },
...
function tbSkill:GetAutoDesc2(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("被攻击<color=gold>%s次<color>后解除本状态\n<color=gold>%s秒<color>内的多次攻击只会被当做1次攻击", tbAutoInfo.nCastCount, FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  --[[local tbMsg = {};
	local szMsg = "";
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("生命降到25%%时触发几率：<color=gold>%d%%<color>\n最多同时影响目标：<color=gold>%d个<color>\n造成周围对手冻结的几率：<color=gold>%s%%<color>，持续<color=gold>%s秒<color>\n自身所有抗性：<color=gold>增加%s<color>，持续<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, tbChildInfo.nMissileHitcount, tbChildInfo.tbWholeMagic["state_freeze_attack"][1], FightSkill:Frame2Sec(tbChildInfo.tbWholeMagic["state_freeze_attack"][2]), tbCCInfo.tbWholeMagic["damage_all_resist"][1], FightSkill:Frame2Sec(tbCCInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("\n<color=green>子招式<color>\n造成会心一击时触发几率：<color=gold>%d%%<color>\n对手移动速度<color=gold>降低%d%%<color>，持续<color=gold>%s秒<color>\n自身<color=gold>免疫受伤、迟缓、定身、麻痹和击退效果<color>，持续<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, -tbChildInfo.tbWholeMagic["fastwalkrun_p"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime), FightSkill:Frame2Sec(tbChildInfo2.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
