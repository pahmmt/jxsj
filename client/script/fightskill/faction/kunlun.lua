...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("\n<color=green>子招式<color>\n生命降到25%%时触发几率：<color=gold>%d%%<color>\n最多同时影响目标：<color=gold>%d个<color>\n造成击退的几率：<color=gold>%s%%<color>，击退距离<color=gold>%s<color>\n自身移动速度：<color=gold>增加%s%%<color>\n受到五行伤害：<color=gold>缩小%s%%<color>\n持续时间：<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, tbCCInfo.nMissileHitcount, tbCCInfo.tbWholeMagic["state_knock_attack"][1], tbCCInfo.tbWholeMagic["state_knock_attack"][2] * tbCCInfo.tbWholeMagic["state_knock_attack"][3], tbChildInfo.tbWholeMagic["fastwalkrun_p"][1], tbChildInfo.tbWholeMagic["redeivedamage_dec_p2"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetExtraDesc(tbInfo)
  ...
  local szMsg = string.format("\n技能命中敌人时自身获得以下效果：\n<color=green>两仪化形<color>\n    发挥基础攻击力：<color=Gold>提高%d%%<color>\n    发挥技能攻击力：<color=Gold>提高%d%%<color>", tbCCInfo1.tbWholeMagic["skilldamageptrim"][1], tbCCInfo1.tbWholeMagic["skillselfdamagetrim"][1])
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  szMsg = szMsg .. string.format("每<color=Gold>%s秒<color>第一次攻击时<color=gold>%s%%<color>释放<color=green>[%s]<color>", FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime), tbAutoInfo.nPercent, szSkillName2)
  return szMsg
end
