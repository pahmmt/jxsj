...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("生命降到25%%时触发几率：<color=gold>%d%%<color>\n自身免疫受伤、迟缓、眩晕、定身状态\n每半秒生命回复：<color=gold>%s%%*生命上限<color>\n持续时间：<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, math.floor(tbChildInfo.tbWholeMagic["replenishlifebymaxhp_p"][1] / 10), FightSkill:Frame2Sec(tbChildInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("\n<color=green>子招式<color>\n被攻击命中时触发几率：<color=gold>%d%%<color>\n最多同时影响目标：<color=gold>%d个<color>\n造成攻击者定身的几率：<color=gold>%d%%<color>，持续<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, tbChildInfo.nMissileHitcount, tbChildInfo.tbWholeMagic["state_fixed_attack"][1], FightSkill:Frame2Sec(tbChildInfo.tbWholeMagic["state_fixed_attack"][2]), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
