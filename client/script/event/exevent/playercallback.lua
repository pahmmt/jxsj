...
function tbPlayerCallBack:OldPlayer_CheckOldPlayerFriend(pMe)
  if pMe.nLevel < 80 then
    return 0, "您的等级不够，必须达到<color=red>80级<color>才可以和老侠客建立召回关系。", { { "我知道了（离开）" } }
  end

  if self:IsOpen(pMe, 1) == 1 then
    return 0, "你是重出江湖的侠客，不能召回其他侠客！", { { "我知道了（离开）" } }
  end
  ...
  if (not tbPlayerId) or not tbTeamMemberList or #tbTeamMemberList ~= 2 or Lib:CountTB(tbPlayerId) ~= 2 then
    return 0, "哪位是要召回的侠客呢？只有与<color=red>他单独组队并在附近<color>，才能召回他。", { { "我知道了（离开）" } }
  end
  ...
  if nFlag ~= 1 then
    return 0, "哪位是要召回的玩家呢？你的附近没有队友，不能建立召回关系。", { { "我知道了（离开）" } }
  end

  if self:IsOpen(pFriend, 1) == 0 then
    return 0, "你的好友不是重出江湖的老侠客！", { { "我知道了（离开）" } }
  end

  if pFriend.nLevel < 79 then
    return 0, "你的好友未到79级！", { { "我知道了（离开）" } }
  end

  -- 表示无法召回
  if self:CheckRelation(pFriend) ~= 0 then
    return 0, "你的好友已经和其他人建立关系，不能重复建立！", { { "我知道了（离开）" } }
  end

  --所有的条件满足，送出祝福
  return 1, "你的好友<color=red>" .. pFriend.szName .. "<color>和你建立了召回关系！", { { "谢谢您（离开）" } }, pFriend
end
