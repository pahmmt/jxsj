...
ActiveGift.nMaxMonthActive = 1310 --最大累计1310点活跃度
ActiveGift.nMaxActiveGrade = 75 --最大累计75点活跃度
...
ActiveGift.tbNameWeek = { [1] = "星期一", [2] = "星期二", [3] = "星期三", [4] = "星期四", [5] = "星期五", [6] = "星期六", [7] = "星期日" }
...
ActiveGift.tbAwardName = { [3] = "白银宝箱", [4] = "黄金宝箱", [5] = "翡翠宝箱" }
--数据表
...
function ActiveGift:LoadActiveAwardFile()
  ...
  if not tbInfo then
    print("文件不存在:" .. self.szActiveAwardFile)
    return
  end
  for nRow, tbRowData in pairs(tbInfo) do
...
function ActiveGift:LoadMonthAwardFile()
  ...
  if not tbInfo then
    print("文件不存在:" .. self.szMonthAwardFile)
    return
  end
  for nRow, tbRowData in pairs(tbInfo) do
    local nId = tonumber(tbRowData.nId) or 0
...
function ActiveGift:CheckCanAddActive(nId)
  ...
  if me.nLevel > tbActive.nDownLevel and tbActive.nDownLevel > 0 then
    return 0, "等级大于" .. tbActive.nDownLevel .. "级"
  end
  if me.nLevel < tbActive.nUpLevel and tbActive.nUpLevel > 0 then
    return 0, "等级不足" .. tbActive.nUpLevel .. "级"
  end
  ...
  if TimeFrame:GetServerOpenDay() < tbActive.nTimerFrame and tbActive.nTimerFrame > 0 then
    return 0, "开服不足" .. tbActive.nTimerFrame .. "天"
  end
  return 1
end
...
function ActiveGift:GetCustomItem(pPlayer, value)
  ...
  if not Task.tbCustomEquip[value] or not Task.tbCustomEquip[value][nFaction] or not Task.tbCustomEquip[value][nFaction][nRoute] or not Task.tbCustomEquip[value][nFaction][nRoute][nSex] then
    return
  end
...
function ActiveGift:CheckCanGetAward(nType, nId)
  if nType == 1 then --检查活跃度
    ...
    if self:CheckBit(nGetIndex, nId) then
      return 0, "您已经领取过这份奖励了。"
    end
    if self.tbActiveAward[nId].nActive > nTotalGrade then
      return 0, "您当前活跃度不足<color=yellow>" .. self.tbActiveAward[nId].nActive .. "<color>点。"
    end
  else
    local nMonth = me.GetTask(self.nTaskGroupId, self.nMonthActiveTime)
    local nMonthActiveGrade = me.GetTask(self.nTaskGroupId, self.nMonthActiveGrade)
    ...
    if self.tbMonthAward[nId].nActive > nMonthActiveGrade then
      return 0, "您当月活跃度不足<color=yellow>" .. self.tbMonthAward[nId].nActive .. "<color>点。"
    end
  end
  return 1
end
