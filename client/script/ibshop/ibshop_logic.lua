...
function IbShop:WriteLog(szVoucher, nRes, tbRet)
  ...
  if 0 == Lib:CountTB(tbRet) then
    ...
    for _, tbInfo in pairs(tbRet) do
      ...
      local tbBaseProp = KItem.GetItemBaseProp(tbWareInfo.nGenre, tbWareInfo.nDetailType, tbWareInfo.nParticular, tbWareInfo.nLevel)
      local szWareName = tbBaseProp.szName
      szLog = szLog .. string.format("对 %s 个 %s 打百分之 %s 的折扣", tbInfo[2], szWareName, tbInfo[3])
    end
  end
