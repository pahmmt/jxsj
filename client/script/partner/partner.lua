...
if MODULE_GAMESERVER then
  ...
  function Partner:UpgradeSkill(pPartner)
    ...
    me.Msg(string.format("恭喜，您的同伴%s的[%s]技能等级提升到%d！", pPartner.szName, KFightSkill.GetSkillName(tbSkill.nId), tbSkill.nLevel))
    self:SendClientMsg(string.format("技能[%s]等级提升到%d级！", KFightSkill.GetSkillName(tbSkill.nId), tbSkill.nLevel))
  ...
  function Partner:CalToUpgradeSkill(pPartner)
    ...
    if #tbSkillIndex == 0 then
      local szMsg = string.format("您的同伴%s已经将所有技能都升满了，不需要再升了！", pPartner.szName)
      return 0, szMsg, 1
    end

    if MathRandom(1, 100) > nTalentLevel then
      local szMsg = "很遗憾，同伴没有提升技能等级，等再升5级后再碰运气吧。"
      return 0, szMsg, 2
    end
  ...
  function Partner:AddExp(pPartner, nExp)
    ...
    if nCurLevel >= self.MAXLEVEL then
      local szMsg = string.format("您的同伴已经达到最大等级，将不能再获得经验了！")
      return 0, szMsg
    end
    ...
    if nCurExp >= nStoreMax then
      local szMsg = string.format("您的同伴%s经验累计已到最大值，为了避免经验丢失，请先该同伴提升等级！", pPartner.szName)
      return 0, szMsg
    end
    ...
    elseif nCurExp < Partner.tbLevelSetting[nCurLevel].nExp and nActualExp >= Partner.tbLevelSetting[nCurLevel].nExp and nCurLevel < self.MAXLEVEL then
  ...
  function Partner:AddTalent(pPartner, nNewLevel, nPointRemain)
    ...
    if nTalentLevel >= self.TALENT_MAX and nNewLevel >= nTalentLevel then
      szMsg = string.format("您的同伴%s已经心领神会，无需再提高领悟度！", pPartner.szName)
      return 0, szMsg
    end

    if nNewLevel < nTalentLevel then -- 领悟度降低了
      if nNewLevel <= self.TAlENT_MIN then
        szMsg = string.format("您的同伴%s心力劳损，领悟度下降%d！已降到最低，不会再降。", pPartner.szName, nTalentLevel - nNewLevel)
      else
        szMsg = string.format("您的同伴%s心力劳损，领悟度下降%d！", pPartner.szName, self.TALENT_DECREASE)
      end
    elseif nNewLevel > nTalentLevel then -- 领悟度提高了
      szMsg = string.format("您的同伴%s的领悟度提高到%d。", pPartner.szName, nNewLevel)
    end
  ...
  function Partner:AddFriendship(pPartner, nAddFriendship)
    ...
    if nFriendship >= self.FRIENDSHIP_MAX and nAddFriendship > 0 then
      local szMsg = string.format("您和您的同伴%s已经亲密无间，无需再提高亲密度。", pPartner.szName)
      return 0, szMsg
    end
    ...
    if nAddFriendship > 0 then
      szMsg = string.format("您和您的同伴%s的亲密度增加到%0.2f。", pPartner.szName, nNewFriendship / 100)
    end
      ...
      if pPartner then
        ...
        if bAdd ~= 0 and (EventManager.IVER_bOpenPartnerExpLimit == 0 or (EventManager.IVER_bOpenPartnerExpLimit == 1 and pPartner.nSkillCount < 5)) then
          ...
          if nRes == 1 then
            ...
            szMsg = string.format("您的同伴%s经验增加了%d", pPartner.szName, nExpAdded)
          end
  ...
  function Partner:CreatePartner(nPlayerId, dwNpcId, nItemId)
    ...
    if MathRandom(1, 100) > nRate then
      ...
      pPlayer.Msg("很遗憾，说服失败，帖子留着下次再用吧！")
      self:SendClientMsg("很遗憾，说服失败，帖子留着下次再用吧！")
    ...
    pPlayer.Msg("恭喜！您可以与这位新同伴一起闯荡江湖了！")
  ...
  function Partner:TurnPartnerToItem(pPlayer, pPartner)
    ...
    if pPartner.GetValue(self.emKPARTNERATTRIBTYPE_LEVEL) > 1 then
      self:SendClientMsg("只有1级的同伴才能转化成道具！")
      pPlayer.Msg("只有1级的同伴才能转化成道具！")
      return 0
    end

    -- 先判断玩家是否能成功添加道具，能添加才将同伴转成道具
    if pPlayer.CountFreeBagCell() < 1 then
      self:SendClientMsg("您的背包空间不足，请先整理！")
      pPlayer.Msg("您的背包空间不足，请先整理！")
      return 0
    end
  ...
  function Partner:SavePartnerInfoToGenInfo(pPlayer, pPartner)
    ...
    if self:DissolveConfirm(pPlayer.nId, pPartner.nPartnerIndex, 1) ~= 1 then
      Dbg:WriteLog("[TurnPartnerToItem]", string.format("%s将同伴%s(%d,%d,%d)放入同伴推荐信失败！", pPlayer.szName, szPartnerName, nPartnerTempId, nSkillCount, nPotentialTemp))
      return
    end

    -- 先添加道具，道具（稚嫩的同伴）添加成功后再删除同伴
    -- n级稚嫩的同伴道具对应n+1个技能个数的同伴
    local pItem = pPlayer.AddItem(self.tbChildPartner.nGenre, self.tbChildPartner.nDetail, self.tbChildPartner.nParticular, nSkillCount - 1)
    if not pItem then
      return 0
    end
  ...
  function Partner:TurnItemToPartner(pPlayer, nPartnerTempId, nPotentialTemp, tbSkillId)
    ...
    if pPlayer.nPartnerCount >= pPlayer.nPartnerLimit then
      pPlayer.Msg("您的同伴列表已满，不能获得新同伴！")
      self:SendClientMsg("您的同伴列表已满，不能获得新同伴！")
      return 0
    end
    ...
    local szMsg = string.format("你获得了同伴%s", szName)
    pPlayer.Msg(szMsg)
    self:SendClientMsg(szMsg)
  ...
  function Partner:RandomPotentialTemp(pPlayer)
    ...
    if nRandom > 0 then
      Dbg:WriteLog("同伴Log:", pPlayer.szName, string.format("潜能模板随机数超出范围！！模板权重总和为%d，随机数为%d", nRateSum, nRandom + nRateSum))
  ...
  function Partner:OnCallPartner(nPlayerId)
    ...
    if pPartner.GetValue(self.emKPARTNERATTRIBTYPE_FRIENDSHIP) >= self.FRIENDSHIP_SHUYUAN then
      ...
      pNpc.SetTitle(string.format("%s的同伴", pPlayer.szName))
  ...
  function Partner:DecreaseFriendship(nPlayerId)
    ...
    if pPartner.GetValue(self.emKPARTNERATTRIBTYPE_FRIENDSHIP) < self.FRIENDSHIP_SHUYUAN then
      ...
      local szMsg = string.format("您与同伴%s的亲密度不足40，同伴不愿再出手相助！", pPartner.szName)
      pPlayer.Msg(szMsg)
      self:SendClientMsg(szMsg)
    end
  end
  ...
  function Partner:RegisterPartnerTimer(pPlayer)
    ...
    if not Partner.bClosePartnerTimer and not pPlayer.GetTempTable("Partner").nPartnerTimer then
      pPlayer.GetTempTable("Partner").nPartnerTimer = Timer:Register(self.EFFECTTIME * 60 * Env.GAME_FPS, self.OnCallPartner, self, pPlayer.nId)
...
function Partner:TryToPersuade(pPlayer, pNpc, nItemLevel)
  ...
  if pPlayer.nLevel < self.PERSUADELEVELLIMIT then -- 100级以后才可以说服同伴
    return 0, "等你成为100级的大侠后，再去寻找自己的同伴吧！"
  end
  ...
  if nPlayerMapId ~= nNpcMapId then
    Dbg:WriteLog("Partner", string.format("玩家%s试图去说服跟自己不在同一地图的NPC成为自己的同伴！！！", pPlayer.szName))
    return 0, "使用道具失败！"
  end
  ...
  if pPlayer.nPartnerCount >= pPlayer.nPartnerLimit then
    local szMsg = string.format("人生得一知己足矣，您已经有多位同伴了，莫贪心。")
    -- 目前先不给玩家能买同伴槽的信息提示
