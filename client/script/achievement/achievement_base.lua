    ...
    for nSubGroupId, tbSubInfo in pairs(tbGroupInfo) do
      for nIndex, tbInfo in pairs(tbSubInfo) do
        ...
        if nCondType ~= 0 and nCondIndex ~= 0 then
          if self.tbCondInfo and self.tbCondInfo[nCondType] and self.tbCondInfo[nCondType][nCondIndex] then
...
function Achievement:GetAchievementInfo(nGroupId, nSubGroupId, nIndex)
  ...
  if not self.tbAchievementInfo[nGroupId] or not self.tbAchievementInfo[nGroupId][nSubGroupId] or not self.tbAchievementInfo[nGroupId][nSubGroupId][nIndex] then
    return
  end
