...
function Npc:OnDialog(szClassName, szParam)
  ...
  if EventManager.IVER_bOpenTiFu == 1 then
    if him.nTemplateId == 3570 then
      ...
      nTaskFlag = 1
      table.insert(tbOpt, 1, { "<color=yellow>体服体验角色获取<color>", tbTiFu.OnDialog, tbTiFu })
    end
  end
  local nSpecFlag = 0
  ...
  if nEventFlag == 1 or nTaskFlag == 1 or nSpecFlag == 1 or nCeilFlag == 1 then
    ...
    if nEventFlag == 1 and nTaskFlag == 1 and nSpecFlag == 1 then
      szMsg = string.format("%s：你来得正好，我这里有些活动和任务要你做。", him.szName)
      szMsg2 = "我不想参加活动和任务"
    elseif nEventFlag == 1 then
      szMsg = string.format("%s：你来得正好，我这里有些活动要你做。", him.szName)
      szMsg2 = "我想问问其他事"
    elseif nTaskFlag == 1 then
      szMsg = string.format("%s：你来得正好，我这里有些任务要你做。", him.szName)
      szMsg2 = "我不想做任务"
  ...
  for _, nItemId in pairs(tbLoseItem.Item or {}) do
    ...
    if pItem then
      ...
      if tbSystemMsgItem[szItem] then
        KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, string.format("【%s】被击杀，在临死前遗留了一个很珍贵的东西【%s】", pNpc.szName, pItem.szName))
      end
    end
