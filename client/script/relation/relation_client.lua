...
Relation.MAX_NUMBER_FRIEND = 300
----------------------------
...
function Relation:OnAddRelation(szPlayer, nType)
  ...
  if (nType == Player.emKPLAYERRELATION_TYPE_BIDFRIEND or nType == Player.emKPLAYERRELATION_TYPE_TMPFRIEND) and nNum and self.MAX_NUMBER_FRIEND <= nNum then
    me.Msg("你的好友人数已达上限。")
    return
  end
...
function Relation:DelRelation(szPlayer, nType)
  ...
  if tbDel.nType == Player.emKPLAYERRELATION_TYPE_TRAINED or tbDel.nType == Player.emKPLAYERRELATION_TYPE_TRAINING or tbDel.nType == Player.emKPLAYERRELATION_TYPE_BUDDY or tbDel.nType == Player.emKPLAYERRELATION_TYPE_INTRODUCTION or tbDel.nType == Player.emKPLAYERRELATION_TYPE_COUPLE then
...
Relation.tbGroupId2Index = {
  [1] = 8,
}

function Relation:ReceivePlayerRelationGroup_Client(nGroupId, tbGroupList)
  local tbRelationGroup = {}
  local tbTempGroup = {}

  tbRelationGroup.nIndex = self.tbGroupId2Index[nGroupId]
  tbRelationGroup.szName = tbGroupList.szGroupName
  for _, tbName in pairs(tbGroupList.tbGroup) do
    tbTempGroup[tbName.szPlayerName] = 1
  end
  tbRelationGroup.tbGroup = tbTempGroup

  me.Relation_AddRelationGroup(nGroupId, tbRelationGroup)
  CoreEventNotify(UiNotify.emCOREEVENT_RELATION_UPDATEPANEL)
end

function Relation:SyncRelationGroupState_Client(tbGroupState)
  if not tbGroupState then
    return 0
  end
  for nGroupId, nState in pairs(tbGroupState) do
    if nState == 0 then
      me.Relation_DelRelationGroup(nGroupId)
    end
  end
  CoreEventNotify(UiNotify.emCOREEVENT_RELATION_UPDATEPANEL)
end

function Relation:AddGroupPlayer_Client(nGroupId, szAddPlayerName)
  local tbGroup = me.Relation_GetRelationGroupList(nGroupId)
  if not tbGroup.tbGroup then
    tbGroup.tbGroup = {}
  end
  tbGroup.tbGroup[szAddPlayerName] = 1
  CoreEventNotify(UiNotify.emCOREEVENT_RELATION_UPDATEPANEL)
end

function Relation:DelRelationGroupPlayer_Client(nGroupId, szDelName)
  local tbGroup = me.Relation_GetRelationGroupList(nGroupId)
  if tbGroup.tbGroup then
    tbGroup.tbGroup[szDelName] = nil
  end
  CoreEventNotify(UiNotify.emCOREEVENT_RELATION_UPDATEPANEL)
end
