...
Faction.tbSeriesRec = { -- {camp, class}
  { 4, 1 }, -- 家族关卡
  { 2, 1 }, -- 宋金扬州
  { 2, 2 }, -- 宋金凤翔
  { 2, 3 }, -- 宋金襄阳
...
function Faction:__RestoreFaction(pPlayer, nFactionId)
  ...
  pPlayer.ApplyAssignPotential(pPlayer.GetTask(nFactionGroupId, self.tbPotential2TaskId.nBaseStrength) - pPlayer.nBaseStrength, pPlayer.GetTask(nFactionGroupId, self.tbPotential2TaskId.nBaseDexterity) - pPlayer.nBaseDexterity, pPlayer.GetTask(nFactionGroupId, self.tbPotential2TaskId.nBaseVitality) - pPlayer.nBaseVitality, pPlayer.GetTask(nFactionGroupId, self.tbPotential2TaskId.nBaseEnergy) - pPlayer.nBaseEnergy)
end
...
function Faction:IsCanSwitchFaction(pPlayer, nFactionId)
  if nFactionId == Env.FACTION_ID_GUMU then
    local nOrgFaction = self:GetOriginalFaction(pPlayer)
    -- 如果主修门派是古墓派就让他切
    if nOrgFaction == Env.FACTION_ID_GUMU then
      return 1
    end
    if pPlayer.GetTask(2027, 265) ~= 1 then
      --如果没有特殊辅修古墓资格，则根据服务器开关判断
    ...
    if nLevel < self.GUMU_FRIEND_REPUTE_CAN_FUXIU_LEVEL then
      return 0, string.format("古墓派为特殊辅修门派，古墓友好度达到%s级之后才能切换至此门派，可前往古墓派寻找苏清泠提升古墓友好度。", self.GUMU_FRIEND_REPUTE_CAN_FUXIU_LEVEL)
    end
  end
  return 1
end
...
function Faction:SwitchFaction(pPlayer, nFactionId)
  ...
  if
    pPlayer.nMapId ~= 255 -- 洗髓岛无cd限制
    and pPlayer.GetSkillState(self.SWITCH_FACTION_BUF_ID) > 0
  then
    return 0, "切换门派需要调息，目前时间未到，尚不能切换。"
  end
  ...
  if pPlayer.nMapId ~= 255 then
    szMsg = "你已成功切换为%s，在30分钟后才能再度切换。获得义军、白虎堂、宋金战场、家族关卡声望时，与你当前五行相同的部分声望才会增加。"
  else
    szMsg = "你已成功切换为%s。"
  end
...
function Faction:CheckGenreFaction(pPlayer, nFactionGenre, nFactionId)
  ...
  if self:__GetFactionUseIndex(pPlayer, nFactionId) > 0 or nCurrFactionId == nFactionId then
    return 0, "你已经修炼该门派"
  end
  ...
  if nFactionId == Env.FACTION_ID_GUMU then
    ...
    if pPlayer.nLevel < 100 then
      return 0, string.format("辅修古墓派需要达到100级！")
    end

    --		local nLevel = pPlayer.GetReputeLevel(self.GUMU_FRIEND_REPUTE_CAMP, self.GUMU_FRIEND_REPUTE_CLASS);
    --		if (nLevel < self.GUMU_FRIEND_REPUTE_CAN_FUXIU_LEVEL) then
    --			return 0, string.format("您的古墓好友度还没达到%s级，不能辅修古墓派！", self.GUMU_FRIEND_REPUTE_CAN_FUXIU_LEVEL);
    --		end
  end
...
function Faction:ChangeGenreFaction(pPlayer, nFactionGenre, nFactionId)
  ...
  self:WriteLog(Dbg.LOG_INFO, "ChangeGenreFaction", pPlayer.szName, nFactionGenre, nCurrFactionId, nFactionId)
...
function Faction:IsOpenGumuZhuxiu()
  if EventManager.IVER_nAutoOpenGuMuFaction == 1 then
    return 1
  end
