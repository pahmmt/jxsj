  ...
  if self.tbTaskDatas[nTaskId] and self.tbTaskDatas[nTaskId].tbAttribute["Repeat"] then
    if self:CanAcceptRepeatTask() ~= 1 then
      if pSharedPlayer then
        pSharedPlayer.Msg(me.szName .. "今日可接的重复任务已达上线，不能接受您的共享")
      end
...
-------------------------------------------------------------------------
-- 接到客户端确认接受
--接受任务时bAccept为1，拒绝时为0
function Task:OnAccept(nTaskId, nReferId, bAccept)
  if self.tbTaskDatas[nTaskId] and not self.tbTaskDatas[nTaskId].tbAttribute["Repeat"] and Task:HaveDoneSubTask(me, nTaskId, nReferId) == 1 then
    me.Msg("您已经完成过此任务！")
  ...
  if self.tbTaskDatas[nTaskId].tbAttribute["Repeat"] then
    if self:CanAcceptRepeatTask() ~= 1 then
      Dialog:SendInfoBoardMsg(me, "重复任务每天只能接10次！")
      return
    end
  end
  ...
  if bAccept == 0 then
    if pSharedPlayer then
      pSharedPlayer.Msg(me.szName .. "拒绝了你共享的任务" .. self.tbTaskDatas[nTaskId].szName)
    end
  ...
  if pSharedPlayer then
    pSharedPlayer.Msg(me.szName .. "接受了你共享的任务" .. self.tbTaskDatas[nTaskId].szName)
  end
...
function Task:DoAccept(nTaskId, nReferId)
  ...
  if not nSaveGroup then
    Dialog:SendInfoBoardMsg(me, "您的任务已经满了！")
    return nil
  end
  ...
  if tbReferData.nParticular then
    ...
    if not self:DelItem(me, tbItemId, 1) then
      Dialog:SendInfoBoardMsg(me, "指定触发物品不存在，无法接受此任务！")
      return nil
    end
  end
...
function Task:OnShare(nTaskId, nReferId)
  ...
  if not tbTeamMembers then
    Dialog:SendInfoBoardMsg(me, "共享失败：没有组队！")
    return
  end
  if nMemberCount <= 0 then
    Dialog:SendInfoBoardMsg(me, "共享失败：队伍没有成员！")
    return
  end

  -- 只有玩家处于这个任务的第一个引用子任务的时候才能共享
  if self:GetFinishedRefer(nTaskId) > 0 then
    me.Msg("共享失败：不在本任务的第一个子任务")
    return
  end
        ...
        if not tbPlayerTask.tbTasks[nTaskId] then
          if self:GetFinishedRefer(nTaskId) <= 0 then -- 只有从没接这个任务的队友才能接受
            if Lib:DoTestFuncs(tbVisable) then
              ...
              plOld.Msg(me.szName .. "没有达到接任务的条件，无法接此任务！")
            end
          else
            plOld.Msg(me.szName .. "已经有这个任务了！")
          end
        end
      else
        plOld.Msg(me.szName .. "太远了，无法共享任务！")
      end
    end
  end
  me = plOld
...
-------------------------------------------------------------------------
-- S 接到客户端请求跳过任务,通知客户端弹出跳过任务窗口
function Task:OnSkip(nTaskId, nReferId)
  local bCanSkip, strWarn = self:IsTaskCanSkip(nTaskId, nReferId)
  if bCanSkip ~= 1 then
    Dialog:SendInfoBoardMsg(me, strWarn)
    return
  end

  -- 判断金钱是否满足
  local bCheck, strWarn = self:SkipTaskCheck(nTaskId, nReferId)
  if bCheck ~= 1 then
    Dialog:SendInfoBoardMsg(me, strWarn)
    return
  end

  self:GetPlayerTask(me).tbAskSkip = { -- 防止客户端作弊(没提出跳过申请)
    nTaskId = nTaskId,
    nReferId = nReferId,
  }
  KTask.SkipTask(me, nTaskId, nReferId) --通知客户端弹出跳过任务窗口
end

-------------------------------------------------------------------------
-- S 跳过任务直接获取奖励
function Task:OnSkipAward(nTaskId, nReferId, nChoice)
  if nChoice == -1 then
    self:GetPlayerTask(me).tbAskSkip = {}
    return
  end
  local bSkip, strWarn = self:IsTaskCanSkip(nTaskId, nReferId)
  if bSkip ~= 1 then
    Dialog:SendInfoBoardMsg(me, strWarn)
    return
  end
  local tbAskSkip = self:GetPlayerTask(me).tbAskSkip
  if not tbAskSkip or tbAskSkip.nTaskId ~= nTaskId or tbAskSkip.nReferId ~= nReferId then
    self:GetPlayerTask(me).tbAskSkip = {}
    return
  end
  self:GetPlayerTask(me).tbAskSkip = {}

  -- 判断金钱是否充足，判断背包,判断次数限制
  local bCheck, strWarn = self:SkipTaskCheck(nTaskId, nReferId)
  if bCheck ~= 1 then
    Dialog:SendInfoBoardMsg(me, strWarn)
    return
  ...
  -- 先扣除金钱，再接取任务，领取奖励，完成任务
  local bTakeCost = self:TakeSkipCost(nTaskId, nReferId)
  if bTakeCost ~= 1 then
    return
  end
  ...
  local tbTask = self:NewTask(nReferId)
  if not tbTask then
    --不知道会不会执行到这里，扣除了玩家的东西但建立任务失败
    print("任务跳过机制,nReferId =：" .. nReferId .. "已经收取玩家:" .. me.szName .. " 的金钱，但建立任务失败导致无法领奖！")
    return nil
  end

  tbTask.nAcceptDate = GetCurServerTime()
  tbTask.nSaveGroup = 2206 --任务跳过用的,只用于占位
  ...
  local nCurSkipCount = me.GetTask(2207, 1) --获取当前的跳过次数
  me.SetTask(2207, 1, nCurSkipCount + 1, 1) -- 跳过次数加1
  self:SetFinishedRefer(nTaskId, nReferId)
  self:CloseTask(nTaskId, "skipfinish") --这里会更新一次客户端的任务界面
  KTask.SendSkipAward(me, nTaskId, nReferId, 0) --通知客户端已经成功跳过并领奖完毕，让客户端刷新任务面板
end
...
function Task:OnAward(nTaskId, nReferId, nChoice)
  ...
  if not TaskCond:HaveBagSpace(tbTask.tbReferData.nBagSpaceCount + nFreeCount) then
    Dialog:SendInfoBoardMsg(me, "请清理出" .. (tbTask.tbReferData.nBagSpaceCount + nFreeCount) .. "个背包空间，再来领奖")
    return
  end
...
function Task:GetSpeOpt_Cost(nTaskId, nReferId, nIndex)
  ...
  if #tbFind < nCostNum then
    szErrMsg = string.format("请随身携带%s个%s再来领取该奖励。", nCostNum, szCostItem)
    return 0, szErrMsg
  end

  local bRet = me.ConsumeItemInBags(nCostNum, unpack(tbCostGDPL))
  if bRet ~= 0 then
    local szLog = string.format("使用%s个%s，失败！！！！", nCostNum, szCostItem)
...
function Task:ChangeExp(nExp, nTaskId)
  return nExp
end
...
function Task:GiveAward(tbAward, nTaskId)
  ...
  if szType == "exp" then
    ...
    nExp = self:ChangeExp(nExp, nTaskId)
  ...
  elseif szType == "money" or szType == "bindmoney" then
    ...
    KStatLog.ModifyAdd("bindjxb", "[产出]" .. self:GetTaskTypeName(nTaskId), "总量", tbAward.varValue)
  ...
  elseif szType == "activemoney" then
    ...
    if tbTask then
      ...
      KStatLog.ModifyAdd("jxb", "[产出]" .. self:GetTaskTypeName(nTaskId), "总量", tbAward.varValue)
  ...
  elseif szType == "repute" then
    ...
    if tbAward.varValue[1] == 1 and tbAward.varValue[2] == 2 then
      me.Msg(string.format("您获得了<color=yellow>%s点<color>军营声望", tbAward.varValue[3]))
    end
    --机关学造诣
    if tbAward.varValue[1] == 1 and tbAward.varValue[2] == 3 then
      me.Msg(string.format("您获得了<color=yellow>%s点<color>机关学造诣", tbAward.varValue[3]))
  ...
  elseif szType == "bindcoin" then
    ...
    KStatLog.ModifyAdd("bindcoin", "[产出]" .. self:GetTaskTypeName(nTaskId), "总量", tbAward.varValue[1])
...
function Task:Get_CustomEquip(nTaskId, nIndex)
  ...
  if not self.tbCustomEquip[nIndex] or not self.tbCustomEquip[nIndex][nFaction] or not self.tbCustomEquip[nIndex][nFaction][nRoute] or not self.tbCustomEquip[nIndex][nFaction][nRoute][nSex] then
    me.Msg("系统问题，请联系GM!")
    Dbg:WriteLog("获得任务装备失败", me.szName, nFaction, nRoute, nSex)
    return
  end
  ...
  if pItem then
    ...
    if nTaskId ~= 10000 then --10000做活跃度调入
      self:AutoEquip(pItem.dwId)
      local szLog = string.format("获得任务装备成功！！！", nFaction, nRoute, nSex)
      Dbg:WriteLog("获得任务装备", me.szName, szLog)
    end
  end
end
...
function Task:GetSpeOpt_FactionEquip(nTaskId, nIndex)
  ...
  if pItem then
    pItem.Bind(1)
    self:AutoEquip(pItem.dwId)
    local szLog = string.format("使用%s个%s，获得一个%s，成功！！！", nCostNum, szCostItem, pItem.szName)
    Dbg:WriteLog("获得新手装备", me.szName, szLog)
  end
end
...
function Task:CloseTask(nTaskId, szReason)
  ...
  if szReason == "giveup" then
    me.Msg("放弃任务：" .. tbTask:GetName())
    me.Msg(tbTask.tbReferData.szGossip)
  elseif szReason == "failed" then
    me.Msg("任务失败：" .. tbTask:GetName())
    me.Msg(tbTask.tbReferData.szGossip)
  elseif szReason == "skipfinish" then
    --任务跳过机制，完成任务后写log
    me.Msg("直接完成：" .. tbTask:GetName())
    local nCurSkipCount = me.GetTask(2207, 1) --获取当前的跳过次数
    local strWarn = "这是您今天第" .. nCurSkipCount .. "次直接完成任务，今天还有" .. (Task.TASK_SKIP_NUM - nCurSkipCount) .. "次机会。"
    me.Msg(strWarn)
    local tbReferData = self.tbReferDatas[tbTask.nReferId]
    local tbSkipData = tbReferData.tbSkipData
    local nType = tbSkipData.nType
    local varValue = tbSkipData.varValue
    local strMoney = "扣除" .. varValue
    if nType == 1 then
      strMoney = strMoney .. "银两"
    elseif nType == 2 then
      strMoney = strMoney .. "绑定银两"
    elseif nType == 3 then
      strMoney = strMoney .. "绑定金币"
    end

    me.CastSkill(self.nFinishTaskSkillId, 1, -1, me.GetNpc().nIndex)
    local szLogMsg = string.format("跳过任务机制，直接完成%s，任务ID：%x，支线任务ID：%x，%s", tbTask:GetName(), tbTask.nTaskId, tbTask.nReferId, strMoney)
    print(szLogMsg)
    ...
    StatLog:WriteStatLog("stat_info", "skip_task", "task", me.nId, tbTask.nTaskId, tbTask.nReferId, varValue, nType, tbReferData.nLevel)
    me.PlayerLog(Log.emKPLAYERLOG_TYPE_SKIPFINISHTASK, szLogMsg)
  elseif szReason == "finish" then
    -- 2. 低于50级以下角色的任务事件，不记入角色历程日志。
    if me.nLevel >= 50 then
      me.Msg("任务完成：" .. tbTask:GetName())
      me.CastSkill(self.nFinishTaskSkillId, 1, -1, me.GetNpc().nIndex)
            ...
            if bOK then
              ...
              if tbSubData then
                ...
                if tbTaskData.tbAttribute and tbTaskData.tbAttribute.TaskType then
                  ...
                  szTaskType = Task.tbemTypeName[nTaskType] or "[任务]"
                end
                tbTaskOption[#tbTaskOption + 1] = {
                  nTaskType,
                  { szTaskType .. tbReferData.szName, TaskAct.TalkInDark, TaskAct, szMsg, self.AskAccept, self, tbTaskData.nId, nReferId },
                }
...
function Task:OnKillNpcForCount(pPlayer, pNpc, tbStudentList, tbTeammateList, tbSharePlayerList)
  ...
  for _, _player in pairs(tbSharePlayerList) do
    if (_player.nTeamId ~= pPlayer.nTeamId or _player.nTeamId == 0) and _player.nId ~= pPlayer.nId then
    ...
    if not tbTask then
      ...
      if not self.tbSubDatas[nSubTaskId] then
        ...
        elseif self:GetTaskType(nSubTaskId) == "LinkTask" then
          ...
          LinkTask:_ReadSubTask(nSubTaskId, tbTaskSub[nSubTaskId].szTaskName, tbTaskSub[nSubTaskId].szTargetName, tbTaskSub[nSubTaskId].tbParams)
        elseif self:GetTaskType(nSubTaskId) == "WantedTask" then
          --官府通缉任务
          local tbTaskSub = Wanted.tbSubTaskData
          Wanted:ReadSubTask(nSubTaskId, tbTaskSub[nSubTaskId].szTaskName, tbTaskSub[nSubTaskId].szTargetName, tbTaskSub[nSubTaskId].tbParams)
        else
...
function Task:GetPosInfo(nTaskId, szTaskName, nCurStep)
  ...
  for _, tbInfo in pairs(self.tbReferDatas) do
    if tbInfo.szName == szTaskName and tbInfo.tbDesc and tbInfo.tbDesc.tbStepsDesc and tbInfo.nTaskId == nTaskId and tbInfo.tbDesc.tbStepsDesc[nCurStep] then
...
function Task:IsSpeOptAward(nTaskId, nReferId, nIndex)
  ...
  for _, tbInfo in pairs(tbSpeOptInfo) do
    if tbInfo.nTaskId == nTaskId and tbInfo.nSubId == nReferId and tbInfo.nIndex == nIndex then
      return 1
    end
  end
...
function Task:GetSpeOptInfo(nTaskId, nReferId, nIndex)
  ...
  for _, tbInfo in pairs(tbSpeOptInfo) do
    if tbInfo.nTaskId == nTaskId and tbInfo.nSubId == nReferId then
      if tbInfo.nTaskId == nTaskId and tbInfo.nSubId == nReferId and tbInfo.nIndex == nIndex and ((tbInfo.nFaction ~= -1 and tbInfo.nFaction == nFaction) or tbInfo.nRoute == -1) and ((tbInfo.nRoute ~= -1 and tbInfo.nRoute == nRouteId) or tbInfo.nRoute == -1) and ((tbInfo.nSeries ~= -1 and tbInfo.nSeries == nSeries) or tbInfo.nSeries == -1) and tbInfo.nSex == me.nSex then
        tbRetInfo = tbInfo
        bFind = 1
        break
      end
...
function Task:LoadSpeOptInfo()
  ...
  for nRow, tbRowData in pairs(tbInfoSetting) do
    ...
    tbTemp.tbGDPL = {}
    tbTemp.tbCostGDPL = {}
    tbTemp.tbGDPL[1], tbTemp.tbGDPL[2], tbTemp.tbGDPL[3], tbTemp.tbGDPL[4] = tonumber(tbGDPL[1]), tonumber(tbGDPL[2]), tonumber(tbGDPL[3]), tonumber(tbGDPL[4])
    tbTemp.tbCostGDPL[1], tbTemp.tbCostGDPL[2], tbTemp.tbCostGDPL[3], tbTemp.tbCostGDPL[4] = tonumber(tbCostGDPL[1]), tonumber(tbCostGDPL[2]), tonumber(tbCostGDPL[3]), tonumber(tbCostGDPL[4])
...
function Task:CheckMantleLevel(nLevel)
  ...
  local szMsg = string.format("你身上没有带上披风或披风等级不足，需要佩戴%s级披风才能够满足条件。", nLevel)
  return nil, szMsg
end

--判断该任务是否可跳过，永不可跳过返回nil，可跳过但不满足条件返回0，可跳过且满足返回1，返回值第二个为错误提醒
function Task:IsTaskCanSkip(nTaskId, nReferId)
  local tbReferData = self.tbReferDatas[nReferId]
  local strWarn = ""
  if not tbReferData or tbReferData.nTaskId ~= nTaskId then
    strWarn = "很遗憾，找不到该任务！"
    return nil, strWarn
  end

  local tbSkipData = tbReferData.tbSkipData
  if not tbSkipData or not tbSkipData.bCanSkip or nTaskId == 0 then
    strWarn = "亲，此任务不支持直接完成！"
    return nil, strWarn
  end

  if me.nLevel - tbSkipData.nCondition < tbReferData.nLevel then
    strWarn = "很抱歉，您的等级不符合直接完成条件"
    return 0, strWarn
  end

  if self.tbTaskDatas[nTaskId].tbAttribute["Repeat"] then -- 还需要验证
    strWarn = "重复性任务不能直接完成哦！"
    return 0, strWarn
  end

  if self:HaveDoneSubTask(me, nTaskId, nReferId) == 1 then
    strWarn = "少侠，此任务已经完成了"
    return 0, strWarn
  end

  -- 判断该任务是否已接,目前是已接到的任务不跳过
  local tbPlayerTask = self:GetPlayerTask(me)
  if tbPlayerTask.tbTasks[nTaskId] then
    if (tbPlayerTask.tbTasks[nTaskId].nCurStep == 0 or not tbPlayerTask.tbTasks[nTaskId].nCurStep) and tbPlayerTask.tbTasks[nTaskId].nSaveGroup == 2206 then
      --如果执行到这里，说明任务跳过机制出错，出现了中间状态
      print("任务跳过机制出现异常，已恢复!")
      tbPlayerTask.tbTasks[nTaskId] = nil
    else
      strWarn = "已接任务是不能直接完成的"
      return 0, strWarn
    end
  end

  if self:GetReferID(nTaskId, self:GetFinishedIdx(nTaskId) + 1) ~= nReferId then
    strWarn = "出错了，好像您前置任务没完成"
    return 0, strWarn
  end
...
-- 跳过任务判断背包和金钱是否满足条件，不满足返回nil，全部满足返回1，第二参数为提示信息
function Task:SkipTaskCheck(nTaskId, nReferId)
  local tbReferData = self.tbReferDatas[nReferId]
  local strWarn = ""
  if not tbReferData or tbReferData.nTaskId ~= nTaskId then
    return nil, strWarn
  ...
  local nType = tbSkipData.nType --扣除-类型
  local varValue = tbSkipData.varValue -- 扣除-数量
  local nCurSkipCount = me.GetTask(2207, 1) --获取当前的跳过次数

  if nCurSkipCount >= Task.TASK_SKIP_NUM then
    strWarn = "您已经用完每天" .. Task.TASK_SKIP_NUM .. "次直接完成任务的机会。"
    return nil, strWarn
  end

  if me.nLevel - tbSkipData.nCondition < tbReferData.nLevel then
    strWarn = "<color=green>您的等级不够，需要达到<color=Red>" .. (tbReferData.nLevel + tbSkipData.nCondition) .. "级<color><color=green>才可直接完成此任务。<color>"
    return nil, strWarn
  end

  local szMapClass = GetMapType(me.nMapId) or ""
  if szMapClass ~= "village" and szMapClass ~= "city" and szMapClass ~= "fight" then
    strWarn = "只能在城市、新手村、野外使用直接完成任务功能。"
    return nil, strWarn
  end

  --判断背包和金钱
  if not TaskCond:HaveBagSpace(tbReferData.nBagSpaceCount) then
    strWarn = "请清理出" .. tbReferData.nBagSpaceCount .. "个背包空间。"
    return nil, strWarn
  end

  --nType为1是银两，为2是绑银，为3是绑金
  if nType == 1 and me.nCashMoney < varValue then
    strWarn = "您的银两不够。"
    return nil, strWarn
  elseif nType == 2 and me.GetBindMoney() < varValue then
    strWarn = "您的绑定银两不够。"
    return nil, strWarn
  elseif nType == 3 and me.nBindCoin < varValue then
    strWarn = "您的绑定金币不够。"
    return nil, strWarn
  elseif nType ~= 1 and nType ~= 2 and nType ~= 3 then
    strWarn = "出错了，未知扣除类型。"
    return nil, strWarn
  end
  return 1, ""
...
-- S 收取跳过任务的费用，收取成功返回1，收取失败返回nil
function Task:TakeSkipCost(nTaskId, nReferId)
  local bCanSkip = self:IsTaskCanSkip(nTaskId, nReferId)
  if bCanSkip ~= 1 then
    return nil
  end
  ...
  if not varValue or varValue < 0 then
    print("跳过任务扣除的金钱不能为负数 " .. "ReferID: " .. nReferId)
    return nil
  end
  ...
  if nType == 1 then
    --银两
    local nWay = Player.emKPAY_SKIPTASK
    bSuccess = me.CostMoney(varValue, nWay)
    strDesc = "" .. varValue .. "银两"
  elseif nType == 2 then
    --绑银
    local nWay = Player.emKBINDMONEY_COST_SKIPTASK
    bSuccess = me.CostBindMoney(varValue, nWay)
    strDesc = "" .. varValue .. "绑定银两"
  elseif nType == 3 then
    --绑金
    local nWay = Player.emKBINDCOIN_COST_SKIPTASK
    bSuccess = me.AddBindCoin(-varValue, nWay)
    strDesc = "" .. varValue .. "绑定金币"
  elseif nType ~= 1 and nType ~= 2 and nType ~= 3 then
    return nil
  end
...
--用于获取直接完成任务的描述信息，返回的第一个参数为在任务面板中的描述，返回第二个参数为在领奖界面的描述
function Task:GetSkipDesc(nTaskId, nReferId)
  local bCheck, strWarn = Task:SkipTaskCheck(nTaskId, nReferId)
  local tbReferData = self.tbReferDatas[nReferId]
  local tbSkipData = tbReferData.tbSkipData
  local nType = tbSkipData.nType --扣除-类型
  local varValue = tbSkipData.varValue -- 扣除-数量
  ...
  local strDesc2 = ""
  if bCheck == 1 then
    strDesc1 = "<color=GREEN>(您可以消耗<color>" .. "<color=yellow>" .. varValue .. strMoney .. "<color>" .. "<color=GREEN>直接完成任务)<color>"
    strDesc2 = "<color=GREEN>(将收取<color>" .. "<color=yellow>" .. varValue .. strMoney .. "<color>" .. "<color=GREEN>直接完成任务)<color>"
  else
    strDesc1 = "<color=GREEN>(您可以消耗<color>" .. "<color=yellow>" .. varValue .. strMoney .. "<color>" .. "<color=GREEN>直接完成任务，" .. strWarn .. ")<color>"
    strDesc2 = "<color=GREEN>(将收取<color>" .. "<color=yellow>" .. varValue .. strMoney .. "<color>" .. "<color=GREEN>直接完成任务，" .. strWarn .. ")<color>"
  end
