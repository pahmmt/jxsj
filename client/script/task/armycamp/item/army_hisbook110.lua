...
tbItem.nTime = 5
tbItem.nRelayTime = 1 * 60 --需要延时10分钟后才能再读
tbItem.nBookMax = 10
tbItem.nSkillId = 798
...
function tbItem:OnUse()
  ...
  if nYear > 0 then
    ...
    if nSec1 < nSec2 then
      me.Msg(string.format("每读一页后必须等待1分钟才可以读取下一页，您还需要等待<color=yellow>%s秒<color>后才能读取下一页。", (nSec2 - nSec1)))
      return 0
    end
  end
  local tbEvent = {
  ...
  GeneralProcess:StartProcess("读书中...", self.nTime * Env.GAME_FPS, { self.SuccessUse, self, it.dwId, me.nId }, { self.FailUse, self, me.nId }, tbEvent)
end
...
function tbItem:SuccessUse(nItemId, nPlayerId)
  ...
  if nR <= self.nRate then
    ...
    if pAddItem then
      pPlayer.Msg(string.format("获得了一个<color=yellow>%s<color>。", pAddItem.szName))
    end
  end
...
function tbItem:GetTip(nState)
  ...
  szTip = szTip .. string.format("<color=yellow>阅读兵书页数: %s/%s<color>", nUseCount, self.nBookMax)
  return szTip
end
