...
function Task:_OnLoginTempLogic()
  local pPlayer = me
  if self:HaveDoneSubTask(pPlayer, tonumber("A0", 16), tonumber("13C", 16)) == 1 or self:HaveDoneSubTask(pPlayer, tonumber("9E", 16), tonumber("13A", 16)) == 1 or Task:HaveDoneSubTask(pPlayer, tonumber("9F", 16), tonumber("13B", 16)) == 1 then
    pPlayer.SetTask(1021, 8, 1, 1)
  end

  if Task:HaveDoneSubTask(pPlayer, tonumber("1", 16), tonumber("1", 16)) == 1 or Task:HaveDoneSubTask(pPlayer, tonumber("6", 16), tonumber("34", 16)) == 1 or Task:HaveDoneSubTask(pPlayer, tonumber("A", 16), tonumber("4B", 16)) == 1 then
...
PlayerEvent:RegisterGlobal("OnLoginOnly", Task._OnLoginTempLogic, Task)
