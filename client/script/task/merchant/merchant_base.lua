  ...
  if Item.tbStone:GetOpenDay() ~= 0 then
    if self:GetTask(self.TASK_STONE_AWARD) == 1 then
      ...
      for _, tbSingleStone in pairs(tbStoneList) do
        StatLog:WriteStatLog("stat_info", "baoshixiangqian", "shanghui", me.nId, string.format("%d_%d_%d_%d,%d", tbSingleStone.nGenre, tbSingleStone.nDetail, tbSingleStone.nParticular, tbSingleStone.nLevel, tbSingleStone.nNum))
      end
    end
  end
...
function Merchant:GetRamdomEvent()
  ...
  local szBlackMsg = string.format("接下来您的第%s个任务是", nStepCount)
  if nStepCount == 1 then
    szBlackMsg = "现在给您的第1个任务是"
  end
...
function Merchant:TryGiveToken_Songjin(pPlayer, nKilledNpcTemplateId)
  local tbTemplateId2KillIndex = {
    [2513] = self.KILL_SONGJIN_DAJIANG,
    [2519] = self.KILL_SONGJIN_DAJIANG, -- 大将
    [2512] = self.KILL_SONGJIN_FUJIANG,
    [2518] = self.KILL_SONGJIN_FUJIANG, -- 副将
    [2511] = self.KILL_SONGJIN_TONGLING,
    [2517] = self.KILL_SONGJIN_TONGLING, -- 统领
  }
