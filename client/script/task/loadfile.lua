  ...
  for _, tb in ipairs(tbXml.children) do
    ...
    for _, tb in ipairs(tb.children) do --这里不会有多个childern吧
      if tb.name == "Value" then
        if szType == "number" or szType == "faction" or szType == "bool" or szType == "itemid" or szType == "taskitem" or szType == "award" then
          szValue = tb.value
    ...
    elseif tb.name == "Managed" then
      ...
      for _, tb in ipairs(tb.children) do
        if tb.name == "Sub" then
          ...
          tbReferData.tbSkipData = {} -- 保存跳过任务信息
          tbReferData.tbSkipData.bCanSkip = false
          ...
          if tbReferData.nAcceptNpcId > 0 then
            ...
            if szNpcName then
              szDefaultInfo = "请找" .. szNpcName .. "接此任务"
            end
          elseif tbReferData.nParticular and tbReferData.nParticular > 0 then
            ...
            if szItemName then
              szDefaultInfo = "使用" .. szItemName .. "接触法任务"
            end
          end
        ...
        elseif tb.name == "Execute" then
          ...
          for _, szExecute in pairs(tbStep.tbExecute) do
            ...
            if string.find(szExecute, "TaskAct:AddExp") then
              ...
              szExecuteDesc = "经验：" .. tostring(nExp) .. "\n"
            elseif string.find(szExecute, "TaskAct:AddItems") then
              ...
              szExecuteDesc = "物品：" .. (KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4]) .. " X " .. tostring(nCount) or "")
            elseif string.find(szExecute, "TaskAct:AddItem") then
              ...
              szExecuteDesc = "物品：" .. (KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4]) or "")
            elseif string.find(szExecute, "TaskAct:AddMoney") then
              ...
              szExecuteDesc = "绑定银两：" .. tostring(nMoney) .. "\n"
            elseif string.find(szExecute, "TaskAct:GiveActiveMoney") then
              ...
              szExecuteDesc = "银两：" .. tostring(nMoney) .. "\n"
            end
            ...
            elseif string.find(szExecute, "TaskAct:SetTaskValueOnStart") or string.find(szExecute, "TaskAct:DelItemOnAccept") or string.find(szExecute, "TaskAct:SetTaskValueBitOnStart") or string.find(szExecute, "TaskAct:AddTaskValueOnStart") then
              tbSubData.tbStartExecute[#tbSubData.tbStartExecute + 1] = szExecute
            elseif string.find(szExecute, "TaskAct:SetTaskValueOnFailed") or string.find(szExecute, "TaskAct:DelItemOnFailed") or string.find(szExecute, "TaskAct:DelTitleOnFailed") then
              tbSubData.tbFailedExecute[#tbSubData.tbFailedExecute + 1] = szExecute
            elseif string.find(szExecute, "TaskAct:SetTaskValueOnFinish") or string.find(szExecute, "TaskAct:DoExecuteOnFinish") or string.find(szExecute, "TaskAct:AddTaskValueOnFinish") then
...
function Task:LoadSkipTaskFile() -- 加载任务跳过表，先加载任务信息后才能调用这函数
  local tbFile = Lib:LoadTabFile("\\setting\\task\\skiptask.txt")
  if not tbFile then
    return
  end
  for i = 1, #tbFile do
    local nReferId = tonumber(tbFile[i].nReferId) -- 子任务ID
    local nCondition = tonumber(tbFile[i].nCondition) -- 人物等级需要比任务等级高过nCondition级别才可跳过
    local varValue = tonumber(tbFile[i].varValue) -- 收取的数值，类型由nType决定，可能是绑银，银两等
    local nType = tonumber(tbFile[i].nType)
    if Task.tbReferDatas[nReferId] then
      --把跳过信息挂在
      Task.tbReferDatas[nReferId].tbSkipData = {}
      Task.tbReferDatas[nReferId].tbSkipData.bCanSkip = true
      Task.tbReferDatas[nReferId].tbSkipData.nCondition = nCondition
      Task.tbReferDatas[nReferId].tbSkipData.varValue = varValue
      Task.tbReferDatas[nReferId].tbSkipData.nType = nType
    end
  end
end
