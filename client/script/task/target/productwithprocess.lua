...
function tb:Init(nDuration, szToke, nRate, szSucMsg, szFailedMsg, szProcessTip, tbTool, szStuffs, tbKeyProduct, nKeyProductNeedCount, szTotleProducts, nArisingsLimitCount, nNeedProuductCount, nMapId, nPosX, nPosY, nRange, nSkillId, nObjId, szDynamicDesc, szStaticDesc, nModel)
...
function tb:Start()
  ...
  if Task:GetItemCount(pPlayer, { 20, 1, self.nToolParticular, 1 }) <= 0 then
    Task:AddItem(pPlayer, { 20, 1, self.nToolParticular, 1 })
  end
...
function tb:Load(nGroupId, nStartTaskId)
  ...
  self.nSucMakeTimes = pPlayer.GetTask(nGroupId, nStartTaskId)
...
function tb:Save(nGroupId, nStartTaskId)
  ...
  pPlayer.SetTask(nGroupId, nStartTaskId, self.nSucMakeTimes)
...
function tb:Close()
  ...
  Task:DelItem(pPlayer, { 20, 1, self.nToolParticular, 1 }, 1)
  self:UnRegister()
end
...
function tb:ResetItemCount()
  ...
  for _, tbStuff in ipairs(self.tbStuffs) do
    local nHaveCount = Task:GetItemCount(pPlayer, tbStuff.tbItem)
    if nHaveCount < (tbStuff.nCount * nRemainTimes) then
      if TaskCond:CanAddCountItemIntoBag(tbStuff.tbItem, (tbStuff.nCount * nRemainTimes)) then
        Task:AddItems(pPlayer, tbStuff.tbItem, (tbStuff.nCount * nRemainTimes))
      end
    elseif nHaveCount > (tbStuff.nCount * nRemainTimes) then
      Task:DelItem(pPlayer, tbStuff.tbItem, nHaveCount - (tbStuff.nCount * nRemainTimes))
    end
  end
end
...
function tb:OnItem()
  ...
  -- 材料是否足够
  for _, tbStuff in ipairs(self.tbStuffs) do
    if Task:GetItemCount(me, tbStuff.tbItem) < tbStuff.nCount then
      me.Msg("材料不足！")
      Setting:RestoreGlobalObj()
      return
    end
...
function tb:OnProgressFull()
  ...
  for _, tbStuff in ipairs(self.tbStuffs) do
    if Task:GetItemCount(pPlayer, tbStuff.tbItem) < tbStuff.nCount then
      pPlayer.Msg("材料不够")
      return
    end
  end
  ...
  if self.nArisingsCount >= self.nArisingsLimitCount then
    ...
    Task:AddItem(me, self.tbKeyProduct)
    ...
    for _, tbProduct in ipairs(self.tbTotleProducts) do
      ...
      if nCurSum > nRandom then
        ...
        Task:AddItem(me, tbProduct.tbItem)
  ...
  for _, tbStuff in ipairs(self.tbStuffs) do
    if Task:DelItem(me, tbStuff.tbItem, tbStuff.nCount) ~= 1 then
      --assert(false);
      --return;
    end
  end
  ...
  if self.nObjId ~= 0 then
    Setting:SetGlobalObj(me, him, it)
  ...
  if self.nModel == 1 then -- 看是否达到次数
    ...
    if MODULE_GAMESERVER and tbSaveTask then -- 自行同步到客户端，要求客户端刷新
      me.SetTask(tbSaveTask.nGroupId, tbSaveTask.nStartTaskId, self.nSucMakeTimes, 1)
      KTask.SendRefresh(me, self.tbTask.nTaskId, self.tbTask.nReferId, tbSaveTask.nGroupId)
    end

    if self:IsDone() then
      me.Msg("目标达成：" .. self:GetStaticDesc())
...
function tb:GetKeyProductCount()
  ...
  return Task:GetItemCount(pPlayer, self.tbKeyProduct)
end

function tb:IsKeyProduct(tbDesItem)
  if tbDesItem[1] == self.tbKeyProduct[1] and tbDesItem[2] == self.tbKeyProduct[2] and tbDesItem[3] == self.tbKeyProduct[3] and tbDesItem[4] == self.tbKeyProduct[4] then
    return 1
  end
...
function tb:AddToke()
  ...
  if self.tbToke.nMakePoint and self.tbToke.nMakePoint > 0 then
    pPlayer.ChangeCurMakePoint(self.tbToke.nMakePoint)
  end
  if self.tbToke.nGatherPoint and self.tbToke.nGatherPoint > 0 then
    pPlayer.ChangeCurGatherPoint(self.tbToke.nGatherPoint)
  end
  if self.tbToke.tbRepute then
    pPlayer.AddRepute(self.tbToke.tbRepute.Camp, self.tbToke.tbRepute.Class, self.tbToke.tbRepute.Delta)
  end
end
