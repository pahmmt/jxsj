  ...
  for _, item in ipairs(self.tbNpcSet) do
    if item.nNpcIdx <= 0 then
      ...
      if item.nReviveTime <= 0 then
        local pNpc = KNpc.Add2(self.nNpcTempId, 1, -1, self.nMapId, item.nMapPosX, item.nMapPosY)
  ...
  if szPos and szPos ~= "" then
    ...
    for i = 1, #tbTrack do
      if tbTrack[i] and tbTrack[i] ~= "" then
        ...
        local pNpc = KNpc.Add2(self.nNpcTempId, 1, -1, self.nMapId, tbRet[i].nMapPosX, tbRet[i].nMapPosY)
        if not pNpc or pNpc.nIndex == 0 then
          print("[Task Error]:" .. self.nNpcTempId .. "  添加失败！")
          return {}
        end
        Task.tbToBeDelNpc[pNpc.dwId] = 1
        tbRet[i].nNpcIdx = pNpc.dwId
