...
-- 弹出MessageBox,非客户端不能使用回调
-- 内容，标题，按钮数量，回调函数
function Dialog:MsgBox(szMsg, szTitle, nOptCount, tbCallback, ...)
  local tbMsg = {}
  tbMsg.szMsg = szMsg or "你好"
  tbMsg.szTitle = szTitle or "提示"
  if MODULE_GAMESERVER then
    tbMsg.nOptCount = 1
    me.CallClientScript({ "UiManager:OpenWindow", "UI_MSGBOX", tbMsg })
  end
  if MODULE_GAMECLIENT then
    tbMsg.nOptCount = nOptCount or 1
    tbMsg.Callback = tbCallback
    UiManager:OpenWindow(Ui.UI_MSGBOX, tbMsg, unpack(arg))
  end
  return
end
