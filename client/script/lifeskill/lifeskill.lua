...
function LifeSkill:NewSkill(pPlayer, nSkillId, nLevel, nCurExp)
  ...
  if not tbSkillData then
    pPlayer.Msg("未知生活技能 - " .. nSkillId)
    return nil
  end

  local tbPlayerLifeSkills = self:GetMyLifeSkill(pPlayer)
  if tbPlayerLifeSkills.tbLifeSkills[nSkillId] then
    pPlayer.Msg("重复技能 - " .. tbPlayerLifeSkills.tbLifeSkills[nSkillId].tbSkillData.Name)
    return nil
  end
...
function LifeSkill:NewRecipe(pPlayer, nRecipeId)
  ...
  if not tbSkill then
    pPlayer.Msg("没有所属技能 - " .. tbRecipeData.Belong .. "," .. tbRecipeData.Name)
    return nil
  end
...
function LifeSkill:AddRecipe(pPlayer, nRecipeId, nMsg, nSync)
  ...
  if nMsg ~= 0 then
    pPlayer.Msg("成功添加了一个配方：" .. tbRecipe.tbRecipeData.Name)
  end
  return 1
end
...
function LifeSkill:RemoveRecipe(pPlayer, nRecipeId)
  ...
  if not tbSkill.tbRecipes[nRecipeId] then
    pPlayer.Msg("删除配方失败，指定配方不存在：", nRecipeId)
  end
...
function LifeSkill:AddSkillExp(pPlayer, nSkillId, nExp)
  ...
  pPlayer.Msg("你获得了" .. nExp .. "点“" .. szName .. "”经验值！")
...
function LifeSkill:MakeProduct(nRecipeId)
  ...
  local szMsg = ""
  if nGene == 0 then
    szMsg = "制造" .. tbRecipeData.Name
  elseif nGene == 1 then
    szMsg = "合成" .. tbRecipeData.Name
  end
...
function LifeSkill:OnMakeProductResult(nRecipeId, bBreak)
  ...
  if self:HasLearnRecipe(me, nRecipeId) == 0 then
    me.Msg("没有学会此配方如何制造成功。")
    me.SynProduceResult(nRecipeId, 0)
    return
  end

  local nLimitJinghuo = me.GetTask(LifeSkill.nGroupId, LifeSkill.nJingHuoLimit)
  local nDecJinghuo = 0 --减少限制的值
  ...
  for _, stuff in ipairs(tbRecipeData.tbStuffSet) do
    if stuff.nCount and stuff.nCount > 0 then
      local nCount = me.GetItemCountInBags(stuff.tbItem[1], stuff.tbItem[2], stuff.tbItem[3], stuff.tbItem[4], stuff.tbItem[5], LifeSkill:GetBindType(stuff.nBind))
      if nCount < stuff.nCount then
        me.Msg("材料不够。" .. szLifeSkillType .. "失败")
        me.SynProduceResult(nRecipeId, 0)
        return
      end
    end
  ...
  for _, stuff in ipairs(tbRecipeData.tbStuffSet) do
    if stuff.nCount and stuff.nCount > 0 then
      ...
      if nStuffCost then
        ...
        nStuffCostAll = nStuffCostAll + stuff.nCount * nStuffCost
      end
    end
  end
  if nStuffCostAll >= nLimitJinghuo then
    nOtherCost = nOtherCost + (nStuffCostAll - nLimitJinghuo) / Spreader.ExchangeRate_Gold2JingHuo
  end
  ...
  elseif tbSkillData.Gene == 0 then
    ...
    if tbRecipeData.Consume == 1 then
      ...
      if tbRecipeData.Cost >= nLimitJinghuo then
        nOtherCost = nOtherCost + (tbRecipeData.Cost - nLimitJinghuo) / Spreader.ExchangeRate_Gold2JingHuo -- 记录ibvalue
        nDecJinghuo = nDecJinghuo + nLimitJinghuo
        nLimitJinghuo = 0
      else
        nDecJinghuo = nDecJinghuo + tbRecipeData.Cost
        nLimitJinghuo = nLimitJinghuo - tbRecipeData.Cost
      end
    else
      nCost = 0 -- 箱子
  ...
  if tbFinalProduct then
    ...
    if pItem then
      if pItem.IsEquip() ~= 1 then
        ...
        pItem.nBuyPrice = pItem.nBuyPrice + nOtherCost
      end
      ...
      me.Msg(szLifeSkillType .. "成功。")

      if pItem.IsEquip() == 1 and nDecJinghuo > 0 then
        me.SetTask(LifeSkill.nGroupId, LifeSkill.nJingHuoLimit, me.GetTask(LifeSkill.nGroupId, LifeSkill.nJingHuoLimit) - nDecJinghuo)
      end
    else
      me.Msg(szLifeSkillType .. "失败。")
    end
  else
    me.Msg(szLifeSkillType .. "失败。")
  end
...
function LifeSkill:AddRecipeForLevelChange(pPlayer, nSkillId, nSkillLevel)
  ...
  for nRecipeId, tbEventRecipe in pairs(self.tbStorageDatas) do
    if (not tbEventRecipe.nBelong or tbEventRecipe.nBelong == nSkillId) and (not tbEventRecipe.nAutoAppend or tbEventRecipe.nAutoAppend == 1) then
      if tbEventRecipe.nStartDate == 0 and tbEventRecipe.nEndDate == 0 then
...
function LifeSkill:ShowMySkill(pPlayer)
  ...
  if tbPlayerLifeSkills then
    for _, tbSkill in pairs(tbPlayerLifeSkills.tbLifeSkills) do
      pPlayer.Msg(tbSkill.tbSkillData.Name .. "等级：" .. tbSkill.nLevel .. "经验：" .. tbSkill.nExp)
