...
function IpStatistics:GetPlayerType(pPlayer)
  ...
  for _, v in ipairs(self.tbLevelDefine) do
    if (not v[1] or (not v[1][1] or nCount <= v[1][1]) and (not v[1][2] or nCount >= v[1][2])) and (not v[2] or (not v[2][1] or nRank <= v[2][1]) and (not v[2][2] or nRank >= v[2][2])) then
      nRet = v[3]
      break
    end
  end
...
function IpStatistics:OnServerStart()
  self:InitDataBuffer()
  PlayerEvent:RegisterGlobal("OnLoginOnly", IpStatistics.OnLogin, IpStatistics)
end
