...
function c2s:ConfirmRecast(nRequest, nEquipId, nIndex)
  ...
  if not me.GetTempTable("Item").tbEquip or me.GetTempTable("Item").tbEquip.dwEquipId ~= nEquipId then
  ...
  elseif nRequest == 1 then
    ...
    elseif nIndex == 2 then
      ...
      local nRet = pEquip.Regenerate(pEquip.nGenre, pEquip.nDetail, pEquip.nParticular, pEquip.nLevel, pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, pEquip.GetGenInfo(), 0, nNewRandSeed, pEquip.nStrengthen, tbRandMa)
      if nRet ~= 1 then
...
function c2s:WriteClientInfoToLog(...)
  ...
  if type(fGamePlayTime) ~= "number" or type(fGameActiveTime) ~= "number" or type(nMouseClickNum) ~= "number" or type(nKeyboardClickNum) ~= "number" then
    return 0
  end
...
function c2s:ApplyAddNewTeamLink(nTeamId, nPlayerId, nKinId, nTongId)
  if type(nTeamId) ~= "number" or type(nPlayerId) ~= "number" or type(nKinId) ~= "number" or type(nTongId) ~= "number" then
    return
  end
...
function c2s:ApplyJoinFaction(nFaction)
  if (type(nFaction) ~= "number") or nFaction <= 0 or nFaction > Env.FACTION_NUM then
    return
  end
  Player:ApplyJoinFaction(nFaction)
end
...
-- 开场动画完成，接新手任务
function c2s:OnCompleteOpenMovie()
  -- 新人直接得到新手任务任务
...
function c2s:ApplyGetConsumePoint()
  SpecialEvent.tbOnlineBankPay:GetRankPoint()
end

function c2s:ApplyGetOnlineBankPayAward(nIndex)
  if type(nIndex) ~= "number" then
    return 0
  end
  local nResult = SpecialEvent.tbOnlineBankPay:GetBankPayAward(nIndex)
end

--手机号领取奖励
function c2s:ApplyColloctPhone_Award(szBackUrl)
  SpecialEvent.tbGetPhoneId_2012:CheckRebackUrl(szBackUrl)
end
...
----------------------------------------------------------------------
--传送到npc处
function c2s:ApplyTransfer_Keju()
  SpecialEvent.JxsjKeju:Transfer2Pos()
end

--选择2个错误答案
function c2s:ApplySelect2Wrong_Keju()
  SpecialEvent.JxsjKeju:Select2Wrong()
end

--双倍积分
function c2s:ApplyDoubleGrade_Keju()
  SpecialEvent.JxsjKeju:DoubleGrade()
end

--选择一个答案
function c2s:ApplySelectAnswerId_Keju(nId)
  if type(nId) ~= "number" then
    return 0
  end
  SpecialEvent.JxsjKeju:AnswerQuesetion(nId)
end

function c2s:ApplyLuckyFan_ActiveGameStep()
  SpecialEvent.XmasLottery2012:ApplyLuckyFan_ActiveGameStep()
end

function c2s:ApplyLuckyFan_GetOneResultCard(nPos)
  if type(nPos) ~= "number" then
    return 0
  end
  SpecialEvent.XmasLottery2012:ApplyLuckyFan_GetOneResultCard(nPos)
end
