...
function Sns:OAuthSignUrl(nSnsId, nSignMethod, szUrl, szHttpMethod, szTokenKey, szTokenSecret, nJobId)
  if type(nSnsId) ~= "number" or type(nSignMethod) ~= "number" or type(szUrl) ~= "string" or type(szHttpMethod) ~= "string" or type(szTokenKey) ~= "string" or type(szTokenSecret) ~= "string" or type(nJobId) ~= "number" then
  ...
  if #szTokenKey == 0 or #szTokenSecret == 0 then
    szSignedUrl = OAuthGetSignedUrl_HMACSHA1(szUrl, szHttpMethod, tbKey.szConsumerKey, tbKey.szConsumerSecret)
  else
    szSignedUrl = OAuthGetSignedUrl_HMACSHA1(szUrl, szHttpMethod, tbKey.szConsumerKey, tbKey.szConsumerSecret, szTokenKey, szTokenSecret)
  end
...
function Sns:SetSnsBind(nSnsId, bEnable)
  if type(nSnsId) ~= "number" or bEnable ~= 1 and bEnable ~= 0 then
...
function Sns:OnClientEvent(nSnsId, nEventKind, tbData)
  if type(nSnsId) ~= "number" or type(nEventKind) ~= "number" or tbData ~= nil and type(tbData) ~= "table" then
  ...
  elseif nEventKind == self.EVENT_FOLLOW or nEventKind == self.EVENT_UNFOLLOW then
    if type(tbData.szTargetPlayerName) ~= "string" or type(tbData.szTargetAccount) ~= "string" or type(tbData.szMyAccount) ~= "string" then
      return
    end
    ...
    StatLog:WriteStatLog("stat_info", "sns", "relation", me.nId, me.GetHonorLevel(), tbSns.nId, tbData.szMyAccount, szOpType, tbData.szTargetAccount, szPlayerAccount, szPlayerName)
  end
end
...
function Sns:SendMsgToFriendChannel(tbSns, tbData)
  if type(tbData.szTweetId) == "string" then
    ...
    local szMsg = string.format("<color=red>%s<color>在<color=red>%s<color>%s <tweet=%d,%s,%s>", me.szName, tbSns.szName, szAction, tbSns.nId, tbData.szAccount, tbData.szTweetId)
    me.SendMsgToFriend(szMsg)
  end
end
function Sns:Test()
