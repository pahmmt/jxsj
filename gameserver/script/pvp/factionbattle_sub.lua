...
function FactionBattle:FlushAwardItem(nMapId, tbPoint, nIndex, nPlayerCount)
  ...
  for i = 1, nItemNum do
    ...
    tbPoint[i] = tbTemp
    Npc.tbXiangZiNpc:AddBox(nMapId, tbPoint[i].nX, tbPoint[i].nY, FactionBattle.TAKE_BOX_TIME * Env.GAME_FPS, tbParam, 1, 60 * 18)
  end
  return 0
end
...
function FactionBattle:AwardChampionStart(nFaction, nWinnerId)
  local pFlagNpc = KNpc.Add2(self.FLAG_NPC_TAMPLATE_ID, 10, -1, self.FACTION_TO_MAP[nFaction], self.FLAG_X, self.FLAG_Y)
...
function FactionBattle:ExcuteAwardChampion(pPlayer, pNpc)
  ...
  if (not tbTemp.tbFactionData) or not tbTemp.tbFactionData.nWinnerId or (tbTemp.tbFactionData.nWinnerId ~= pPlayer.nId) then
    return 0
  end
  ...
  if self._MODEL_OLD == FactionBattle.FACTIONBATTLE_MODLE then
    self:FlushAwardItem(pNpc.nMapId, self.tbBoxPoint[9], 5, nPlayerCount) -- 临时
  end
  ...
  pNpc.Delete()
  if tbData then
    tbData:MsgToMapPlayer("新人王诞生，在领奖台周围能持续获得丰厚经验奖励。")
    tbData:ShutDown(1) -- 圆满结束了
  end

  -- 冠军可以领取奖励
  if FactionBattle.FACTIONBATTLE_MODLE == FactionBattle._MODEL_NEW and tbData then
    Dialog:Say("是否现在兑换门派竞技奖励，你也可以选择以后到门派掌门人处领取奖励！", {
      { "是", self.ExchangeExp, self, pPlayer.nId, 0, 1 },
      { "否" },
    })
  end
end
...
function FactionBattle:PromotionAward(nMapId, nArenaId, nIndex, nPlayer1Id, nPlayer2Id, nPlayerCount)
  ...
  if FactionBattle._MODEL_OLD == FactionBattle.FACTIONBATTLE_MODLE then
    Timer:Register(self.ADD_BOX_DELAY * Env.GAME_FPS, self.FlushAwardItem, self, nMapId, self.tbBoxPoint[nArenaId], nIndex, nPlayerCount)
  end
end
...
function FactionBattle:ExchangeExp(nPlayerId, bConfirm, bIgnor)
  ...
  if nFlag == 1 and bIgnor ~= 1 then
    Dialog:Say("门派竞技正在进行，不能兑换积分")
    return 0
  end
  if nScore == 0 and nElimat <= 2 then
    Dialog:Say("你目前没有可以兑换的积分")
    return 0
  end
  ...
  if nElimat > 2 then
    if self.BOX_NUM[nElimat - 2][1] == 1 then
      szTitle = "获得<color=red>冠军<color>"
    else
      szTitle = "进入<color=red>" .. self.BOX_NUM[nElimat - 2][1] .. "强<color>"
    end
  ...
  if bConfirm == 1 then
    ...
    if pPlayer.CountFreeBagCell() < nBagNeed then
      local szError = string.format("你的背包空间不足,你需要<color=green>%s<color>个空间领取奖励", nBagNeed)
      pPlayer.Msg(szError)
      return 0
    end
    ...
    if tbStoneAward then
      for _, tbSingleAward in pairs(tbStoneAward) do
        ...
        StatLog:WriteStatLog("stat_info", "baoshixiangqian", "menpai", me.nId, string.format("%d_%d_%d_%d,%d", tbSingleAward[1], tbSingleAward[2], tbSingleAward[3], tbSingleAward[4], tbSingleAward[6]))
      end
    end
  ...
  local szMsg = string.format("你目前的积分为<color=green>%s<color>，可以兑换<color=green>%s<color>经验,<color=green>%s<color>个箱子\n", nScore, nExp, nXiangZi)
  local szModeMsg = ""
  if nElimat > 2 then
    szMsg = szMsg .. string.format("鉴于你在竞技比赛表现出色，" .. szTitle .. "，特此给予额外奖励：<color=green>%s<color>经验,<color=green>%s<color>个箱子\n", nExpEx, nXiangZiEx)
    szModeMsg = string.format("你在竞技比赛表现出色，" .. szTitle .. "，特此给予额外奖励：<color=green>%s<color>经验,<color=green>%s<color>个箱子\n", nExpEx, nXiangZiEx)
  end
  local szStoneMsg = ""
  if tbStoneAward then -- 宝石
    ...
    szStoneMsg = string.format("同时你将会得到<color=green>%s个蒙尘的宝石<color>", nStoneCount)
  end
  ...
  Dialog:Say(szMsg, {
    { "我要兑换", self.ExchangeExp, self, me.nId, 1, bIgnor },
    { "我再考虑一下" },
  })
end
...
function FactionBattle:DescribNewModel(nFaction)
  ...
  local szMsg = string.format("\n  3、新模式下最少参赛人数为%d人。", FactionBattle.MIN_ATTEND_PLAYER)
  Dialog:Say("  1、晋级赛阶段，诸位侠士将在多个场地中分为两个阵营对战。通过击败敌人获得积分，同时单场胜利一方阵营会有10%积分加成。\n  2、缩短各阶段活动时间。" .. szMsg, {
    { "打开F12详细帮助", funOpenWindow },
    { "我知道了" },
  })
end
