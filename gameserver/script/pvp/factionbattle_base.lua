  ...
  if pPlayer then
    ...
    if self.nState == FactionBattle.SIGN_UP then
      if nCurPlayerCount >= FactionBattle.MAX_ATTEND_PLAYER then
        szMsg = string.format("报名人数已达到%d人上限，不能再报名了", FactionBattle.MAX_ATTEND_PLAYER)
...
function tbBaseFaction:TimerStart(szFunction)
  ...
  self.nTimerId = Timer:Register(tbTimer[2] * Env.GAME_FPS, self.TimerStart, self, tbTimer[3]) -- 开启新的定时
  -- 新模式，在开始的时候都要提醒
  if FactionBattle.FACTIONBATTLE_MODLE == FactionBattle._MODEL_NEW then
    local szMsgBlack = ""
    local szMsgSys = ""
    ...
    if self.nState == FactionBattle.SIGN_UP then
      szMsgSys = string.format("30秒后进入晋级赛第1/4阶段。请注意，新赛制中，4轮晋级赛每轮均随机平均分配敌我两个阵营，根据重伤他人的数量获得门派竞技积分，进阶16强。", self.nMeleeCount + 1)
      szMsgBlack = string.format("30秒后进入晋级赛第1/4阶段。")
    elseif self.nState == FactionBattle.MELEE_REST then
      szMsgBlack = string.format("30秒后将进入晋级赛第%d/4阶段，将自动重新分配阵营", self.nMeleeCount + 1)
      szMsgSys = szMsgBlack
    elseif self.nState == FactionBattle.READY_ELIMINATION then
      if self.nEliminationCount < 4 then
        szMsgSys = "30秒后将进入下一阶段比赛，请做好准备！"
      elseif nEliminationCount == 4 then
        szMsgSys = "30秒后将进入决赛阶段，请做好准备！"
      end
      szMsgBlack = szMsgSys
    end
  ...
  for i, nId in ipairs(tbTmpId) do
    ...
    if pPlayer then
      ...
      if FactionBattle._MODEL_OLD == FactionBattle.FACTIONBATTLE_MODLE then
        szMsg = string.format("当前累计连胜：%s\n\n目前排名：%s", tbPlayer[nId].nScore, tbPlayer[nId].nSort)
        if self.nMeleeCount and self.nMeleeCount > 4 then -- 混战结束休息时间
          szMsg = ""
        end
      else
        szMsg = string.format("本场积分：%d\n\n总积分：%d\n\n目前排名：%d", tbPlayer[nId].OnceScore, tbPlayer[nId].nScore, tbPlayer[nId].nSort)
      end
...
function tbBaseFaction:UpdateMeleePlayerTimer(nPlayerId, bShowMsg)
  ...
  local szTimeFmt = ""
  if FactionBattle.FACTIONBATTLE_MODLE == FactionBattle._MODEL_NEW then
    szTimeFmt = "<color=green>" .. self.nMeleeCount .. "/4轮晋级比赛阶段：<color><color=white>%s<color>\n"
  else
...
function tbBaseFaction:BeginAddExp()
  if FactionBattle._MODEL_OLD == FactionBattle.FACTIONBATTLE_MODLE then
    self:BoardMsgToMapPlayer("你进入了门派竞技场，在活动期间，会持续获得经验奖励!")
  end
  Timer:Register(FactionBattle.ADDEXP_SECOND_PRE_TIME * Env.GAME_FPS, self.AddExp, self)
end
...
function tbBaseFaction:AddExp()
  ...
  Timer:Register(1, self._AddExp, self, 1) -- 分帧+经验
end
...
function tbBaseFaction:_AddExp(nIndex)
  ...
  Timer:Register(1, self._AddExp, self, nIndex + 1)
  return 0
end
...
function tbBaseFaction:AssignPlayerToMelee()
  ...
  -- 混战保护时间
  self.nFightTimerId = Timer:Register(FactionBattle.MELEE_PROTECT_TIME * Env.GAME_FPS, self.ChangeFight, self)
  return 1
end
...
function tbBaseFaction:RestartMelee()
  ...
  -- 混战保护时间
  self.nFightTimerId = Timer:Register(FactionBattle.MELEE_RESTART_PROTECT * Env.GAME_FPS, self.ChangeFight, self)
end
...
function tbBaseFaction:KickPlayerFromArena(nPlayerId)
  ...
  if tbPlayer[nPlayerId] then
    ...
    if nRet ~= 1 then -- 结束该混战区域活动
      ...
      Timer:Register(FactionBattle.END_DELAY * Env.GAME_FPS, self.CloseArena, self, nArenaId)
    end
  end
end
...
function tbBaseFaction:OnDeathInMelee(pKillerNpc)
  ...
  if tbPlayer[pKillerPlayer.nId] and self.nState == FactionBattle.MELEE then -- 混战模式下加分处理
    ...
    tbPlayer[me.nId].nTimerId = Timer:Register(FactionBattle.RETURN_TO_MELEE_TIME[nDeath] * Env.GAME_FPS, self.ReturnToMelee, self, me.nId)
    ...
    me.Msg("你被<color=yellow>" .. pKillerPlayer.szName .. "<color>击败了，<color=green>" .. FactionBattle.RETURN_TO_MELEE_TIME[nDeath] .. "秒<color>后重新投入战斗！")
  end
end
...
function tbBaseFaction:OnDeathInElimin(pKillerNpc)
  ...
  Timer:Register(FactionBattle.END_DELAY * Env.GAME_FPS, self.AutoRevivePlayer, self, me.nId)
...
function tbBaseFaction:AssignPlayerToElimination()
  ...
  for i = 1, math.ceil(nPlayerNum / 2) do
    ...
    if pPlayer1 and pPlayer2 and pPlayer1.nMapId == self.nMapId and pPlayer2.nMapId == self.nMapId then
      ...
      if 4 == self.nEliminationCount then
        szMsg = "决赛阶段开始，30秒后进入战斗，请注意"
      elseif self.nEliminationCount < 4 then
        local nNum = self.nEliminationCount
        if nNum <= 0 then
          nNum = 1
        end
        szMsg = string.format("%d进%d淘汰赛开始，30秒后开始战斗，请注意！", nPlayerNum / nNum, math.ceil(nPlayerNum / (2 * nNum)))
      end
      if szMsg ~= "" then
        pPlayer2.Msg(szMsg)
        pPlayer1.Msg(szMsg)
  ...
  -- 进入保护时间
  self.nFightTimerId = Timer:Register(FactionBattle.ELIMI_PROTECT_TIME * Env.GAME_FPS, self.ChangeFight, self)
end
...
function tbBaseFaction:SetEliminationWinner(nArenaId, nWinnerId, nLoserId, bCalcDamiage)
  ...
  if pLoser then
    ...
    if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
      if self.nEliminationCount < 3 then
        szMsg = "淘汰赛结束，很遗憾您没进晋级，您可观看决赛并抢宝箱"
      elseif self.nEliminationCount == 3 then
        szMsg = "淘汰赛结束，很遗憾您没进入决赛，您可观看决赛并抢宝箱"
      elseif self.nEliminationCount == 4 then
        szMsg = "决赛阶段结束，很遗憾您未获得冠军，再接再厉！"
      end
    else
      szMsg = "你落败了，失去了晋级下一轮的资格"
    end
    pLoser.Msg(szMsg)
    if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
  ...
  if self.tbSportscast[nWinnerId].nWinCount >= 4 then -- nWinCount ==淘汰赛胜利次数
    if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
      pWinner.Msg("决赛阶段结束，恭喜您获得了本届冠军！")
      Timer:Register(Env.GAME_FPS, self.SendBlackMsg2Player, self, pWinner.nId, "决赛阶段结束，恭喜您获得了本届冠军！")
    else
      pWinner.Msg("恭喜你获得冠军！请到地图中央领奖台处领取冠军旗帜")
      self:BoardMsgToMapPlayer("恭喜[" .. pWinner.szName .. "]获得冠军，冠军选手可以到中央领奖台领取冠军旗帜")
    end
    self:MsgToMapPlayer("恭喜<color=yellow>" .. pWinner.szName .. "<color>获得冠军，同时获得候选人资格！")

    --向门派竞技冠军玩家推送SNS通知
    local szPopupMessage = "祝贺您获得<color=yellow>门派竞技冠军<color>！\n把这个好消息分享给朋友们吧！"
    local szTweet
    if pLoser then
      szTweet = string.format("#剑侠世界# 我打败%s赢得门派竞技冠军！呵呵……", pLoser.szName)
    else
      szTweet = "#剑侠世界# 刚刚赢得了门派竞技冠军！呵呵……"
    end
    ...
    pWinner.SendMsgToFriend("您的好友[" .. pWinner.szName .. "]" .. szWinMsg)
    Player:SendMsgToKinOrTong(pWinner, szWinMsg, 1)
    if pLoser then
      pLoser.SendMsgToFriend("您的好友[" .. pLoser.szName .. "]" .. szLoseMsg)
    ...
    if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
      if self.nEliminationCount == 3 then
        szMsg = "淘汰赛结束，恭喜您进入了决赛，加油啊！"
      else
        szMsg = "你击败了对手，获得了晋级下一轮的资格！"
      end
  ...
  if self.tbSportscast[nWinnerId].nWinCount == 2 then
    ...
    pWinner.SendMsgToFriend("您的好友[" .. pWinner.szName .. "]" .. szMsg)
  ...
  elseif self.tbSportscast[nWinnerId].nWinCount == 3 then
    --进入决赛
    szMsg = "在" .. Player:GetFactionRouteName(pWinner.nFaction) .. "的门派竞技赛中进入决赛。"
    pWinner.SendMsgToFriend("您的好友[" .. pWinner.szName .. "]" .. szMsg)
    Player:SendMsgToKinOrTong(pWinner, szMsg, 0)
  end
...
function tbBaseFaction:StartMelee()
  ...
  self:MsgToMapPlayer("本门竞技比赛正式开始！首先进入自由切磋阶段")
...
function tbBaseFaction:EndMelee()
  ...
  self.nMeleeCount = self.nMeleeCount + 1
  Timer:Register(FactionBattle.MELEE_END_ANOUNCE_TIME * Env.GAME_FPS, self.AnounceTime, self)
end
...
function tbBaseFaction:StartElimination()
  ...
  if FactionBattle.BOX_NUM[self.nEliminationCount][1] > 2 then
    szMsg = FactionBattle.BOX_NUM[self.nEliminationCount][1] .. "强比赛开始了！参赛选手将被传入指定擂台"
  else
  ...
  if self.nEliminationCount < 4 then -- 决赛不需要检测
    self.nEliminationTimerId = Timer:Register(FactionBattle.ELIMI_PROTECT_TIME * Env.GAME_FPS, self.CheckEliminationIsOver, self)
  end
  ------------------------------------------------------------------------------------------
end
...
function tbBaseFaction:EndElimination()
  ...
  if nIndex > 2 and FactionBattle._MODEL_OLD == FactionBattle.FACTIONBATTLE_MODLE then
    szMsg = string.format("%s强比赛结束，%s强选手已产生，请晋级选手做好准备，比赛将在7分钟后进行", tonumber(nIndex), tonumber(math.ceil(nIndex / 2)))
    self:MsgToMapPlayer(szMsg)
  end
  if self.tbArena then
    for i in pairs(self.tbArena) do
...
function tbBaseFaction:AnounceTime()
  ...
  if FactionBattle.BOX_NUM[self.nEliminationCount + 1][1] > 2 then
    self:MsgToMapPlayer(FactionBattle.BOX_NUM[self.nEliminationCount + 1][1] .. "强淘汰赛还有60秒请晋级选手做好准备")
  else
...
function tbBaseFaction:ShutDown(bCamplete)
  ...
  if bCamplete == 1 then
    if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE then
      self:MsgToMapPlayer("本届门派竞技圆满结束，希望各位努力提升武艺，再接再厉！")
      self:BoardMsgToMapPlayer("本届门派竞技圆满结束，希望各位努力提升武艺，再接再厉！")
    else
      self:MsgToMapPlayer("此次本门竞技比赛圆满结束！")
      self:BoardMsgToMapPlayer("此次本门竞技比赛圆满结束！")
    end
  else
    self:MsgToMapPlayer("门派竞技比赛关闭！")
  end
