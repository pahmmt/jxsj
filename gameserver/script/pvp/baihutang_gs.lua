...
function BaiHuTang:TimerStart(szFunction)
  ...
  Timer:Register(tbTimer[2], self.TimerStart, self, tbTimer[3]) -- 开启新的定时
  return 0
end
...
function BaiHuTang:ShowTimeInfo(pPlayer, IsPK)
  ...
  local szMsgFormat = ""
  if IsPK == 1 then
    szMsgFormat = "<color=green>剩余时间：<color> <color=white>%s<color>"
  else
...
function BaiHuTang:AddFriendFavor(pSelf, tbPlayerList, nFavor)
  ...
  for _, pPlayer in ipairs(tbPlayerList) do
    if pPlayer.nId ~= pSelf.nId and pPlayer.nTeamId == pSelf.nTeamId and pSelf.IsFriendRelation(pPlayer.szName) == 1 then
      Relation:AddFriendFavor(pSelf.szName, pPlayer.szName, nFavor)
      pSelf.Msg(string.format("您与<color=yellow>%s<color>好友亲密度增加了%d点。", pPlayer.szName, nFavor * 2))
    end
  end
end
...
function BaiHuTang:GetFloor(nMapId)
  if (nMapId >= 226 and nMapId <= 229) or (nMapId >= 275 and nMapId <= 278) or (nMapId >= 334 and nMapId <= 337) or (nMapId >= 234 and nMapId <= 237) or (nMapId >= 822 and nMapId <= 825) then
    return 1
  elseif (nMapId >= 230 and nMapId <= 231) or (nMapId >= 279 and nMapId <= 280) or (nMapId >= 338 and nMapId <= 339) or (nMapId >= 238 and nMapId <= 239) or (nMapId >= 826 and nMapId <= 827) then
    return 2
...
function BaiHuTang:WritePlayerInfo__GS()
  local tbMapId = { 226, 227, 228, 229, 234, 235, 236, 237, 275, 276, 277, 278, 334, 335, 336, 337, 822, 823, 824, 825 }
  for _, nMapId in ipairs(tbMapId) do
    if SubWorldID2Idx(nMapId) >= 0 then
      for _, pPlayer in pairs(KPlayer.GetMapPlayer(nMapId)) do
        if pPlayer.GetExtMonthPay() < self.nMinPay then
          GCExcute({ "BaiHuTang:WritePlayerInfo__GC", pPlayer.szAccount, pPlayer.szName, pPlayer.GetPlayerIpAddress() })
        end
      end
    end
  end
