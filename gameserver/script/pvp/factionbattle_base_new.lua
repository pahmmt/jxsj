...
function tbBaseFaction:BrushXiangZi(nTimes, nMaps, nItemNum, nPickTime, nDuration, nInterval, tbCallBack)
  ...
  for i, point in pairs(tbPoint) do
    Npc.tbXiangZiNpc:AddBox(self.nMapId, point.TRAPX, point.TRAPY, nPickTime * Env.GAME_FPS, tbParam, 1, nDuration * Env.GAME_FPS)
  end
  self.nBoxTimerId = 0
  -- 注册
  if nTimes - 1 > 0 then
    self.nBoxTimerId = Timer:Register(nInterval * Env.GAME_FPS, self.BrushXiangZi, self, nTimes - 1, nMaps, nItemNum, nPickTime, nDuration, nInterval, tbCallBack)
  end
...
function tbBaseFaction:EliminationAword()
  ...
  if tbAword[3] == 1 then
    self:BrushXiangZi(tbAword[1], tbAword[2], nBoxNum, FactionBattle.TAKE_BOX_TIME, FactionBattle.N_BOX_EXSIT_TIME, FactionBattle.N_BOX_INTERVAL_TIME, tbCallBack)
  else
    self.nBoxTimerId = Timer:Register(FactionBattle.N_BOX_INTERVAL_TIME, self.BrushXiangZi, self, tbAword[1], tbAword[2], nBoxNum, FactionBattle.TAKE_BOX_TIME, FactionBattle.N_BOX_EXSIT_TIME, FactionBattle.N_BOX_INTERVAL_TIME, tbCallBack)
  end
end
-- 冠军点击旗帜，刷箱子
...
function tbBaseFaction:ChampionAword_New(pPlayer)
  ...
  if self.nChampionAwordCount < FactionBattle.CHAMPION_AWARD_COUNT then
    ...
    self.nChampionAwordTimerId = Timer:Register(FactionBattle.CHAMPION_AWARD_CD_TIME * Env.GAME_FPS, self.ChampionAwordCoolDown, self, pPlayer.nId)
    -- 发奖励提示
    if self.nChampionAwordCount ~= FactionBattle.CHAMPION_AWARD_COUNT then
      self:UpdateMapPlayerInfo()
    else
