...
function FactionBattle:StartSignUp(nModel)
  ...
  local szMsg = "门派竞技赛已经开启，请玩家从门派掌门人处进入门派竞技场!"
...
function FactionBattle:OnLeaveMap(nFaction)
  ...
  if tbData then
    ...
    if tbData:FindAttendPlayer(nPlayerId) == 1 and tbAttendPlayer then -- 暂时恢复原来状态
      ...
      -- 移除称号
      if FactionBattle._MODEL_NEW == FactionBattle.FACTIONBATTLE_MODLE and tbInfo.nCamp and FactionBattle.TB_TITLE and (tbInfo.nCamp == FactionBattle.CAMP_RED or tbInfo.nCamp == FactionBattle.CAMP_BLUE) then
