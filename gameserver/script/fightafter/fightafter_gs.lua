...
function FightAfter:EnterRoomForMeReady()
  ...
  Dialog:Say(szMsg, { { "进入得悦舫", self.ApplyFreeRoomForMe, self }, { "我再考虑考虑" } })
end
...
function FightAfter:Award(pPlayer, szInstanceId)
  ...
  for nIndex, tbItemInfo in ipairs(tbPlayerAward.tbItemList) do
    nNeedCount = nNeedCount + KItem.GetNeedFreeBag(tbItemInfo.tbItemId[1], tbItemInfo.tbItemId[2], tbItemInfo.tbItemId[3], tbItemInfo.tbItemId[4], nil, tbItemInfo.nItemCount)
  end
  ...
  local nAddCount = 0
  for nIndex, tbItemInfo in ipairs(tbPlayerAward.tbItemList) do
    nAddCount, szItemName = pPlayer.AddStackItem(tbItemInfo.tbItemId[1], tbItemInfo.tbItemId[2], tbItemInfo.tbItemId[3], tbItemInfo.tbItemId[4], { bForceBind = 1 }, tbItemInfo.nItemCount)
...
function FightAfter:AwardLog(tbInstance, szPlayerName, szItemName, nItemCount)
  if tbInstance.nType == self.emTYPE_TREASURE then
    TreasureMap2:AwardLog(tbInstance.nTreasureId, tbInstance.nLevel, szPlayerName, szItemName, nItemCount, tbInstance.bRandom)
  end
end
