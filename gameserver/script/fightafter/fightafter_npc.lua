...
function tbDialogNpc:OnDialog()
  local tbOpt = {}
  local szMsg = "想要评价并获得奖励吗？去参加些活动吧！想要再找到我吗？去找包万同附近的义军军需官吧！\n"

  local tbInstanceList = FightAfter:GetExpiryInstanceList(me)
  if #tbInstanceList ~= 0 then
    szMsg = szMsg .. "<color=orange>（请在下列时间点后24小时内领奖）<color>\n"
  else
    szMsg = szMsg .. "<color=orange>（您没有未领取的奖励）<color>\n"
  end
  ...
  table.insert(tbOpt, { "我要离开这儿", self.LeaveHere, self })
...
function tbDialogNpc:GeTime(nSec)
  ...
  if nCurDate ~= nCompleteDate then
    szMsg = "<color=yellow>昨日，" .. szCompleteHour
  else
    szMsg = "<color=orange>今日，" .. szCompleteHour
  end
