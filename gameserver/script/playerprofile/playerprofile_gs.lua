...
function PProfile:ApplyEditStrInfo(nOper, szStr, nVersion)
  ...
  elseif nOper == self.emPF_BUFTASK_TTENCENT or nOper == self.emPF_BUFTASK_TSINA then
...
function PProfile:ApplyEditAllInfo2(szRealName, nSex, nBirth, szCity, nReins, szQianming, nFriendOnly)
  if type(szRealName) ~= "string" or type(nSex) ~= "number" or type(nBirth) ~= "number" or type(szCity) ~= "string" or type(nReins) ~= "number" or type(szQianming) ~= "string" or type(nFriendOnly) ~= "number" then
    return
  end
...
function PProfile:GetSnsInfo(nClientRequestId, tbPlayerName)
  if (type(nClientRequestId) ~= "number" or type(tbPlayerName) ~= "table") or #tbPlayerName == 0 then
