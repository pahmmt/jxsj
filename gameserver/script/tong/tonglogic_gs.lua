...
function Tong:StopMasterVote_GS2(nTongId, nKinId, nMaxBallot)
  ...
  if cKinNewMaster then
    ...
    local szName = KGCPlayer.GetPlayerName(nMasterId)
    KTong.Msg2Tong(nTongId, "本届帮会竞选结束，<color=white>" .. cKinNewMaster.GetName() .. "<color>家族的族长<color=green>" .. szName .. "<color>以<color=red>" .. (nMaxBallot / 100) .. "%<color>的票数当选为新一任帮主！")
  end
  --解除帮主锁定状态
  cTong.SetMasterLockState(0)
  return 1
...
function Tong:ApplyJoin_GS1(nPlayerId, bConfirm)
  ...
  if not bConfirm or 1 ~= bConfirm then
    if nKinFund > 0 and nKinFund > (self.MAX_BUILD_FUND - nBuildFund) then
      ...
      Dialog:Say(szMsg, {
        { "确定", self.ApplyJoin_GS1, self, nPlayerId, 1 },
        { "取消" },
      })
      return 0
    end
...
function Tong:InviteAddReply_GS1(nPlayerId, bAccept)
  ...
  if cTagetKin.GetBelongTong() ~= 0 then
    me.Msg("您的家族已经有帮会了。")
    return 0
  end
  if cTong.GetKinCount() >= self.MAX_KIN_NUM then
...
function Tong:FireKin_GS2(nTongId, nSelfKinId, nPlayerId, nTagetKinId)
  ...
  tbData.nTimerId = Timer:Register(self.FIREKIN_APPLY_LAST, self.CancelExclusiveEvent_GS, self, nTongId, nPlayerId, self.REQUEST_KICK_KIN)
  local cKin = KKin.GetKin(nTagetKinId)
  if not cKin then
    return 0
  end
...
function Tong:ApplyDispense_GS2(nTongId, nKinId, nPlayerId, nType, nAmount, nRequset)
  ...
  -- 状态持10分钟
  tbData.ApplyRecord.nTimerId = Timer:Register(self.DISPENSE_APPLY_LAST, self.CancelExclusiveEvent_GS, self, nTongId, nPlayerId, nRequset)
  local szPlayerName = KGCPlayer.GetPlayerName(nPlayerId)
  -- 寻找拥有资金权限的人员通知有申请
  local cTong = KTong.GetTong(nTongId)
  if not cTong then
...
function Tong:ApplyTakeFund_GS2(nTongId, nKinId, nPlayerId, nMoney)
  ...
  tbData.ApplyRecord.nTimerId = Timer:Register(self.TAKEFUND_APPLY_LAST, self.CancelExclusiveEvent_GS, self, nTongId, nPlayerId, self.REQUEST_TAKE_FUND)
  local szPlayerName = KGCPlayer.GetPlayerName(nPlayerId)
  -- 寻找拥有资金权限的人员通知有申请
  local cTong = KTong.GetTong(nTongId)
  if not cTong then
...
function Tong:TakeFund_GS2(nTongId, nPlayerId, nDataVer, nMoney, nCurFund)
  ...
  cPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_TONGPAYOFF, string.format("玩家：%s, 帐号:%s, 从帮会：%s 领取了%d的资金,帮会还有%d的资金", cPlayer.szName, cPlayer.szAccount, cTong.GetName(), nMoney, nCurFund))
end
...
function Tong:SetCapital_GS1(nTongId, nDomainId, bConfirm)
  ...
  if bConfirm == 1 then
    ...
    local szMsg = string.format("您要将帮会的主城更换为<color=green>%s<color>。这是您的帮会<color=green>第%d次<color>更换主城，需要支付<color=green>%d<color>帮会建设资金。", szCapital, nChangeCount, nCost)
    Dialog:Say(szMsg, {
...
function Tong:SetCapital_GS2(nTongId, nDomainId, nCost, nChangeCount, nDataVer)
  ...
  if szCapital then
    ...
    KTong.Msg2Tong(nTongId, string.format("帮会扣除建设资金<color=green>%d<color>，主城更改为<color=green>%s<color>。", nCost, szCapital))
  end
end
...
function Tong:AddBuildFund_GS2(nKinId, nMemberId, nMoney, bTongShow, bSelfShow)
  ...
  if pPlayer and bSelfShow == 1 then
    pPlayer.Msg(string.format("您的个人资产增加<color=yellow>%d<color>", nMoney))
  end
  if nTongId ~= 0 and bTongShow == 1 then
    KTong.Msg2Tong(nTongId, string.format("<color=white>%s<color>增加了帮会建设资金<color=green>%d<color>", KGCPlayer.GetPlayerName(nPlayerId), nMoney))
  end
end
...
function Tong:AddBuildFund2_GS2(nTongId, nMoney)
  ...
  if pTong then
    ...
    KTong.Msg2Tong(nTongId, string.format("帮会建设资金增加了<color=green>%d<color>", nMoney))
  end
end
...
function Tong:CostBuildFund_GS2(nTongId, nCostedBuildFund, nMoney, bNeedMsg)
  ...
  if bNeedMsg >= 1 then
    KTong.Msg2Tong(nTongId, string.format("使用了<color=green>%d<color>帮会建设资金， 本周使用建设资金额度剩余<color=green>%d。", nMoney, nRest))
  end
end

-- 设置帮会建设资金使用上限
function Tong:SetBuildFundLimit_GS1(nTongId, nKinId, nMemberId, nMoneyLimit)
  if not nTongId or not nKinId or not nMemberId or not nMoneyLimit or 0 == Lib:IsInteger(nTongId) or 0 == Lib:IsInteger(nKinId) or 0 == Lib:IsInteger(nMemberId) or 0 == Lib:IsInteger(nMoneyLimit) or nMoneyLimit < 0 or nMoneyLimit > self.MAX_BUILD_FUND then
    return 0
  end
  local pTong = KTong.GetTong(nTongId)
  if not pTong then
...
function Tong:AddStockBaseCount_GS1(nPlayerId, nStockBaseCount, nPersonalRate, nTongRate, nCaptainRate, nAssistantRate, nMasterRate, nType)
  ...
  if not nPersonalRate or nPersonalRate < 0 then
    nPersonalRate = 0
  end
  if not nTongRate or nTongRate < 0 then
    nTongRate = 0
  end
  if not nCaptainRate or nCaptainRate < 0 then
    nCaptainRate = 0
  end
  if not nAssistantRate or nAssistantRate < 0 then
    nAssistantRate = 0
  end
  if not nMasterRate or nMasterRate < 0 then
    nMasterRate = 0
  end
...
-- 个人官衔维护成功
function Tong:OfficialMaintain_GS2(nTongId, nKinId, nMemberId, nPlayerId, nPersonalStock, nTotalStock, nMaintainNo, nOfficialLevel, nDataVer)
  ...
  if tbTitles then
    for _, pTitle in ipairs(tbTitles) do
      if pTitle.byTitleGenre == Tong.OFFICIAL_TITLE_GENRE then
        pPlayer.RemoveTitle(pTitle.byTitleGenre, pTitle.byTitleDetailType, pTitle.byTitleLevel, 0)
      end
    end
  end
  return 1
...
function Tong:ElectGreatMember_GS2(nSelfPlayerId, nTagetKinId, nTagetMemberId, nVote)
  ...
  if pTagetMember then
    ...
    local nMemberVoteNo = KGblTask.SCGetDbTaskInt(DBTASK_GREAT_MEMBER_VOTE_NO)
    Dbg:WriteLog("ElectGreatMember", "优秀成员投票成功", "投票者：" .. szSelfName, "受票者：" .. szTagetName, "票数：" .. nVote, "流水号" .. nMemberVoteNo)
  end
...
function Tong:ApplyStorageFundToKin_GS2(nTongId, nKinId, nPlayerId, nMoney, nTargetKinId)
  ...
  tbData.ApplyRecord.nTimerId = Timer:Register(self.TAKEFUND_APPLY_LAST, self.CancelExclusiveEvent_GS, self, nTongId, nPlayerId, self.REQUEST_STORAGE_FUND_TO_KIN)
