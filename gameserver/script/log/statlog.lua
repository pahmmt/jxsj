...
function StatLog:WriteStatLog(szKeyWord, szBigType, szSubType, nPlayerId, ...)
  ...
  -- lqy add
  -- 传入玩家ID的方式在玩家离线后会失效。这里新增了传入玩家信息table的方式
  -- 传入格式为{szAccount, szName}
  local szAccount = ""
  local szName = ""
  if type(nPlayerId) == "number" then --如果是Id
    szAccount, szName = self:__WriteStatLog_GetAccName(nPlayerId)
  elseif type(nPlayerId) == "table" then --如果是table
    if not nPlayerId[1] or not nPlayerId[2] then
      return
    end
    szAccount, szName = nPlayerId[1], nPlayerId[2]
  else
    return
  end
