...
-- 合并2个非顺序表，用于下标默认的表
function Lib:MergeTable2(tableA, tableB)
  local tbMerge = {}
...
function Lib:TransferDigit2CnNum(nDigit)
  ...
  for i = 1, #tbModelUnit do
    ...
    if n4LenNum ~= 0 then
      ...
      if
        (nPreNum > 0 and nPreNum < 1000) -- 两个数字之间有0,所以要加"零"
        or (math.floor(n4LenNum % 10) == 0 and i > 1)
      then
        szNumTmp = szNumTmp .. "零"
      end
    end
...
function Lib:StrFillC(szStr, nLen, szFilledChar)
  ...
  szStr = string.rep(szFilledChar, nLeftCharNum) .. szStr .. string.rep(szFilledChar, nRightCharNum) -- 补齐
  return szStr
end
