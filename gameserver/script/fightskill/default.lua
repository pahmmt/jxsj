...
function tbDefault:OnLevelUp() end
...
function tbDefault:GetDesc(tbThisInfo, bNext, bShowTitle, bNoNeedLevel) -- bNoNeedLevel 不需要自身能级也能取到对应技能TIP
  ...
  if not bNext or bShowTitle then
    ...
    if tbHorseSkill[tbInfo.nId] and ((tbInfo.nBaseLevel or tbInfo.nLevel) < KFightSkill.GetSkillMaxLevel(tbInfo.nId)) then
      tbMsg[#tbMsg + 1] = string.format("<color=green>装备%s时处于修炼珠的开启修炼修为状态下打怪可以修炼此技能<color>", tbHorseSkill[tbInfo.nId])
    end
  end
  ...
  if not bNext or bShowTitle then
    ...
    if tbInfo.nSeries ~= 0 then
      -- or 后面的部分用于出错保护
      tbMsg[#tbMsg + 1] = string.format("\n五行：<color=%s>%s系<color>", tbSeriesColor[tbInfo.nSeries] or "", Env.SERIES_NAME[tbInfo.nSeries] or tostring(tbInfo.nSeries))
    end
    ...
    if tbInfo.nWeaponLimited ~= 0 then
      ...
      tbMsg[#tbMsg + 1] = string.format("武器限制：<color=gold>%s<color>", tbWeaponName[tbInfo.nWeaponLimited] or tostring(tbInfo.nWeaponLimited))
    end
    ...
    if tbInfo.nHorseLimited ~= 0 then
      ...
      tbMsg[#tbMsg + 1] = string.format("%s\n", tbHoresName[tbInfo.nHorseLimited] or ("骑马限制？：" .. tostring(tbInfo.nHorseLimited)))
    else
      tbMsg[#tbMsg + 1] = ""
    end
  end
...
function tbDefault:GetDescAboutLevel(tbMsg, tbInfo, bShow)
  ...
  if bShow == 1 then
    ...
    if tbInfo.nCost ~= 0 then
      tbMsg[#tbMsg + 1] = string.format("<color=blue>%s消耗：<color><color=gold>%d点<color>", FightSkill.COSTTYPE_NAME[tbInfo.nCostType] or tostring(tbInfo.nCostType) .. "?", tbInfo.nCost)
    end
    ...
    if tbInfo.nMinTimePerCast > 0 then
      if tbInfo.nMinTimePerCast == tbInfo.nMinTimePerCastOnHorse or tbInfo.nMinTimePerCastOnHorse <= 0 or tbInfo.nHorseLimited == 1 then
        ...
        tbMsg[#tbMsg + 1] = string.format("<color=blue>施展间隔：<color><color=gold>%s秒<color>", Frame2Sec(nMinTimePerCast))
      else
        local nMinTimePerCast = math.max(0, tbInfo.nMinTimePerCastOnHorse - KFightSkill.GetDecreaseSkillCastTime(tbInfo.nId))
        tbMsg[#tbMsg + 1] = string.format("<color=blue>施展间隔：<color><color=gold>%s秒<color>\n<color=blue>骑马施展间隔时间：<color><color=gold>%s秒<color>", Frame2Sec(tbInfo.nMinTimePerCast), Frame2Sec(nMinTimePerCast))
      end
    end
  end
  ...
  if tbInfo.nStateTime > 0 and tbInfo.nIsAura ~= 1 then
    if tbInfo.nId ~= 1259 then --隐藏反两仪刀法技能持续时间显示
      tbMsg[#tbMsg + 1] = string.format("<color=white>持续时间：<color><color=gold>%s秒<color>", Frame2Sec(tbInfo.nStateTime))
    end
  end
...
-- 技能参数说明
function tbDefault:GetParamDesc(tbMsg, nParam1, nParam2) end
