...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("生命降到25%%时触发几率：<color=gold>%d%%<color>\n不受任何伤害技能影响，持续<color=Gold>%s秒<color>\n攻击会心一击值<color=gold>增加%s<color>\n造成受伤的几率<color=gold>增加%s<color>，持续<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, FightSkill:Frame2Sec(tbChildInfo.nStateTime), tbCCInfo.tbWholeMagic["deadlystrikeenhance_r"][1], tbCCInfo.tbWholeMagic["state_hurt_attackrate"][1], FightSkill:Frame2Sec(tbCCInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  szMsg = szMsg .. string.format("生命降到25%%时触发几率：<color=gold>%d%%<color>\n", tbAutoInfo.nPercent)
  szMsg = szMsg .. "    " .. "<color=green>天王本生<color>\n"
  for i = 1, #tbMsg do
...
function tbSkill:GetAutoDesc2(tbAutoInfo, tbSkillInfo)
  ...
  if tbAutoInfo.nPercent >= 1 then
    szMsg = szMsg .. "\n被击中时，有<color=Gold>" .. tbAutoInfo.nPercent .. "%<color>的概率获得以下效果：\n"
    szMsg = szMsg .. "    " .. "<color=green>不屈<color>\n"
    local tbMsg = {}
    FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
    for i = 1, #tbMsg do
      szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  szMsg = szMsg .. "攻击命中时自动叠加以下2种状态：\n"
  szMsg = szMsg .. "    " .. "<color=green>连环夺命<color>\n"
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  --子技能描述
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  szMsg = szMsg .. "被攻击时自动叠加以下状态：\n"
  szMsg = szMsg .. "    " .. "<color=green>乾坤倒悬<color>\n"
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n触发间隔时间：<color=Gold>" .. FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime) .. "秒<color>"
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = "状态中，附近友方也可获得定身以外的免疫效果"
  return szMsg
end
