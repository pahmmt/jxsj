...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  szMsg = szMsg .. "每<color=Gold>" .. FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime) .. "秒<color>自动释放：\n"
  szMsg = szMsg .. "    <color=green>无形毒蛊<color>\n"
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n    毒蛊存在时间：<color=gold>10秒<color>"
  return szMsg
end
...
function tbSkill:GetExtraDesc(tbInfo)
  ...
  szMsg = szMsg .. "<color=green>[星空破] " .. tbInfo.nLevel .. "级<color>\n"
  for i = 1, #tbMsg do
    szMsg = szMsg .. "" .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  return szMsg
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  szMsg = szMsg .. "攻击时自动对目标施加以下诅咒：\n"
  szMsg = szMsg .. "    <color=green>瘟蛊之气<color>\n"
  szMsg = szMsg .. "    生命降到25%时获得以下状态：\n"
  ...
  szMsg = szMsg .. "\n    持续时间：<color=Gold>" .. FightSkill:Frame2Sec(tbChildInfo.nStateTime) .. "秒<color>"
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  szMsg = szMsg .. "生命降到25%时触发几率：<color=gold>" .. tbAutoInfo.nPercent .. "%<color>\n"
  szMsg = szMsg .. "    <color=green>驱毒术<color>\n"
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = ""
  szMsg = szMsg .. "造成会心时<color=gold>" .. tbAutoInfo.nPercent .. "<color>%几率对敌释放：\n"
  szMsg = szMsg .. "    <color=green>截脉式<color>\n"
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("\n<color=green>子招式<color>\n造成会心一击时触发几率：<color=gold>%d%%<color>\n最多同时影响目标：<color=gold>%d个<color>\n造成对手定身的几率：<color=gold>%d%%<color>，持续<color=Gold>%s秒<color>\n造成对手虚弱的几率：<color=gold>%d%%<color>，持续<color=Gold>%s秒<color>\n自身清除并免疫<color=gold>受伤、迟缓、眩晕<color>，持续<color=gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, tbChildInfo.nMissileHitcount, tbChildInfo.tbWholeMagic["state_fixed_attack"][1], FightSkill:Frame2Sec(tbChildInfo.tbWholeMagic["state_fixed_attack"][2]), tbChildInfo.tbWholeMagic["state_weak_attack"][1], FightSkill:Frame2Sec(tbChildInfo.tbWholeMagic["state_weak_attack"][2]), FightSkill:Frame2Sec(tbCCInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = string.format("\n击中敌人时对目标附加以下状态：\n毒攻抗性：<color=gold>减少%d点<color>\n最多叠加：<color=gold>%d次<color>\n持续时间：<color=Gold>%s秒<color>", -tbChildInfo.tbWholeMagic["damage_poison_resist"][1], tbChildInfo.tbWholeMagic["superposemagic"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  szMsg = szMsg .. "每<color=gold>" .. FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime) .. "秒<color>对周围敌人施加以下状态：\n"
  local tbMsg = {}
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n攻击命中可使<color=green>[失心蛊]<color>攻击增加效果减少1/3，持续4秒"
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = ""
  szMsg = szMsg .. "攻击命中时使<color=green>[失心蛊]<color>攻击增加效果减少1/3，持续4秒"
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  local szMsg = ""
  szMsg = szMsg .. "攻击命中时自动释放以下诅咒：\n"
  szMsg = szMsg .. "    <color=green>万蛊蚀心<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  szMsg = szMsg .. "攻击命中时自动释放以下诅咒：\n"
  szMsg = szMsg .. "    <color=green>悲魔血光<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
