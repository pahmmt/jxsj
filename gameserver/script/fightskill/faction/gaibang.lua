...
local tb = {
  ...
  kanglongyouhui = { --亢龙有悔_20
    appenddamage_p = { { { 1, 50 * 0.4 * 0.8 + 1 }, { 20, 50 * 0.4 + 1 }, { 21, 50 * 0.4 * nA0 + 1 } } },
    firedamage_v = {
      [1] = { { 1, 640 * 0.9 * 0.4 * 0.8 }, { 20, 640 * 0.9 * 0.4 }, { 21, 640 * 0.9 * 0.4 * nA0 } },
      [3] = { { 1, 640 * 1.1 * 0.4 * 0.8 }, { 20, 640 * 1.1 * 0.4 }, { 21, 640 * 1.1 * 0.4 * nA0 } },
    },
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format(
    "攻击时使<color=gold>%s<color>施展间隔减少<color=gold>%s秒<color>",
    --tbAutoInfo.nPercent,
    szSkillName,
...
function tbSkill:GetExtraDesc(tbInfo)
  ...
  szMsg = szMsg .. "\n"
  szMsg = szMsg .. "发招时获得<color=green>[棒打狗头] " .. tbInfo.nLevel .. "级<color>\n"
  for i = 1, #tbMsg do
    szMsg = szMsg .. "" .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n击中每个敌人可使效果时间延长<color=gold>3秒<color>，最多<color=gold>18秒<color>"
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  local szMsg = ""
  szMsg = szMsg .. "下<color=gold>" .. tbAutoInfo.nCastCount .. "<color>次攻击命中时触发：\n"
  szMsg = szMsg .. "    <color=green>时乘六龙<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  local szMsg = ""
  szMsg = szMsg .. "攻击自动对敌释放<color=green>[迷花乱蝶]<color>：\n"
  szMsg = szMsg .. string.format("使该状态下的敌人的限制技能每击中一个目标时都有<color=gold>%d%%<color>几率陷入以下状态：\n", tbAutoInfo2.nPercent)
  szMsg = szMsg .. "    <color=green>[醉眼看花]<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo2, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n    <color=green>[醉眼看花]<color>触发<color=red>" .. tbAutoInfo2.nCastCount .. "<color>次后<color=green>[迷花乱蝶]<color>效果自动解除"
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  local szMsg = ""
  szMsg = szMsg .. "限制技能击中每个敌人有<color=gold>" .. tbAutoInfo.nPercent .. "%<color>几率使自身陷入以下状态：\n"
  szMsg = szMsg .. "    <color=green>醉眼看花<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = szMsg .. "\n最多触发：<color=gold>" .. tbAutoInfo.nCastCount .. "<color>次"
  szMsg = szMsg .. "\n触发间隔：<color=gold>" .. FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime) .. "<color>秒"
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = string.format("生命降到50%%时触发几率：<color=gold>%d%%<color>\n反弹伤害的抗性：<color=gold>增加%s%%<color>\n发挥基础攻击力：<color=gold>%s%%<color>\n发挥技能攻击力：<color=gold>%s%%<color>\n持续时间：<color=Gold>%s秒<color>\n获得<color=gold>%d级[时乘六龙]<color><color>效果\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, -tbChildInfo.tbWholeMagic["damage_return_receive_p"][1], tbChildInfo.tbWholeMagic["skilldamageptrim"][1], tbChildInfo.tbWholeMagic["skillselfdamagetrim"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime), tbChildInfo.tbEvent.nLevel, FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local tbMsg = {}
  local szMsg = ""
  szMsg = szMsg .. "击中敌人时有<color=gold>" .. tbAutoInfo.nPercent .. "%<color>几率获得以下状态：\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
  szMsg = string.gsub(szMsg, "清除并免疫负面状态", "    清除并免疫负面状态")
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = string.format("被攻击命中时触发几率：<color=gold>%d%%<color>\n造成攻击者击退的几率：<color=gold>%d%%<color>，击退距离<color=gold>%d<color>", tbAutoInfo.nPercent, tbChildInfo.tbWholeMagic["state_knock_attack"][1], tbChildInfo.tbWholeMagic["state_knock_attack"][2] * tbChildInfo.tbWholeMagic["state_knock_attack"][3])
  return szMsg
end
