...
local tb = {
  ...
  qiwu120_child = { --气武120_子_10
    deadlystrikeenhance_r = { { { 1, 40 * 0.7 }, { 10, 40 }, { 11, 40 * nA0 } } },
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = string.format("<color=orange>纯阳无极<color>被破盾时触发几率：<color=gold>%d%%<color>\n攻击会心一击值：<color=gold>%d<color>，持续<color=Gold>%s秒<color>\n<color=gold>无敌%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbAutoInfo.nPercent, tbChildInfo.tbWholeMagic["deadlystrikeenhance_r"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime), FightSkill:Frame2Sec(tbChildInfo2.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end
