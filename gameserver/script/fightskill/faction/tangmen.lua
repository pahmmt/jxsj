...
function tbSkill:GetParamDesc(tbMsg, nParam1, nParam2, tbInfo)
  tbMsg[#tbMsg + 1] = string.format("移动最大距离：<color=gold>%d<color>", nParam1)
end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  szMsg = szMsg .. "每<color=gold>" .. FightSkill:Frame2Sec(nPerCastTime) .. "秒<color>第一次攻击时自动释放：\n"
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  local szSkillName = FightSkill:GetSkillName(tbAutoInfo.nSkillId)
  szMsg = szMsg .. "<color=green>子招式<color>\n"
  szMsg = szMsg .. "    受到会心一击时触发概率：<color=gold>100%<color>\n"
  FightSkill:GetClass("default"):GetDescAboutLevel(tbMsg, tbChildInfo, 0)
  for i = 1, #tbMsg do
    szMsg = szMsg .. "    " .. tostring(tbMsg[i]) .. (i ~= #tbMsg and "\n" or "")
  end
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  ...
  local szMsg = ""
  local szSkillName = FightSkill:GetSkillName(tbAutoInfo.nSkillId)
  szMsg = szMsg .. "\n每<color=gold>" .. FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime) .. "<color>秒自动对队友释放以下状态：\n"
...
function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
  local tbChildInfo = KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel)
  local szMsg = string.format("攻击会心一击值：<color=gold>增加%d<color>\n外功系出招速度：<color=gold>增加%s<color>\n造成受伤的几率：<color=gold>增加%s<color>\n持续时间：<color=Gold>%s秒<color>\n触发间隔时间：<color=Gold>%s秒<color>", tbChildInfo.tbWholeMagic["deadlystrikeenhance_r"][1], tbChildInfo.tbWholeMagic["attackspeed_v"][1], tbChildInfo.tbWholeMagic["state_hurt_attackrate"][1], FightSkill:Frame2Sec(tbChildInfo.nStateTime), FightSkill:Frame2Sec(tbAutoInfo.nPerCastTime))
  return szMsg
end

--[[
local tbSkill	= FightSkill:GetClass("xiujian120_child1");

function tbSkill:GetAutoDesc(tbAutoInfo, tbSkillInfo)
	local tbChildInfo	= KFightSkill.GetSkillInfo(tbAutoInfo.nSkillId, tbAutoInfo.nSkillLevel);
	local szMsg	= string.format("<color=red>增加自身会心效果每<color><color=gold>%s<color><color=red>秒降低<color><color=gold>1/6<color>",
