...
function Spreader:OnItemConsumed(nCount, nCosumeMode)
  -- 卖店/叠加等消耗途径不算
  if nCosumeMode == self.emITEM_CONSUMEMODE_SELL or nCosumeMode < self.emITEM_CONSUMEMODE_REALCONSUME_START or nCosumeMode > self.emITEM_CONSUMEMODE_REALCONSUME_END then
    return
  end
...
function Spreader:CalcCosumeValue(nCount, nCosumeMode, bIsIbShop)
  ...
  if it.IsIbItem() == 1 then
    if
      nCosumeMode == self.emITEM_CONSUMEMODE_EXPIREDTIMEOUT --保值期到
      or nCosumeMode == self.emITEM_CONSUMEMODE_USINGTIMEOUT --物品超时
    then
      -- 过期删除可以按原价
...
--奇珍阁消耗统计
function Spreader:IbShopAddConsume(nItConsumed, nFlag)
  if nItConsumed <= 0 then
    return 0
  end
  local nTotleConSume = Spreader:IbShopGetConsume() + nItConsumed
...
--获得当月的消耗值
function Spreader:IbShopGetConsume()
...
function Spreader:OnItemBound(pItem)
  if pItem and pItem.nBuyPrice > 0 then
    ...
    if 1 == nResult then
      Dbg:WriteLog("Spreader:OnItemBound", "推广员消耗记录：", self.ZoneGroup or self:ExtractZoneGroup(), me.szAccount, string.format("[%s]绑定装备[%s],添加消耗记录[%s]", me.szName, pItem.szName, tostring(pItem.nBuyPrice)))
    end
...
function Spreader:OnGouhunyuRepute(nReputeAdded)
  ...
  if 1 == nResult then
    Dbg:WriteLog("Spreader:OnGouhunyuRepute", "推广员消耗记录：", self.ZoneGroup or self:ExtractZoneGroup(), me.szAccount, string.format("[%s]使用勾魂玉，得到声望[%d], 添加消耗记录[%s]", me.szName, nReputeAdded, tostring(self.GOUHUNYU * nConsume)))
  end
end
...
function Spreader:Flush(pPlayer)
  ...
  if nConsumeToSend > 0 then
    ...
    pPlayer.SetTask(self.TASK_GROUP, self.TASKID_CONSUME, nConsume - nConsumeToSend * self.ExchangeRate_Rmb2Gold)
    ...
    if nLaoWanJia == 1 then
      ...
      SendSpreaderConsume(self.ZoneGroup or self:ExtractZoneGroup(), pPlayer.szAccount, pPlayer.szName, nRealConsume, pPlayer.nId, self.emKTYPE_REDUX_PLAYER)

      Dbg:WriteLog("Spreader:Flush", "老玩家活动：", self.ZoneGroup or self:ExtractZoneGroup(), pPlayer.szAccount, pPlayer.szName, nRealConsume)
    end

    if self:_IsIntroducee_Spreader(pPlayer) == 1 then
      SendSpreaderConsume(self.ZoneGroup or self:ExtractZoneGroup(), pPlayer.szAccount, pPlayer.szName, nConsumeToSend * self.ExchangeRate_Rmb2Gold, pPlayer.nId, self.emKTYPE_SPREADER)

      Dbg:WriteLog("Spreader:Flush", "推广员消耗记录：", self.ZoneGroup or self:ExtractZoneGroup(), pPlayer.szAccount, pPlayer.szName, nConsumeToSend * self.ExchangeRate_Rmb2Gold)
    end
  end
end
