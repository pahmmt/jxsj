...
function tbNpc:_OnDialog(bSure, dwHimId, nMeId)
  ...
  if bSure == 0 then
    ...
    local tbOpt = {
      { "确定", self._OnDialog, self, 1, him.dwId, me.nId },
      { "取消" },
    }
...
function tbNpc:CheckLockTime()
  ...
  if tbTemplate.LOCK_BOAT == 1 then
    if GetFrame() - tbTemplate.LOCK_TIME >= self.LOCK_INTERVAL then
      ...
      local pBoat = KNpc.Add2(self.nBoatNpcId, 1, -1, nMapId, unpack(tbTemplate.tbBoatPos))
      if pBoat then
        tbTemplate.nBoatId = pBoat.dwId
...
function tbNpc:ClimbBoat(nTemplateId, nMapId, nFlag)
  ...
  local nBoatId = self.tbTemplate[nTemplateId].nBoatId or 0
  local pBoat = KNpc.GetById(nBoatId)
  if nFlag then
    if not pBoat then
      local pNpc = KNpc.Add2(self.nBoatNpcId, 1, -1, nMapId, unpack(tbBoatPos))
      if not pNpc then
        return 0
      end
      self.tbTemplate[nTemplateId].nBoatId = pNpc.dwId
      pNpc.SetCarrierIntData(1, self.tbTemplate[nTemplateId].nBoatLine) -- 设置航线
      pBoat = pNpc
    end
