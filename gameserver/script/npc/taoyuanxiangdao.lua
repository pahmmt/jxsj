...
function tbNpc:OnDialog_Original()
  ...
  Dialog:Say("桃源向导：这是个隐蔽的地方哦，难道你是用了非法软件或者利用游戏漏洞进来的，赶快使用GM面板<color=red>联系GM<color>说明你的情况吧。也许你可以打死守卫，拿到剑侠世界的终极装备再走，不过桃源的宝藏，可是不好拿的哦。", tbDlg)
end
...
function tbNpc:OnDialog_MultiRunGame()
  ...
  if nNow - nArrestTime < nJailTerm then
    ...
    Dialog:Say("桃源向导：捕快发现，你通过非法客户端多开游戏，在" .. nMinute .. "分钟内不能离开桃源。", tbDlg)
  else
    local tbDlg = {
...
function tbNpc:OnDialog_WG()
  ...
  if nNow - nArrestTime < nJailTerm then
    ...
    Dialog:Say("桃源向导：捕快发现，你使用了非法客户端或插件，在" .. nMinute .. "分钟内不能离开桃源。", tbDlg)
  else
    local tbDlg = {
      { "离开桃源", self.GetOutOfTaoYuan, self },
      { "关闭" },
