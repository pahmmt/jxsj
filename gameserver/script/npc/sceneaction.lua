...
SceneAction.nCheckTime = 30 * 60 * Env.GAME_FPS

SceneAction.szFileName = "\\setting\\npc\\sceneaction.txt"

SceneAction.tbMapInfo = {
  ["taoxizhen"] = { 2154, 2254, 2255, 2256, 2257, 2258, 2259 },
}
...
function SceneAction:ServerStart()
  ...
  self.nFinishCheckId = Timer:Register(self.nCheckTime, self.CheckIsFinsh, self)
end

function SceneAction:CheckIsFinsh()
  for i, tb in ipairs(self.tbActionEvent) do
    local bFlag = 0
    if tb.nTypeObj == "Npc" then
      for _, nNpcId in ipairs(tb.tbObjInfo) do
        local pNpc = KNpc.GetById(nNpcId)
        if pNpc then
          bFlag = 1
          break
    ...
    if bFlag == 0 then
      self.tbActionEvent[i] = {}
      self.tbActionEventEx[i] = 1
    end
  end
  return
end
...
function SceneAction:GoBack(nIndex, bCanClearTB)
  ...
  if self.tbActionEvent[nIndex].nMinStep == Lib:CountTB(tbParam) then
    ...
    self:GoNext(nIndex, bCanClearTB)
  end
end

function SceneAction:GoNext(nIndex, bCanClearTB)
  if not self.tbActionEvent[nIndex] then
    return
  end
  local nEventIndex = self.tbActionEvent[nIndex].nEventIndex
  local nStep = self.tbActionEvent[nIndex].nStep
  if not bCanClearTB then
    if not self.tbActionList[nEventIndex] or not self.tbActionList[nEventIndex].tbAction[nStep] then
