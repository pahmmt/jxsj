...
function tbXiangZiNpc:AddBox(nSubWorld, nX, nY, nTakeTime, tbParam, nBagCell, nExistTime)
  ...
  if nExistTime and nExistTime > 0 then
    tbTemp.tbBoxData.nTimerId = Timer:Register(nExistTime, self.DelBox, self, pNpc.dwId)
  end
  return pNpc
end
...
function tbJiangLiXiangZi:OnDialog()
  ...
  -- 进度条
  GeneralProcess:StartProcess("", tbTemp.tbBoxData.nTakeTime, { self.CompleteProcess, self, him.dwId, me.nId }, nil, tbEvent)
end
