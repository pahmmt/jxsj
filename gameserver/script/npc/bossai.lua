...
function Npc:Npc10256Skill_01(pTargeNpc)
  ...
  local tbUseNpc = { [10278] = 1, [10256] = 1 }
  local pPlayer = pTargeNpc.GetPlayer()
  if not him or not tbUseNpc[him.nTemplateId] then
    print("Npc:Npc10256Skill_01 Error", ((him and him.szName) or "无npc"), debug.traceback("Npc10256Skill_01"))
    return
  end
  if pPlayer then
    local szName = him.szName or "敌人"
    local szMsg = string.format("触碰<color=gold>黄色气劲<color>反噬%s对其造成致命伤害", szName)
