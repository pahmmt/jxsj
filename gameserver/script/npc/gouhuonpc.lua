          ...
          for _, nId in pairs(tbPlayerId2) do
            if nId ~= nPlayerId then
              ...
              if pMemPlayer and pPlayer.IsFriendRelation(pMemPlayer.szName) == 1 then
                ...
                pPlayer.Msg(string.format("您与<color=yellow>%s<color>好友亲密度增加了%d点。", pMemPlayer.szName, 2))
                pMemPlayer.Msg(string.format("您与<color=yellow>%s<color>好友亲密度增加了%d点。", pPlayer.szName, 2))
              end
            end
          end
        end
  ...
  if nAnnouce == 1 then
    if nJiuMax == 0 then
      if szEventJiuName then
        szMsg = string.format("<color=blue>使用%s，", szEventJiuName)
      else
        szMsg = string.format("<color=blue>没有使用酒，")
      end
      szMsg = string.format("%s篝火现在获得经验的倍数为<color=yellow>%s％<color=blue>。", szMsg, nQuotiety)
    else
      if szEventJiuName then
        szMsg = string.format("队友正在分享%s和%s，经验加成为%s％", szJiuName, szEventJiuName, nQuotiety)
      else
        szMsg = string.format("队友正在分享%s，经验加成为%s％", szJiuName, nQuotiety)
      end
