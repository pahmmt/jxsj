...
function tbJiaZuLingPaiNpc:StartToWork(nKinId, nMapId, nMapX, nMapY)
  ...
  local nKinBadge = cKin.GetKinBadge()
  ...
  Timer:Register(self.ADD_EXP_TIME, self.AddKinExp, self, nKinId, pNpc.dwId)
  ...
  if tbPlayer then
    ...
    for i in pairs(tbPlayer) do
      ...
      if nTagetKin ~= 0 and nTagetKin == nKinId then
        ...
        if cMember and (cMember.GetFigure() <= Kin.FIGURE_REGULAR or (bIsOldPAction == 1 and cMember.GetFigure() <= Kin.FIGURE_RETIRE)) and tbPlayer[i].nLevel >= 10 then
