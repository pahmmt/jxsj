...
function tbStoneThrower:CanOperate(nOpeSeq)
  if not self.tbCanOperationCallBacks or not self.tbCanOperationCallBacks[nOpeSeq] then
    return 1
  end
...
function tbStoneThrower:Operate(nOpeSeq)
  if not self.tbOperateCallBacks or not self.tbOperateCallBacks[nOpeSeq] then
    return 1
  end
...
-- 需要司机的提示信息时钟
function tbStoneThrower:OnNeedDriverTimer()
  local tbCPS = him.GetCarrierPassengers()
  if not tbCPS then
    return 0
  end
  if not tbCPS[0] or tbCPS[1] then
    self.nNeedDriver = nil
    return 0
  end
  NewBattle:SendMsg2Player(tbCPS[0], "你的位置是攻击位，你需要一个驾驶位的战友来移动战车。", NewBattle.SYSTEMBLACK_MSG)
  return nil
end
-- 是否需要老司机
function tbStoneThrower:CheckDriver()
  if Timer:GetRestTime(self.nNeedDriver) ~= -1 then
    Timer:Close(self.nNeedDriver)
  end
  self.nNeedDriver = Timer:Register(20 * 18, self.OnNeedDriverTimer, self)
end
...
function tbStoneThrower:OnPlayerLandIn(pPlayer, nSeat)
  if MODULE_GAMESERVER then
    if nSeat == 0 then
      NewBattle:SendMsg2Player(pPlayer, "你的位置是攻击位，你还要一个驾驶位的战友来移动战车。", NewBattle.SYSTEMBLACK_MSG)
      self:CheckDriver()
    else
      NewBattle:SendMsg2Player(pPlayer, "你的位置是驾驶位，只有你才能控制战车移动。", NewBattle.SYSTEMBLACK_MSG)
    end
...
function tbStoneThrower:OnPlayerLandOff(pPlayer, nSeat)
  ...
  if MODULE_GAMESERVER then
    ...
    if nSeat == 0 then
      ...
      --换了Leader，重新安装快捷键
      NewBattle:SendMsg2Player(pPassenger, "你的位置是攻击位，你需要一个驾驶位的战友来移动战车。", NewBattle.SYSTEMBLACK_MSG)
      self:CheckDriver()
...
if MODULE_GAMESERVER then
  ...
  function tbStoneThrower:LandInCarrier(pPlayer, nSeat)
    ...
    if pLeader then
      ...
      if not nMemCount then
        pPlayer.Msg("载具队伍人数异常！")
        return
      end
      if (not pPlayer.nTeamId or pPlayer.nTeamId ~= pLeader.nTeamId) and nMemCount >= 6 then
  ...
  function tbStoneThrower:SetLeader(pCarrier, pPlayer)
    ...
    if not pPlayer then
      ...
      -- 设置显示名字、血条
      pCarrier.GetTempTable("Npc").szOldName = pCarrier.GetTempTable("Npc").szOldName or pCarrier.szName
      local szName = string.format("%s | %s的载具", string.sub(pCarrier.szName, 1, 2), pPlayer.szName)
