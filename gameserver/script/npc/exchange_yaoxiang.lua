...
function tbYX:CheckItemType(pItem)
  for _, tbType in ipairs(tbItemType) do
    if pItem.nGenre == tbType[1] and pItem.nDetail == tbType[2] and pItem.nParticular == tbType[3] and pItem.nLevel == tbType[4] then
      return 1
    end
  end
...
function tbYX:DoExchange(tbGDPL, tbItemObj)
  ...
  for i = 1, nItemCount do
    ...
    if pItem.nGenre ~= tbGDPL[1] or pItem.nDetail ~= tbGDPL[2] or pItem.nParticular ~= tbGDPL[3] or pItem.nLevel ~= tbGDPL[4] then
      bBind = pItem.IsBind()
      pItem.Delete(me)
