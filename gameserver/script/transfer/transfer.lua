...
function Transfer:PlayerLogout(nPlayerId)
  self:SetTransferStatus(0) --清除跨服标志
end
...
--跨服后设置跨服标志
function Transfer:SetTransferStatus(nStatus)
  nStatus = nStatus or 1
  me.SetTask(self.tbTransferStatus[1], self.tbTransferStatus[2], nStatus, 1)
end
--获取跨服标志，跨服后为1，否则为0
function Transfer:GetTransferStatus()
...
function Transfer:NewWorld2GlobalMap(pPlayer, tbPos)
  ...
  self:SetPlayerKinAndTongName(pPlayer)
  self:TransferData() --同步数据
  pPlayer.GlobalTransfer(nMapId, nMapX, nMapY)
end

function Transfer:SetPlayerKinAndTongName(pPlayer)
  if not pPlayer then
    return
  end
  local pTong = KTong.GetTong(pPlayer.dwTongId)
  if pTong then
    pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_TONGNAME, pTong.GetName())
  else
    pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_TONGNAME, "_notong")
  end

  local pKin = KKin.GetKin(pPlayer.dwKinId)
  if pKin then
    pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_KINNAME, pKin.GetName())
  else
    pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_KINNAME, "_nokin")
  end
end

function Transfer:ClearPlayerKinAndTongName(pPlayer)
  if not pPlayer then
    return
  end
  pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_TONGNAME, "")
  pPlayer.SetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_KINNAME, "")
end

function Transfer:GetPlayerTongName(pPlayer)
  --TODO:玩家在全局服时，帮会变更了，此函数获取可能出错，用KGC_PlayerManager.GetGlobalPlayerTongName (未导出到脚本)
  if not pPlayer then
    return
  end
  local szTongName = pPlayer.GetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_TONGNAME)
...
function Transfer:GetPlayerKinName(pPlayer)
  if not pPlayer then
    return
  end
  local szKinName = pPlayer.GetTaskStr(self.TASK_GROUP_GLOBAL_LOGIN, self.TASK_ID_GLOBAL_LOGIN_KINNAME)
