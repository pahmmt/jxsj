...
function BaseGame:JoinNextGame(pPlayer)
  ...
  if nTeamId ~= 0 then
    if self.tbNextGameTeam[nTeamId] == nil and self.nNextTeamCount < XoyoGame.MAX_TEAM then
      ...
      KStatLog.ModifyAdd("xoyogame", "本日参加逍遥谷的队伍", "总量", 1)
    ...
    print("有角色在逍遥谷内登陆了", pPlayer.szName)
    Dbg:WriteLog("XoyoGame", "有角色在逍遥谷内登陆了")
    Dialog:SendBlackBoardMsg(pPlayer, "太危险了！你擅自闯入逍遥谷内，将在5秒后回到城内！")
...
function BaseGame:OnJoin(nGroupId)
  ...
  if self.nTimerId and self.nTimerId > 0 then
    ...
    if (nCurTime >= XoyoGame.START_TIME1 and nCurTime < XoyoGame.END_TIME1) or (nCurTime >= XoyoGame.START_TIME2 and nCurTime < XoyoGame.END_TIME2) then
      -- 计算下一场的开启剩余时间
      nLastFrameTime = ((30 - (nCurTime % 100) % 30) * 60 - tonumber(os.date("%S", GetTime())) + XoyoGame.LOCK_MANAGER_TIME) * Env.GAME_FPS
    end
  end
  ...
  for _, nPlayerId in ipairs(tbMember) do
    ...
    if pPlayer then
      if not self.tbAddXoyoTimesPlayerId[pPlayer.nId] then
        pPlayer.Msg("你是怎么进来逍遥的？别以为老夫年纪大了，眼睛就不好使了。")
...
function BaseGame:RandomRoom(tbTeam, nRoomLevel)
  ...
  while nTeamCount > 0 do
    ...
    if nWhileTimes >= 200 then
      for i = 1, nTeamCount do
        self:KickTeam(tbTeam[i], nil, "找不到合适的房间。")
      end
...
function BaseGame:EndRoomTime(nRoomId)
  ...
  if nCurRoomCount == XoyoGame.PLAY_ROOM_COUNT and nLevel == nFinalLevel and bWin == 1 then
    XoyoGame:CalcTotalTime(XoyoGame:GetDifficuty(nTeamId), XoyoGame.tbStartTime[nTeamId], nTeamId)
  end
  ...
  for i = 1, nCount do
    ...
    if pPlayer then
      ...
      if nDifficuty == 9 then
        if self.tbTeam[nTeamId].nWinRoomCount >= 5 then -- 简单逍遥谷过5关给2点威望
          local nTaskPrestige = pPlayer.GetTask(XoyoGame.TASK_GROUP, XoyoGame.EASY_PRESTIGE)
          local nRemain = XoyoGame.MAX_EASY_PRESTIGE - nTaskPrestige
          if nRemain > 0 then
            local nValue = XoyoGame.ONCE_EASY_PRESTIGE
            if nRemain < nValue then
              nValue = nRemain
            end
            pPlayer.AddKinReputeEntry(nValue)
            pPlayer.SetTask(XoyoGame.TASK_GROUP, XoyoGame.EASY_PRESTIGE, nTaskPrestige + nValue)
          end
        end
      else -- 9是简单难度，不计入侠客
        ...
        if (nDifficuty == 1 and self.tbTeam[nTeamId].nWinRoomCount >= XoyoGame.GUMUREPUTE_TASK_WIN_COUNT) or (nDifficuty > 1 and self.tbTeam[nTeamId].nWinRoomCount >= XoyoGame.GUMUREPUTE_TASK_WIN_COUNT - 1) then
          Faction:AchieveTask(pPlayer, Faction.TYPE_XOYO)
        end
      end
      SpecialEvent.tbMessagePush:AchieveXoyo(nDifficuty, pPlayer)
...
function BaseGame:AddRoom(nRoomId, nDifficuty, ...)
  ...
  if type(tbRoomSetting.nMapIndex) == "table" then
    ...
    self.tbRoom[nRoomId]:InitRoom(self, tbRoomSetting, self.tbMap[nIndex][nMapIndex], nRoomId, tbRoomExp)
  else
    self.tbRoom[nRoomId]:InitRoom(self, tbRoomSetting, self.tbMap[tbRoomSetting.nMapIndex], nRoomId, tbRoomExp)
  end
