...
BaseRoom.TSK_GROUP_TASK_MAIN = 1025
BaseRoom.TSK_GROUP_TASK_SUB = 89
BaseRoom.TASK_MAIN_ID = "214"
BaseRoom.TASK_SUB_ID = "2EE"
BaseRoom.TASK_STEP = 1
...
function BaseRoom:RoomLevelUp()
  if self.nTimerId then
    local fnExcute = function(pPlayer)
      Dialog:SetBattleTimer(pPlayer, "<color=green>本关剩余时间：%s<color>", XoyoGame.DELAY_ENDTIME * Env.GAME_FPS)
    end
...
function BaseRoom:NewMsgPlayerGroup(szMsg, nType)
  ...
  if nType == XoyoGame.TOALLGAMESERVER then
    szMsg = "<color=green>" .. szName .. "<color>带领的队伍" .. szMsg
...
function BaseRoom:TeamAward(tbTeam, nMinuteExp, nRepute, nPrestige, nOffer, bWinner, nTollGatePoint)
  ...
  local fnExcute = function(pPlayer)
    ...
    if bWinner == 1 then
      ...
      if tbTeamPlayer and tbTeamPlayer[1] and tbTeamPlayer[1] == pPlayer.nId and XoyoGame.HONOR[self.tbSetting.nRoomLevel] and XoyoGame.HONOR[self.tbSetting.nRoomLevel][nCount] then
        PlayerHonor:AddPlayerHonorById_GS(tbTeamPlayer[1], PlayerHonor.HONOR_CLASS_LINGXIU, 0, XoyoGame.HONOR[self.tbSetting.nRoomLevel][nCount])
      end
      ...
      if TimeFrame:GetState("Keyimen") == 1 and self.tbLongWenYinBiAward[self.tbSetting.nRoomLevel] then
        ...
        if nActualCount ~= tbAward[5] then
          pPlayer.Msg("您的背包空间不足，未能获得足够的龙纹银币！")
        end
      end

      -- 不管是什么难度什么关卡，只要过关了就完成逍遥谷教育任务
    ...
    if XoyoGame.LOG_ATTEND_OPEN == 1 then
      ...
      Dbg:WriteLog("xoyogame", "attend 奖励LOG 玩家:" .. pPlayer.szName, "房间等级" .. self.tbSetting.nRoomLevel, "经验:" .. nExp, "贡献度:" .. nOffer, "声望:" .. nRepute, "江湖威望:" .. nPrestige .. "," .. bPrestige)
    end
  end
  self:TeamPlayerExcute(fnExcute, tbTeam)
  -- 添加亲密度
  ...
  for i = 1, #tbTeamPlayer do
    for j = i + 1, #tbTeamPlayer do
      if tbTeamPlayer[i] and tbTeamPlayer[j] and 1 == tbTeamPlayer[i].IsFriendRelation(tbTeamPlayer[j].szName) then
        ...
        tbTeamPlayer[i].Msg(string.format("您与<color=yellow>%s<color>好友亲密度增加了%d点。", tbTeamPlayer[j].szName, 16))
        tbTeamPlayer[j].Msg(string.format("您与<color=yellow>%s<color>好友亲密度增加了%d点。", tbTeamPlayer[i].szName, 16))
      end
    end
  end
end
...
function BaseRoom:SetTagetInfo(nPlayerGroup, szInfo)
  ...
  self:GroupPlayerExcute(fnExcute, nPlayerGroup)
  --成功LOG统计 比较差的实现
  if XoyoGame.LOG_ATTEND_OPEN == 1 and szInfo == "任务完成" and self.nTimerId then
    --查找所在Group
    local tbGroup
    if nPlayerGroup and nPlayerGroup > 0 then
      if (not self.tbPlayerGroup) or not self.tbPlayerGroup[nPlayerGroup] then
...
function BaseRoom:JoinRoom(...)
  ...
  for i = 1, #arg do
    ...
    for j = 1, nNum do
      ...
      -- TODO
      if pPlayer and self.tbGame == XoyoGame:GetPlayerGame(pPlayer.nId) and not self.tbPlayer[tbMember[j]] then
...
function BaseRoom:DefaultDeath(pKiller)
  ...
  if self.nTimerId then
    local nLastFrameTime = tonumber(Timer:GetRestTime(self.nTimerId))
    Dialog:SetBattleTimer(me, "<color=green>离本关结束时间还剩:%s<color>", nLastFrameTime)
    Dialog:SendBattleMsg(me, "请耐心等待本关结束")
  end
end
...
function BaseRoom:SendPlayer2RestRoom(pPlayer)
  ...
  if self.nTimerId then
    local nLastFrameTime = tonumber(Timer:GetRestTime(self.nTimerId))
    Dialog:SetBattleTimer(pPlayer, "<color=green>离本关结束时间还剩:%s<color>", nLastFrameTime)
    Dialog:SendBattleMsg(pPlayer, "请耐心等待本关结束")
  end
end
...
function BaseRoom:PKDeath(pKillNpc)
  ...
  self:SetTagetInfo(nWinTeam, string.format("敌方击败我方成员数：%d\n我方击败敌方成员数：%d", self.tbTeam[nKilledTeam].nCount, self.tbTeam[nWinTeam].nCount))
  self:SetTagetInfo(nKilledTeam, string.format("敌方击败我方成员数：%d\n我方击败敌方成员数：%d", self.tbTeam[nWinTeam].nCount, self.tbTeam[nKilledTeam].nCount))
end
