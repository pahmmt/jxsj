...
function XoyoChallenge:LoadCommonEntry(nRowNum, tbRow)
  ...
  local tb = {
    ["tbCardGDPL"] = { g, d, p, l },
    ["szCardGDPL"] = string.format("%d,%d,%d,%d", g, d, p, l),
    ["id"] = id,
    ["szDesc"] = tbRow.CARD_DESC,
    ["nWeight"] = tonumber(tbRow.CARD_WEIGHT),
    ["nProbability"] = tbRow.PROBABILITY,
  }
  return tb, id
end
...
function XoyoChallenge:InitCardStorage()
  ...
  for i = 1, #tbCtrl - 1 do
    ...
    for _, v in pairs(info[1]) do
      ...
      if self.tbCardStorage[szKey] and not (self.tbCardStorage[szKey][3] == i and self.tbCardStorage[szKey][4] == v.id) then -- npc表有多个npcId对应同一个卡片的情况
        assert(false, string.format("prev i:%d, prev id:%d, curr i:%d, curr id:%d, gdpl:%s", self.tbCardStorage[szKey][3], self.tbCardStorage[szKey][3], i, v.id, szKey)) -- 卡片gdpl不能重复
      end
...
function XoyoChallenge:HandUpItemForCard(pPlayer, tbItems, tbItem2Data)
  ...
  for k, tbItem in pairs(tbItemsSorted) do
    if self:HasItem(pPlayer, self.tbItem2Data[k].tbCardGDPL) == 0 and self:GetCardState(pPlayer, self.tbItem2Data[k].szCardGDPL) ~= 2 then
...
function XoyoChallenge:GetAward(pPlayer)
  ...
  if
    not ((0 < nRank and nRank <= EventManager.IVER_nXoyoGameTaskAwardMaxRank) or nCardNum >= 24) -- 最后一个条件是排不上名次，但收集了24+张卡
    or (nYearMonth <= 200909 and self:GetGatheredCardNum(pPlayer) <= 0) -- 上个月卡片数为0
  then
    ...
    local szLog = string.format("玩家名：%s 无奖励。 名次: %d, 分数: %d, 卡片收集数:% d", pPlayer.szName, nRank, nLastMonthPoint, nCardNum)
  ...
  for _, tbTemp in ipairs(tbGiveAward) do
    ...
    Item:CheckXJRecord(Item.emITEM_XJRECORD_EVENT, "逍遥录奖励", { tbTemp.tbItem[1], tbTemp.tbItem[2], tbTemp.tbItem[3], tbTemp.tbItem[4], 1, nAddCount })
  end
  ...
  --
  --log
  local szLog = string.format("玩家名：%s 奖励: %s, 名次: %d, 分数: %d 卡片收集数: %d", pPlayer.szName, szAwardName, nRank, nLastMonthPoint, nCardNum)
  Dbg:WriteLog("XoyoChallenge:GetAward", szLog)
  pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, "逍遥谷挑战领奖：" .. szLog)
  ...
  if 1 <= nRank and nRank <= 100 then
    if nCanGetKinAward == 1 then
      pPlayer.SendMsgToFriend(string.format("您的好友[%s]在上个月的逍遥录收集任务中排名第%d位，家族的地狱逍遥积分排名前10，获得%s奖励。", pPlayer.szName, nRank, szAwardName))
    else
      pPlayer.SendMsgToFriend(string.format("您的好友[%s]在上个月的逍遥录收集任务中排名第%d位，获得%s奖励。", pPlayer.szName, nRank, szAwardName))
    end
  end
...
function XoyoChallenge:GetAwardRemind()
  ...
  if nYearMonth == 200907 and Task.IVER_nXoyo_GetAward_Fix == 1 then
    if nPrevGetMonth == nPrevMonth and nGetAwardCopy == 0 and KGblTask.SCGetDbTaskInt(DBTASK_XOYO_FINAL_LADDER_MONTH) == nYearMonth then
      me.Msg("你上月逍遥录收集奖励还未领取，请到逍遥谷入口晓菲处领取奖励")
    end
  else
    if nPrevGetMonth == nPrevMonth and nGetAwardMonth < nPrevMonth and KGblTask.SCGetDbTaskInt(DBTASK_XOYO_FINAL_LADDER_MONTH) == nYearMonth then
      me.Msg("你上月逍遥录收集奖励还未领取，请到逍遥谷入口晓菲处领取奖励")
    end
  end
end
