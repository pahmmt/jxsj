...
function tbYuanding:Create(nMapId, nLevel, tbRoom)
  ...
  self.nControlTimerId = Timer:Register(self.tbStepInfo[self.nCurrentStep].nBeginDelay * Env.GAME_FPS, self.StepControl, self, pNpc.dwId, nMapId)
  self.nFixSkillTimerId = Timer:Register(self.nFixSkillDelay * Env.GAME_FPS, self.CastFixSkill, self, pNpc.dwId, nMapId)
  return pNpc
end
...
function tbYuanding:StepControl(nNpcId, nMapId)
  ...
  if pNpc then
    ...
    if tbStepInfo then
      ...
      tbYuanding.nStepTimerId = Timer:Register(tbStepInfo.nSpace * Env.GAME_FPS, self.StepProcess, self, pNpc.dwId, nMapId)
    end
    if not tbYuanding.nObserverTimerId then
      tbYuanding.nObserverTimerId = Timer:Register(6 * Env.GAME_FPS, self.Observer, self, pNpc.dwId)
    end
    return 0
  end
  return 0
