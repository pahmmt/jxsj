...
function tbMission:AddAword(nTemplateId, nKillerId)
  ...
  pPlayer.SetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY, pPlayer.GetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY) + TowerDefence.NPC_AWORD[nTemplateId][2])
...
function tbMission:AddMoney(nNumber)
  ...
  for nGroup = 1, self:GetGroupNum() do
    for _, pPlayer in pairs(self:GetPlayerList(nGroup)) do
      pPlayer.SetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY, pPlayer.GetTask(TowerDefence.TSK_GROUP, TowerDefence.TSK_MONEY) + self.MONEY_PER[nMoney] + nNumber * 4)
    end
  end
  self:GenerateAndSendMsg()
end
