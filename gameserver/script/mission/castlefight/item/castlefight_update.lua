...
function tbItem:OnUseEx(nNpcId, nSure, nProcess)
  ...
  if not pNpc then
    CastleFight:SendMsgAndBroadMsg(me, "请先选中要升级的建筑。")
    return 0
  end
  ...
  if nDistance > 30 then
    CastleFight:SendMsgAndBroadMsg(me, "要靠近才能升级哦。")
    return
  end
  ...
  GeneralProcess:StartProcess("升级中", CastleFight.NPC_TEMPLATE[CastleFight:GetNpcId(pNpc)].nLevelUpCD * Env.GAME_FPS, { self.OnUseEx, self, nNpcId, 1, 1 }, { self.OnUpdateBreak, self, nNpcId }, tbBreakEvent)
  return
end
