...
function tbBaseCamp:CanBuildBuilding(pPlayer, nId)
  ...
  if nPos == 0 then
    CastleFight:SendMsgAndBroadMsg(pPlayer, "位置不对，请站在本方的蓝色地基上建造。")
    return 0
  end
  ...
  if nCurMoney < tbNpcInfo.nProductMoney then
    CastleFight:SendMsgAndBroadMsg(pPlayer, string.format("您的军饷不够，不能建造。需要军饷<color=yellow>%d<color>。", tbNpcInfo.nProductMoney))
    return 0
  end
...
function tbBaseCamp:CanUpdateBuilding(pPlayer, pBuilding)
  if CastleFight:GetNpcOwnerId(pBuilding) ~= pPlayer.nId then
    CastleFight:SendMsgAndBroadMsg(pPlayer, "这不是你的建筑，无法升级。")
    return 0
  end
  ...
  if tbOldInfo.nLevelUpNpc == 0 then
    CastleFight:SendMsgAndBroadMsg(pPlayer, "大本营、哨塔或者已经满级的建筑不可升级。")
    return 0
  end

  local nCurMoney = CastleFight:GetPlayerMoney(pPlayer)
  if nCurMoney < tbOldInfo.nLevelUpMoney then
    CastleFight:SendMsgAndBroadMsg(pPlayer, string.format("您还缺军饷<color=yellow>%s<color>，不能升级，升级此建筑需要军饷<color=yellow>%s<color>。", tbOldInfo.nLevelUpMoney - nCurMoney, tbOldInfo.nLevelUpMoney))
    return 0
  end
...
function tbBaseCamp:AddNpcBuff(nBuffId)
  ...
  for nNpcId in pairs(self.tbNpcList) do
    ...
    if pNpc then
      pNpc.AddSkillState(tbBuffInfo.nId, tbBuffInfo.nLevel, 2, tbBuffInfo.nSec * Env.GAME_FPS, 1, 0, 1)
    end
  end
...
function tbBaseCamp:BuffEffectNpc(pNpc)
  for nBuffId, nStart in pairs(self.tbBuffList) do
    ...
    if tbBuffInfo.nSec + nStart > GetTime() then
      pNpc.AddSkillState(tbBuffInfo.nId, tbBuffInfo.nLevel, 2, (nStart + tbBuffInfo.nSec - GetTime()) * Env.GAME_FPS, 1, 0, 1)
    else
