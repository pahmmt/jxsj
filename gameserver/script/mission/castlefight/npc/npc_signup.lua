...
function tbNpc:OnDialog()
  ...
  local szMsg = string.format(
    [[    丰年迎盛世，卯兔耀中华。在这新春来临之际，夜岚关石破天托我秦洼和大家传个话：为广纳天下有勇有谋的侠客，请大家到夜岚关一聚。那里有思考的静谧、激情的火花、谋略的智慧、协作的快感。
    挑战次数可通过使用<color=yellow>夜岚明灯<color>获得。夜岚明灯在月影之石商店有售。
    
<color=green>您还有剩余挑战次数<color=yellow>%s<color>次<color>
<color=green>您已经参加挑战的次数<color=yellow>%s/%s<color>次<color>
<color=green>您今天可以使用夜岚明灯的次数<color=yellow>%s<color>次<color>]],
    nCountSum,
    nTotal,
    CastleFight.DEF_MAX_TOTAL_NUM,
    nUseItem
  )

  local tbOpt = {
    { "排行榜查询", self.OnOpenRank, self },
    { "月影之石商店", self.GetExCount, self },
    { "决战夜岚关活动规则", self.OnAbout, self },
    { "随便看看" },
  }

  local szJoinMsg = "<color=yellow>报名参加决战夜岚关<color>"

  if CastleFight:IsSignUpByAward(me) > 0 then
    table.insert(tbOpt, 1, { "<color=yellow>领取单场奖励<color>", self.GetAward, self })
  end

  if tbConsole:CheckState() ~= 1 then
    szMsg = "<color=red>目前活动尚未开放。<color>\n<color=yellow>玉兔吉祥，金兔送福 ，卯兔兆丰年。夜岚关关主石破天为找出智勇双全的真英雄，特地托我向武林之上侠客发出邀请，参加决战夜岚关活动。<color>\n开放时间：\n<color=yellow>1月18日-2月17日\n上午10:00-晚上11:30<color>\n整点和半点开始报名，报名时间5分钟"
    szJoinMsg = "<color=gray>报名参加决战夜岚关<color>"
  end

  if tbConsole:CheckAwardState() == 1 then
    table.insert(tbOpt, 1, { "领排行榜奖励", self.GetFinishAward, self })
...
function tbNpc:GetFinishAward()
  ...
  local tbConsole = CastleFight:GetConsole()
  if not tbConsole then
    Dialog:Say("目前活动尚未开放！开放时间在1月25日-2月14日每天上午10：00-晚上11：30，每逢半点、整点请在我这里报名吧。")
    return 0
  end
...
function tbNpc:OnOpenRank()
  ...
  local szMsg = string.format(
    [[可以帮你打开荣誉排行榜，你在界面下面的“其他排行榜”下的“活动排行”中查询到排行信息，需要我帮你打开排行榜吗？
	<color=green>
	---参赛信息---
...
function tbNpc:GetExCount()
  ...
  --	Dialog:OpenGift("放入月影石【每个月影之石可以兑换两次参加决战夜岚关活动的资格哦】", {"CastleFight:CheckGiftSwith"}, {self.OnOpenGiftOk, self});
end
...
function tbNpc:OnOpenGiftOk(tbItemObj)
  ...
  if nTotal + nCount + nExCount >= CastleFight.DEF_MAX_TOTAL_NUM then
    me.Msg("您已经参加活动的次数和剩余挑战资格已经超过最大参加活动的次数，不能兑换！")
    return 0
  end
  ...
  me.Msg(string.format("您获得了<color=yellow>%s次<color>额外参赛资格。", nTotalNum))
end
...
tbNpc.tbAbout = {
  ...
  [2] = [[
   活动开启后，60级以上的玩家可以去各新手村找晏若雪报名参加，每个玩家每天有2次机会。参加比赛必须有自己的夜岚明灯或夜岚翡翠灯，拥有夜岚翡翠灯，杀死敌方兵可以额外获得15%的军饷。
]],
  [3] = [[
...
function tbNpc:OnAbout(nSel)
  if nSel then
    Dialog:Say(self.tbAbout[nSel], { { "返回上层", self.OnAbout, self } })
    return 0
  end
  local szMsg = "决战夜岚关"
  local tbOpt = {
