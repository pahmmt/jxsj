...
function tbMission:StartGame()
  self:SetMissionBattleTimer("<color=green>距比赛开始还有：<color=white>%s<color>\n", CastleFight.WAIT_TIME_FPS, 1)
  self:BroadcastBlackBoardMsg("请查看快捷栏，比赛中您可以站在蓝色地基上使用它们。")
...
function tbMission:BeginPlay()
  self.nIsPlaying = 1
  self:BroadcastBlackBoardMsg("活动开始，请在蓝色地基上建造建筑开展攻防战吧！")
  self:BroadcastSystemMsg("<color=green>【决战夜岚关-玩法简介】<color><color=yellow>您必须通过消耗军饷来建造与升级建筑，方可获得攻防能力。建造与击毁可获得军饷、积分。<color>")
  self:SetMissionBattleTimer("<color=green>距比赛结束还有：<color=white>%s<color>\n", CastleFight.MATCH_TIME_FPS)
...
function tbMission:EndPlay()
  ...
  if self.nLoseCamp == 0 then
    self:BroadcastBlackBoardMsg(string.format("比赛结束！"))
    self:BroadcastSystemMsg(string.format("比赛结束！"))
  else
    for i = 1, 2 do
      if self.nLoseCamp == i then
        if 2 ~= self.nMatchType then
          self:BroadcastBlackBoardMsg(string.format("比赛结束，本队未能取胜，请再接再厉！稍候将离开这里！"), i)
        end

      --	self:BroadcastBlackBoardMsg(string.format("比赛结束！很遗憾你们输了。"),i);
      --	self:BroadcastSystemMsg(string.format("比赛结束！很遗憾你们输了。"),i);
      else
        if 2 ~= self.nMatchType then
          self:BroadcastBlackBoardMsg(string.format("比赛结束！恭喜你和你的队友取得了最终胜利，稍候将离开这里！"), i)
        else
        ...
        --	self:BroadcastBlackBoardMsg(string.format("比赛结束！恭喜你们获得比赛的胜利"),i);
        --	self:BroadcastSystemMsg(string.format("比赛结束！恭喜你们获得比赛的胜利"),i);
      end
    end
  end
  ...
  self:SetMissionBattleTimer("<color=green>剩余休息时间：<color=white>%s<color>\n", CastleFight.ENDREST_TIME_FPS)
...
function tbMission:OnClose() end
...
function tbMission:GenerateAndSendMsg()
  ...
  tbMsg = { "\n队伍积分表：" }
  for i = 1, 2 do
    table.insert(tbMsg, string.format("%s<color=white>%d<color>", CastleFight:AddStrColor(string.format("%-16s", (self.tbGroupCaptain[i] or "") .. "队"), i), self:GetCampScore(i)))
  end
...
function tbMission:CalcPlayerInfo()
  ...
  if #tbMoney > 0 then
    self:BroadcastSystemMsg(string.format("本场获得军饷最多的侠士是:<color=yellow>%s<color>", tbMoney[1][1]))
  end

  if #tbBuild > 0 then
    self:BroadcastSystemMsg(string.format("本场建造建筑最多的侠士是:<color=yellow>%s<color>", tbBuild[1][1]))
  end

  if #tbKillNpc > 0 then
    self:BroadcastSystemMsg(string.format("本场杀敌最多的侠士是    :<color=yellow>%s<color>", tbKillNpc[1][1]))
  end
end
