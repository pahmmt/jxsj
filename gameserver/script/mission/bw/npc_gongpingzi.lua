...
function BiWu:SignUpFinal(nMapId, nMemberCount)
  ...
  for i = 1, #tbPlayer do
    ...
    Dialog:Say(string.format("%s：你们约定于1分钟后在[%s]进行[%dV%d]的对战。请尽快进行准备并入场。%s", him.szName, GetMapNameFormId(nMapId), nMemberCount, nMemberCount, szKeyMsg), { { "恩，我知道了", self.OnCancel } })
  ...
  local szMsg = string.format("%s 与 %s 约定于1分钟后在[%s]的[%s]进行[%dV%d]的对战。", tbCaptainName[1], tbCaptainName[2], szSignMapName, self.tbMission[nMapId].szMapName, nMemberCount, nMemberCount)
...
function BiWu:OnHelp()
  Dialog:Say("公平子：比武擂台是武林侠士互相切磋武艺、一决高下的场所。擂台给比赛双方提供一个公平的场地。" .. "要进行比武擂台赛，首先在我这边进行擂台比赛的申请。" .. "由于我们场地有限，当所有场地正在进行比赛的时候，不接受新的比赛申请。", {
    { "下一页", self.OnHelp1, self },
    { "就这样吧", self.OnCancel },
  })
end
function BiWu:OnHelp1()
  Dialog:Say("公平子：申请成功后，双方有<color=yellow>1分钟<color>入场时间。待入场时间结束后，比赛正式开始！" .. "比赛时间分为<color=yellow>1分钟<color>准备时间和<color=yellow>10分钟<color>战斗时间。时间结束时双方未分胜负，则依照双方的伤害量来判决胜负。", {
...
function BiWu:OnLook()
  ...
  if #tbOpp == 0 then
    Dialog:Say(string.format("%s；对不起，现在没有供观看的比赛，你稍后再来吧。", him.szName), { { "结束对话", self.OnCancel } })
    return 0
  end
  table.insert(tbOpp, { "结束对话", BiWu.OnCancel, self })
...
function BiWu:OnLookMap(nMapId)
  ...
  if me.szName == self.tbMission[nMapId].tbCaptainName[1] or me.szName == self.tbMission[nMapId].tbCaptainName[2] then
    ...
    local szMsg = string.format("%s：%s 与 %s 将在[%s]进行%s的对战！你确定要观战么？", him.szName, self.tbMission[nMapId].tbCaptainName[1], self.tbMission[nMapId].tbCaptainName[2], self.tbMission[nMapId].szMapName, self.tbMission[nMapId].szType)
    Dialog:Say(szMsg, {
...
function BiWu:ErrorMsg(nErrorId)
  ...
  Dialog:Say(szMsg, { { "结束对话", self.OnCancel } })
  return
end
...
function BiWu:DoShowMyGrade(nType)
  ...
  Dialog:Say(string.format("你%dV%d的参赛总场次数：%d场", nType, nType, nCount), {
