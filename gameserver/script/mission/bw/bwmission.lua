...
function tbTest:InitGame(nMapId, nType, tbTeamKey, tbCaptainName, tbCaptainId)
  ...
  self.tbMisCfg = {
    tbEnterPos = {
      [1] = { nMapId, unpack(BiWu.TB_MAPTYPE_MAPDEC[nMapType].tbCampPos[1]) },
      [2] = { nMapId, unpack(BiWu.TB_MAPTYPE_MAPDEC[nMapType].tbCampPos[2]) },
      [3] = { nMapId, unpack(BiWu.TB_MAPTYPE_MAPDEC[nMapType].tbCampPos[3]) },
    }, -- 进入坐标
    tbDeathRevPos = {
      [1] = { nMapId, unpack(BiWu.TB_MAPTYPE_MAPDEC[nMapType].tbCampPos[1]) },
      [2] = { nMapId, unpack(BiWu.TB_MAPTYPE_MAPDEC[nMapType].tbCampPos[2]) },
  ...
  for i = 1, 2 do
    ...
    if pPlayer ~= nil then
      ...
      if self.nType == 1 then
        ...
        if nMstGroup <= 0 then
          local szMsg = string.format("你是<color=yellow> %s<color>擂台赛的参赛方，比赛为<color=yellow> %d<color>对<color=yellow> %d<color>的比赛，请进快入场。", self.szMapName, self.nType, self.nType)
          me.Msg(szMsg)
          me.Msg(szTimeMsg)
        end
      else
        nKey = self.tbTeamKey[i] --GetMissionV(MS_TEAMKEY[i]);
        szMsg = string.format("你是<color=yellow> %s<color>擂台赛的参赛方，比赛为<color=yellow> %d<color>对<color=yellow> %d<color>的比赛，入场号为<color=yellow>[%d]<color>，请快点通知你的队友入场。", self.szMapName, self.nType, self.nType, nKey)
        me.Msg(szMsg)
        local nMstGroup = self:GetPlayerGroupId(KPlayer.GetPlayerObjById(self.tbCaptainId[i]))
        if nMstGroup <= 0 then
          me.Msg(szTimeMsg)
...
function tbTest:OnReportSignState(nValue)
  if nValue == BiWu.GO_TIME then
    ...
    local szMsg = string.format("%s比武场 <color=yellow>%s<color>小组 与 <color=yellow>%s<color>小组 的比武擂台赛入场已截止，比赛正式开始！", self.szMapName, self.tbCaptainName[1], self.tbCaptainName[2])
  ...
  if szBiWuEvent == "OnTotalTimer" then
    ...
    if nGroup1PlayerCount == nGroup2PlayerCount then --比赛结束时 双方人数相同
      if nGroup1Damage < nGroup2Damage then --被伤害量比较 小的一方获胜
        szMsg = string.format(tbCloseMsg[szBiWuEvent][4], self.szMapName, szCaptainName[1], szCaptainName[2])
        nWinGroup = 1
      elseif nGroup1Damage > nGroup2Damage then
        szMsg = string.format(tbCloseMsg[szBiWuEvent][4], self.szMapName, szCaptainName[2], szCaptainName[1])
        nWinGroup = 2
      else --不然就打平
        szMsg = string.format(tbCloseMsg[szBiWuEvent][2], self.szMapName, szCaptainName[1], szCaptainName[2])
        nWinGroup = 0
      end
    elseif nGroup1PlayerCount > nGroup2PlayerCount then --人数多的一方 获胜
      szMsg = string.format(tbCloseMsg[szBiWuEvent][3], self.szMapName, szCaptainName[1], szCaptainName[2])
      nWinGroup = 1
    else
      szMsg = string.format(tbCloseMsg[szBiWuEvent][3], self.szMapName, szCaptainName[2], szCaptainName[1])
      nWinGroup = 2
    end
  else
    if nGroup1PlayerCount <= 0 and nGroup2PlayerCount <= 0 then --比赛过程中 只考虑人数为0时结束比赛
      szMsg = string.format(tbCloseMsg[szBiWuEvent][1], self.szMapName, szCaptainName[1], szCaptainName[2])
      nWinGroup = 0
    elseif nGroup1PlayerCount <= 0 then
      szMsg = string.format(tbCloseMsg[szBiWuEvent][2], self.szMapName, szCaptainName[2], szCaptainName[1])
      nWinGroup = 2
...
function tbTest:OnDeath(pKillerNpc)
  ...
  if pKillerPlayer then
    ...
    if nKillerGroupId == 1 or nKillerGroupId == 2 then
      szMsg = string.format("%s多人擂台赛战斗中，<color=yellow>%s<color>队的<color=yellow>%s<color>在比赛中击倒了 <color=yellow>%s<color>队的<color=yellow>%s<color>。", self.szMapName, szCaptainName[nKillerGroupId], szKillerName, szCaptainName[nDeathGroupId], szDeathName)
      self:BroadcastMsg(szMsg)
    end
  end
