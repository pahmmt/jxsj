...
function Fun:ExeStoneStackItem(pPlayer, value)
  ...
  if pPlayer.CountFreeBagCell() < nBagNeed then
    pPlayer.Msg(string.format("由于您的背包空间已满，无法获得<color=yellow>%s<color>", KItem.GetNameById(value[1], value[2], value[3], value[4])))
    return 0
  end
  --value[6] = nil;
  --table.insert(value, 5, nil);
  pPlayer.AddStackItem(value[1], value[2], value[3], value[4], nil, value[5])
  StatLog:WriteStatLog("stat_info", "baoshixiangqian", "advanced", pPlayer.nId, string.format("%d_%d_%d_%d", value[1], value[2], value[3], value[4]), value[5])
end
