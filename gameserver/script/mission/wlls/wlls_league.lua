...
function Wlls:LeagueRank(nFinalType, nUpdateNews)
  local szPrimName = "本届初级联赛榜"
  local szAdvName = "本届高级联赛榜"
...
function Wlls:LeagueRankFrame(nGameLevel, nType, nFinalType)
  ...
  if MODULE_GC_SERVER and nFinalType == 1 then
    local szLastPrimName = "上届初级联赛榜"
    local szLastAdvName = "上届高级联赛榜"
    if GLOBAL_AGENT and GbWlls:CheckOpenGoldenGbWlls() == 1 then
      szLastPrimName = "上届高级联赛榜"
      szLastAdvName = "上届黄金联赛榜"
    end
...
function Wlls:LeagueClearSession()
  ...
  self.ClsLeagueId = 1
  while pLeague do
    if pLeague.GetTask(Wlls.LGTASK_MTYPE) ~= self:GetMacthType() or pLeague.GetTask(Wlls.LGTASK_MSESSION) ~= self:GetMacthSession() then
