...
function Wlls:OnGetAwardSingleWithWldhRep()
  ...
  local szMsg = [[你可以使用<color=yellow>1枚武林大会英豪令<color>兑换<color=yellow>%s点武林大会声望<color>。
你当前可兑换武林大会声望次数为<color=yellow>%s<color>次，距离到达购买<color=yellow>%s<color>资格还差<color=yellow>%s次<color>兑换次数。每月参加高级武林联赛获胜<color=yellow>%s次<color>即可获得<color=yellow>全部<color>兑换次数，加油哦。
<color=green>注意：每个月的兑换次数仅限当月使用，请及时兑换哦！<color>]]
  ...
  if nCanFinialChange > 0 then
    tbOpt[#tbOpt + 1] = { "<color=yellow>兑换获得白银/黄金戒指声望<color>", self.OnChangeWldhReput, self }
    tbOpt[#tbOpt + 1] = { "月影之石购买兑换道具[武林大会英豪令]", self.OnOpenYueYingShop, self }
  end

  tbOpt[#tbOpt + 1] = { "<color=yellow>购买白银/黄金武林大会声望戒指<color>", self.OnOpenWldhReputeShop, self }
  tbOpt[#tbOpt + 1] = { "如何获得武林大会声望兑换次数", self.AboutWldhReput, self }

  local tbNpc = Npc:GetClass("wlls_guanyuan3")
  tbOpt[#tbOpt + 1] = { "了解其他内容", tbNpc.OnDialog, tbNpc }
...
function Wlls:AboutWldhReput()
  local szMsg = [[
    1.每个月参加<color=yellow>高级<color>武林联赛<color=red>获胜18次<color>即可获得当月全部兑换次数。
    2.每个月的兑换次数可<color=yellow>累计兑换<color>但仅限<color=yellow>当月使用<color>，请及时兑换。
    3.你只需要积累110次兑换次数就可获得武林大会白银戒指<color=yellow>[门派技能+1]<color>的购买资格。
]]
  Dialog:Say(szMsg, {
    { "返回查看其它选项", self.OnGetAwardSingleWithWldhRep, self },
    { "我已了解" },
  })
end
...
function Wlls:OnChangeWldhReput()
  ...
  local szMsg = string.format(
    [[您<color=yellow>本月当前累计<color>可兑换白银/黄金戒指的声望次数是<color=yellow>%s次<color>，您可通过使用<color=green>武林大会英豪令<color>兑换，或者直接<color=yellow>消耗金币<color>进行兑换。

温馨提示：
1）每个月可兑换的最大系数已支持<color=yellow>累计兑换<color>，你不用再担心忘记兑换。
2）若您没有足够的兑换道具，<color=yellow>推荐<color>您直接使用金币消耗兑换。
]],
    nMaxCount
  )
  local tbOpt = {}
  tbOpt[#tbOpt + 1] = { "使用<color=yellow>武林大会英豪令<color>兑换", self.OpenAskNumber, self, 1 }
  tbOpt[#tbOpt + 1] = { "<color=yellow>使用金币直接兑换（660金币/每次）<color>", self.OpenAskNumber, self, 2 }
  tbOpt[#tbOpt + 1] = { "返回了解其他内容", self.OnGetAwardSingleWithWldhRep, self }
  tbOpt[#tbOpt + 1] = { "在考虑考虑" }
  Dialog:Say(szMsg, tbOpt)
end
...
function Wlls:OpenAskNumber(nType)
  ...
  local nMaxCount = self.YINGXIONGLING_MAX_TIMES - nTaskTimes
  if nMaxCount <= 0 then
    Dialog:Say("您本月兑换次数已经用完了！")
    return 0
  end
  local nReputeCount = self:GetPlayerMaxReputeChangeCount(me)
  nMaxCount = math.min(nMaxCount, nHaveCount)
  nMaxCount = math.min(nReputeCount, nMaxCount)
  Dialog:AskNumber("请输入数量：", nMaxCount, self.DoChangeWldhRepute, self, nType)
end

function Wlls:DoChangeWldhRepute(nType, nChangeCount)
  if 1 == nType then
    local szMsg = string.format("您确定使用%s个武林大会英豪令兑换武林大会声望吗？", nChangeCount)
    Dialog:Say(szMsg, {
      { "确定", self.OnChangeWldhReputeByItem, self, nChangeCount },
      { "返回", self.OnChangeWldhReput, self },
    })
  elseif 2 == nType then
    local szMsg = string.format("您确定花费%s金购买武林大会声望吗？", nChangeCount * self.BUY_WLDH_COIN)
    Dialog:Say(szMsg, {
      { "确定", self.OnChangeWldhReputeByCoin, self, nChangeCount },
      { "返回", self.OnChangeWldhReput, self },
    })
  end
end
...
function Wlls:OnChangeWldhReputeByItem(nChangeCount)
  ...
  if nCount < nChangeCount then
    Dialog:Say(string.format("你身上没有<color=yellow>%s个武林大会英豪令<color>，赶快去龙五太爷处购买再来领奖。", nChangeCount))
    return
  end
  ...
  me.SendMsgToFriend(string.format("您的好友[<color=yellow>%s<color>]成功使用武林大会英豪令兑换了武林大会声望。", me.szName))
  Player:SendMsgToKinOrTong(me, "成功使用武林大会英豪令兑换了武林大会声望。", 1)
end
...
function Wlls:OnChangeWldhReputeByCoin(nChangeCount)
  ...
  if me.nCoin < nTotalCoin then
    Dialog:Say(string.format("您的金币不足%s元，不能兑换武林大会声望", nTotalCoin))
    return 0
  end
  ...
  StatLog:WriteStatLog("stat_info", "ring_repute", "exchange", me.nId, nChangeCount)
  me.SendMsgToFriend(string.format("您的好友[<color=yellow>%s<color>]成功兑换了武林大会声望。", me.szName))
  Player:SendMsgToKinOrTong(me, "成功兑换了武林大会声望。", 1)
end
