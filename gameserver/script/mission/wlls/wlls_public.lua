  ...
  if GLOBAL_AGENT then
    if MODULE_GAMESERVER then
      ...
      if GbWlls:CheckOpenGoldenGbWlls() == 0 then
        if (self:GetMacthSession() >= self.MACTH_ADV_START_MISSION) and ((nMoneyRank > 0 and nMoneyRank <= GbWlls.DEF_ADV_MAXGBWLLS_MONEY_RANK) or (nWllsRank > 0 and nWllsRank <= GbWlls.DEF_ADV_MAXGBWLLS_WLLS_RANK)) then
          return self.MACTH_ADV
        end
      else
        if (self:GetMacthSession() >= self.MACTH_ADV_START_MISSION) and ((nMoneyRank <= 0 or nMoneyRank > GbWlls.DEF_ADV_MAXGBWLLS_MONEY_RANK) and (nWllsRank <= 0 or nWllsRank >= GbWlls.DEF_ADV_MAXGBWLLS_WLLS_RANK)) then
          return 0
        end
...
function Wlls:SendGbWllsStarServer_GC(tbStarResult)
  ...
  for i, tbServer in ipairs(tbStarResult) do
    if tbServer.nGate == nNowGateWay then
      self:SendGbWllsStarServerMail_GC(i)
...
function Wlls:SendGbWllsStarServerMail_GC(nRank)
  if not MODULE_GC_SERVER then
    return 0
  end
  local szTitle = string.format("明星服务器奖励发放")
  local szContent = string.format(
    [[致各位侠客：
    本届跨服联赛已于昨晚光荣落幕，恭喜你们获得<color=yellow>明星服务器<color>的光荣称号。贵服所有参赛战队的总积分排名位于大区<color=yellow>第%s名<color>，特发放礼花奖励一份，请所有<color=yellow>财富排行榜前5000名<color>的侠客于今日起前往临安助威鼓领取，每天领取一次，共可领取<color=yellow>20天<color>。

    让我们绽放幸运的礼花共同庆祝这胜利的时刻吧！
             跨服武林联赛主办方]],
    nRank
  )
  local nEndDate = tonumber(os.date("%Y%m%d%H", GetTime() + 3600 * 24 * 20))
