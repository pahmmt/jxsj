    ...
    for i = 1, nMaxCount do
      if tbAreaLeague[i] then
        ...
        for nAreaLeagueId, tbTempLeague in pairs(tbTempAreaLeague) do
          if not tbAreaLeague[i].tbHistory[tbTempLeague.nNameId] and not tbTempLeague.tbHistory[tbAreaLeague[i].nNameId] then
            table.insert(tbLeagueA, tbAreaLeague[i])
...
Wlls.tbDirectFinishAchievementSession = { 3, 7 }

function Wlls:DirectFinishAchievement()
  for _, nSession in pairs(self.tbDirectFinishAchievementSession) do
    local nAchieId = self.tbAchievementApply[nSession]
    if Achievement:CheckFinished(nAchieId) == 0 then
      Achievement:__FinishAchievement(nAchieId)
    end

    local tbListId = self.tbAchievementWin[nSession]
    for _, nAchieId in pairs(tbListId) do
      if Achievement:CheckFinished(nAchieId) == 0 then
        Achievement:__FinishAchievement(nAchieId)
      end
    end

    tbListId = self.tbAchievementRank[nSession]
    for _, tbInfo in pairs(tbListId) do
      if Achievement:CheckFinished(tbInfo[2]) == 0 then
        Achievement:__FinishAchievement(tbInfo[2])
      end
    end

    nAchieId = self.tbAchievementAttend[nSession]
    if Achievement:CheckFinished(nAchieId) == 0 then
      Achievement:__FinishAchievement(nAchieId)
    end
  end
