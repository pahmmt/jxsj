...
function tbMission:Reset()
  ...
  tbMission.tbMonsterList = { --nNpcId	nNpcCount
    [2459] = 0, --进度1事件一小怪
    [2460] = 0, --进度1事件二蛮族首领
  ...
  for _, tbValue in ipairs(Mentor.tbSetting) do
    if tonumber(tbValue.nDegree) == self.nDegree and tonumber(tbValue.nStep) == self.nStep then
      ...
      if tonumber(tbValue.bRoute) == 0 then
        local pNpc = KNpc.Add2(tonumber(tbValue.nNpcId), tonumber(nLevel), -1, self.nMisMapId, tbValue.nX / 32, tbValue.nY / 32, 0, tonumber(tbValue.nNpcType))
...
function tbMission:GoNextStep()
  ...
  if self.nDegree > Mentor.WEEKLY_SCHEDULE then
    local pMasterPlayer = KPlayer.GetPlayerObjById(self.nMasterPlayerId)
    local pApprenticePlayer = KPlayer.GetPlayerObjById(self.nApprenticePlayerId)
    Dbg:WriteLog("Mentor_shitufuben", string.format("队伍{%s, %s}在师徒副本中的进度大于当前最大限制！！", pMasterPlayer.szName, pApprenticePlayer.szName))
  ...
  if fnCall == nil then
    ...
    if pApprenticePlayer then
      ...
      elseif nTaskDegree == self.nDegree then
        ...
        --走到这里，是遇到了异常情况
        local pMasterPlayer = KPlayer.GetPlayerObjById(self.nMasterPlayerId)
        Dbg:WriteLog("Mentor_shitufuben", string.format("队伍{%s, %s}在师徒副本中的进度异常！！", pMasterPlayer.szName, pApprenticePlayer.szName))
      end
    end
