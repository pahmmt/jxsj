...
--重置一个地图到初始状态
function Mentor:ResetMap(nDynMapId) end
...
function Mentor:CheckEnterCondition(nPlayerId)
  ...
  if pOtherPlayer and pPlayer.GetTrainingTeacher(self.RELATIONTYPE_TRAINING) ~= pOtherPlayer.szName and pOtherPlayer.GetTrainingTeacher(self.RELATIONTYPE_TRAINING) ~= pPlayer.szName then
...
function Mentor:GetApprentice(nPlayerId)
  ...
  --判断队伍的两个成员间是否有未出师的师徒关系
  if pPlayer.GetTrainingTeacher(self.RELATIONTYPE_TRAINING) ~= pOtherPlayer.szName and pOtherPlayer.GetTrainingTeacher(self.RELATIONTYPE_TRAINING) ~= pPlayer.szName then
    --print("师徒关系出了问题")
    return nil
  end
...
function Mentor:GetMission(pNpc)
  ...
  for i = 1, Mentor.MAX_MAP_COUNT do
    if Mentor.tbMissList[i].nApprenticePlayerId == nId or Mentor.tbMissList[i].nMasterPlayerId == nId or Mentor.tbMissList[i].dwProtecNpcId == nId then
      return Mentor.tbMissList[i]
    end
  end
end

function Mentor:SetGame1(tbMission) end

function Mentor:SetGame2(tbMission) end

function Mentor:SetGame3(tbMission) end

function Mentor:GetNpcList(nStep) end
