...
local tbRuleBase = Battle:GetRuleClass(3, "夺旗")
...
function tbRuleBase:AddFlag(nCampId)
  ...
  local szMsg = string.format("<color=yellow>%s<color>军战旗出现在<color=green>(%d, %d)<color>。", Battle.NAME_CAMP[nCampId], tbPos[2] / 8, tbPos[3] / 16)
...
function tbRuleBase:DeletePlayerFlag(pPlayer)
  ...
  if (2 == self.tbMission.nState) and (1 == tbBattleInfo.bHaveFlag) and (tbBattleInfo.pPlayer.nId == tbBattleInfo.tbCamp.nPlayerIsFlag) then
    ...
    local szMsg = string.format("%s军战旗掉落在(%d, %d)。", Battle.NAME_CAMP[nCampId], nPosX / 8, nPosY / 16)
    self.tbMission:BroadcastMsg(szMsg)
...
function tbRuleBase:ReachToFlagDes(nCampId)
  ...
  GeneralProcess:StartProcess("立旗中...", 3 * Env.GAME_FPS, { self.ReachToFlagDesSuc, self, tbCamp, pPlayer.nId }, { self.ForceCloseTimeBar, self, tbCamp }, tbEvent)
