...
local tbRuleBase = Battle:GetRuleClass(6, "保护龙柱")
...
tbRuleBase.TOTEM_NAME = {
  [Battle.CAMPID_NEUTRAL] = "中立龙柱",
  [Battle.CAMPID_SONG] = "宋方龙柱",
  [Battle.CAMPID_JIN] = "金方龙柱",
}

tbRuleBase.TOTEM_TITLE = {
  [Battle.CAMPID_NEUTRAL] = string.format("<bclr=gray><color=white>%s<color><bclr>", "龙柱"),
  [Battle.CAMPID_SONG] = string.format("<bclr=red><color=yellow>%s<color><bclr>", "龙柱"),
  [Battle.CAMPID_JIN] = string.format("<bclr=purple><color=#FF400080>%s<color><bclr>", "龙柱"),
}
...
function tbRuleBase:OnTimer_WaitSeizeEnd(dwNpcId, nPlayerId)
  ...
  if pPlayer ~= nil then
    ...
    self.tbMission:BroadcastMsg(string.format("<color=yellow>%s<color>军%s<color=yellow>%s<color>占领了%s<enter>(<pos=%d,%d,%d>)", self.tbCamps[tbNewData.nCampId].szCampName, Battle.NAME_RANK[tbBattleInfo.nRank], pPlayer.szName, "龙柱", nMapId, nX, nY))
  else
    self.tbMission:BroadcastMsg(string.format("<color=yellow>%s<color>军占领了%s<enter>(<pos=%d,%d,%d>)", self.tbCamps[tbNewData.nCampId].szCampName, "龙柱", nMapId, nX, nY))
  end

  local tbAddPointTimer = self.tbMission:CreateTimer(self.CAMP_POINT_INTERVAL * Env.GAME_FPS, self.OnTimer_AddCampPoint, self, tbNewNpc.dwId)
...
function tbRuleBase:OnTimer_AddCampPoint(dwNpcId)
  ...
  -- 处于争夺状态
  if tbData.nChangingToCampId ~= Battle.CAMPID_NEUTRAL or tbData.nCampId == Battle.CAMPID_NEUTRAL then
    return 0 -- 停止TIMER
  end
    ...
    if pPlayer and pPlayer.IsDead() == 0 then
      ...
      if tbBattleInfo.tbCamp.nCampId == tbData.nCampId then
        ...
        if nDx * nDx + nDy * nDy <= nDistanceSqure then
          szMsg = string.format("你保护%s有功，获得奖励<color=yellow>%d<color>积分。", "龙柱", self.PERSONAL_POINT_PER_ONCE)
          pPlayer.Msg(szMsg)
...
function tbRuleBase:ProcessSeriesBouns(tbKillerBattleInfo, tbDeathBattleInfo)
  ...
  if 5 >= (nPLRank - nMeRank) then
    ...
    if math.fmod(nSeriesKill, 3) == 0 then
      ...
      tbKillerBattleInfo.pPlayer.Msg(string.format("%s军%s %s连续击退敌人%d名，获得%d积分的连斩奖励。", Battle.NAME_CAMP[tbKillerBattleInfo.tbCamp.nCampId], Battle.NAME_RANK[tbKillerBattleInfo.nRank], tbKillerBattleInfo.pPlayer.szName, tbKillerBattleInfo.nSeriesKill, Battle.SERIESKILLBOUNS))
    end
