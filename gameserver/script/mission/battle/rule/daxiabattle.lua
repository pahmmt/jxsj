...
local tbRuleBase = Battle:GetRuleClass(5, "大侠保卫战")
...
function tbRuleBase:AddProtectBouns(tbCamp, tbBoss, nRankId, nCampId, nDistanceSqure)
  ...
  local szMsg = string.format("你护卫%s军%s有功，获得奖励<color=yellow>%d<color>积分。", tbCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns)
  local tbBossPos = {}
...
function tbRuleBase:GiveKillBossCampBouns(nCampId, nRankId)
  ...
  local szMsg = string.format("%s军%s已被我军将士奋勇击退，你获得奖励<color=yellow>%d<color>积分。", tbOppCamp.szCampName, Battle.NAME_RANK[nRankId], nAddBouns)
  if 10 == nRankId then
...
function tbRuleBase:AddPlayerNpc(nCampId)
  ...
  local szMsg = string.format("<color=yellow>%s军战神丹<color>出现，%s军将士们赶紧去寻找战神丹！", Battle.NAME_CAMP[nCampId], Battle.NAME_CAMP[nCampId])
