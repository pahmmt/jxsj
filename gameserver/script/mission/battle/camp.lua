...
function tbCampBase:OnEnd(nResult, szWinMsg)
  -- 发送胜败消息
  local tbPlayerList = self.tbMission:GetPlayerList(self.nCampId)
  local szMsg = string.format(Battle.MSG_CAMP_RESULT[nResult], self.tbMission:GetFullName(), self.szCampName)
  for _, pPlayer in pairs(tbPlayerList) do
...
function tbCampBase:OnProtectFlag(tbFlagPlayer)
  ...
  local szFirMsg = string.format("%s军 %s <color=green>%s<color>成功护送战旗到达目的地", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], Battle.NAME_RANK[tbFlagPlayer.nRank], tbFlagPlayer.pPlayer.szName)
  local szSecMsg = string.format("目前%s方成功护送战旗数量为： <color=red>%d<color>", Battle.NAME_CAMP[tbFlagPlayer.tbCamp.nCampId], tbFlagPlayer.tbCamp.nFlags)
