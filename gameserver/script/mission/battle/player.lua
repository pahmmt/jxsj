...
function tbPLBase:AddBounsWithoutCamp(nBouns)
  ...
  if nNewBouns >= self.nLimitBouns then
    ...
    self.pPlayer.Msg(string.format("你的战场积分当前为：%d，已达到本场积分上限。", nNewBouns))
  end
...
function tbPLBase:ShowRightBattleInfo()
  ...
  if 3 == self.tbMission.tbRule.nRuleType then
    szMsg = string.format("<color=green>当前排名：<color> <color=0xa0ff>%d<color>\n<color=green>个人积分：<color> <color=yellow>%d<color>\n<color=green>伤敌玩家： <color><color=red>%d<color>\n<color=green>成功护旗：<color> <color=yellow><bclr=red>%d<bclr><color>", self.nListRank, self.nBouns, self.nKillPlayerNum, self.nFlagNum)
  else
    szMsg = string.format("<color=green>当前排名：<color> <color=0xa0ff>%d<color>\n<color=green>个人积分：<color> <color=yellow>%d<color>\n<color=green>伤敌玩家： <color><color=red>%d<color>", self.nListRank, self.nBouns, self.nKillPlayerNum)
  end
...
function tbPLBase:GiveKillNpcBouns(pNpc)
  ...
  if 1 == nMidMsg then
    ...
    local szAllMsg = string.format("<color=yellow>%s<color>军 <color=yellow>%s<color> 小队浴血奋战，斩杀<color=yellow>%s<color>。", Battle.NAME_CAMP[self.tbCamp.nCampId], self.pPlayer.szName, Battle.NAME_RANK[nRankId])
    local tbPlayerList = self.tbMission:GetPlayerList()
    szMsg = string.format("<color=yellow>%s<color>军 <color=yellow>%s<color> 被 <color=yellow>%s军<color>将士奋勇斩杀。", Battle.NAME_CAMP[self.tbCamp.tbOppCamp.nCampId], Battle.NAME_RANK[nRankId], Battle.NAME_CAMP[self.tbCamp.nCampId])
  ...
  elseif 0 == nMidMsg then
    if nRankId > 1 then
      local szAllMsg = string.format("%s军 %s %s 杀死了 %s军 %s", Battle.NAME_CAMP[self.tbCamp.nCampId], Battle.NAME_RANK[self.nRank], self.pPlayer.szName, Battle.NAME_CAMP[self.tbCamp.tbOppCamp.nCampId], Battle.NAME_RANK[nRankId])
