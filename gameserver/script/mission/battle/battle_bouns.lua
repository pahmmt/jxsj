...
function Battle:ProcessSeriesBouns(tbKillerBattleInfo, tbDeathBattleInfo)
  ...
  if 5 >= (nPLRank - nMeRank) then
    ...
    if math.fmod(nSeriesKill, 3) == 0 then
      ...
      tbKillerBattleInfo.pPlayer.Msg(string.format("%s军%s %s连续击退敌人%d名，获得%d积分的连斩奖励。", Battle.NAME_CAMP[tbKillerBattleInfo.tbCamp.nCampId], Battle.NAME_RANK[tbKillerBattleInfo.nRank], tbKillerBattleInfo.pPlayer.szName, tbKillerBattleInfo.nSeriesKill, self.SERIESKILLBOUNS))
    end
...
function Battle:AwardFinalShengWang(tbPlayerList)
  ...
  for i = 1, #tbPlayerList do
    ...
    tbBattleInfo.pPlayer.Msg(string.format("排名为：<color=green>%d<color>，你获得了<color=white>%d<color>点战场声望值奖励。", i, tbBattleInfo.nShengWang))
  end
end
...
function Battle:AwardFinalHonor(tbPlayerList)
  ...
  for i = 1, #tbPlayerList do
    ...
    tbBattleInfo.pPlayer.Msg(string.format("排名为：<color=green>%d<color>，你获得了<color=white>%d<color>点战场荣誉值奖励。", i, tbBattleInfo.nHonor))
  end
end
...
function Battle:AwardFinalBouns(tbPlayerList, nBattleLevel)
  ...
  for i = 1, #tbPlayerList do
    ...
    tbBattleInfo.pPlayer.Msg(string.format("排名为：<color=green>%d<color>，你获得了<color=white>%s<color>倍的积分奖励。", i, nNowTimes))
  end
end
...
function Battle:AwardFinalGoods(tbPlayerList, nBattleLevel)
  ...
  for i = 1, #tbPlayerList do
    ...
    if self.tbAWARDBOUNS[1] <= nBouns then
      ...
      Dialog:SendInfoBoardMsg(pPlayer, "请到战场报名点募兵校尉处领取奖励。")
...
function Battle:GetFactionNumber(pPlayer)
  ...
  if 0 == nFaction then
    Battle:DbgOut("GetFactionNumber", pPlayer.szName, "未加入门派！")
    return 0
  end
  local nRouteId = pPlayer.nRouteId
  if 0 == nRouteId then
    Battle:DbgOut("GetFactionNumber", pPlayer.szName, " 未选定路线，不能获得本门秘籍！")
    return 0
  end
  ...
  function Battle:TiFuFinalAward(tbPlayerList, nBattleLevel)
    ...
    for i = 1, #tbPlayerList do
      ...
      if nCurRank == -1 then -- 得的是积分上限奖励
        szMsg = szMsg .. string.format("积分为：<color=green>%d<color>, 你将获得", tbBattleInfo.nBouns)
      else -- 得的是排名奖励
        szMsg = szMsg .. string.format("排名为：<color=green>%d<color>, 你将获得", i)
      end
      for nLevel, nCount in pairs(tbXuanjing) do
        ...
        szMsg = szMsg .. string.format("%d个%s，", nCount, szName)
      end
      szMsg = szMsg .. string.format("%d银两, %d分钟的真元历练经验, 请到战场报名点募兵校尉处领取奖励。", nMoney, nZhenYuanTime)
      pPlayer.Msg(szMsg)
