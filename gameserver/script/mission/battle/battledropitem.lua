...
function Battle:GetItemList()
  ...
  for nLine = 2, nLineCount do
    ...
    if nProbability >= 0 then
      tbClassItemList[nHonorLevel].nMaxProp = tbClassItemList[nHonorLevel].nMaxProp + nProbability
    end
  end
  ...
  for nitem = 1, #self.tbItemList[nkind] do
    ...
    if nRate <= nRateSum and self.tbItemList[nkind][nitem].nProbability ~= -1 then
      if self:GetItem(pKiller, self.tbItemList[nkind][nitem]) == 0 then
        ...
        local szMsg = string.format("玩家<color=yellow>%s<color>击伤<color=yellow>%s<color>获得<color=yellow>%s<color>物品", pKiller.szName, pDeather.szName, szItemName)
...
function Battle:DeathDropProp(pKiller, pDeather)
  ...
  if not self.tbKillDropProp or not self.tbKillDropProp[nKillHonorLevel] or not self.tbKillDropProp[nKillHonorLevel][nDeathHonorLevel] then
    return 0
  end
