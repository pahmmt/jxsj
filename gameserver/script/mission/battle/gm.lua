function Battle:GM()
  local tbOpt = {}
  tbOpt[#tbOpt + 1] = { "测试用装备领取", "Battle:GM_GetEquip" }
  ...
  if tbMission then
    if tbMission.nState == 1 then
      tbOpt[#tbOpt + 1] = { "结束报名，强制开战", "Battle:GM_Start" }
    end
    tbOpt[#tbOpt + 1] = { "结束当前战役", "Battle:GM_Close" }
  else
    for nRuleId, tbRule in ipairs(self.tbRuleBases) do
      tbOpt[#tbOpt + 1] = { "开启战役-" .. tbRule.szRuleName, "Battle:GM_Open", nRuleId }
    end
    for nRuleId, tbRule in ipairs(self.tbRuleBases) do
      tbOpt[#tbOpt + 1] = { "开启战役-九曲之战" .. tbRule.szRuleName, "Battle:GM_OpenSp", nRuleId }
    end
    for nRuleId, tbRule in ipairs(self.tbRuleBases) do
      tbOpt[#tbOpt + 1] = { "开启战役-蟠龙谷之战" .. tbRule.szRuleName, "Battle:GM_OpenSp1", nRuleId }
    end
    for nRuleId, tbRule in pairs(self.tbRuleBases) do
      if tbRule.szRuleName then
        tbOpt[#tbOpt + 1] = { "开启战役-嘉峪关之战" .. tbRule.szRuleName, "Battle:GM_OpenSp_JiaYuGuan", nRuleId }
      end
    end
  end
  ...
  --tbOpt[#tbOpt + 1]	= {"宋金专用号", "Battle:GM_Equipment"};
  tbOpt[#tbOpt + 1] = { "宋金体验专用角色方案2", "Battle:GM_Equipment1" }

  tbOpt[#tbOpt + 1] = { "结束对话" }

  Dialog:Say("我的宋金，我做主！<pic=42>", tbOpt)
end
...
function Battle:GM_GotoOneSignUp(nCampId, nBattleSeq)
  ...
  if self.MAPID_LEVEL_CAMP[nLevel][nBattleSeq] then
    ...
    me.Msg(string.format("根据您的等级，已将您传送至%s方%s报名点%d！", Battle.NAME_CAMP[nLevel], Battle.NAME_GAMELEVEL[nLevel], nBattleSeq))
  end
end
