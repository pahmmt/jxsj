...
function tbMSBase:OnDeath(pKillerNpc)
  ...
  if pKillerPlayer then
    ...
    -- 击伤敌对玩家获得物品奖励
    if Battle.FLAG_OPEN_DROP_ITEM and Battle.FLAG_OPEN_DROP_ITEM[self.nRuleType] and Battle.FLAG_OPEN_DROP_ITEM[self.nRuleType] == 1 then
      Battle:DeathDropProp(pKillerPlayer, pPlayer)
    end

    local tbKillerBattleInfo = Battle:GetPlayerData(pKillerPlayer)

    local szMsg = string.format("%s军%s <color=yellow>%s<color> 击退了 %s军%s <color=yellow>%s<color>", Battle.NAME_CAMP[nKillerGroupId], Battle.NAME_RANK[tbKillerBattleInfo.nRank], tbKillerBattleInfo.pPlayer.szName, Battle.NAME_CAMP[nGroupId], Battle.NAME_RANK[tbDeathBattleInfo.nRank], tbDeathBattleInfo.pPlayer.szName)
...
function tbMSBase:_SetStateLog2(tbPlayerList)
  ...
  for i = 1, #tbPlayerList do
    local tbPlayer = tbPlayerList[i]
    Dbg:WriteLogEx(Dbg.LOG_INFO, "Mission", "Battle", string.format("【宋金】%s\t玩家：%s\t积分：%s\t宋方积分：%s\t宋方人数：%s\t金方积分：%s\t金方人数：%s", szBtSeqKey, tbPlayer.pPlayer.szName, tbPlayer.nBouns, self.tbCampSong.tbOppCamp.nBouns, Lib:CountTB(self:GetPlayerIdList(1)), self.tbCampJin.tbOppCamp.nBouns, Lib:CountTB(self:GetPlayerIdList(2))))
  end
end
