...
function Union:DlgCreateUnion(szUnionName, bConfirm)
  ...
  --创建确认
  if bConfirm ~= 1 then
    Dialog:Say("创建联盟，你确定要创建吗？", { { "是的，我要创建", self.DlgCreateUnion, self, szUnionName, 1 }, { "让我再考虑一下" } })
    return 0
  end
...
function Union:DlgTongJoin(bConfirm)
  ...
  if bConfirm ~= 1 then
    Dialog:Say("你确定要把队员的帮会加入联盟吗？", { { "是的，把队员的帮会加入联盟", self.DlgTongJoin, self, 1 }, { "让我再考虑一下" } })
    return 0
  end
...
function Union:DlgTongLeave(bConfirm)
  ...
  if bConfirm ~= 1 then
    Dialog:Say("你的帮会确定要退出联盟吗？", { { "是的，我要退出", self.DlgTongLeave, self, 1 }, { "让我再考虑一下" } })
    return 0
  end
...
function Union:DlgChangeUnionMaster(nChoose, bConfirm)
  ...
  if bConfirm ~= 1 then
    Dialog:Say("你的帮会确定要移交盟主吗？", { { "是的，我要移交", self.DlgChangeUnionMaster, self, nChoose, 1 }, { "让我再考虑一下" } })
    return 0
  end
...
function Union:DlgDispenseDomain(nTargetDomainId, nTargetTongId, bConfirm)
  ...
  if not bConfirm then
    ...
    Dialog:Say("你确定把" .. szDomainName .. "要分配给" .. pTargetTong.GetName() .. "吗？", { { "确定", self.DlgDispenseDomain, self, nTargetDomainId, nTargetTongId, 1 }, { "让我再考虑一下" } })
    return 0
  end
