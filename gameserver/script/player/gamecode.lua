...
function Player:ShowCheckHistory(szFlag)
  ...
  for nId, tbErrorInfo in pairs(Player.tbErrorHistory) do
    if tbErrorInfo.nErrorTimes > 0 then
      print(string.format("%s:	%s	%s	%s	%d	%d	%d	%s", szFlag, tbErrorInfo.szName, tbErrorInfo.szAccount, tbErrorInfo.szIp, tbErrorInfo.nLevel, tbErrorInfo.nErrorTimes, tbErrorInfo.nWarnTimes, os.date("%m-%d %H:%M:%S", tbErrorInfo.nLastTime)))
    end
  end
end
