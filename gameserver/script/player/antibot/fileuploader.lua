...
function tbFileUploader:ReceiveClientPackage(pPlayer, tbPackage)
  ...
  if self.bLogPrint == 1 then
    print(string.format("Player[%s], File[%s] Receive Package: State[%d], BeginPos[%d], UploadSize[%d], ReqSize[%d]。", pPlayer.szName, self.tbRequestList[pPlayer.nId][2], tbPackage.nState, tbPackage.nBeginPos, tbPackage.nUploadSize, tbPackage.nReqSize))
  end

  if tbPackage.nState == self.CLIENT_RET_SUCCESS_NOT_LAST or tbPackage.nState == self.CLIENT_RET_SUCCESS_LAST then -- 成功
...
function tbFileUploader:RemoveRequest(varKey)
  ...
  if RemoveClientUpLoadFile(szPlayerName, tbRequest[2]) == 0 then
    ...
    print(string.format("Player[%s], File[%s] delete Failed!", szPlayerName, tbRequest[2]))
  end
...
function tbFileUploader:_OnTimer()
  -- 队列中已经没有请求了，关掉计时器
  if not self.tbRequestList or Lib:CountTB(self.tbRequestList) == 0 then
    self.nTimerId = nil
    return 0 -- 会关掉
  end
