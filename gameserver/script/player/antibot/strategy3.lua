...
function tbStrategy3:GetLogMsg(pPlayer)
  local szMsg = string.format("策略名：随机登陆10-16次并且在线时间超过1个小时丢入天牢\t被判定为外挂后，登陆%d次丢入天牢\t判定为外挂后的在线时间%.1f小时", pPlayer.GetTask(Player.tbAntiBot.TSKGID, self.TSK_DEFINEPOINT), pPlayer.GetTask(Player.tbAntiBot.TSKGID, self.TSK_ONLINE_TIME) / 3600)
  return szMsg
end
