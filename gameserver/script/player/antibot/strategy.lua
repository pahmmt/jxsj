...
function tbBase:ImmediateAgent(nPlayerId)
  ...
  if pPlayer.GetArrestTime() == 0 then
    local szLogMsg = string.format("[反外挂]：直接丢天牢(使用了第三方辅助工具)：\t账号：%s\t角色：%s\t等级：%d\tIP地址：%s\t丢入天牢的时间：%s\t处理成功。", pPlayer.szAccount, pPlayer.szName, pPlayer.nLevel, pPlayer.GetPlayerIpAddress(), GetLocalDate("%Y\\%m\\%d  %H:%M:%S"))
...
function tbBase:Agent(pPlayer, nIndex)
  ...
  if tbAnti.DEFAULT_OPERATE == 0 then --只写日志记录，不丢入天牢
    ...
    local szLogMsg = string.format("[反外挂]：丢天牢处理结果：\t账号：%s\t角色：%s\t等级：%d\tIP地址：%s\t时间：%s\t策略名：%s\t策略信息：%s\t%s\t只是写日志记录，并未做丢入天牢的处理", pPlayer.szAccount, pPlayer.szName, pPlayer.nLevel, pPlayer.GetPlayerIpAddress(), GetLocalDate("%Y\\%m\\%d  %H:%M:%S"), tbBase.tbStrategyList[nIndex].szName, tbBase.tbStrategyList[nIndex].fnGetLogMsg(tbBase.tbStrategyList[nIndex].obj, pPlayer), tbAnti.tbScore:ScoreLog(pPlayer))
    pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_ANTIBOT_PROCESS, szLogMsg)
    return 0
  end
  ...
  if pPlayer.GetTask(tbAnti.TSKGID, tbAnti.TSK_ACTUAL_SCORE) < tbAnti.CRITICAL_VALUE then
    ...
    --写log日志
    local szLogMsg = string.format("[反外挂]：分数小于临界值不做丢天牢的处理：\t账号：%s\t角色：%s\t等级：%d\tIP地址：%s\t时间：%s\t策略名：%s\t策略信息：%s\t%s", pPlayer.szAccount, pPlayer.szName, pPlayer.nLevel, pPlayer.GetPlayerIpAddress(), GetLocalDate("%Y\\%m\\%d  %H:%M:%S"), tbBase.tbStrategyList[nIndex].szName, tbBase.tbStrategyList[nIndex].fnGetLogMsg(tbBase.tbStrategyList[nIndex].obj, pPlayer), tbAnti.tbScore:ScoreLog(pPlayer))
    pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_ANTIBOT_PROCESS, szLogMsg)
    --print(szLogMsg);
    return 0
  end
  ...
  --如果玩家不在天牢中
  if pPlayer.GetArrestTime() == 0 then
    local szLogMsg = string.format("[反外挂]：丢天牢处理结果：\t账号：%s\t角色：%s\t等级：%d\tIP地址：%s\t丢入天牢的时间：%s\t策略名：%s\t策略信息：%s\t%s\t处理成功。", pPlayer.szAccount, pPlayer.szName, pPlayer.nLevel, pPlayer.GetPlayerIpAddress(), GetLocalDate("%Y\\%m\\%d  %H:%M:%S"), tbBase.tbStrategyList[nIndex].szName, tbBase.tbStrategyList[nIndex].fnGetLogMsg(tbBase.tbStrategyList[nIndex].obj, pPlayer), tbAnti.tbScore:ScoreLog(pPlayer))
    pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_ANTIBOT_PROCESS, szLogMsg)
    Player:Arrest(pPlayer.szName, 0) --打入天牢
  end
  return 0 --返回0结束Timer
