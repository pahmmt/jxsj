...
PlayerEvent.bOpenMovieFlag = 1 --CG开关
...
function PlayerEvent:OnEvent(szEvent, ...)
  ...
  if szEvent == "OnLogin" then
    --如果登陆，调用单纯登陆和通用登陆表
    --如果跨服，调用通用登陆表
    --减少跨服消耗，并且了解到跨服必须使用哪些功能
    local tbEvent1 = self.tbGlobalEvent["OnLogin"]
    local tbEvent2 = self.tbGlobalEvent["OnLoginOnly"]
    local tbExeEvent = tbEvent1
    if arg and arg[1] ~= 1 then
      --登陆，合并调用
      tbExeEvent = Lib:MergeTable2(tbEvent1, tbEvent2)
    end
    self:_CallBack(tbExeEvent, arg)
  else
...
function PlayerEvent:OnFirstLogin()
  -- 通知客户端首次登录
  if self.bOpenMovieFlag == 1 then
    me.CallClientScript({ "Ui:OnPlayMovie", 1 })
    me.SetTask(2204, 1, 1) -- 开场动画播放标记
  end
  --设置快捷栏前三格
  for i = 1, 3 do
  ...
  --by lqy 新开前30天的服务器，发豪华礼包优惠券
  local nOpenTime = KGblTask.SCGetDbTaskInt(DBTASD_SERVER_STARTTIME)
  if GetTime() < nOpenTime + 30 * 24 * 60 * 60 then
    me.AddItem(18, 1, 1902, 1)
  end
