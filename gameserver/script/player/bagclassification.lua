...
function Player:ItemSortOnSort(nContainerType, nRemainCellsType)
  ...
  for i = 1, #tbCells do
    ...
    for j = i + 1, #tbCells do
      ...
      if (pItem1 and pItem2) and (pItem1.nGenre == pItem2.nGenre) and (pItem1.nDetail == pItem2.nDetail) and (pItem1.nParticular == pItem2.nParticular) and (pItem1.nLevel == pItem2.nLevel) then
        ...
        if (pItem1.IsStackable() == 1 and nTimeOut1 == 0) and (pItem2.IsStackable() == 1 and nTimeOut2 == 0) then
          if pItem1.IsBind() == pItem2.IsBind() then
            local nRet = me.SwitchItem(tbCells[j].nContainerId, tbCells[j].nRow, tbCells[j].nColumn, tbCells[i].nContainerId, tbCells[i].nRow, tbCells[i].nColumn)
  ...
  for i = 1, #tbCells do
    ...
    if tbItemMVI.nNum ~= i then
      local nRet = me.SwitchItem(tbCells[i].nContainerId, tbCells[i].nRow, tbCells[i].nColumn, tbCells[tbItemMVI.nNum].nContainerId, tbCells[tbItemMVI.nNum].nRow, tbCells[tbItemMVI.nNum].nColumn)
      if nRet ~= 1 then
        return 0
      end
    end
...
-- 排序规则，可以添加和调换顺序
Player.tbSortRule = Player.tbSortRule or {
  Player.ItemSortCompareGenre,
  Player.ItemSortCompareDetail,
  Player.ItemSortCompareClassName,
  Player.ItemSortCompareBindType,
  Player.ItemSortCompareLevel,
  Player.ItemSortCompareName,
  Player.ItemSortCompareCount,
}
