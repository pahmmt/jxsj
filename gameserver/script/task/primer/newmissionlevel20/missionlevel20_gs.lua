...
function NewPrimerLv20:OnStartGame(nPlayerId, nMapId)
  ...
  Dialog:PlayIlluastration(pPlayer, {
    szImage = "chahua_xingkong.spr",
    szTalk = "打斗后的疲惫让我渐渐睡去，醒来后也不知过了几个时辰……夜色凉如水，该是去到花灯会的时候了。",
  })
  Dialog:SendBlackBoardMsg(pPlayer, "不知不觉，夜色正浓...")
...
function NewPrimerLv20:ApplyGameFailed(nPlayerId, nFlag)
  ...
  Dialog:SendBlackBoardMsg(pPlayer, "天色还早，你可以先去逛逛再来。")
end
...
function NewPrimerLv20:AddYanhua(nNpcId)
  ...
  self:SayZishuqing(me, "以后每年都一起放烟花吧……")
...
function NewPrimerLv20:AddLight()
  ...
  self:SayZishuqing(me, "一花一安平，不知道要多少花灯，才能祈得这天下安平。")
end
...
function NewPrimerLv20:OnFinalBossPercent(nNpcId, nPlayerId)
  ...
  pNpc.SendChat("前辈并非想伤你们……只是不能让游龙珏再留于这世上！")
  local tbNpcList = KNpc.GetAroundNpcListByNpc(nNpcId, 20)
...
function NewPrimerLv20:AddOtherZiqing(nNpcId, nMapId, nX, nY, nPlayerId)
  ...
  pNpc.Delete()
  Dialog:PlayIlluastration(pPlayer, { szImage = "chahua_xueguang.spr", szTalk = "<color=yellow>子书青<color>：啊......" })
...
function NewPrimerLv20:AddJuQing1()
  ...
  Player:RegisterTimer(25 * 18, self.AddJuQing1Ex, self, me.nId)
end
...
function NewPrimerLv20:AddJuQing1Ex(nPlayerId)
  ...
  if pPlayer then
    ...
    Player:RegisterTimer(36, self.AddJuQing1Ex2, self, nPlayerId)
    Setting:RestoreGlobalObj()
    pPlayer.GetNpc().SendChat("发生什么事了...")
  end
  return 0
end
...
function NewPrimerLv20:AddJuQing2()
  ...
  Player:RegisterTimer(18 * 18, self.AddJuQing2_black, self, me.nId)
  Player:RegisterTimer(34 * 18, self.AddJuQing2Ex, self, me.nId)
end
...
function NewPrimerLv20:GetLevelPos(pPlayer)
  local tbMap = { 2154, 2254, 2255, 2256, 2257, 2258, 2259 }
  local tbTasks = Task:GetPlayerTask(pPlayer).tbTasks[self.TASK_MAIN_ID]
  if tbTasks and (tbTasks.nReferId ~= self.TASK_SUB_ID_NEXT or tbTasks.nCurStep ~= -1) then
...
PlayerEvent:RegisterGlobal("OnLoginOnly", NewPrimerLv20.OnLogin, NewPrimerLv20)
...
function NewPrimerLv20:UseXiFu()
  ...
  Player:RegisterTimer(23 * 18, self.UseXiFuEx, self, me.nId) --18秒后释放
end
