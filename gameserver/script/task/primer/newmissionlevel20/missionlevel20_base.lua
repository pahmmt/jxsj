...
function tbBase:WaringMsg()
  ...
  if self.nCurSec % 300 == 0 then
    self:AllBlackBoard("当前距离副本关闭还有" .. math.floor((NewPrimerLv20.MAX_TIME - self.nCurSec) / 60) .. "分钟")
  end
...
function tbBase:FireYanhua()
  if #self.tbYanhuaList <= 0 then
    self.tbYanhuaList = KNpc.GetMapNpcWithName(self.nMapId, "烟花")
  end
end
...
function tbBase:OnLeave()
  ...
  if tbLevelPos and tbLevelPos[1] == 55 or tbLevelPos[1] == 2286 then
    me.SetRevivePos(tbLevelPos[1], 1)
  end
