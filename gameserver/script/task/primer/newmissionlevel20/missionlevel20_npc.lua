...
function tbShiBing:OnDeath(pNpcKiller)
  ...
  if tbTasks and tbTasks.nReferId == NewPrimerLv20.TASK_SUB_ID_NEXT and tbTasks.nCurStep == 1 and him.nTemplateId == 10258 then
...
function tbShenmi:OnDeath(pNpcKiller)
  ...
  if pPlayer then
    ChangeWorldWeather(him.nMapId, 1)
    ...
    Player:RegisterTimer(30 * 18, self.OnDeathEx, self, me.nId)
...
function tbShenmi:AddOtherZiqing(nNpcId, nMapId, nX, nY, nPlayerId)
  ...
  Dialog:SendBlackBoardMsg(pPlayer, "<color=yellow>子书青<color>：啊......")
...
function tbXuShiwei:OnDeath(pNpcKiller)
  ...
  if pPlayer then
    ...
    Player:RegisterTimer(90, self.OnDeathEx, self, me.nId)
    Setting:RestoreGlobalObj()
  end
end
