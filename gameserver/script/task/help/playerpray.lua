...
function tbPlayerPray:OnApplyGetResult()
  ...
  self:GivePrayResult()
  SpecialEvent.tbMessagePush:AchievePray(me)
end
...
function tbPlayerPray:GivePrayResult()
  ...
  if 0 < nPrayFlag then
    if 1 == nPrayFlag then
      pPlayer.Msg("对不起你等级还没达到50级，不能进行祈福！！")
    elseif 2 == nPrayFlag then
      pPlayer.Msg("因服务器原因，祈福功能现已禁止！！")
    end
...
function tbPlayerPray:GivePrayAnnouns(pPlayer, tbAward)
  ...
  for i = 1, #tbAward do
    if tbAward[i].szType == "playertitle" then
      szPopupMessage = "哇，您今天的<color=yellow>运气<color>真不错哦！把这个消息<color=yellow>截图<color>分享给朋友们吧！"
...
function tbPlayerPray:GetAnnounsMsg(pPlayer, nElement)
  ...
  local szTweet = ""
  if 1 == nElement then
    szMsg = " 祈福出5个金！获得500点声望和 <color=255,181,0>[独孤求败]<color> 称号"
    szTweet = [["#剑侠世界# 刚才祈福祈出了5个金！还获得了"独孤求败"称号！哈哈！"]]
  elseif 2 == nElement then
    szMsg = " 祈福出5个木！获得500点声望和 <color=255,181,0>[天下无双]<color> 称号"
    szTweet = [[#剑侠世界# 刚才祈福祈出了5个木！还获得了"天下无双"称号！哈哈！]]
  elseif 3 == nElement then
    szMsg = " 祈福出5个水！获得500点声望"
    if pPlayer.nSex == 0 then
      szMsg = szMsg .. "和 <color=255,181,0>[风一样的男子]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个水！还获得了"风一样的男子"称号！哈哈！]]
    elseif pPlayer.nSex == 1 then
      szMsg = szMsg .. "和 <color=255,181,0>[绝代芳华]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个水！还获得了"绝代芳华"称号！哈哈！]]
    end
  elseif 4 == nElement then
    szMsg = " 祈福出5个火！获得500点声望"
    if pPlayer.nSex == 0 then
      szMsg = szMsg .. "和 <color=255,181,0>[江湖最后一位侠客]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个火！还获得了"江湖最后一位侠客"称号！哈哈！]]
    elseif pPlayer.nSex == 1 then
      szMsg = szMsg .. "和 <color=255,181,0>[江湖最后一位侠女]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个火！还获得了"江湖最后一位侠女"称号！哈哈！]]
    end
  elseif 5 == nElement then
    szMsg = " 祈福出5个土！获得500点声望"
    if pPlayer.nSex == 0 then
      szMsg = szMsg .. "和 <color=255,181,0>[天之骄子]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个土！还获得了"天之骄子"称号！哈哈！]]
    elseif pPlayer.nSex == 1 then
      szMsg = szMsg .. "和 <color=255,181,0>[天之娇女]<color> 称号"
      szTweet = [[#剑侠世界# 刚才祈福祈出了5个土！还获得了"天之骄女"称号！哈哈！]]
    end
  end
...
function tbPlayerPray:ResetGiveCount(pPlayer)
  ...
  if pPlayer.GetTask(self.TSKGROUP, self.TSK_IFGETEXTRACOUNT) < nCoZoneTime and GetTime() < nCoZoneTime + 10 * 24 * 60 * 60 and pPlayer.nLevel >= 50 then
...
if MODULE_GAMESERVER then
  ...
  PlayerEvent:RegisterGlobal("OnLoginOnly", tbPlayerPray.Pray_OnLogin, tbPlayerPray)
end
