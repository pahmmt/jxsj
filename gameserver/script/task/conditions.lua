...
function TaskCond:HaveTitleAE(byTitleGenre, byTitleDetailType, byTitleLevel)
  ...
  for _, tbTitle in ipairs(tbTitles) do
    if tbTitle.byTitleGenre == byTitleGenre and tbTitle.byTitleDetailType == byTitleDetailType and tbTitle.byTitleLevel >= byTitleLevel then
      return 1
    end
  end
  return nil, "称号尚未达到要求"
...
function TaskCond:HaveTitle(byTitleGenre, byTitleDetailType, byTitleLevel, dwTitleParam)
  local tbTitles = me.GetAllTitle(nCamp, nClass)
  for _, tbTitle in ipairs(tbTitles) do
    if tbTitle.byTitleGenre == byTitleGenre and tbTitle.byTitleDetailType == byTitleDetailType and tbTitle.byTitleLevel == byTitleLevel and tbTitle.dwTitleParam == dwTitleParam then
      return 1
    end
  end
  return nil, "称号尚未达到要求"
  ...
  if nKinId and nKinId > 0 then
    ...
    if cKin then
      ...
      if cMember then
        ...
        if nFigure == Kin.FIGURE_CAPTAIN or nFigure == Kin.FIGURE_ASSISTANT or nFigure == Kin.FIGURE_REGULAR then
          return 1
        end
      end
    end
