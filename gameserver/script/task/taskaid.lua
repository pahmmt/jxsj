...
function tbGiveForm:DecreaseItemInList(pFind, tbItemList)
  for _, tbItem in ipairs(tbItemList) do
    if tbItem[1] == pFind.nGenre and tbItem[2] == pFind.nDetail and tbItem[3] == pFind.nParticular and (tbItem[4] == pFind.nLevel or tbItem[4] == -1) and (tbItem[5] == pFind.nSeries or tbItem[5] == -1) then
...
function tbGiveForm:DecreaseItemInListWithName(pFind, tbItemList)
  if tbItemList[1] == pFind.szOrgName and tbItemList[2] == pFind.szSuffix then
    return pFind.nCount
  end
  return 0
end
...
function Task:_Specil_SelectCountry()
  ...
  local szMsg = "想去哪个新手村："
  local tbOpt = {
    { "云中镇", me.NewWorld, 1, 1388, 3098 },
    { "江津村", me.NewWorld, 5, 1598, 3121 },
    { "永乐镇", me.NewWorld, 3, 1589, 3196 },
  }
...
function Task:IsSameItem(tbItem1, tbItem2)
  if tbItem1[1] ~= tbItem2[1] or tbItem1[2] ~= tbItem2[2] or tbItem1[3] ~= tbItem2[3] or tbItem1[4] ~= tbItem2[4] or tbItem1[5] ~= tbItem2[5] then
    return 0
  end
...
function Task:DoRepairTaskValue(bExchangeServerComing)
  ...
  if self:HaveDoneSubTask(pPlayer, tonumber("04", 16), tonumber("26", 16)) == 1 or self:HaveDoneSubTask(pPlayer, tonumber("05", 16), tonumber("33", 16)) == 1 or self:HaveDoneSubTask(pPlayer, tonumber("09", 16), tonumber("4A", 16)) == 1 or self:HaveDoneSubTask(pPlayer, tonumber("0C", 16), tonumber("5F", 16)) == 1 or self:HaveDoneSubTask(pPlayer, tonumber("0D", 16), tonumber("60", 16)) == 1 then
...
function Task:DailyEvent()
  ...
  me.SetTask(2207, 1, 0, 1) --跳过任务次数每天重置
end
...
PlayerEvent:RegisterGlobal("OnLoginOnly", Task.RepairTaskValue, Task)
PlayerEvent:RegisterGlobal("OnLoginOnly", Task.RepairBook, Task)
PlayerEvent:RegisterGlobal("OnLoginOnly", Task.RepairCampTask, Task)
