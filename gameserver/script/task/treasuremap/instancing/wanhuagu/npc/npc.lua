...
function tbNpc_Boss_2:OnLifePercentReduceHere(nLifePercent)
  ...
  if nLifePercent == 50 and tbInstancing.tbBossLifePoint[2] == 0 then
    -- 在这里生产可重生的熊
    for i = 14, 16 do
      KNpc.Add2(tbInstancing.tbNpcPos[i][1], tbInstancing.tbNpcPos[i][2], -1, nMapId, tbInstancing.tbNpcPos[i][4], tbInstancing.tbNpcPos[i][5], 1, 0, 1)
    end
...
function tbNpc_TalkNpc_3:OnDialog()
  ...
  Dialog:Say("客官来嘞，怎样？在岛上是否已经游玩够了？如果想回去的话，小六这就送您一程。", { "是的", self.SendOut, self, me, nMapId, nMapX, nMapY }, { "暂不" })
end
...
function tbNpc_Boss_3_talk:OnDialog()
  Dialog:Say("好吧，你们赢了，想要做什么药就和我说吧，天下没有我做不出来的药剂，只要你们能把材料搞来！", { "我想做隐身药[需要10块熊皮]", self.MakeIt, self, me }, { "离开" })
  return
end
