...
function WeekendFish:CheckPlayerLimit(pPlayer)
  if pPlayer.nLevel < self.PLAYER_LEVEL_LIMIT or pPlayer.nFaction <= 0 then
    return 0, "只有达到30级并且加入门派的玩家才能使用鱼竿哦，加油练级吧。"
  end
  return 1
end
...
function WeekendFish:CheckCanMakeBait(pPlayer, nNum)
  ...
  if self:CheckPlayerLimit(pPlayer) ~= 1 then
    return 0, "只有达到30级并且加入门派的玩家才允许制作鱼饵哦。"
  end
...
function WeekendFish:MakeFishBaitDlg(nCount, nSure)
  ...
  if not nSure then
    local szMsg = string.format("制作<color=yellow>%s个<color>鱼饵一共需要消耗精力、活力各<color=yellow>%s点<color>以及鱼饵粉<color=yellow>%s份<color>。用这种鱼饵做引钓到的鱼儿将比用普通的粗制鱼饵钓到的鱼更好！\n\n确定制作吗？", nCount, nCount * self.NUM_GTPMKP_MAKE, nCount)
    local tbOpt = {
      { "加工鱼饵", self.MakeFishBaitDlg, self, nCount, 1 },
      { "我再想想" },
    }
  ...
  GeneralProcess:StartProcess("精致鱼饵加工中", 5 * Env.GAME_FPS, { self.MakeFishBait, self, me.nId, nCount }, nil, tbEvent)
end
...
function WeekendFish:CheckCanChangeAward(pPlayer)
  if self:CheckPlayerLimit(pPlayer) ~= 1 then
    return 0, "只有达到30级且加入了门派的侠客才可以换鱼哦。"
  end
  local nRes, szMsg = self:CheckAwardTaskTime(pPlayer)
  if nRes ~= 1 then
    return 0, szMsg
...
function WeekendFish:CheckCanDetectFish(pPlayer)
  if self:CheckOpen() ~= 1 then
    return 0, "活动还未开启，请在每周六和周日再来找我吧。周六和周日10：00-13：00；16：00-19：00是钓鱼时间，领奖时间则持续到周六周日晚上23：30分。"
  end
  if self:CheckPlayerLimit(pPlayer) ~= 1 then
    return 0, "只有30级加入门派的侠士才能使用道具"
  end
  --if not Task:GetPlayerTask(pPlayer).tbTasks[self.TASK_MAIN_ID] then
...
function WeekendFish:CancelTask(pPlayer)
  if Task:GetPlayerTask(pPlayer).tbTasks[self.TASK_MAIN_ID] then
    pPlayer.Msg("很遗憾，你的钓鱼任务因为过期未完成而失败。钓鱼任务必须当天指定时间内完成。")
...
function WeekendFish:DetectFinishSort(nPlayerId, dwFishId, dwAllodesmusId)
  ...
  if not pNpcFish then
    pPlayer.Msg("你鉴定太慢了，这群鱼已经被别人钓完了，你还是再找找其他鱼群吧。")
    return 0
  end
  local tbFishInfo = pNpcFish.GetTempTable("Npc").tbFishInfo
  if not tbFishInfo then
    pPlayer.Msg("这个鱼群有点奇怪，我竟然鉴别不出")
    return 0
  end
  if tbFishInfo.nSortIndex > 0 then
    pPlayer.Msg(string.format("经水产大师鉴定，这群鱼是<color=green>%s<color>。", KItem.GetNameById(unpack(WeekendFish.ITEM_FISH_ID[tbFishInfo.nSortIndex]))))
  else
    pPlayer.Msg("眼神儿不行，还真没看出来这是什么鱼群。")
  end
end
...
function WeekendFish:HarvestFailure(nPlayerId, dwFloatId, dwFishId)
  ...
  if pPlayer then
    ...
    pPlayer.Msg("真可惜，鱼儿上钩却又逃走了")
    Dialog:SendBlackBoardMsg(pPlayer, "真可惜，鱼儿上钩却又逃走了")
  end
  return 0
end
...
function WeekendFish:FinishFishing(nPlayerId)
  ...
  if nFloatId <= 0 then
    pPlayer.Msg("太可惜了，鱼儿上钩了又跑掉了")
    Dialog:SendBlackBoardMsg(pPlayer, "太可惜了，鱼儿上钩了又跑掉了")
    pPlayer.CloseGenerProgress()
    return 0
  end
  ...
  if tbFloatInfo.nState == 1 then
    ...
    pPlayer.Msg("鱼儿还没上钩，要有耐心哦")
    Dialog:SendBlackBoardMsg(pPlayer, "鱼儿还没上钩，要有耐心哦")
    pPlayer.CloseGenerProgress()
    return 0
  end
end
...
function WeekendFish:GetRandFish(pPlayer, dwFishId)
  ...
  if pPlayer.CountFreeBagCell() < 1 then
    pPlayer.Msg("背包满了，没地方放鱼，眼睁睁看着鱼儿逃走了")
    Dialog:SendBlackBoardMsg(pPlayer, "背包满了，没地方放鱼，眼睁睁看着鱼儿逃走了")
    return 0
  end
  ...
  if nIsFish == 1 then
    ...
    if pItem then
      ...
      if nWeight == 50 then
        ...
        -- 公告
        pPlayer.SendMsgToFriend(string.format("您的好友[<color=yellow>%s<color>]在钓鱼时竟然钓上了一条重达<color=yellow>50斤的%s<color>，真是可喜可贺啊！", pPlayer.szName, pItem.szName))
        --       			Player:SendMsgToKinOrTong(pPlayer, string.format("在钓鱼时竟然钓上了一条重达50斤的%s，真是可喜可贺啊！", pItem.szName));
        KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, string.format("%s在钓鱼时竟然钓上了一条重达50斤的%s，真是可喜可贺啊！", pPlayer.szName, pItem.szName))
      end
      if nWeight >= 40 and nWeight < 50 then
        pPlayer.SendMsgToFriend(string.format("您的好友[<color=yellow>%s<color>]在钓鱼时竟然钓上了一条重达<color=yellow>%s斤的%s<color>，真是可喜可贺啊！", pPlayer.szName, nWeight, pItem.szName))
        --       			Player:SendMsgToKinOrTong(pPlayer, string.format("在钓鱼时竟然钓上了一条重达%s斤的%s，真是可喜可贺啊！", nWeight, pItem.szName));
      end
      pItem.SetGenInfo(1, nWeight) -- 设置鱼的重量
      pItem.Sync()
      local nTodayNum = pPlayer.GetTask(self.TASK_GROUP, self.TASK_TODAY_FISHTIMES)
      pPlayer.Msg(string.format("恭喜你钓上了今天的第<color=yellow>%s<color>条鱼，这是一条重达<color=yellow>%s斤<color>的<color=green>%s<color>", nTodayNum + 1, nWeight, pItem.szName))
...
function WeekendFish:RefreshFish_GS(tbRefreshFishSort)
  ...
  self.tbFishInMapList = {} -- 生成每个鱼群所在的地图列表
  for i = 1, self.MAX_FISH_KIND do
    self.tbFishInMapList[i] = {}
  end
  for nMapId, tbSort in pairs(tbRefreshFishSort) do
    if SubWorldID2Idx(nMapId) >= 0 then
      self.tbRefreshFishSort[nMapId] = tbSort
    end
    for _, nFishId in pairs(tbSort) do
      if nFishId > 0 and nFishId <= self.MAX_FISH_KIND then
        self.tbFishInMapList[nFishId][nMapId] = 1
      else
        print("WeekendFish:RefreshFish_GS fishid Error", nFishId)
      end
    end
  end
...
function WeekendFish:ShowAwardDialog()
  ...
  GeneralAward:SendAskAward(szAwardTalk, tbGeneralAward, { "WeekendFish:AwardFinish" })
end
-- 领奖回调
