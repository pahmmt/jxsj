...
-- 玩家申请一个FB
function TreasureMap2:CreateInstancing(pPlayer, nTreasureId, nTreasureLevel, nCityMapId, IsRandMap)
  --地图分配
  ...
  if not nApplyMapId then
    ...
    local tbMapInfo = TreasureMapEx:Id2MapInfo(nTreasureId)
    if Map:LoadDynMap(Map.DYNMAP_TREASUREMAP, tbMapInfo.nTemplateMapId, { self.OnLoadMapFinish, self, pPlayer.nId, nCityMapId, nTreasureId, nTreasureLevel, IsRandMap }) == 1 then
      self.MapTempletList.tbBelongList[pPlayer.nId] = { nCityMapId, 0, nTreasureId, nTreasureLevel, IsRandMap }
    ...
    --GlobalExcute({"TreasureMap2:SyncMap",  pPlayer.nId, nCityMapId});
    self:OpenInstance(pPlayer.nId, nTreasureId, nTreasureLevel, nApplyMapId, nCityMapId, 0, IsRandMap)
    return 1
  end
end
...
-- GC载入地图完毕 GC回调
function TreasureMap2:OnLoadMapFinish(nPlayerId, nCityMapId, nTreasureId, nTreasureLevel, IsRandMap, nMapId)
  ...
  self:OpenInstance(nPlayerId, nTreasureId, nTreasureLevel, nMapId, nCityMapId, 1, IsRandMap)
end

-- 开启副本地图
function TreasureMap2:OpenInstance(nPlayerId, nTreasureId, nTreasureLevel, nMapId, nCityMapId, bNewMap, IsRandMap)
  GCExcute({ "TreasureMap2:Apply_GC", { nPlayerId, nCityMapId, nTreasureId, nTreasureLevel, IsRandMap } }) -- OPEN的时候再同步比较好
  self.MapTempletList.tbBelongList[nPlayerId][2] = nMapId

  self:MapInit(nTreasureId, nTreasureLevel, nMapId, nPlayerId, bNewMap)

  local tbInstancingBase = TreasureMap2:GetInstancingBase(nTreasureId, nTreasureLevel)
  ...
  tbInstancing:StartGame(nPlayerId, nMapId, nCityMapId, nTreasureId, nTreasureLevel, IsRandMap)
end
...
function TreasureMap2:SyncMap(tbData)
  self:GetMapTempletList()
  local nPlayerId = tbData[1]
  if not self.MapTempletList.tbBelongList[nPlayerId] then
    self.MapTempletList.tbBelongList[nPlayerId] = { tbData[2], 0, tbData[3], tbData[4], tbData[5] }
  end
end
...
function TreasureMap2:GetTreasure(nTreasureId)
  local szMapTypeName = TreasureMapEx.tbId2Type[nTreasureId]
  if not szMapTypeName or not TreasureMapEx.tbMapInfo[szMapTypeName] then
    print("[ERR] GetTreasure", szMapTypeName)
    assert(false)
    return
  end
  return TreasureMapEx.tbMapInfo[szMapTypeName]
end
...
function TreasureMap2:CheckPlayer(nPlayerId, nMapId, nTreasureId, nTreasureLevel)
  return TreasureMap.TreasureMapEx:CheckPlayer(nPlayerId, nMapId, nTreasureLevel)
end
...
function TreasureMap2:MapInit(nTreasureId, nTreasureLevel, nMapId, nPlayerId, bNewMap)
  ...
  ClearMapObj(nMapId)
  local tbTreasure = self:GetTreasure(nTreasureId)
  local tbNpcInfo = tbTreasure.tbNpcInfo
  if not tbNpcInfo or not tbNpcInfo then
    print("[ERR] TreasureMap2:MapInit", nTreasureId, nTreasureLevel)
    assert(false)
  end
  local nNpcLevel = TreasureMapEx:Star2NpcLevel(nTreasureLevel)
  -- 刷NPC
  for _, tbInstanceNpc in ipairs(tbNpcInfo) do
    if tbInstanceNpc.nManually ~= 1 then --只刷新非手动刷新的怪
      ...
      for i = 1, tbInstanceNpc.nNpcCount do
        ...
        local pNpc = KNpc.Add2(tbInstanceNpc.nTemplateId, tbInstanceNpc.nNpcLevel or nNpcLevel, -1, nMapId, tbInstanceNpc.tbNpcPos[nIndex][1], tbInstanceNpc.tbNpcPos[nIndex][2])
        if pNpc then
          ...
          if tbInstanceNpc.tbNpcScore then
            nNpcScore = tbInstanceNpc.tbNpcScore[nTreasureLevel]
          end
          pNpc.GetTempTable("TreasureMap2").nNpcScore = nNpcScore or tbInstanceNpc.nNpcScore
...
function TreasureMap2:GetPlayerAwardInfo(nTreasureLevel, nGrade, bRand)
  local tbPlayerAward = {}
  local nCount, tbItem = TreasureMapEx:Star2AwrrowCount(nTreasureLevel, nGrade)
  if bRand == 1 then --是随机副本
    nCount = nCount + 3
  end
  tbPlayerAward.tbItem = tbItem
  tbPlayerAward.nCount = nCount
...
function TreasureMap2:AwardLog(nTreasureId, nTreasureLevel, szPlayerName, szItemName, nItemCount, bRand)
  local tbTreasure = self:GetTreasure(nTreasureId)
  self:WriteLog("奖励获得", string.format("%s,%s,%s,%d,%s", szPlayerName, tbTreasure.szName, szItemName, nItemCount, bRand == 1 and "随机" or "指定"))
end
