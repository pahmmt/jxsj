...
--改成以副本ID的形式
function TreasureMap2:GetInstancingBase(nTreasureId, nTreasureLevel)
  if not TreasureMap2.tbInstancingLib[nTreasureId] then
    TreasureMap2.tbInstancingLib[nTreasureId] = Lib:NewClass(self.Mission)
  end
  if nTreasureLevel then
    TreasureMap2.tbInstancingLib[nTreasureId].nNpcLevel = TreasureMap.TreasureMapEx:Star2NpcLevel(nTreasureLevel)
  end
...
function TreasureMap2:AddFriendFavor(tbTeamList, nMapId, nFavor)
  ...
  for i = 1, #tbTeamList do
    for j = i + 1, #tbTeamList do
      if tbTeamList[i].nMapId == nMapId and tbTeamList[j].nMapId == nMapId and tbTeamList[i].IsFriendRelation(tbTeamList[j].szName) == 1 then
...
function TreasureMap2:OnHorseApplyRequest_biluogu(tbInstancKey, nResult)
  ...
  if tbInstance.nCaptainId ~= tbInstancKey[2] or tbInstance.nStartTime ~= tbInstancKey[3] then
    return
  end
