...
function tbNpc_Captain:OnDeath(pNpc)
  ...
  TreasureMap2:AddInstanceScore(tbInstancing, 15)
...
function tbNpc_Boss_1:OnDeath()
  ...
  if tbInstancing.nTaoZi_Death == 0 then
    ...
    -- 护送陶子成功
    TreasureMap2:AddInstanceScore(tbInstancing, 25)
  end
...
function tbNpc_Boss_2:OnLifePercentReduceHere(nLifePercent)
  ...
  if nLifePercent == 50 and tbInstancing.tbBossLifePoint[2] == 0 then
    -- 在这里生产可重生的熊
    for i = 14, 16 do
      KNpc.Add2(tbInstancing.tbNpcPos[i][1], tbInstancing.nLevel, -1, nMapId, tbInstancing.tbNpcPos[i][4], tbInstancing.tbNpcPos[i][5], 1, 0, 1)
    end
...
function tbNpc_Door:OnDialog()
  ...
  if nKeys > 0 then
    GeneralProcess:StartProcess("开启中……", 10 * 18, { self.OnOpened, self, me.nId, him.dwId }, { me.Msg, "开启被打断" }, tbEvent)
  end
end
...
function tbNpc_Bag:OnOpened(nPlayerId, dwNpcId)
  ...
  if pNpc then
    ...
    if tbInstancing.nBoss_1 == 1 and tbInstancing.nBoss_3 == 0 then
      ...
      -- 通知附近的玩家
      TreasureMap2:NotifyAroundPlayer(pPlayer, "<color=yellow>" .. pPlayer.szName .. "得到了一把钥匙！<color>")
    elseif tbInstancing.nBoss_3 == 1 then
      pPlayer.AddItem(18, 1, 1016, 1)
      pPlayer.Msg("<color=yellow>你得到了一坛女儿红！<color>")
      -- 通知附近的玩家
      TreasureMap2:NotifyAroundPlayer(pPlayer, "<color=yellow>" .. pPlayer.szName .. "得到了一坛女儿红！<color>")
    end
    pNpc.Delete()
  end
end
...
function tbNpc_Bag_2:OnOpened(nPlayerId, dwNpcId)
  ...
  if pNpc then
    ...
    -- 通知附近的玩家
    TreasureMap2:NotifyAroundPlayer(pPlayer, "<color=yellow>" .. pPlayer.szName .. "得到了一支笛子！<color>")
    pNpc.Delete()
  end
end
...
function tbNpc_Boss_3_talk:OnDialog()
  Dialog:Say("好吧，你们赢了，想要做什么药就和我说吧，天下没有我做不出来的药剂，只要你们能把材料搞来！", { "我想做隐身药[需要10块熊皮]", self.MakeIt, self }, { "离开" })
  return
end
...
function tbNpc_Box:OnDialog()
	GeneralProcess:StartProcess("开启宝箱", 10 * 18, {self.OpenTreasureBox, self, me.nId, him.dwId}, {me.Msg, "开启被打断"}, tbEvent);
end;
