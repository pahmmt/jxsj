...
tbHorseCallNpc = {
  { 9781, 0, { 49568 / 32, 101952 / 32 }, { 49632 / 32, 102208 / 32 }, { 49696 / 32, 101728 / 32 }, { 49888 / 32, 101888 / 32 }, { 49856 / 32, 102144 / 32 } },
} --召唤npc

tbBossStep1 = {
  { 9780, 15, { 49728 / 32, 101984 / 32 } },
} --曹公公
...
tbGrass = {
  { 9784, 0, { 48000 / 32, 100448 / 32 }, { 48512 / 32, 100288 / 32 }, { 48736 / 32, 100512 / 32 } },
  { 9785, 0, { 48032 / 32, 98912 / 32 }, { 47904 / 32, 99616 / 32 }, { 48320 / 32, 99264 / 32 } },
  { 9786, 0, { 47264 / 32, 100832 / 32 }, { 47136 / 32, 101568 / 32 }, { 47936 / 32, 101888 / 32 } },
} --草药，采集
...
tbOpenSwitch = {
  { 9798, 0, { 46240 / 32, 92736 / 32 }, { 46720 / 32, 93440 / 32 }, { 47552 / 32, 92032 / 32 }, { 47264 / 32, 94464 / 32 }, { 48192 / 32, 93600 / 32 } },
} --机关

tbEnemyStep3 = {
  { 9799, 3, { 46400 / 32, 93920 / 32 }, { 46080 / 32, 93728 / 32 }, { 46432 / 32, 93632 / 32 }, { 46688 / 32, 93824 / 32 }, { 46784 / 32, 94784 / 32 } },
  { 9800, 3, { 46080 / 32, 93376 / 32 }, { 46208 / 32, 94048 / 32 }, { 46240 / 32, 94624 / 32 }, { 46400 / 32, 94784 / 32 }, { 46304 / 32, 94240 / 32 } },
  { 9801, 3, { 46624 / 32, 93568 / 32 }, { 46848 / 32, 94112 / 32 }, { 47040 / 32, 94080 / 32 }, { 47072 / 32, 94304 / 32 }, { 47040 / 32, 94656 / 32 } },
} --小怪
...
tbSwitchName = { "宫", "商", "角", "羽", "徵" } --机关名字

------room 5 ------------------
tbBossStep5 = {
  { 9802, 30, { 49216 / 32, 87392 / 32 } },
} --月姬

tbEnemyStep5 = {
  { 9803, 3, { 48512 / 32, 88416 / 32 }, { 48160 / 32, 88256 / 32 }, { 48864 / 32, 88416 / 32 }, { 48960 / 32, 88832 / 32 }, { 48640 / 32, 88896 / 32 } },
} --月姬随从
...
tbCallNpcStep6 = {
  { 9807, 0, { 50368 / 32, 92800 / 32 }, { 50656 / 32, 92448 / 32 }, { 50976 / 32, 92800 / 32 }, { 50688 / 32, 93184 / 32 } },
} --自爆召唤npc

tbBossStep6 = {
  { 9808, 40, { 50688 / 32, 92800 / 32 } },
} --镖局分舵boss
...
tbEnemyStep7 = {
  ...
  { 9812, 3, { 52032 / 32, 96192 / 32 }, { 52672 / 32, 96448 / 32 } },
  { 9813, 3, { 52352 / 32, 96096 / 32 } },
} --小怪

tbBossStep7 = {
  { 9815, 48, { 53056 / 32, 95264 / 32 } },
} --客栈boss

tbYuejiEnemy = {
  { 9816, 30, { 53152 / 32, 95296 / 32 } },
} --客栈内月姬
...
function tbRoom:StartStep1()
  ...
  local nNpcLevel = tbInstance.nNpcLevel --	TreasureMap2.TEMPLATE_LIST[self.tbBase.nTreasureId].tbNpcLevel[self.tbBase.nTreasureLevel];
      ...
      for _, tbPos in pairs(tbInfo) do
        if type(tbPos) == "table" then
          ...
          if pNpc then
            ...
            pNpc.GetTempTable("TreasureMap2").nNpcScore = nScore
...
function tbRoom:OnDefenseEnemyDeath()
  ...
  if self.nDefenseEnemyCount <= 0 then
    ...
    local szMsg = string.format("报────有人闯关了！")
...
function tbRoom:OnBossStep1Death()
  ...
  local szMsg = string.format("盟约不在曹公公身上，询问前方侠士如何通过荆棘林")
  self.tbBase:SendBlackBoardMsgByTeam(szMsg)
...
function tbRoom:ProcessGrass()
  if #self.tbHasGiveGrass ~= self.nNeedGrassCount then
    local szMsg = string.format("还需要%s种草药，继续在荆棘林中寻找", self.nNeedGrassCount - #self.tbHasGiveGrass)
    self.tbBase:SendBlackBoardMsgByTeam(szMsg)
  else
    self.tbTrapOpenState[3] = 1 --可以通过城门
    self.tbBase:DelTrapNpc(3)
    self:StartStep3()
    local szMsg = "清淡的药香自炉中升起，毒雾被驱散了"
    self.tbBase:SendBlackBoardMsgByTeam(szMsg)
  end
end
...
function tbRoom:ProcessStep3()
  ...
  self.tbBase:NpcTalk(pNpc.dwId, "无知中原人！古族封印的恶煞被你放出来了！")
  local nNpcLevel = tbInstance.nNpcLevel --TreasureMap2.TEMPLATE_LIST[self.tbBase.nTreasureId].tbNpcLevel[self.tbBase.nTreasureLevel];
...
function tbRoom:OnBossStep3Death()
  ...
  local pNpc = KNpc.GetById(self.nEnterNpcStep3Id)
  if pNpc then
    self.tbBase:NpcTalk(pNpc.dwId, "他乃我族大祭司好友，大祭司化骨后他煞气攻心…唉")
  end
  self.tbBase:AddKillBossNum(him)
  self.tbTrapOpenState[4] = 1 --可以通过城门
  self.tbBase:DelTrapNpc(4)
  self:StartStep4() --开启第4关
  local szMsg = string.format("前方地形甚是诡异，似有一名旅人在等着我们")
  self.tbBase:SendBlackBoardMsgByTeam(szMsg)
end
    ...
    if nTemplateId then
      ...
      for nIndex, tbPos in pairs(tbNewPos) do
        ...
        if pNpc then
          ...
          pNpc.SetTitle("<color=green>龙门五阵<color>")
...
function tbRoom:ProcessOpenSwitch(nNpcId)
  ...
  if nIsOrderd ~= 1 then
    --黑条提示开启错误
    self.tbBase:SendMsgByTeam("机关纹丝不动，似乎与旅人所说顺序不合")
    self.tbBase:SendBlackBoardMsgByTeam("机关纹丝不动，似乎与旅人所说顺序不合")
    ...
    if #self.tbOpenSwitchStep4Order == #self.tbNeedOpenStep4Order then --按顺序开启过了
      ...
      self.tbBase:SendBlackBoardMsgByTeam("“轰隆”一声，挡住迷宫出口的屏障打开了")
    end
  end
end
...
function tbRoom:OnCastLunhui()
  ...
  self.tbBase:NpcTalk(pBoss.dwId, "曲终人散，谁知道散的又是哪一双？")
  if #tbPlayer > 0 then
    for _, pPlayer in pairs(tbPlayer) do
  ...
  if pRemoveBuffNpc then
    self.tbBase:NpcTalk(pRemoveBuffNpc.dwId, "此曲莫闻！速寻谭某消抵！")
  end
...
function tbRoom:OnEnemyStep5Death()
  ...
  if self.nEnemyStep5Count <= 0 then
    self:AddBossStep5()
    self.tbBase:SendBlackBoardMsgByTeam("何人偷看月姬大人沐浴！")
  end
end
...
function tbRoom:StartWalk(nId)
  ...
  self.tbBase:NpcTalk(pNpc.dwId, "少侠勿怒，月姬误会了少侠，愿领少侠前去龙门分舵")
  self.tbBase:SendBlackBoardMsgByTeam("少侠勿怒，月姬误会了少侠，愿领少侠前去龙门分舵")
  self.nStartWalkTimer = 0
  return 0
end
...
function tbRoom:OnBossStep5Arrive(nId)
  ...
  self:StartStep6(nId)
  self.tbBase:SendBlackBoardMsgByTeam("龙门镖局押镖在此，休要擅闯，去别处投宿！")
end
      ...
      if nTemplateId then
        for _, tbPos in pairs(tbInfo) do
          if type(tbPos) == "table" then
            ...
            if pNpc then
              self.tbBase:NpcTalk(pNpc.dwId, "龙门镖局的人，个个不惧生死！")
...
function tbRoom:OnBossStep6Death()
  ...
  self.tbBase:AddKillBossNum(him)
  self:StartStep7()
  self.tbBase:SendBlackBoardMsgByTeam("看来龙门镖局的人并不知实情，进客栈会会携卖国盟约的贼子")
end
...
function tbRoom:OnBossStep7Death()
  ...
  self.tbBase:SendBlackBoardMsgByTeam("他身上果然带着议和盟约，就此销毁，保我大宋江山")
end
