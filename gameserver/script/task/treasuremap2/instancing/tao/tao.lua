...
local tbInstancing = TreasureMap2:GetInstancingBase(2)
...
function tbInstancing:OnNew()
  ...
  for i = 1, #tbStele_1 do
    local pNpc = KNpc.Add2(2707, 1, -1, self.nMapId, tbStele_1[i][1], tbStele_1[i][2])
    pNpc.szName = " "
    table.insert(self.tbStele_1_Idx, pNpc.dwId)
  end

  for i = 1, #tbStele_2 do
    local pNpc = KNpc.Add2(2707, 1, -1, self.nMapId, tbStele_2[i][1], tbStele_2[i][2])
    pNpc.szName = " "
