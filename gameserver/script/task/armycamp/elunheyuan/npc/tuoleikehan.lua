...
function tbNpcQuestion:StartQuestion(nNpcId, nStep, nQuestionId, nAnswerId)
  ...
  if nStep > self.nPassAnswerNum then -- 答对了五题通关
    ...
    Dialog:SendBlackBoardMsg(me, "看来你了解的不错，放你进来了")
...
function tbNpcQuestion:TalkEnd(nPlayerId)
  ...
  if pPlayer then
    Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "可汗似乎起疑，看来不得不与拖雷战斗一场。", 15)
    pPlayer.Msg("可汗似乎起疑，看来不得不与拖雷战斗一场。")
  end
end
...
function tbNpcQuestion:CallTeamate(nNpcId, nPlayerId)
  ...
  if GetTime() >= tbInstancing.tbKehandazhang.nCallTeamateTime + self.nCallTeamateSepTime then
    ...
    tbInstancing:SendPrompt(string.format("队友[%s]被传送进了大帐", pPlayer.szName), 0, 1, 0, 0)
    Dialog:SendBlackBoardMsg(pPlayer, "你被队友召唤进了大帐")
    self:AllComeIn(tbInstancing)
  else
    Dialog:Say("你还有队友没有通过答题考验。不过看在你对草原了解甚深的面子上，你可以帮助你的队友免除考验直接进入大帐。不过每45秒你只能帮助1名尚未通过的队友。在草原上行走，你们要了解的东西还有很多，一定要将你的经验多传授给他们！")
  end
end

-- 对话托雷
local tbNpcTuolei_Dialog = Npc:GetClass("elunheyuan_dlg_tuolei")
tbNpcTuolei_Dialog.tbWineName = { "杏花村", "稻香村", "西北望", "杜康" }
function tbNpcTuolei_Dialog:OnDialog()
  local nSubWorld = him.GetWorldPos()
  local tbInstancing = Task.tbArmyCampInstancingManager:GetInstancing(nSubWorld)
  assert(tbInstancing)
  if tbInstancing.tbTollgateReset[6] ~= 0 then
    Dialog:Say(string.format("%s：%s，你好！", him.szName, me.szName))
    return
  end
  ...
  if tbNpcData.nFailure == 1 then
    if tbNpcData.nDrinkState == 1 then
      ...
      szDrinkMsg = string.format("你需要按%s的顺序喝，才能尽的酒中真味啊！", szDrinkMsg)
      Dialog:Say(szDrinkMsg)
    else
      Dialog:Say("你们这几个家伙的确有些本领。")
    end
    return
  end
  -- 检查是否全部玩家都已经进入大帐
  for nPlayerId, _ in pairs(tbInstancing.tbAttendPlayerList) do
    if not tbInstancing.tbKehandazhang.tbEnterDazhangList[nPlayerId] then
      local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
      if pPlayer then
        local szMsg = string.format("队友[%s]还没有进入大帐，只有当所有参加挑战的勇士都进入大帐才能开始挑战", pPlayer.szName)
        local tbOpt = {
          { "召唤该队友进入大帐", tbNpcQuestion.CallTeamate, tbNpcQuestion, him.dwId, nPlayerId },
          { "我再想想" },
        }
        Dialog:Say(szMsg, tbOpt)
        return
  ...
  local szMsg = "既然父汗有命，纳托雷只有得罪了。不过别在意，这也是你们的表现机会，是不是？来，是好汉就全力以赴吧！"
  local tbOpt = {
    { "我们开始吧", self.StartFight, self, me.nId, him.dwId },
    { "我随便看看" },
  }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbNpcTuolei_Fight:OnDeath(pKiller)
  ...
  for nPlayerId, nFlag in pairs(tbInstancing.tbAttendPlayerList) do
    ...
    if pPlayer then
      ...
      Setting:RestoreGlobalObj()
      pPlayer.Msg("按照正确的顺序喝酒，否则会难以对抗铁木真。")
    end
  end
  -- 开启饮酒，过一分钟自动进入战斗
...
function tbNpcTuolei_Fight:CallSoldier(nBossId)
  ...
  if pBoss.GetTempTable("Task").nCallSoldierPrompt ~= 1 then
    ...
    tbInstancing:SendPrompt("若让士兵靠近拖雷，你会受到严重的伤害！", 0, 1, 1, 0)
  end
...
function tbNpcTiemuzhen_Dialog:OnDrink(nPlayerId, nNpcId, nType)
  ...
  if #tbInstancing.tbKehandazhang.tbPlayerDrinkInfo[nPlayerId] == #tbInstancing.tbKehandazhang.tbWineOrder then
    Dialog:SendBlackBoardMsg(pPlayer, "勇士不用再喝了，留点给别人喝吧")
    return
  end
  ...
  if nFlag == 0 then
    ...
    Dialog:SendBlackBoardMsg(pPlayer, "喝酒的顺序不对，重新喝")
  else
    -- 5种酒喝完了
    if #tbInstancing.tbKehandazhang.tbPlayerDrinkInfo[nPlayerId] == #tbInstancing.tbKehandazhang.tbWineOrder then
      -- 添加一个buff
      me.AddSkillState(2677, 1, 1, 18000)
      Dialog:SendBlackBoardMsg(pPlayer, "你痛饮了美酒，酣畅淋漓！")
      pPlayer.Msg("你痛饮了美酒，酣畅淋漓！")
    else
      Dialog:SendBlackBoardMsg(pPlayer, "不错，接着喝")
      pPlayer.Msg(string.format("你饮下一杯<color=yellow>%s<color>", pNpc.szName))
    end
  end
end
...
function tbNpcTiemuzhen_Fight:OnLifePercentReduceHere(nLifePercent)
  ...
  elseif nLifePercent == 70 then
    ...
    if pYoulongzhenqi then
      ...
      for _, teammate in ipairs(tbPlayList) do
        Task.tbArmyCampInstancingManager:ShowTip(teammate, string.format("请%s选择游龙真气", KGCPlayer.GetPlayerName(tbNpcInfo.nAssignPlayerId)), 20)
        teammate.Msg(string.format("请%s选择游龙真气", KGCPlayer.GetPlayerName(tbNpcInfo.nAssignPlayerId)))
      end
...
function tbNpcWine1:OnDialog()
  ...
  GeneralProcess:StartProcess("品尝中", 2 * Env.GAME_FPS, { tbNpcTiemuzhen_Dialog.OnDrink, tbNpcTiemuzhen_Dialog, me.nId, him.dwId, 1 }, nil, tbEvent)
end
...
function tbNpcWine2:OnDialog()
  ...
  GeneralProcess:StartProcess("品尝中", 2 * Env.GAME_FPS, { tbNpcTiemuzhen_Dialog.OnDrink, tbNpcTiemuzhen_Dialog, me.nId, him.dwId, 2 }, nil, tbEvent)
end
...
function tbNpcWine3:OnDialog()
  ...
  GeneralProcess:StartProcess("品尝中", 2 * Env.GAME_FPS, { tbNpcTiemuzhen_Dialog.OnDrink, tbNpcTiemuzhen_Dialog, me.nId, him.dwId, 3 }, nil, tbEvent)
end
...
function tbNpcWine4:OnDialog()
  ...
  GeneralProcess:StartProcess("品尝中", 2 * Env.GAME_FPS, { tbNpcTiemuzhen_Dialog.OnDrink, tbNpcTiemuzhen_Dialog, me.nId, him.dwId, 4 }, nil, tbEvent)
end
...
function tbNpcWine5:OnDialog()
  ...
  GeneralProcess:StartProcess("品尝中", 2 * Env.GAME_FPS, { tbNpcTiemuzhen_Dialog.OnDrink, tbNpcTiemuzhen_Dialog, me.nId, him.dwId, 5 }, nil, tbEvent)
end
...
function tbNpcYoulongzhenqi:SwitchAward(nNpcId, nSelectPlayerId, nIndex)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    teammate.Msg(string.format("恭喜你获得%s级别奖励增加了%s经验", self.tbAwardInfo[nLevel][1], self.tbAwardInfo[nLevel][2]))
    Dialog:SendBlackBoardMsg(teammate, string.format("恭喜你获得%s级别奖励", self.tbAwardInfo[nLevel][1]))
  end
  -- 选择的玩家有几率获得额外奖励
