...
function tbManager:OnDialog()
  ...
  if tbInstancing.tbTollgateReset[2] ~= 2 then
    Dialog:Say("先去套马吧，完成了套马再我来这里吧。")
    return
  end
  local szMsg = "足够的猎物，摆在我面前！多，过关，重赏！少，乖乖回家去！哈哈哈哈哈！"
  local tbOpt = {
    { "<color=yellow>打猎高手排行榜<color>", self.ViewRank, self },
    { "我随便看看" },
  }
  if tbInstancing.tbTollgateReset[3] == 0 then
    szMsg = szMsg .. "\n你的队友正在跟捕猎中，你先耐心等候"
  elseif tbInstancing.tbTollgateReset[3] == 1 then
    table.insert(tbOpt, 1, { "我要开始狩猎！", self.StartHunting, self, him.dwId })
    szMsg = szMsg .. "\n第一次挑战我还会有特殊任务给你们，等挑战成功后再来我这里看。"
  elseif tbInstancing.tbTollgateReset[3] == 2 then
    szMsg = szMsg .. "\n小伙子干的不错，老夫还算满意。"
    table.insert(tbOpt, 1, { "前往祭祀台", self.GoNext, self, him.dwId })
    -- 第一次挑战
    if tbInstancing.tbHuntingGround.nFirstChallenge == 1 then
      local nType = tbInstancing.tbHuntingGround.nExtraAwardType
      szMsg = szMsg .. string.format("\n<color=yellow>最近%s出没甚少，我手上也没有足够的存货，如果你有足够的数量可以交给我，我可以用好东西跟你交换哦。<color>", self.tbExtraAward[nType][1])
      table.insert(tbOpt, 1, { "<color=green>我正好猎到了你要的猎物！<color>", self.ExtraAward, self, him.dwId })
    end
  end
...
function tbManager:ViewRank()
  ...
  for nRank = 1, #tbRankList do
    ...
    szMsg = szMsg .. string.format("积分：<color=yellow>%s<color>\n", tbRankList[nRank].nPoint)
    szMsg = szMsg .. "参与队员："
...
function tbManager:StartHunting(nNpcId)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    Dialog:SendBlackBoardMsg(teammate, "赶快击杀小动物")
  end
...
function tbManager:GetExtraAward(nNpcId)
  ...
  if tbPlayerInfo[nType] and tbPlayerInfo[nType] >= self.tbExtraAward[nType][2] and tbInstancing.tbHuntingGround.tbExtraAwardFlag[me.nId] ~= 1 then
    ...
    Dialog:SendBlackBoardMsg(me, "恭喜你获得猎人给予的大量经验。")
...
function tbManager:KillAnimal(nNpcId, nMapId, nPlayerId, nType, nIsKing)
  ...
  if self:AddKillPoint(tbInstancing, pKillerPlayer.nId, nType, nIsKing) == 1 then
    ...
    if math.mod(tbInstancing.tbHuntingGround.tbAnimalAccumulationNum[nType], self.tbKingAnimalRandInfo[nType][2]) == 0 and tbInstancing.tbHuntingGround.tbKingAnimalRrefreshNum[nType] < self.tbKingAnimalRandInfo[nType][3] then
...
function tbManager:AddSpecialState(nNpcId, nMapId, nPlayerId, nType)
  ...
  if nType == 1 then -- 双倍
    ...
    tbInstancing:SendPrompt("双倍积分了，快打啊", 0, 1, 1, 0)
  elseif nType == 2 then -- 大波怪
    ...
    tbInstancing:SendPrompt("猎物马上要集体出动了，注意了", 0, 1, 1, 0)
  elseif nType == 3 then -- 加攻击
    ...
    Dialog:SendBlackBoardMsg(pKillerPlayer, "攻击增加了，快打啊！")
  elseif nType == 4 then -- debuff
    ...
    tbInstancing:SendPrompt("真倒霉，打到不该打的", 0, 1, 1, 0)
  end
  Achievement:FinishAchievement(pKillerPlayer, 491)
end
...
function tbManager:GetPlayerInfoTxt(tbInstancing, nPlayerId)
  ...
  local szGrade = ""
  for i = 1, #self.tbExtraAward do
    szGrade = szGrade .. string.format("%s:%s  %s个\n", i, self.tbExtraAward[i][1], tbPlayerInfo[i] or 0)
  end
  szMsg = szMsg .. szGrade
  return szMsg
end
...
function tbManager:HuntingTimerEnd(nNpcId)
  ...
  if tbInstancing.tbHuntingGround.nTotalPoint >= self.tbPointAward[1][1] then
    ...
    for _, teammate in ipairs(tbPlayList) do
      ...
      teammate.Msg(string.format("恭喜成功通过%s级别狩猎考验！请与猎官对话前往下一关。", nLevel))
      Dialog:SendBlackBoardMsg(teammate, "请与猎官对话前往下一关。")
    end
    for nPlayerId, nFlag in pairs(tbInstancing.tbAttendPlayerList) do
      local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
      if pPlayer then
        ...
        Dialog:SendBlackBoardMsg(pPlayer, string.format("恭喜你达到%s级别，请与猎官对话前往下一关。", self.tbPointAward[nLevel][3]))
