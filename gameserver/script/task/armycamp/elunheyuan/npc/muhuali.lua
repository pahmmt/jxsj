...
function tbNpcMuhuali_Dialog:CallBoss(nNpcId)
  ...
  if pBoss then
    ...
    tbInstancing:SendPrompt("破除金狼阵，击败木华黎。", 0, 0, 1, 0)
  else
    return Env.GAME_FPS
  end
  return 0
...
function tbNpcMuhuali_Fight:OnLifePercentReduceHere(nLifePercent)
  ...
  if nLifePercent == 77 then
    ...
    tbInstancing:SendPrompt("木华黎马上就要布置军阵了，小心！", 0, 1, 1, 0)
    tbInstancing:SendPrompt("有些阵旗会在一段时间内受到加护，任意攻击的话会受到严重伤害！请仔细辨识再进行攻击", 0, 0, 1, 0)
    return
  end
  ...
  if nLifePercent == 75 or nLifePercent == 25 then
    ...
    him.SendChat("布阵")
    tbInstancing:SendPrompt("直接攻击木华黎收效甚微！需要击毁阵旗才能破除军阵！", 0, 1, 1, 0)
    tbInstancing:SendPrompt("木华黎布置阵旗会使你的攻击力大幅降低，及时采集长生草解除。", 0, 0, 1, 1)
  end
...
function tbNpcMuhuali_Fight:OnDeath(pKiller)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    Setting:RestoreGlobalObj()
    teammate.Msg("恭喜通过木华黎的考验！请前往可汗大帐！")
    Dialog:SendBlackBoardMsg(teammate, "恭喜通过木华黎的考验！请前往可汗大帐！")
  end
...
function tbNpcChangshengcao:OnDialog()
  ...
  GeneralProcess:StartProcess("采集中", Env.GAME_FPS, { self.ClearBuff, self, me.nId, him.dwId }, nil, tbEvent)
end
