...
function tbNpcYongshi_Dialog:OnDialog()
  ...
  local szMsg = string.format("%s：哈哈哈哈哈，有勇气站在这里的都是勇士！你们准备好要挑战我们了么？站在台上，要么得胜下台，要么被揍下台，你可要想清楚了！", him.szName)
  local tbOpt = {
    { "我要开始挑战！", self.StartFight, self, me.nId, him.dwId },
    { "我再准备准备" },
  }
...
function tbNpcYongshi_Dialog:CallBoss(nLevel, nNpcId)
  ...
  elseif nLevel == 3 then -- 高级勇士1
    ...
    if pMengguyongshi then
      ...
      tbInstancing:SendPrompt("踩在地面的光圈上可以清除受到伤害增加的负面状态！", 0, 1, 1, 0)
...
function tbNpcYongshi_Dialog:CallWoodNpc(nBossId1, nBossId2)
  ...
  if #tbInstancing.tbBiwuchangInfo.tbNpcWoodId >= 8 then
    tbInstancing:SendPrompt("迅速清除比武台周围的机关！", 0, 1, 1, 0)
    return
  end
...
function tbNpcYongshi_Fight_2:OnDeath(pKiller)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    Setting:RestoreGlobalObj()
    teammate.Msg("挑战草原勇士，击败查木儿，鄂木儿兄弟。")
  end
  -- 删除机关
...
function tbNpcYongshi_Fight_3_1:ChallengeSuccess(tbInstancing)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    Setting:RestoreGlobalObj()
    teammate.Msg("成功通过了比武场的考验！赶快前往下一关套马场吧！")
  end
  -- 删除机关
  for _, nWoodId in ipairs(tbInstancing.tbBiwuchangInfo.tbNpcWoodId) do
    tbInstancing:DeleteNpc(nWoodId)
...
function pNpcJiguan:OnDialog()
  ...
  GeneralProcess:StartProcess("开启中", 1 * Env.GAME_FPS, { self.CloseJiguan, self, him.dwId, tbNpcData.nBelongBossId1, tbNpcData.nBelongBossId2 }, nil, tbEvent)
end
