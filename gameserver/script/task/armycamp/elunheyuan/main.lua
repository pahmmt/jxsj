...
tbInstancing.tbTrapNpcPosList = {
  ...
  [4] = {
    ["jishichang_enter"] = {
      tbAddState = { 0 },
      tbPosList = { { 1742, 3407 }, { 1744, 3405 }, { 1710, 3405 }, { 1712, 3406 }, { 1710, 3376 }, { 1711, 3374 }, { 1741, 3374 }, { 1743, 3376 } },
    },
...
function tbInstancing:OnBreath()
  ...
  if self.nCurSec % 600 == 0 then
    Task.tbArmyCampInstancingManager:Tip2MapPlayer(self.nMapId, "当前距离" .. self.tbSetting.szName .. "关闭还有" .. math.floor((self.tbSetting.nInstancingExistTime - self.nCurSec) / 60) .. "分钟")
  end
end
...
function tbInstancing:OnPlayerEnter(nPlayerId)
  ...
  Dialog:SetTimerPanel(pPlayer, "<color=Gold>军营副本：<color>\n<color=White>距副本关闭还有：<color>", (self.tbSetting.nInstancingExistTime - self.nCurSec))
  ...
  if nAllDeath == 1 then
    for nPlayerId, nFlag in pairs(self.tbAttendPlayerList) do
      ...
      if pPlayer then
        ...
        pPlayer.SetFightState(0)
        Dialog:ShowBattleMsg(pPlayer, 0, 0)
        Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "可以通过路路通传送到之前的关卡继续挑战", 20)
      end
      self.tbAttendPlayerList[nPlayerId] = nil
    end
    -- 初始化当前失败的关卡
...
-- 重置玩家FB相关的属性
function tbInstancing:ResetPlayerAttr(nPlayerId) end
...
function tbInstancing:ReviveInHuntingGround(nPlayerId)
  ...
  if self.tbAttendPlayerList[nPlayerId] and self.tbHuntingGround.tbReviveList then
    ...
    if self.tbHuntingGround.tbReviveList[nPlayerId] < self.nReviveTimesInHuntingGround then
      ...
      Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "你消耗了一次复活机会", 10)
      pPlayer.Msg("你消耗了一次复活机会。")
      return 1
    else
      local szMsg = Npc:GetClass("elunheyuan_animalmanager"):GetPlayerInfoTxt(self, nPlayerId)
      Dialog:SendBattleMsg(pPlayer, szMsg)
