...
function tbGuWang:OnDeath(pNpc)
  ...
  for _, player in ipairs(tbPlayList) do
    ...
    if (tbTask1 and tbTask1.nReferId == 565) or (tbTask2 and tbTask2.nReferId == 622) or (tbTask3 and tbTask3.nReferId == 703) or (tbTask4 and tbTask4.nReferId == 701) then
  ...
  if nEntryWayRate < 50 then
    ...
    for _, teammate in ipairs(tbPlayList) do
      Task.tbArmyCampInstancingManager:ShowTip(teammate, "一道神秘的闸门出现在祭台上")
    end
  end
end
...
function tbGuSheng:OnDeath(pNpc) end
...
function tbJinDiZhiMen:OnDialog()
  ...
  Dialog:Say("是否现在进入？", { "好", self.Enter, self, me, him.dwId, him.nMapId }, { "暂时不去" })
end
