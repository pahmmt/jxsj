...
function tbLuo:OnDialog()
  ...
  GeneralProcess:StartProcess("敲锣", 5 * Env.GAME_FPS, { self.OnPlay, self, me.nId, tbInstancing, nSubWorld }, { self.BreakPlay, self, me.nId, tbPlayList, szSound, "被打断！" }, tbEvent)
end
...
function tbWenZhu:OnDeath(pNpc)
  ...
  if tbInstancing.nWenZhu >= 200 and tbInstancing.nPlayDrumCount == 0 then
    local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
    for _, teammate in ipairs(tbPlayList) do
      Task.tbArmyCampInstancingManager:ShowTip(teammate, "现在可以打鼓了！")
...
function tbGu:OnDialog()
  ...
  GeneralProcess:StartProcess("打鼓", 5 * 18, { self.OnPlay, self, me.nId, tbInstancing, nSubWorld }, { self.BreakPlay, self, me.nId, tbPlayList, szSound, "被打断！" }, tbEvent)
end
...
function tbGu:OnPlay(nPlayerId, tbInstancing, nSubWorld)
  ...
  local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
  for _, teammate in ipairs(tbPlayList) do
    Task.tbArmyCampInstancingManager:ShowTip(teammate, "神蛛使已出现")
  end
end
...
function tbZhuMu:OnDeath(pNpc)
  ...
  local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
  for _, teammate in ipairs(tbPlayList) do
    Task.tbArmyCampInstancingManager:ShowTip(teammate, "您可以前往灵蝎峰了！")
  end
end
...
function tbZhuMu:OnLifePercentReduceHere(nLifePercent)
  ...
  if nLifePercent == 10 then
    ...
    for _, teammate in ipairs(tbPlayList) do
      ...
      teammate.SetFightState(1)
      Task.tbArmyCampInstancingManager:ShowTip(teammate, "神蛛使用情网大阵陷害你和你的队友！")
    end
  end
end
