...
function tbLaoMen:OnDialog()
  ...
  --
  GeneralProcess:StartProcess("正在开启", 5 * Env.GAME_FPS, { self.Open, self, me.nId, him.dwId, tbInstancing }, { me.Msg, "开启被打断" }, tbEvent)
end
...
function tbLaoMen:Open(nPlayerId, nNpcId, tbInstancing)
  ...
  if nSuccess < 30 then
    Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "铁门已打开！")
...
function tbTieGongJi:OnDialog()
  ...
  Dialog:Say("给我10只蝎尾，我可以帮你们除掉灵蝎使。", {
    { "给蝎尾", self.Give, self, tbInstancing, me.nId, him.dwId },
    { "结束对话" },
  })
end

function tbTieGongJi:Give(tbInstancing, nPlayerId, nNpcId)
  Task:OnGift("给铁嘴公鸡10只蝎尾。", self.tbNeedItemList, { self.Pass, self, tbInstancing, nPlayerId, nNpcId }, nil, { self.CheckRepeat, self, tbInstancing }, true)
end
...
function tbTieGongJi:Pass(tbInstancing, nPlayerId, nNpcId)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    Task.tbArmyCampInstancingManager:ShowTip(teammate, "铁嘴金鸡的鸣叫已令灵蝎使的金钟罩失去效力")
  end
end
...
function tbLingXieShi:OnDeath(pNpc)
  ...
  for _, teammate in ipairs(tbPlayList) do
    Task.tbArmyCampInstancingManager:ShowTip(teammate, "您可以前往天绝峰了！")
