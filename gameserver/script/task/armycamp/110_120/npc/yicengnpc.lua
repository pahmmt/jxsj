...
tbNpc.tbDesc = {
  "第一轮游戏开启：答案将在<color=red>6-36<color>之间随机产生。",
  "第二轮游戏开启：答案将在<color=red>5-30<color>之间随机产生。",
  "第三轮游戏开启：答案将在<color=red>4-24<color>之间随机产生。",
}
...
function tbNpc:OnDialog()
  ...
  if tbInstancing.nGuessState1 == 0 then
    local tbOpt = {
      { "开始游戏", self.GuessStart, self, tbInstancing },
      { "结束对话" },
    }
    ...
    if me.nId == tbInstancing.nCurGuessPlayer then
      ...
      if not tbInstancing.nPassGuess[tbInstancing.nCurGuessPlayer] or tbInstancing.nPassGuess[tbInstancing.nCurGuessPlayer] ~= 1 then
        if not tbInstancing.nReturnGuess[tbInstancing.nCurGuessPlayer] or tbInstancing.nReturnGuess[tbInstancing.nCurGuessPlayer] ~= 1 then
          tbOpt[#tbOpt + 1] = { "通过", self.InputNo, self, me.nId, him.dwId, tbInstancing, 4 }
          tbOpt[#tbOpt + 1] = { "返回", self.InputNo, self, me.nId, him.dwId, tbInstancing, 5 }
        end
      end
      Dialog:Say(szMsg, tbOpt)
    else
      Dialog:SendInfoBoardMsg(me, "您现在不能猜点，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      me.Msg("您现在不能猜点，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
    end
  end
end
...
function tbNpc:GuessStart(tbInstancing)
  ...
  if pPlayer ~= nil then
    ...
    for _, teammate in ipairs(tbPlayList) do
      Dialog:SendInfoBoardMsg(teammate, "猜点游戏开始，本轮的答案在<color=green>" .. (tbInstancing.nCurGuess1No + 1) .. " - " .. tbInstancing.nGuess1Max .. "<color>之间，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      teammate.Msg("猜点游戏开始，本轮的答案在<color=green>" .. (tbInstancing.nCurGuess1No + 1) .. " - " .. tbInstancing.nGuess1Max .. "<color>之间，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
    end
...
function tbNpc:OnBreath(nId, tbInstancing)
  ...
  szMsg = szMsg .. "<color>"
  Dialog:SendInfoBoardMsg(pPlayer, "您在30秒内未猜点，系统为您选随机择了" .. szMsg .. "。")
  pPlayer.Msg("您在30秒内未猜点，系统为您选随机择了" .. szMsg .. "。")
...
function tbNpc:InputNo(nId, dwId, tbInstancing, nCount)
  ...
  if nCount == 4 then
    ...
    local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
    for _, teammate in ipairs(tbPlayList) do
      Dialog:SendInfoBoardMsg(teammate, "<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>通过<color>，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      teammate.Msg("<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>通过<color>，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      tbInstancing.nCurGuessPlayer = pPlayer.nId
    end
  ...
  elseif nCount == 5 then
    ...
    local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
    for _, teammate in ipairs(tbPlayList) do
      Dialog:SendInfoBoardMsg(teammate, "<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>返回<color>，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      teammate.Msg("<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>返回<color>，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      tbInstancing.nCurGuessPlayer = pPlayer.nId
    end
    ...
    if tbInstancing.nGuessNo1 >= tbInstancing.nCurGuess1No + 1 and tbInstancing.nGuessNo1 <= tbInstancing.nCurGuess1No + nCount then
      ...
      if tbInstancing.nYiCengGuessCount ~= 3 then
        for _, teammate in ipairs(tbPlayList) do
          Dialog:SendInfoBoardMsg(teammate, "很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，请进行下一轮猜点")
          teammate.Msg("很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，请进行下一轮猜点")
        end
      ...
      for _, teammate in ipairs(tbPlayList) do
        Dialog:SendInfoBoardMsg(teammate, "<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>" .. szMsg .. "<color>，未猜中答案，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
        teammate.Msg("<color=yellow>" .. pCurPlayer.szName .. "<color>选择<color=green>" .. szMsg .. "<color>，未猜中答案，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      end
...
function tbNpc:GameOver(nNpcId, tbInstancing)
  ...
  for _, teammate in ipairs(tbPlayList) do
    ...
    if self.GUESS_GIFT[nWinCount] then
      ...
      Dialog:SendInfoBoardMsg(teammate, "很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，游戏结束。您获得了一个<color=yellow>" .. self.GUESS_GIFT[nWinCount][1] .. "<color>!")
      teammate.Msg("很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，游戏结束。您获得了一个<color=yellow>" .. self.GUESS_GIFT[nWinCount][1] .. "<color>!")
    else
      Dialog:SendInfoBoardMsg(teammate, "很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，游戏结束。")
      teammate.Msg("很不幸<color=yellow>" .. pCurPlayer.szName .. "<color>猜到了倒霉数字，游戏结束。")
    end
  end
...
tbJiGuan.szText = "<npc=4224>：你们找到了我，很好，但是要想通过这里，你们必须帮我找到三个答案。<end><npc=4224>：忘了说了，答到数字的人将会受到惩罚。<end><npc=4224>：想好了就开始游戏，你们尽情享受吧，哈哈哈哈……<end>"
...
function tbJiGuan:OnDialog()
  ...
  GeneralProcess:StartProcess("正在开启", 5 * 18, { self.OnOpen, self, him.dwId, me.nId, tbInstancing }, { me.Msg, "开启被打断！" }, tbEvent)
end
...
function tbJiGuan:OnOpen(dwNpcId, nPlayerId, tbInstancing)
  ...
  if tbNpcData.nNo ~= (tbInstancing.nOpenJiGuan + 1) then
    local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
    for _, teammate in ipairs(tbPlayList) do
      Task.tbArmyCampInstancingManager:ShowTip(teammate, "开启顺序不对，请从风重新开启!")
      teammate.Msg("开启顺序不对，请从风重新开启!")
    end
  ...
  local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
  for _, teammate in ipairs(tbPlayList) do
    Task.tbArmyCampInstancingManager:ShowTip(teammate, string.format("%s开启成功！", pNpc.szName))
  end
  KTeam.Msg2Team(pPlayer.nTeamId, string.format("%s开启成功！", pNpc.szName))
...
tbZhiYin.szDianShu =
  "猜点游戏规则：\n    1， 首先由系统在规定范围内之间随机挑一个数字（三轮的范围分别是6-36，5-30，4-24）。\n    2， 玩家轮流报数，第一个玩家从最小的数字开始报，以第一轮为例，可以报6，67，678，三种选择方式，如果报数中没有系统选定的数字，则安全通过。第二个玩家延续第一个玩家的报数顺序往下报，也是三种选择，以此类推如果有一位玩家的报数与系统挑中的数字相同，那么他就输掉了比赛。\n    3， 在数字的三种组合选择之外，玩家还可以选择通过和返回，顾名思义，觉得下个数字危险，选择通过下家不报数，或者返回给上家报数，将报数的顺序逆过来。在一局比赛中，玩家只能选择一次通过或者返回，使用之后，这两个选项将不会在面板中出现。\n    4， 当一个玩家猜中倒霉数字，则被标记为输掉本轮比赛。待到三轮结束后，按照综合成绩颁发奖励，一次未输者奖励最高。"
...
function tbSend1:OnDialog()
  ...
  Dialog:Say("是否传送？", { "好", self.Enter, self, me.nId, tbInstancing }, { "暂时不去" })
end
