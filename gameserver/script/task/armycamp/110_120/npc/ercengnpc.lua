...
tbNpc.szDesc = "数字将会在<color=red>1-100<color>之间随机产生，请按照顺序说出你们的答案。"
...
function tbNpc:OnDialog()
  ...
  if tbInstancing.nGuessState2 == 0 then
    local tbOpt = {
      { "开始游戏", self.GuessStart, self, tbInstancing, him.dwId },
      { "结束对话" },
    }
  ...
  if tbInstancing.nGuessState2 == 1 then
    ...
    if me.nId == tbInstancing.nCurGuessPlayer then
      Dialog:AskNumber("请输入您猜的点：", tbInstancing.nGuess2Max, self.InputNo, self, tbInstancing, him.dwId, me.nId)
    else
...
function tbNpc:GuessStart(tbInstancing, dwId)
  ...
  if pPlayer ~= nil then
    ...
    for _, teammate in ipairs(tbPlayList) do
      Dialog:SendInfoBoardMsg(teammate, "猜点游戏开始，答案在<color=green>" .. tbInstancing.nCurGuess2No .. " - " .. tbInstancing.nGuess2Max .. "<color>之间，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
      teammate.Msg("猜点游戏开始，答案在<color=green>" .. tbInstancing.nCurGuess2No .. " - " .. tbInstancing.nGuess2Max .. "<color>之间，请<color=yellow>" .. pPlayer.szName .. "<color>猜点")
    end
...
function tbNpc:InputNo(tbInstancing, nNpcId, nId, nCount)
  ...
  if tbInstancing.nGuessNo2 == nCount then
    ...
    local tbPlayList, _ = KPlayer.GetMapPlayer(tbInstancing.nMapId)
    for _, teammate in ipairs(tbPlayList) do
      Dialog:SendInfoBoardMsg(teammate, "恭喜<color=yellow>" .. pCurPlayer.szName .. "<color>猜中了点数, 获得了一个黄金宝箱！")
      teammate.Msg("恭喜<color=yellow>" .. pCurPlayer.szName .. "<color>猜中了点数, 获得了一个黄金宝箱！")
    end
    ...
    for _, teammate in ipairs(tbPlayList) do
      ...
      Dialog:SendInfoBoardMsg(teammate, "数字<color=green>" .. nCount .. "<color>未猜中，请<color=yellow>" .. pPlayer.szName .. "<color>继续猜点！答案在<color=green>" .. tbInstancing.nCurGuess2No .. " - " .. tbInstancing.nGuess2Max .. "<color>之间。")
      teammate.Msg("数字<color=green>" .. nCount .. "<color>未猜中，请<color=yellow>" .. pPlayer.szName .. "<color>继续猜点！答案在<color=green>" .. tbInstancing.nCurGuess2No .. " - " .. tbInstancing.nGuess2Max .. "<color>之间。")
    end
...
function tbErCengSend:OnDialog()
  ...
  Dialog:Say("是否进入？", { "好", self.Enter, self, me.nId, him.dwId, tbInstancing }, { "暂时不去" })
end
...
function tbSend2:OnDialog()
  ...
  Dialog:Say("是否进入？", { "好", self.Enter, self, me.nId, tbInstancing }, { "暂时不去" })
end
