...
function tbBoss5:OnDeath(pNpc)
  ...
  for _, player in ipairs(tbPlayList) do
    ...
    if (tbTask1 and tbTask1.nReferId == 565) or (tbTask2 and tbTask2.nReferId == 622) or (tbTask3 and tbTask3.nReferId == 703) then
...
function tbNpc:OnDialog()
  ...
  GeneralProcess:StartProcess("灭火中...", Env.GAME_FPS, { self.CrushOut, self, him.dwId }, { me.Msg, "开启被打断！" }, tbEvent)
end
...
function tbDiYuNpc:OnDialog()
  ...
  GeneralProcess:StartProcess("开启中...", Env.GAME_FPS * 5, { self.Open, self, him.dwId, tbInstancing }, { me.Msg, "开启被打断！" }, tbEvent)
end
...
function tbDiYuNpc:Open(dwId, tbInstancing)
  ...
  Task.tbArmyCampInstancingManager:Tip2MapPlayer(tbInstancing.nMapId, "机关已开启，请及时熄灭周围冥火，否则机关会受到损伤!")
...
function tbDiYuNpc:TimeOver(nNo, tbInstancing)
  ...
  Task.tbArmyCampInstancingManager:Tip2MapPlayer(tbInstancing.nMapId, "冥火烧毁了机关，请您重新开启。")
