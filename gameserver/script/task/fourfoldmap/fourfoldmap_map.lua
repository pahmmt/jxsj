...
function tbMap:OnEnter()
  ...
  if nRes == 1 then
    ...
    if nOffLineTime < nLeftTime then
      ...
      local szMsg = string.format("您今天的离线托管时间还剩%s，在秘境中将有%s的时间不能获得离线托管经验。", Lib:TimeDesc(nOffLineTime), Lib:TimeDesc(nLeftTime - nOffLineTime))
      me.Msg(szMsg)
    end
    local nBaiJuTime = Player.tbOffline:GetLeftBaiJuTime(me)
    if nBaiJuTime < nLeftTime and nBaiJuTime < nOffLineTime then
      me.Msg("您的剩余白驹时间不足保持到秘境结束，建议补充白驹时间以免浪费在秘境内的在线托管经验。")
    end
  end
end
...
function tbPlayer:OnPlayer()
  ...
  local nPosX = Fourfold.PlayerTempList[me.nId].nPosX
  local nPosY = Fourfold.PlayerTempList[me.nId].nPosY
  GeneralProcess:StartProcess("离开中...", 3 * Env.GAME_FPS, { self.OnPlayerLeave, self, me.nId, nMapId, nPosX, nPosY }, nil, tbEvent)
end
