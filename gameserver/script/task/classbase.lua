...
function tbClassBase:GoNextStep()
  ...
  for _, tbCurTag in ipairs(self.tbCurTags) do
    if not tbCurTag:IsDone() then
      Dialog:SendInfoBoardMsg(pPlayer, "目标尚未完成：" .. tbCurTag:GetStaticDesc())
      return
    end
  end
  local tbStep = self:GetCurStep()
...
function tbClassBase:SetCurStep(nStep, bLoading)
  ...
  if tbStep then --还有步骤，说明还没有做完此任务
    ...
    if tbEvent.nType == 1 then --如果触发为npc
      self:AddNpcMenu(tbEvent.nValue, 0, self:GetName(), TaskAct.Talk, TaskAct, { self.GetNpcDialog, self }, Task.Active, Task, self.nTaskId, self.nReferId, nStep)
    elseif tbEvent.nType == 2 then --如果触发为Item
      self:AddItemUse(tbEvent.nValue, Task.Active, Task, self.nTaskId, self.nReferId, nStep)
    end
    ...
    if MODULE_GAMESERVER and not bLoading then
      if TaskCond:HaveBagSpace(self.tbReferData.nBagSpaceCount) then
        ...
        Dialog:SendInfoBoardMsg(me, "请清理出" .. self.tbReferData.nBagSpaceCount .. "个背包空间，再来领奖！")
      end
    end
    ...
    if not self.tbReferData.nReplyNpcId or self.tbReferData.nReplyNpcId <= 0 then
      ...
      if tbEvent.nType == 1 then
        self:AddNpcMenu(tbEvent.nValue, 0, self:GetName() .. "-领奖", unpack(tbAwardCall))
      else
        assert(false or "没地方领奖了!")
      end
    else
      self:AddNpcMenu(self.tbReferData.nReplyNpcId, 0, self:GetName() .. "-领奖", unpack(tbAwardCall))
    end
  end
...
function tbClassBase:IsItemToBeCollect(tbItem)
  for _, item in ipairs(self.tbCollectList) do
    if tbItem[1] == item[1] and tbItem[2] == item[2] and tbItem[3] == item[3] and tbItem[4] == item[4] then
      return 1
    end
  end
