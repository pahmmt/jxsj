...
function Merchant:NewRefer(nReferId, bAddStoneAward)
  ...
  tbReferData.szName = "商会任务"
  tbReferData.tbDesc = {
    szMainDesc = string.format("帮助商会完成%s个步骤的任务即可获得大量银两和玄晶奖励", self.TASKDATA_MAXCOUNT),
    tbStepsDesc = { "" },
  }
  ...
  tbReferData.szReplyDesc = "请找商会老板领取奖励" -- 回复文字
  ...
  for _, tb in pairs(self.TaskAwardFile[self.TASKDATA_MAXCOUNT]) do
    if tb.nMoney ~= 0 then
      local tbTemp = {
        ...
        nSprIdx = "",
        szDesc = "",
        szCondition1 = "TaskAwardCond:None",
        szCondition2 = "TaskAwardCond:None",
        szCondition3 = "TaskAwardCond:None",
      }
    ...
    if tb.nGenre ~= 0 and tb.nDetail ~= 0 and tb.nParticular ~= 0 then
      local tbTemp = {
        szType = "item",
        varValue = { tb.nGenre, tb.nDetail, tb.nParticular, tb.nLevel, tb.nSeries, -1 },
        nSprIdx = "",
        szDesc = "",
        szCondition1 = "TaskAwardCond:None",
        szCondition2 = "TaskAwardCond:None",
...
function Merchant:CheckFixItem(tbFix, szType, varValue)
  for nId, tbTemp in pairs(tbFix) do
    if tbTemp.szType == szType then
      if tbTemp.varValue[1] == varValue[1] and tbTemp.varValue[2] == varValue[2] and tbTemp.varValue[3] == varValue[3] and tbTemp.varValue[4] == varValue[4] and tbTemp.varValue[5] == varValue[5] then
        return nId
      end
    end
  end
...
function Merchant:GetStepAward(nStep, nExp, nMoney)
  ...
  if nExp > 0 then
    szAwardDesc = szAwardDesc .. string.format("  获得%s经验\n", nExp)
    table.insert(tbAward, string.format("TaskAct:AddExp(%s)", nExp))
  end

  if nMoney and nMoney > 0 then
    szAwardDesc = szAwardDesc .. string.format("  获得%s银两\n", nMoney)
  ...
  if self.TaskAwardFile[nStep] and nStep ~= self.TASKDATA_MAXCOUNT then
    szAwardDesc = szAwardDesc .. string.format("<color=gold>%s步额外奖励：<color=white>\n", nStep)
    for _, tb in pairs(self.TaskAwardFile[nStep]) do
      ...
      if tb.nBindMoney ~= 0 then
        ...
        szAwardDesc = szAwardDesc .. string.format("  获得%s绑定银两\n", tb.nBindMoney)
      end
      if tb.nBaseExp ~= 0 then
      ...
      if tb.nGenre ~= 0 and tb.nDetail ~= 0 and tb.nParticular ~= 0 then
        ...
        szAwardDesc = szAwardDesc .. string.format("  获得%s个%s\n", tb.nNum, tb.szName)
...
function Merchant:InitStep(nTypeId, nStepType, nLevelType, nTaskId, tbTargetFile)
  ...
  local szSelect = "商会任务"
  local szMsg1 = "您好，您是来完成商会任务的吧？把东西给我吧。"
  local szMsg2 = "您好，您已经完成。"
  local ItemSet = nil
  local szDesc = ""
    ...
    if nTypeId == Merchant.TYPE_DELIVERITEM or nTypeId == Merchant.TYPE_DELIVERITEM_NEW then
      ...
      if #tbFind1 <= 0 and #tbFind2 <= 0 then
        if me.CountFreeBagCell() >= 1 then
          ...
          me.Msg("您的背包空间不足，无法拿到商会信笺，请到商人处再领取。")
        end
      end
    end
  end
  if szTargetName == "GiveItem" then
    ...
    szDesc = string.format("给予物品：\n %s个%s", nNum, KItem.GetNameById(nGenre, nDetail, nParticular, nLevel))
  end

  if szTargetName == "GiveItemWithName" then
    ItemSet = { szItemName, szSuffix, nNum }
    szDesc = string.format("给予物品：\n %s个%s·%s", nNum, szItemName, szSuffix)
  end
...
function Merchant:LoadDate(nTaskId)
  ...
  if nTaskId == self.TASKDATA_ID then
    --更换类型，步骤做相应兑换
    if MODULE_GAMESERVER and self:GetTask(self.TASK_TYPE) <= 4 and self:GetTask(self.TASK_TYPE) > 0 and self:GetTask(self.TASK_RESET_NEWTYPE) == 0 then
