...
local tbQuestItem = { --npcId，物品Id，重生时间
...
function tbNpc:OnDialog()
  ...
  if me.CountFreeBagCell() < 1 then
    Dialog:Say("对不起，您的背包空间不足。")
    return 0
  end
  GeneralProcess:StartProcess("采集中...", DELAY_TIME * Env.GAME_FPS, { self.GetQuest, self, me.nId, him.dwId }, nil, tbEvent)
end
...
function tbNpc:GetQuest(nPlayerId, nNpcId)
  ...
  if nGenre > 0 and nDetail > 0 and nParticular > 0 and nLevel > 0 then
    ...
    local tbFind1 = me.FindItemInBags(nGenre, nDetail, nParticular, nLevel)
    if #tbFind1 >= nNum then
      Dialog:SendBlackBoardMsg(pPlayer, string.format("您已经收集齐了%s个%s", nNum, szName))
    end
  end
