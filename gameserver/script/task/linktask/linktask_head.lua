...
function LinkTask.tbGiftDialog:OnOK()
  ...
  if nAccordNum == tbNeed.nNeedCount then
    ...
    Dialog:Say("你放入的物品数量不符合，检查一下是否多了或者少了些什么东西？")
    tbDelItem = {}
    return
  end
end
...
function LinkTask:OnAward()
  ...
  SpecialEvent.tbMessagePush:AchieveLaobao(me)
  -- 调用奖励函数发奖
...
function LinkTask:Team_ShowTaskInfo(pPlayer, szCaptainName, nTaskType, nSubTaskId)
  ...
  Dialog:Say("你所在队伍的队长<color=yellow>" .. szCaptainName .. "<color>想要与你共享义军任务：<color=green>" .. szTaskName .. "<color>，你愿意接受吗？", {
