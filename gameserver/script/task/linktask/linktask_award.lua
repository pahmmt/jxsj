...
function LinkTask:_AssignItemRate(tbFile)
  ...
  for i = 1, nRow do
    ...
    table.insert(self.tbAwardItemRate[nGroupId], { nRate, nGenre, nDetail, nParticular, nLevel, nValue, szName, nBind })
  end
end
...
function LinkTask:CountAwardItem(nBit)
  ...
  -- 返回：名字, tbItem, 是否绑定
  return tbGroupItem[nRow][7], { tbGroupItem[nRow][2], tbGroupItem[nRow][3], tbGroupItem[nRow][4], tbGroupItem[nRow][5], 0, 0, 0, nil, 0, 0, tbGroupItem[nRow][8] }
end
...
function LinkTask:ShowAwardDialog(tbAward)
  ...
  local szAwardTalk = "很好！年轻人，这是你应得的奖励，来看看你喜欢些什么东西吧？" -- 奖励时说的话
  ...
  if nDailyTaskNum == 9 and nDailyAward ~= tonumber(GetLocalDate("%Y%m%d")) then
    ...
    table.insert(tbGeneralAward.tbFix, { szStatLogName = "包万同任务", szType = "exp", varValue = nFixExp, nSprIdx = 0, szDesc = "额外经验" })

    -- 将老包所有的固定银两都改为绑银 by peres 2009/02/16
    table.insert(tbGeneralAward.tbFix, { szStatLogName = "包万同任务", szType = "bindmoney", varValue = nFixMoney * 1.5, nSprIdx = 1, szDesc = "额外银两" })

    -- 去除额外给的精活 by peres 2009/06/11
    --		table.insert(tbGeneralAward.tbFix,
    --				{szType="makepoint",varValue=nMakePoint,nSprIdx=0,szDesc="额外精力"}
    --			);
    --		table.insert(tbGeneralAward.tbFix,
    --				{szType="gatherpoint",varValue=nGatherPoint,nSprIdx=0,szDesc="额外活力"}
    --			);
    ...
    table.insert(tbGeneralAward.tbFix, { szStatLogName = "包万同任务", szType = "item", varValue = { 18, 1, 9, nTreaMapItemLevel, 0, 0, 0, nil, 0, 0 }, nSprIdx = 0, szDesc = "藏宝图" })

    szAwardTalk = szAwardTalk .. "\n\n因为你在一天内连续完成了<color=green> 10 次任务<color>而得到以下特别奖励："
  elseif nDailyTaskNum > 10 and math.fmod(nDailyTaskNum + 1, 10) == 0 then
    if self:GetTask(self.tbExMoneyAward[nDailyTaskNum + 1]) == 0 then
      ...
      table.insert(tbGeneralAward.tbFix, { szStatLogName = "包万同任务", szType = "bindmoney", varValue = nFixMoney * 1.5, nSprIdx = 1, szDesc = "额外金钱" })
    end
  end
  ...
  table.insert(tbGeneralAward.tbFix, { szType = "linktask_repute", varValue = { 1, 1, nRepute }, nSprIdx = 0, szDesc = "义军声望 " .. nRepute .. " 点" })

  for i = 1, 3 do
    if tbAward[i] == 1 then -- 经验
      nValue = self:CountAwardExp()
      tbSelect[i] = { szStatLogName = "包万同任务", szType = "exp", varValue = nValue, nSprIdx = 0, szDesc = "经验 " .. nValue }
    elseif tbAward[i] == 2 then -- 银两
      nValue = self:CountAwardMoney()
      tbSelect[i] = { szStatLogName = "包万同任务", szType = "bindmoney", varValue = nValue, nSprIdx = 0, szDesc = "绑定银两 " .. nValue }
    elseif tbAward[i] == 3 then -- 物品
      szItemName, tbItem = self:CountAwardItem(i + 3)
      tbSelect[i] = { szStatLogName = "包万同任务", szType = "item", varValue = tbItem, nSprIdx = 0, szDesc = szItemName }
    elseif tbAward[i] == 4 then -- 取消机会
      tbSelect[i] = { szStatLogName = "包万同任务", szType = "linktask_cancel", varValue = 10, nSprIdx = 0, szDesc = "一次取消机会" }
    end
  end
  ...
  GeneralAward:SendAskAward(szAwardTalk, tbGeneralAward, { "LinkTask:AwardFinish", LinkTask.AwardFinish })
end
