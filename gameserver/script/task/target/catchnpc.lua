...
function tb:GetDesc()
  ...
  szMsg = szMsg .. string.format("%s并捉到%s：%d/%d", self.szNpcName, self.szItemName, self:GetCount(), self.nNeedCount)
  return szMsg
end
...
function tb:GetStaticDesc()
  ...
  szMsg = szMsg .. string.format("%s%d 个", self.szNpcName, self.nNeedCount)
  return szMsg
end
...
function tb:OnKillNpc(pPlayer, pNpc)
  ...
  local szMsg = string.format("捉到一个 %s ", self.szNpcName)
  pPlayer.Msg(szMsg)
