...
function tb:Dialog2Fight()
  ...
  local szTitle = "由<color=yellow>" .. pPlayer.szName .. "<color>的队伍护送"
...
function tb:OnRunTimer()
  ...
  if self.nRunElapseTime > self.nLimitTime then -- 到了指定时间，护送失败
    pFightNpc.Delete()
    pPlayer.Msg("没能在指定时间内成功护送" .. self.szMoveNpcName .. "，请返回起点重新对话开启护送任务。")
    self.nMoveNpcId = nil
    self.nRegisterRunTimerId = nil
    return 0
  else
    ...
    if not bAtPos then
      pFightNpc.Delete()
      pPlayer.Msg("您距离被护送的" .. self.szMoveNpcName .. "太远，护送任务失败，请返回起点重新对话开启护送任务。")
      self.nMoveNpcId = nil
      self.nRegisterRunTimerId = nil
      return 0
    end
    ...
    if self:IsDone() then -- 本目标是一旦达成后不会失效的
      pPlayer.Msg("目标达成：" .. self:GetStaticDesc())
      self:UnReg_Npc_RunTimer()
      self:UnRegisterTalk()
        ...
        for _, tbTask in pairs(Task:GetPlayerTask(tbTeamMembers[i]).tbTasks) do
          for _, tbCurTag in pairs(tbTask.tbCurTags) do
            if tbCurTag.szTargetName == self.szTargetName then
              if tbCurTag.nMoveNpcTempId == self.nMoveNpcTempId and (tbCurTag.nMapId == 0 or tbCurTag.nMapId == self.nMapId) then
...
function tb:OnDeath()
  ...
  self.nMoveNpcId = nil
  pPlayer.Msg(self.szMoveNpcName .. "已死亡，护送任务失败，请返回起点重新对话开启护送任务。")
end
