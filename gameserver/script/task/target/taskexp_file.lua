  ...
  for nId, tbParam in ipairs(tbFile) do
    if nId >= 1 then
      ...
      if nIndex > 0 and nGenre > 0 and nDetailType > 0 and nParticularType > 0 and nLevel > 0 then
        self.tbItem[nIndex] = {
          { nGenre, nDetailType, nParticularType, nLevel },
          szName,
          nPer,
          nPerEx,
          { nGenreEx, nDetailTypeEx, nParticularTypeEx, nLevelEx },
          szNameEx,
          nRate,
          nCheckMaker,
          { nBuy_Genre, nBuy_DetailType, nBuy_ParticularType, nBuy_Level },
          { nCheckTask, nTaskGroup, nTaskId, nMaxCount },
        }
      end
    end
  end
