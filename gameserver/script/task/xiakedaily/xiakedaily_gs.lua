...
function XiakeDaily:ShowAwardDialog()
  ...
  local szAwardTalk = "   言必行,行必果,己诺必诚,不爱其躯,赴士之阨困,千里诵义者也。荀悦曰,立气齐,作威福,结私交,以立强于世者,谓之侠。\n   你聪明智慧过我十倍，将来成就定然远胜于我，这是不消说的。只盼你心头牢牢记着“为国为民，侠之大者”这八个字，日后名扬天下，成为受万民敬仰的真正大侠。\n   这是给予你的奖励，请务必再接再厉！\n" -- 奖励时说的话
  ...
  if self:GetWeekTimes() + 1 == self.AWARDEX_WEEK_TIMES then
    ...
    if Item.tbStone:GetOpenDay() ~= 0 then
      ...
      szAwardTalk = szAwardTalk .. string.format("\n由于你本周完成了<color=yellow>%s<color>次，特给予你额外的<color=yellow>%s<color>个侠客令", self.AWARDEX_WEEK_TIMES, self.AWARD_EXTRA)
      szAwardTalk = szAwardTalk .. string.format("\n<color=yellow>%s<color>个蒙尘的宝石和<color=yellow>%s<color>个解玉锤", self.AWARD_STONE, self.AWARD_STONE_KEY)
    else
      nFreeCount = 1
      szAwardTalk = szAwardTalk .. string.format("\n由于你本周完成了<color=yellow>%s<color>次，特给予你额外的<color=yellow>%s<color>个侠客令", self.AWARDEX_WEEK_TIMES, self.AWARD_EXTRA)
    end
  ...
  if me.CountFreeBagCell() < nFreeCount + nFreeCountEx then
    Dialog:Say(string.format("您背包空间不足。请整理%s格空余背包空间", nFreeCount + nFreeCountEx))
    return 1
  end
  if nNum > 0 then
  ...
  if nStoneKey > 0 then
    table.insert(tbGeneralAward.tbFix, {
      szType = "item",
      varValue = { self.ITEM_STONE_KEY[1], self.ITEM_STONE_KEY[2], self.ITEM_STONE_KEY[3], self.ITEM_STONE_KEY[4], 0, 0, 0, 0, 0, 0, 1 },
      nSprIdx = "",
      szDesc = "解玉锤",
  ...
  GeneralAward:SendAskAward(szAwardTalk, tbGeneralAward, { "XiakeDaily:AwardFinish", nTaskLogId, nNum, me.nId, me.GetHonorLevel() })
end
...
function XiakeDaily:AwardFinish(nTaskLogId, nNum, nPlayerId, nHonorLevel)
  ...
  if Item.tbStone:GetOpenDay() ~= 0 then
    if self:GetWeekTimes() == self.AWARDEX_WEEK_TIMES then
      ...
      StatLog:WriteStatLog("stat_info", "baoshixiangqian", "xiake", me.nId, string.format("%d_%d_%d_%d,%d,%d_%d_%d_%d,%d", self.ITEM_STONE[1], self.ITEM_STONE[2], self.ITEM_STONE[3], self.ITEM_STONE[4], self.AWARD_STONE, self.ITEM_STONE_KEY[1], self.ITEM_STONE_KEY[2], self.ITEM_STONE_KEY[3], self.ITEM_STONE_KEY[4], self.AWARD_STONE_KEY))
    end
  end
end
...
function XiakeDaily:CancelTask()
  if Task:GetPlayerTask(me).tbTasks[self.TASK_MAIN_ID] then
    me.Msg("很遗憾，你的侠客任务因为过期未完成而失败。侠客任务必须当天接取当天完成。")
...
function XiakeDaily:AchieveTask(pPlayer, nType, nDetail)
  ...
  if self:GetTask(self.TASK_FIRST_TARGET) == 1 and self:GetTask(self.TASK_SECOND_TARGET) == 1 then
    Dialog:SendBlackBoardMsg(pPlayer, "你已完成侠客任务，尽快前往临安夏侯元韬处领奖吧！")
  end
  Setting:RestoreGlobalObj()
end
