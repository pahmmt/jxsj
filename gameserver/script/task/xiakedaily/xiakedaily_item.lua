...
function tbClass:OnUse()
  ...
  if me.CountFreeBagCell() < nNeedFreeBag then
    local szMsg = string.format("背包空间不足，打开礼包需要<color=yellow>%s<color>格背包空间。", nNeedFreeBag)
    me.Msg(szMsg)
    --Dialog:Say(szMsg)
    return 0
  end
  local szMsg = "成功使用侠义礼包，获得"
  for nIndex, tbTemp in ipairs(self.tbAward) do
    szMsg = szMsg .. string.format("<color=yellow>%s<color>个%s", tbTemp[6], tbTemp[8])
    for i = 1, tbTemp[6] do
