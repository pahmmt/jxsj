...
function tbBoss:TeamFinishTask(pPlayer, tbTeamMembers, nMemberCount)
  ...
  for i = 1, nMemberCount do
    if (pPlayer.nPlayerIndex ~= tbTeamMembers[i].nPlayerIndex) and (Task:AtNearDistance(pPlayer, tbTeamMembers[i]) == 1) and Task:GetPlayerTask(tbTeamMembers[i]).tbTasks[Wanted.TASK_MAIN_ID] then
      for _, tbCurTag in ipairs(Task:GetPlayerTask(tbTeamMembers[i]).tbTasks[Wanted.TASK_MAIN_ID].tbCurTags) do
        if tbCurTag.OnKillNpc then
  ...
  if nTaskId and Wanted.TaskFile[nTaskId] then
    ...
    if nRandCallBoss > 0 and nBossId > 0 then
      ...
      if pCur <= nRandCallBoss then
        ...
        if pNpc then
          local szMsg = string.format("%s在<color=green>%s(%s,%s)<color>抓住了大盗<color=white>%s<color>时，突然大盗叫来了他们的头领<color=white>%s<color>！", pPlayer.szName, GetMapNameFormId(nMapId), math.floor(nPosX / 8), math.floor(nPosY / 16), him.szName, pNpc.szName)
