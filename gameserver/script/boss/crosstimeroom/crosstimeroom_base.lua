...
function tbBase:EndGame()
  ...
  StatLog:WriteStatLog("stat_info", "yinyangshiguangdian", "over", self.nPlayerId, self.tbStartCount[1] or 0, self.tbStartCount[2] or 0, self.tbStartCount[3] or 0, self.tbStartCount[4] or 0, self.tbStartCount[5] or 0, nPassRoom) --数据埋点
...
function tbBase:OnDeath()
  ...
  if self.nCurrentRoomId ~= 2 then --第二关是特殊处理
    ...
    if nCanRevive == 1 then
      GeneralProcess:StartProcess("方士正殒身救你...", 5 * Env.GAME_FPS, { self.DoRevive, self, me.nId }, { self.Revive, self, me.nId }, {})
    else
      self:RemovePlayerInRoom(me.nId)
      local nIsFailed = self.tbRoom[self.nCurrentRoomId]:CheckFailed()
      if nIsFailed == 1 then
...
function tbBase:OnRevive(nPlayerId)
  ...
  if pPlayer and pPlayer.nMapId == self.nMapId then
    if nCanRevive == 1 then
      GeneralProcess:StartProcess("方士正殒身救你...", 5 * Env.GAME_FPS, { self.DoRevive, self, nPlayerId }, { self.Revive, self, nPlayerId }, {})
    else
