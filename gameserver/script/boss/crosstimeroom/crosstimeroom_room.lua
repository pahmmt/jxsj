...
function tb1stRoom:StartRoom()
  ...
  self.tbBase:AllBlackBoard("前方似乎有位白衣侠士，去问问他此为何地？")
end

function tb1stRoom:FailedRoom()
  self.nIsFailed = 1
  self:ClearRoom()
  self.tbBase:AllBlackBoard("书剑难销红尘曲，看来你还是拦不住他")
end
...
function tb1stRoom:TalkS1End()
  if self.pBossS1 then
    ...
    elseif self.nTalkState == 2 then
      ...
      self.nTalkTimer = 0
      self.tbBase:UpdateUiState("<color=yellow>阻止王遗风获悉文小月死讯<color>")
      return 0
    else
      self.nTalkTimer = 0
      return 0
...
function tb1stRoom:OnBossS1Death()
  ...
  self.tbBase:AllBlackBoard("王遗风似乎预感到文小月遭遇不测")
  self.nTalkState = 1
...
function tb1stRoom:OnBossS2Death()
  --刷箱子
  CrossTimeRoom:AddChest(1, self.tbBase)
...
function tb2ndRoom:FailedRoom()
  self.nIsFailed = 1
  self:ClearRoom()
  self.tbBase:AllBlackBoard("媚娘一死狐妖替，你还是没能改变历史")
end
...
function tb2ndRoom:TalkEnd()
  if self.pMeiniang then
    ...
    elseif self.nTalkState == 3 then
      ...
      self:SetBossLiAi()
      self.nTalkTimer = 0
      self.tbBase:UpdateUiState("<color=yellow>从道士袁天罡手中救下少女武媚娘<color>")
      return 0
    else
      self.nTalkTimer = 0
      return 0
...
function tb2ndRoom:OnBossYuanDeath()
  --刷箱子
  CrossTimeRoom:AddChest(2, self.tbBase)
...
function tb3rdRoom:FailedRoom()
  self.nIsFailed = 1
  self:ClearRoom()
  self.tbBase:AllBlackBoard("现在的你尚难抵挡，何况是年幼的你……")
end
...
function tb3rdRoom:TalkEnd()
  if self.pMan then
    ...
    elseif self.nTalkState == 6 then
      ...
      self.tbBase:UpdateUiState("<color=yellow>打败小太监，保护怀璧其罪的王爷<color>")
      self.nTalkTimer = 0
      return 0
    else
      self.nTalkTimer = 0
...
function tb3rdRoom:OnBossDeath()
  --刷箱子
  CrossTimeRoom:AddChest(3, self.tbBase)
  local tbPlayer = self.tbBase:GetPlayerList()
  for _, pPlayer in pairs(tbPlayer) do
    if pPlayer then
      local nIsNear = self:IsPlayerNear(pPlayer.nId)
...
function tb4thRoom:FailedRoom()
  self.nIsFailed = 1
  self:ClearRoom()
  self.tbBase:AllBlackBoard("好歹毒的心魔阵，连你也迷失其中")
end
...
function tb4thRoom:CastTianxianzi()
  ...
  self.tbBase:AllBlackBoard("天仙下凡，迷惑众生.....")
...
function tb4thRoom:OnBossNalanDeath()
  --刷箱子
  CrossTimeRoom:AddChest(4, self.tbBase)
...
function tb5thRoom:FailedRoom()
  self.nIsFailed = 1
  self:ClearRoom()
  self.tbBase:AllBlackBoard("你心力不济，恐怕也要陷入时间的深渊了")
end
...
function tb5thRoom:TalkEnd()
  if self.pBoss then
    ...
    elseif self.nTalkState == 3 then
      ...
      self.tbBase:UpdateUiState("<color=yellow>打败大司命，闯出阴阳时光殿<color>")
      self.nTalkTimer = 0
      return 0
    else
      self.nTalkTimer = 0
...
function tb5thRoom:BornChild(nPercent)
  ...
  elseif nPercent == 80 then
    ...
    if self.nKuileiCount > 0 and self.pBoss then
      self.tbBase:AllBlackBoard("迷失在过去的尸人正企图逃离")
      self.pBoss.SetCurCamp(6)
    end
  end
end
...
function tb5thRoom:ProcessStep(nPercent)
  if nPercent == 70 then --第二阶段
    self:ProcessStep2()
    self.tbBase:AllBlackBoard("幽冥犬似乎要被欲望撑破了") --test
  elseif nPercent == 50 then --第三阶段
    self:ProcessStep3()
    self.tbBase:AllBlackBoard("心魔阵开启，去看看你内心的选择") --test
  elseif nPercent == 20 then --第四阶段
    self:ProcessStep4()
    self.tbBase:AllBlackBoard("时空深渊的守护者出现了") --test
  end
end
...
function tb5thRoom:TransPlayerToNewPlace()
  if self.nCurrentNpcCount and self.nCurrentNpcCount ~= 0 and self.nInNewPlacePlayerId and self:IsPlayerNear(self.nInNewPlacePlayerId) == 1 then
...
function tb5thRoom:OnBossDeath()
  --刷箱子
  CrossTimeRoom:AddChest(5, self.tbBase)
  local tbPlayer = self.tbBase:GetPlayerList()
  for _, pPlayer in pairs(tbPlayer) do
    if pPlayer then
      local nIsNear = self:IsPlayerNear(pPlayer.nId)
