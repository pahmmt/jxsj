...
function tbTrap:OnPlayer()
  if self.nMapX and self.nMapY and self.nFightState then
    if self.nFightState == 1 then
      ...
      Atlantis:SendMessage(me, Atlantis.MSG_BOTTOM, "您进入了这片广袤的沙漠，注意提防所有意图不轨的人！")
      Atlantis:SendMessage(me, Atlantis.MSG_CHANNEL, "您进入了这片广袤的沙漠，注意提防所有意图不轨的人！")
    else
      if Atlantis:CheckHaveSuper(me.szName) == 1 then
        Atlantis:SendMessage(me, Atlantis.MSG_MIDDLE, "神器杀气太重，无法带回后营！")
        return 0
      end
      Atlantis:SendMessage(me, Atlantis.MSG_BOTTOM, "您回到了商队的营地，这里还是很安全的！")
