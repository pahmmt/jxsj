...
function tbNpc:OnDialog()
  ...
  GeneralProcess:StartProcess("拾取中", 10 * Env.GAME_FPS, { self.OnGetEquip, self, him.dwId, me.nId }, { self.OnBreak, self, me.nId }, tbBreakEvent)
...
function tbNpc:OnGetEquip(pNpcDwId, nPlayerId)
  ...
  if tbItem then
    ...
    if pItem then
      ...
      Atlantis:SendMessage(Atlantis.MSG_BOTTOM, "<color=yellow>您从这道光芒中，获得了楼兰古国封印的神器！<color>")
      Atlantis:BroadCast(Atlantis.MSG_CHANNEL, "<color=yellow>一位被先祖选中的人拿起了楼兰神器！<color>")
      KTeam.Msg2Team(pPlayer.nTeamId, string.format("[%s]拾取了楼兰神器！", pPlayer.szName))
