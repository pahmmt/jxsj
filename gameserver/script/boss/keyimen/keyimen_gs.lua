...
function Keyimen:UpdateRightUI(pPlayer)
  ...
  for nBossDwId, tbInfo in pairs(self.tbDamageList) do
    if tbInfo.nMapId == pPlayer.nMapId then
      ...
      szMsg = szMsg .. string.format("<color=yellow>【%s】\n<color>", tbInfo.szName)
      for i = 1, 3 do
        ...
        if pKin then
          ...
          szMsg = szMsg .. string.format("<color=green>%s. 家族：<color=cyan>%s<color>\n   <color=green>伤害：<color=cyan>%s<color>\n   <color=green>时间：<color=cyan>%s<color>\n<color>", i, szKinName, nDamage, nTime)
        else
          szMsg = szMsg .. string.format("<color=green>%s. - 无信息 -\n<color>", i)
        end
      end
    end
  end
  szMsg = szMsg .. "<taskid=60002>"
...
function Keyimen:CheckPlayer(pPlayer)
  ...
  if self:CheckIsOpen() ~= 1 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，地图尚未开放，请稍后再来。")
    return 0
  end
  ...
  local pKin = KKin.GetKin(nKinId)
  if not pKin then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "加入<color=yellow>家族<color>后，方可进入该地图。")
    return 0
  end

  -- 记名限制
  if pPlayer.nKinFigure == Kin.FIGURE_SIGNED then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "家族<color=yellow>正式成员<color>和<color=yellow>荣誉成员<color>，方可进入该地图。")
    return 0
  end

  -- 等级限制
  if pPlayer.nLevel < self.MIN_LEVEL then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "等级达到<color=yellow>100级<color>后，方可进入该地图。")
    return 0
  end

  -- 披风限制
  if self:GetMantleLevel(pPlayer) < self.MIN_MANTLE then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "前方凶险万千，披风等级达到<color=yellow>凌绝<color>的侠士方可进入该地图。")
    return 0
  end
...
function Keyimen:AddPlayer(pPlayer)
  ...
  self:SendMessage(pPlayer, self.MSG_CHANNEL, "您已自动获得<color=yellow>硝烟蚀甲<color>状态。")
  ...
  if nCamp > 0 and nCamp == 3 - Keyimen:GetTongCamp(nTongId) then
    ...
    self:SendMessage(pPlayer, self.MSG_CHANNEL, "您已自动获得<color=yellow>尖兵利刃<color>状态。")
  end

  pPlayer.SetTask(self.TASK_GID, self.TASK_PROTECT, 1)
  self:SendMessage(pPlayer, self.MSG_BOTTOM, "您已进入<color=blue>家族帮会PK模式<color>地图。")
...
function Keyimen:UpdateBossDamage(nNpcDwId)
  ...
  -- 排序表
  table.sort(tbDamage, function(a, b)
...
function Keyimen:UpdateBoss_GS(nBossId, nMapId, nMapX, nMapY, nCamp)
  ...
  if pNpc then
    self:BroadCast(self.MSG_BOTTOM, string.format("传说中的<color=cyan>%s<color>终于现身在克夷门战场！", pNpc.szName))
    self:BroadCast(self.MSG_GLOBAL, string.format("<color=cyan>突如其来的一阵地动山摇，电闪雷鸣，那传说中的%s终于现身在克夷门战场！<color>", pNpc.szName))
...
function Keyimen:UpdateGuard_GS(nGuardId, nMapId, nMapX, nMapY, nCamp)
  ...
  if pNpc then
    self:BroadCast(self.MSG_BOTTOM, string.format("危急时刻，猛将<color=green>%s<color>现身克夷门战场！<color>", pNpc.szName))
    self:BroadCast(self.MSG_CHANNEL, string.format("<color=green>危急时刻，猛将%s现身克夷门战场，大家快去找找吧！<color>", pNpc.szName))
    self.tbBossList[pNpc.dwId] = {}
...
function Keyimen:UpdateMonster_GS(nMonsterId, nMapId, nMapX, nMapY)
  ...
  if pNpc then
    local szMsg = string.format("<color=yellow>%s<color>出现了，大家快去找找吧！", pNpc.szName)
...
-- 刷出赤焰龙魂
function Keyimen:UpdateDialogNpc(nNpcdwId, nTongId, szTongName, pPlayer, szDragonName)
  local tbInfo = self.tbDragonList[nNpcdwId]
  if not tbInfo then
    return 0
  end
  ...
  if nFind > 0 then
    ...
    if pNpc then
      ...
      pNpc.SetTitle(string.format("<color=gold>%s的龙魂<color>", szTongName))
      Timer:Register(self.DIALOG_TIME * Env.GAME_FPS, self.OnDelDialogNpc, self, pNpc.dwId, nNpcdwId)
      local nKinId, nMemberId = pPlayer.GetKinMember()
      local pKin = KKin.GetKin(nKinId)
      if pKin then
        local szMsg = string.format("家族成员[%s]在克夷门战场击倒了%s<pos=%s,%s,%s>，大家赶紧前往吧。", pPlayer.szName, szDragonName, tbInfo.nMapId, tbInfo.nMapX, tbInfo.nMapY)
...
function Keyimen:KinFlagFailed_GS(nPlayerId, nKinId)
  ...
  if pPlayer then
    self:SendMessage(pPlayer, self.MSG_CHANNEL, "插旗失败，请稍后重试。")
  end
end
...
function Keyimen:KinFlagSuccess_GS(nPlayerId, nTongId, nKinId, tbPos)
  ...
  if SubWorldID2Idx(nMapId) >= 0 then
    local pPlayer = KPlayer.GetPlayerObjById(nPlayerId)
    if pPlayer then
      self:SendMessage(pPlayer, self.MSG_CHANNEL, "插旗成功，快通知大家传送过来吧。")
      KKin.Msg2Kin(nKinId, string.format("<color=yellow>%s<color>在对方阵营插了个大旗，家族成员可以直接传送过去啦！", pPlayer.szName), 0)
    end
    local nCamp = self:GetTongCamp(nTongId)
    if self.CAMP_FLAG_LIST[nCamp] then
      ...
      if pNpc then
        local pKin = KKin.GetKin(nKinId)
        if pKin then
          pNpc.szName = string.format("【%s】家族战旗", pKin.GetName())
          local nKinBadge = pKin.GetKinBadge()
          local nDetail = math.floor(nKinBadge / 10000)
          local nLevel = math.fmod(nKinBadge, 10000)
          if nDetail > 0 and nLevel > 0 then
            pNpc.AddSpeTitle(15, nDetail, nLevel)
          else
            pNpc.AddSpeTitle(15, 1, 1)
          end
        end
...
function Keyimen:FinishTaskAward(pPlayer)
  ...
  local szAwardTalk = "\n    恭喜你，终于重新封印了赤焰龙魂，神州大陆重归和平！这是赠与你的勇士礼物，快打开看看吧！\n" -- 奖励时说的话
  ...
  if pPlayer.CountFreeBagCell() < nNeed then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, (string.format("请留出%s格背包空间。", nNeed)))
    return 0
  end
  ...
  for _, tbTaskMain in pairs(tbTask) do
    for i = tbTaskMain[1], tbTaskMain[2] do
      if Task:GetPlayerTask(me).tbTasks[i] then
        self:SendMessage(me, self.MSG_CHANNEL, "很遗憾，你昨天的克夷门战场任务因为过期未完成而失败了。")
...
if Keyimen.nEventLoginId then
  PlayerEvent:UnRegisterGlobal("OnLoginOnly", Keyimen.nEventLoginId)
end
Keyimen.nEventLoginId = PlayerEvent:RegisterGlobal("OnLoginOnly", Keyimen.OnPlayerLogin, Keyimen)
