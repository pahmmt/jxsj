...
function tbBoss:OnDeath(pNpcKiller)
  ...
  for _, tbInfo in pairs(tbDropItem) do
    ...
    if pItem then
      ...
      local szKeyName = pPlayer and pPlayer.szName or "-无人拾取-"
  ...
  local szKinName = pKin and pKin.GetName() or "-未知-"

  -- 频道广播
  local szMsg = string.format("<color=cyan>[%s]<color>家族战胜了克夷门战场统帅<color=yellow>%s<color>！", szKinName, him.szName)
  ...
  for szPlayerName, tbItemInfo in pairs(tbList) do
    local szTmpMsg = ""
    if szPlayerName ~= "-无人拾取-" then
      szTmpMsg = string.format("<color=cyan>%s<color> 获得物品：", szPlayerName)
    else
      szTmpMsg = string.format("<color=green>下列物品散落到地上：<color>")
    end
    for szItemName, nCount in pairs(tbItemInfo) do
      szTmpMsg = szTmpMsg .. string.format("\n<color=yellow>%s - %s个<color>", szItemName, nCount)
    end
...
function tbBoss:OnLifePercentReduceHere(nLifePercent)
  ...
  for i, tbPercent in ipairs(Keyimen.BOSS_STEP) do
    if nLifePercent == tbPercent[1] and tbInfo.nStep == i then
      -- 说话
      local szMsg = string.format("%s说道：%s", him.szName, tbPercent[2])
  ...
  if nLifePercent == 20 then
    -- 说话
    local szMsg = string.format("%s说道：霞光万道，助我杀敌！", him.szName)
    Keyimen:BroadCast(Keyimen.MSG_CHANNEL, szMsg)
    -- 刷小怪
    local nMapId, nMapX, nMapY = him.GetWorldPos()
...
function tbGuard:OnDeath(pNpcKiller)
  ...
  -- 频道广播
  local szMsg = string.format("<color=yellow>[%s]<color>家族战胜了猛将<color=yellow>%s<color>！", szKinName, him.szName)
  Keyimen:BroadCast(Keyimen.MSG_GLOBAL, szMsg)
  KKin.Msg2Kin(nKinId, szMsg, 0)
...
function tbMonster:OnDeath(pNpcKiller)
  Keyimen.tbActiveList[him.nTemplateId] = nil
  Keyimen:BroadCast(Keyimen.MSG_CHANNEL, string.format("[<color=yellow>%s<color>]已经被人击杀了。", him.szName))
...
function tbDragon:OnDeath(pNpcKiller)
  local pPlayer = pNpcKiller.GetPlayer()
  if not pPlayer then
    Keyimen:RebornDragon_GS(him.dwId)
    return 0
  end
  ...
  if pTong then
    Keyimen:UpdateDialogNpc(him.dwId, nTongId, pTong.GetName(), pPlayer, him.szName)
  else
    Keyimen:RebornDragon_GS(him.dwId)
  end
end
...
function tbDialog:OnDialog()
  ...
  if me.GetTask(Keyimen.TASK_GID, Keyimen.TASK_FINISH[nIndex]) == 0 then
    ...
    GeneralProcess:StartProcess("释放龙魂中", 5 * Env.GAME_FPS, { self.OnFinish, self, me.nId, him.dwId }, nil, tbBreakEvent)
  end
end
...
function tbDialog:OnFinish(nPlayerId, nNpcdwId)
  ...
  if pPlayer and pNpc then
    ...
    if pPlayer.GetTask(Keyimen.TASK_GID, Keyimen.TASK_FINISH[nIndex]) == 0 then
      ...
      Keyimen:SendMessage(pPlayer, Keyimen.MSG_CHANNEL, string.format("你完成了任务：释放<color=yellow>%s<color>", pNpc.szName))
