...
function tbBase:RoomFinish()
  ...
  if self.nCurrentRoomId == ChenChongZhen.MAX_ROOM_COUNT then
    ...
    self:PlayerMsg("风岫和浮樱留在了火海里，这座名为辰虫的小镇再不曾出现过，稍后即将离去！")
  end
  if self.nCurrentRoomId < ChenChongZhen.MAX_ROOM_COUNT then
...
function tbBase:InitGame(nMapId, nServerId, nPlayerId)
  ...
  self.tbRoomOpenBox = { 0, 0, 0, 0, 0, 0, 0 } --记录某关是否已经开过箱子了
...
function tbBase:EndGame()
  ...
  StatLog:WriteStatLog("stat_info", "chenchongzhen", "over", self.nPlayerId, self.tbStartCount[1] or 0, self.tbStartCount[2] or 0, self.tbStartCount[3] or 0, self.tbStartCount[4] or 0, self.tbStartCount[5] or 0, self.tbStartCount[6] or 0, self.tbStartCount[7] or 0, nPassRoom, nUseTime) --数据埋点
...
function tbBase:OnDeath()
  ...
  if self.nCurrentRoomId ~= 7 then
    self:BlackBoard(me, "你已经身受重伤")
  else
    self:BlackBoard(me, "你失去了异兽的庇护，已被火海烧成重伤")
  end
  local nIsFailed = self:CheckAllPlayerDeath()
  if nIsFailed == 1 then
    self.tbRoom[self.nCurrentRoomId]:FailedRoom()
...
function tbBase:GetUnlockPlayerCount()
  local nCount = 0
  local tbPlayer = self:GetPlayerList()
  for _, pPlayer in pairs(tbPlayer) do
    if pPlayer then
...
function tbBase:ProcessTrap(szTrapName)
  if szTrapName == "trap_join" then
    if self:CheckAllPlayerLock() == 0 and self.nTransferRoomMaxId > 1 and me.GetTask(2191, 5) == 1 then
      Dialog:SendBlackBoardMsg(me, "请等待当前关卡结束后，再前往战斗。")
      me.SetFightState(0)
  ...
  elseif szTrapName == "trap_room4" then --第三关到第四关/小木屋
    if self.tbRoom[3]:IsRoomFinished() == 1 then
      if self.tbRoom[4]:IsRoomFinished() == 1 then
        local tbPos = ChenChongZhen.tbMapTrapName["trap_back5"]
        if tbPos then
          me.NewWorld(me.nMapId, tbPos[1], tbPos[2])
        end
      else
  ...
  elseif szTrapName == "trap_room5" then --第五关传送点
    if self.tbRoom[4]:IsRoomFinished() == 1 then
      if self.tbRoomOpenBox[4] == 1 then --第四关宝箱是否打开
        local tbPos = ChenChongZhen.tbMapTrapName[szTrapName]
        if tbPos then
          me.NewWorld(me.nMapId, tbPos[1], tbPos[2])
        ...
        self:BlackBoard(me, "宝物当前，还是打开后再前进吧。")
        return 0
      end
    else
      return 0
  ...
  elseif szTrapName == "trap_room5_trans" then --第四关到第三关
    me.NewWorld(me.nMapId, 1783, 2937)
  elseif szTrapName == "trap_back5" then --第五关到小木屋
    if self.tbRoom[5]:IsRoomFinished() == 1 then
      local tbPos = ChenChongZhen.tbMapTrapName[szTrapName]
      if tbPos then
        me.NewWorld(me.nMapId, tbPos[1], tbPos[2])
...
function tbBase:UpdateEndUi()
  ...
  local szMsg = "<color=green>距离副本关闭还有<color><color=white>%s<color>"
  for _, pPlayer in pairs(tbPlayer) do
    if pPlayer then
...
function tbBase:DropBox()
  ...
  local nPlayerCount = self:GetPlayerCount()
  local nLevel = (nPlayerCount < 3) and 1 or (nPlayerCount - 2)
  local nNpcId = ChenChongZhen.tbDropRateInfoNpc[nLevel]
  local pNpc = KNpc.Add2(nNpcId, 1, -1, self.nMapId, tbPos[1], tbPos[2])
  if pNpc then
  ...
  local szMsg = ChenChongZhen.tbDropBoxMsg[nLevel]
  local tbPlayer = self:GetPlayerList()
  if szMsg then
    for _, pPlayer in pairs(tbPlayer) do
      pPlayer.Msg(szMsg)
...
function tbBase:MoveAllPlayer(nMapId, nMapX, nMapY)
  local tbPlayer = self:GetPlayerList()
  for _, pPlayer in pairs(tbPlayer) do
