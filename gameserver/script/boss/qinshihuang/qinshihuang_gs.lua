    ...
    for nPlayerId, tbPlayerMap in pairs(self.tbPlayerList) do
      ...
      if pPlayer then
        ...
        if nUseTime + nTime > self.MAX_DAILY_TIME or self:_CheckState() ~= 1 then
          ...
          local szMsg = "秦陵内的毒气实在厉害，身体已经无力支撑。今天还是先歇息吧！"
        ...
        elseif self:_CheckTime() == 1 and pPlayer.nFightState > 0 and pPlayer.nPkModel ~= Player.emKPK_STATE_TONG then
          ...
          local szMsg = "秦始皇即将出没，你的战斗模式变为帮会家族模式！"
          self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
          self:SendMessage(pPlayer, self.MSG_BOTTOM, szMsg)
        end
      else
...
function tbQinshihuang:OpenRightUI(pPlayer, nRemainFrame)
  -- 右侧显示
  local szMsg = "<color=green>秦陵内剩余时间：<color=white>%s<color>"
...
function tbQinshihuang:DailyEvent()
  ...
  if Task:GetPlayerTask(me).tbTasks[529] then
    me.Msg("很遗憾，你昨天的秦始皇陵任务因为过期未完成而失败。")
  ...
  -- 使用美玉清0
  me.SetTask(self.TASK_GROUP_ID, self.TASK_USE_MEIYU, 0)
end
...
function tbQinshihuang:OnUseYemingzhu(nPlayerId)
  ...
  if not self.tbPlayerList[nPlayerId] then
    Dialog:SendInfoBoardMsg(pPlayer, "<color=red>该物品只能在秦陵内使用！<color>")
    return
  end
  ...
  if not self.tbYemingzhu[nMapLevel][nHonorLevel] or self.tbYemingzhu[nMapLevel][nHonorLevel] <= 0 then
    local szMsg = "你的身体目前还可以抵御毒气，不必使用！"
    self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
  ...
  if nSkillLevel > 0 then
    ...
    if nSkillLevel > nMapLevel - 1 then
      local szMsg = string.format("你身上已经有%s层夜明珠的效果了，不能在%s层使用！", nSkillLevel + 1, nMapLevel)
      self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
      self:SendMessage(pPlayer, self.MSG_MIDDLE, szMsg)
      return
    ...
    elseif nSkillLevel == nMapLevel - 1 then
      if nSkillFrame + nBaseFrame > 10 * 60 * 60 * Env.GAME_FPS then
        Dialog:Say(string.format("对不起，你身上的%s层夜明珠效果即将超过10小时，无法继续累积。", nMapLevel))
        return
      else
...
function tbQinshihuang:OnMapEffect(nPlayerId, nMapLevel)
  ...
  self:SendMessage(pPlayer, self.MSG_BOTTOM, "秦陵毒瘴弥漫，你已寸步难行，须得使用<color=yellow>夜明珠<color>来抵御毒气！")
  self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你现在进入了秦陵%d层，由于秦陵毒瘴弥漫，你已寸步难行，<color=yellow>须得使用江湖中传说的夜明珠来抵御此毒气<color>。", nMapLevel))
...
function tbQinshihuang:OnProtectBoss(nIndex, nTempId, nStep, tbDamage)
  ...
  -- 公告
  local szMsg = string.format("%s的伤害输出最高队伍：\n", tbBoss.szName)
  ...
  for i = 1, 3 do
    if tbSort[i] then
      ...
      if szCaptainName then
        szMsg = szMsg .. string.format("<color=green>第%d名：<color>", i) .. "<color=yellow>" .. szCaptainName .. "小队<color>\n"
      end
    end
  end
...
function tbQinshihuang:RecoverBoss(nRate, nIndex)
  ...
  local pNpc = KNpc.Add2(tbBoss.nNpcId, nLevel, -1, tbBoss.tbPos[1], tbBoss.tbPos[2], tbBoss.tbPos[3])
  ...
  if pNpc then
    ...
    for i = 1, 3 do
      if tbBoss.tbDamage[i] then
        pNpc.SetDamageTable(i, tbBoss.tbDamage[i].nTeamId, tbBoss.tbDamage[i].nPlayerId, tbBoss.tbDamage[i].nTime, tbBoss.tbDamage[i].nDamage)
      end
    end
  end
end
...
function tbQinshihuang:OnBossReal_GS(nRealMap)
  ...
  -- 刷传送点
  local tbPos = {
    [1] = { 1536, 1401, 3620, "传送阵3层" },
    [2] = { 1536, 1591, 3449, "传送阵5层" },
    [3] = { 1539, 1609, 3899, "传送阵5层" },
    [4] = { 1539, 1985, 3532, "传送阵5层" },
  }
  for i = 1, 4 do
...
function tbQinshihuang:OnBossCallOut(pNpc)
  ...
  -- 秦始皇
  if nType == 1 then
    local tbFloor = { [1536] = "一层", [1538] = "三层", [1540] = "五层" }
    ...
    -- 秦始皇
    local szMsg = string.format("<color=green>据盗墓的人传说，%s的%s此时在秦陵中苏醒了！<color>", tbFloor[nMapId] or "神秘", pNpc.szName)
  ...
  elseif nType == 2 then
    -- 小boss
    local szMsg = string.format("听闻在不远处%s出现了！", pNpc.szName)
    self:BroadCast(self.MSG_CHANNEL, szMsg)
...
function tbQinshihuang:DoUpdateQinBoss(nTemplateId, nMapId, nMapX, nMapY)
  ...
  -- add npc
  local nLevel = self:GetNpcLevel()
  local pNpc = KNpc.Add2(nTemplateId, nLevel, -1, nMapId, nMapX, nMapY, 0, 1)
