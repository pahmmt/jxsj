...
function tbQinshihuangBoss:OnDeath(pNpcKiller)
  ...
  -- 频道公告
  local szMsg = "您的好友[" .. pPlayer.szName .. "]打败了" .. him.szName .. "。"
  pPlayer.SendMsgToFriend(szMsg)
  Player:SendMsgToKinOrTong(pPlayer, "打败了" .. him.szName .. "。", 0)

  local szMsg = string.format("<color=green>%s<color>所在的小队于秦始皇陵中战胜了%s！", pPlayer.szName, him.szName)
  Boss.Qinshihuang:BroadCast(Boss.Qinshihuang.MSG_CHANNEL, szMsg)

  if nReal == 1 then
    local szMsg = string.format("<color=green>%s<color>所在的小队战胜了真正的秦始皇！！！", pPlayer.szName)
...
function tbQinshihuangBoss:OnLifePercentReduceHere(nLifePercent)
  ...
  if nLifePercent == 80 then
    if Boss.Qinshihuang:GetBossStep(nIndex) == 0 then
      ...
      -- 增加4个兵马桶
      local nLevel = Boss.Qinshihuang:GetNpcLevel()
      KNpc.Add2(2439, nLevel, -1, nMapId, nMapX - 15, nMapY)
      KNpc.Add2(2439, nLevel, -1, nMapId, nMapX + 15, nMapY)
      KNpc.Add2(2439, nLevel, -1, nMapId, nMapX, nMapY - 15)
      KNpc.Add2(2439, nLevel, -1, nMapId, nMapX, nMapY + 15)
  ...
  elseif nLifePercent == 50 then
    if Boss.Qinshihuang:GetBossStep(nIndex) == 1 then
      ...
      -- 增加4个招魂师
      local nLevel = Boss.Qinshihuang:GetNpcLevel()
      KNpc.Add2(2440, nLevel, -1, nMapId, nMapX - 15, nMapY)
      KNpc.Add2(2440, nLevel, -1, nMapId, nMapX + 15, nMapY)
      KNpc.Add2(2440, nLevel, -1, nMapId, nMapX, nMapY - 15)
      KNpc.Add2(2440, nLevel, -1, nMapId, nMapX, nMapY + 15)
...
function tbSmallBoss:OnDeath(pNpcKiller)
  ...
  pPlayer.SendMsgToFriend("您的好友[" .. pPlayer.szName .. "]打败了" .. him.szName .. "。")
  Player:SendMsgToKinOrTong(pPlayer, "打败了" .. him.szName .. "。", 0)
  Boss.Qinshihuang:BroadCast(Boss.Qinshihuang.MSG_CHANNEL, string.format("<color=green>%s<color>所在的小队击败了%s！", pPlayer.szName, him.szName))
