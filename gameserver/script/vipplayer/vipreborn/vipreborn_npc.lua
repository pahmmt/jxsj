...
function tbNpc:OnSelectGate(szGateName)
  ...
  tbOpt[#tbOpt + 1] = { "返回上层", self.ApplyReborn, self }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbNpc:OnEnterAccount(szGateName, szServerName, szGateway, szAccount)
  ...
  local szMsg = string.format(
    [[您确定转往区服<color=yellow>[%s-%s]<color>
...
不绑定价值量：<color=green>%s<color>	
  ...
  local tbOpt = {
    { "确定", self.OnConfirm, self, szAccount, szGateway, nSpec },
    { "我再想想" },
  }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbNpc:OnConfirm(szAccount, szGateway, nSpec)
  ...
  me.Msg("你已经顺利完成转服申请！")
  Dbg:WriteLog("VipReborn", "vip转服申请", me.szAccount, me.szName, "绑定价值量：" .. tbValue.nBindValue, "不绑定价值量：" .. tbValue.nNobindValue, "转向账号：" .. szAccount, "转向网关：" .. szGateway, "扩展点：" .. nExtPoint)
...
function tbNpc:FinishReborn()
  ...
  local szMsg = string.format(
    [[完成转服后，您将获得相应的转服价值量，可以根据需要兑换成绑金、绑银、银两。
		
	您的转服价值量为：
<color=yellow>绑定价值量：%d<color>
<color=yellow>非绑价值量：%d<color>
]],
    tbVipReborn.tbGlobalBuffer[me.szAccount].nBindValue,
    tbVipReborn.tbGlobalBuffer[me.szAccount].nNobindValue
  )
...
本月未兑的价值量为：<color=yellow>%d<color>
    ...
    nNobindValue,
    me.nMonCharge * 100,
    math.min(nMonLimit, me.nMonCharge) * tbVipReborn.MONTH_RATE,
    nMonthValue,
    nFreeValue
  )
...
function tbNpc:OnGetAward(nType, nValue)
  ...
  Dialog:AskNumber("请输入数量：", nValue, self.OnGetAwardSure, self, nType)
end
...
function tbNpc:OnGetAwardSure(nType, nValue, nSure)
  ...
  if not nSure then
    ...
    local tbOpt = {
      { "确定", self.OnGetAwardSure, self, nType, nValue, 1 },
      { "我再想想" },
    }
    Dialog:Say(szMsg, tbOpt)
    return 0
