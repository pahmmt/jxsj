...
function tbNpc:OnEnterAccount(nGateId, szGateway, szAccount)
  ...
  local szMsg = string.format("您确定转往区服<color=yellow>[%s-%s]<color>，转入通行证：<color=yellow>%s<color>", VipPlayer.VipTransfer.tbServerName[nGateId].szZoneName, VipPlayer.VipTransfer.tbServerName[nGateId][szGateway], szAccount)
...
不绑定价值量：<color=green>%s<color>
    ...
    tbValue.nBindValue,
    tbValue.nNoBindValue
  )

  szMsg = szMsg .. "\n\n" .. szValue

  local tbOpt = {
    { "确定", self.OnConfirm, self, szAccount, szGateway },
    { "我再想想" },
  }
  Dialog:Say(szMsg, tbOpt)
end
...
function tbNpc:OnConfirm(szAccount, szGateway)
  ...
  -- 记本地log
  Dbg:WriteLog("VipTransfer", "Vip转服申请", me.szAccount, me.szName, "绑定价值量：" .. tbInfo.nBindValue, "不绑定价值量：" .. tbInfo.nNoBindValue, "转向账号：" .. tbInfo.szNewAccount, "转向网关：" .. tbInfo.szNewGateway, "保留声望数量：" .. #tbInfo.tbRepute, "扩展点：" .. tbInfo.nExtPoint)
...
function tbNpc:GetAward()
  ...
  local szMsg = string.format("您的转服奖励如下：\n\n" .. "<color=yellow>\t绑定金币：\t%d\n<color>" .. "<color=yellow>\t绑定银两：\t%d\n<color>" .. "<color=yellow>\t银两：\t%d\n<color>", VipPlayer.VipTransfer.tbGlobalBuffer.tbApplyIn[me.szAccount].nBindCoin, VipPlayer.VipTransfer.tbGlobalBuffer.tbApplyIn[me.szAccount].nBindMoney, VipPlayer.VipTransfer.tbGlobalBuffer.tbApplyIn[me.szAccount].nMoney)
...
function tbNpc:OnGetAward()
  ...
  -- 记本地log
  Dbg:WriteLog("VipTransfer", "Vip转服领奖", me.szAccount, me.szName, "绑定金币：" .. nBindCoin, "绑定银两：" .. nBindMoney, "银两：" .. nMoney, "等级：" .. me.nLevel, "保留声望数量：" .. #tbRepute)
