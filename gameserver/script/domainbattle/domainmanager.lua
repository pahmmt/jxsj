...
function Domain:LoadNpcPos(nCountryId)
  ...
  for _, tbRowData in pairs(tbFile) do
    ...
    table.insert(self.tbNpcPos[nMapId][nTemplateId], { nX = tonumber(tbRowData.X), nY = tonumber(tbRowData.Y), nReactNpcId = nReactNpcId })
  end
end
...
function Domain:LoadBossNpcPos()
  ...
  if tbFile then
    for _, tbRowData in pairs(tbFile) do
      ...
      table.insert(self.tbBossPos[nMapId][nTimes], { nTemplateId = tonumber(tbRowData.nTemplateId), nX = tonumber(tbRowData.nX), nY = tonumber(tbRowData.nY) })
    end
  end
end
