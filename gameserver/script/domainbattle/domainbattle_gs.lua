...
function Domain:StartDomainBattle_GS2(nBattleNo, nDataVer)
  ...
  local szMsg = "领土争夺战进入宣战期，各帮前往领土官员处确定宣战目标"
...
function Domain:CheckAndAddExp(pPlayer)
  ...
  pPlayer.Msg(string.format("您获得领土争夺战发放的<color=yellow>第%d次<color>经验", self.nCurExpFlag))
end
...
function Domain:SetTongScore_GS2(nMapId, nId, nScore, tbSortResult)
  ...
  if not self.tbSort then
    ...
    --GC数据准确
    print("scriptdomainbattledomainbattle_gs.lua 248", "Domain:SetTongScore_GS2", "Server Dump Data Error!")
    return 0
  end
  ...
  for nCurId, nSort in pairs(tbSortResult) do
    ...
    if nCurId ~= 0 then
      if KTong.GetTong(nCurId) then
        KTongGs.TongClientExcute(nCurId, { "Domain:s2cBattleMinInfo", nil, { [nMapId] = self.tbSort[nCurId][nMapId] } })
      elseif KUnion.GetUnion(nCurId) then
        Union:UnionClientExcute(nCurId, { "Domain:s2cBattleMinInfo", nil, { [nMapId] = self.tbSort[nCurId][nMapId] } })
      end
    end
  end
  if nId ~= 0 then
    if KTong.GetTong(nId) then
      return KTongGs.TongClientExcute(nId, { "Domain:s2cBattleMinInfo", { [nMapId] = nScore }, { [nMapId] = tbSortResult[nId] } })
    elseif KUnion.GetUnion(nId) then
      return Union:UnionClientExcute(nId, { "Domain:s2cBattleMinInfo", { [nMapId] = nScore }, { [nMapId] = tbSortResult[nId] } })
    end
  end
end
...
function Domain:NotifyNpcReact_GS2(nTongId, tbDomainId)
  for i = 1, #tbDomainId do
    ...
    if szName then
      local nDataVer = self.nDataVer + 1
      self.nDataVer = nDataVer
      KTong.Msg2Tong(nTongId, string.format("本帮的领土[%s]受到当地流亡军队反扑，请速速前往守卫！", szName))
    end
  end
end
  ...
  if nOrgOwnerId ~= 0 then
    ...
    if pTong and szName then
      ...
      if pTong.GetCapital() == nDomainId then
        pTong.SetCapital(0)
        KTong.Msg2Tong(nOrgOwnerId, "帮会的主城[" .. szName .. "]被攻陷！")
      elseif self:GetDomainType(nDomainId) == "village" then
        KTong.Msg2Tong(nOrgOwnerId, "新手村[" .. szName .. "]占领权被收回！")
      else
        KTong.Msg2Tong(nOrgOwnerId, "帮会的领土[" .. szName .. "]被攻陷！")
      end

      -- add log
      Dbg:WriteLog("DomainBattle", string.format("帮会%s失去领土[%s]", pTong.GetName(), szName))
    end
    ...
    if pUnion then
      ...
      if self:GetDomainType(nDomainId) == "village" then
        Union:Msg2UnionTong(nOrgOwnerId, "新手村[" .. szName .. "]占领权被收回！")
      else
        if bIsDispense == 1 then
          local pTong = KTong.GetTong(nNewOwnerId)
          if pTong then
            Union:Msg2UnionTong(nOrgOwnerId, "联盟的领土[" .. szName .. "]分配给帮会成员[" .. pTong.GetName() .. "]了！")
          end
        else
          Union:Msg2UnionTong(nOrgOwnerId, "联盟的领土[" .. szName .. "]被攻陷！")
        end
      end

      -- add log
      Dbg:WriteLog("DomainBattle", string.format("联盟%s失去领土[%s]", pUnion.GetName(), szName))
    end
  end
  -- 增加到新归属
  if nNewOwnerId ~= 0 then
    ...
    if pTong and szName then
      ...
      if pDomain then
        ...
        if bIsDispense == 1 then
        --					KTong.Msg2Tong(nNewOwnerId, "联盟的领土["..szName.."]分配给帮会成员["..pTong.GetName().."]了！");
        else
          KTong.Msg2Tong(nNewOwnerId, "帮会占领了领土[" .. szName .. "]")
          -- add log
          Dbg:WriteLog("DomainBattle", string.format("帮会%s占领领土[%s]", pTong.GetName(), szName))
        end
      end
    end
    ...
    if pUnion and szName then
      if pUnion.AddDomain(nDomainId, nTime) == 1 then
        Union:Msg2UnionTong(nNewOwnerId, "联盟占领了领土[" .. szName .. "]")
        -- add log
        Dbg:WriteLog("DomainBattle", string.format("联盟%s占领领土[%s]", pUnion.GetName(), szName))
      end
    end
  end
...
function Domain:DeclareWar_GS2(nDomainId, nTongId)
  ...
  if szCapital and szFightMap then
    if cTong.GetBelongUnion() == 0 then
      KTong.Msg2Tong(nTongId, string.format("您所在的帮会已经向[<color=red>%s<color>]宣战，请前往[<color=red>%s<color>]参加战斗。", szCapital, szFightMap))
    else
      Union:Msg2UnionTong(cTong.GetBelongUnion(), string.format("您联盟下的帮会[%s]已经向[<color=red>%s<color>]宣战，联盟其他帮会成员同样可以对其进行征战，请前往[<color=red>%s<color>]参加战斗。", cTong.GetName(), szCapital, szFightMap))
    end
  end
...
function Domain:_HasBattleRight(nTongId, nMapId)
  ...
  -- 1.已经宣战的区域
  if
    (self:IsTongDeclareDomain(nTongId, nDomainId) == 1 or self:IsUnionDeclareDomain(nUnionId, nDomainId) == 1) -- 本帮或联盟已宣
    and (not self.tbUnionState[nUnionId] or self.tbUnionState[nUnionId] >= 0)
  then -- 无联盟或联盟可攻打
    return 1
  end
  if self:GetDomainType(nDomainId) == "village" then
  ...
  if
    (tbAdj and tbAdj[nDomainId] and tbAdj[nDomainId] ~= 0) -- 帮会邻接
    or (tbUnionAbj and tbUnionAbj[nDomainId] and tbUnionAbj[nDomainId] ~= 0)
  then -- 联盟邻接
    return 1
  end
...
function Domain:CheckAwardCondition(cPlayer, nTongId, nAwardType)
  ...
  elseif nGlobalBattleNo ~= nMyBattleNo then
    szMsg = "您没出席上一场领土争夺战，不能领取奖励。"
    Dialog:Say(szMsg)
    return 0
  elseif nCurTongId ~= nPreTongId then
    szMsg = "您已经离开原来的帮会，不能获得上场领土争夺战的奖励。"
    Dialog:Say(szMsg)
    return 0
  elseif nScoreLevel <= 0 then
    szMsg = string.format("您当前功勋<color=green>%d点<color>不足，领奖最低功勋要求<color=green>%d点<color>。", cPlayer.GetTask(self.TASK_GROUP_ID, self.SCORE_ID), nMinScore)
    Dialog:Say(szMsg)
    return 0
  end
  return 1
...
function Domain:ReciveSystemAward(bConfirme)
  ...
  -- 确认认领
  if bConfirme == 0 then
    Dialog:Say("1、您的帮会占领了<color=green>" .. pTong.GetDomainCount() .. "<color>块领土；\n" .. "2、您在本次领土争夺战中获得<color=green>" .. nScore .. "点<color>个人功勋，达到了<color=green>" .. self.DOMAINBATTLE_SCORE_RATE_TABLE[nScoreLevel][3] .. "<color>，功勋级别<color=green>" .. nScoreLevel .. "<color>级）；\n" .. "    鉴于以上两点，特奖励予您<color=green>" .. nDirectRepute .. "<color>点领土声望，<color=green>" .. nBoxAward .. "<color>个领土争夺战宝箱。", {
      { "我要领取", self.ReciveSystemAward, self, 1 },
      { "我再想想" },
    })
    return 0
  end
  ...
  Dbg:WriteLog("DomainBattle", "玩家获得声望" .. nDirectRepute, "箱子：" .. nGiven .. "/" .. nBoxAward, "剩余：" .. nRemain, me.szName, me.szAccount)
...
function Domain:SetTongAward(nLevel, bConfirme)
  ...
  if nRet ~= 1 or nCurTime < self.TONG_AWARD_START_TIME or nCurTime > self.TONG_AWARD_END_TIME then
    Dialog:Say(string.format("请在星期%s、星期天的21：30到22：00之间来设置军饷额度。", UiManager.IVER_szDomainBattleTime))
    return 0
  end
  ...
  if cTong.GetDomainAttendNo() ~= nGblBattleNo or cTong.GetDomainAttendNum() == 0 then
    Dialog:Say("您的帮会在上场领土争夺战中无人参加战，不能设置军饷。")
    return 0
  end
  ...
  if bConfirme == 0 then
    ...
    Dialog:Say("    首领可以通过设置军饷额度，消耗部分帮会建设资金来奖励在领土战中建立卓越功勋的帮会成员。不同的军饷额度下，帮会成员能够领取的奖励也不同，额度越大成员获得的奖励也会越多。\n    <color=yellow>您的帮会当前还没有设置奖励额度。<color>\n\n我要设置奖励额度：", tbOpt)
...
function Domain:SetTongAwardComfirme(nLevel)
  Dialog:Say("    您确定要选择<color=green>" .. self.DOMAINBATTLE_AWARD_TABLE[nLevel] .. "<color>这个军饷额度，并扣除帮会建设资金吗？", { "我确定了", self.SetTongAward, self, nLevel, 1 }, { "我再想想" })
end
...
function Domain:ReciveTongAward(bConfirme, nChoose)
  ...
  if bConfirme == 0 then
    ...
    table.insert(tbOpt, { "转为帮会的奖励基金", self.TongAwardToGreatBonus, self, me.dwTongId, me.nId, nPersonalAwardValue })
    table.insert(tbOpt, { "结束对话" })

    Dialog:Say("1、您的帮会此次领土战设置的军饷额度总计为<color=green>" .. nAwardAmount .. "<color> 您还有<color=green>" .. nRestValue .. "<color>额度没领；\n" .. "2、您在此次领土战的表现达到了<color=green>“" .. self.DOMAINBATTLE_SCORE_RATE_TABLE[nScoreLevel][3] .. "”<color>，功勋级别<color=green>" .. nScoreLevel .. "<color>级;\n" .. "    鉴于以上两点，您可以选择领取如下中的一种奖励：", tbOpt)
  else
    -- 领取奖励
...
function Domain:ReciveTongAwardConfirme(nChoose, szSay)
  if self:GetBattleState() == self.PRE_BATTLE_STATE or self:GetBattleState() == self.BATTLE_STATE then
    Dialog:Say("宣战期和征战期无法领取军饷")
    return 0
  end

  Dialog:Say("您选择了" .. szSay .. "是吧？", {
    { "是的", self.ReciveTongAward, self, 1, nChoose },
    { "我要重新选择" },
  })
end
...
function Domain:TongAwardToGreatBonus(nTongId, nPlayerId, nPersonalAwardValue)
  ...
  KTong.Msg2Tong(nTongId, cPlayer.szName .. "把本次价值" .. math.floor(nPersonalAwardValue) .. "的军饷，转赠为帮会奖励基金。")
  return 1
end
...
function Domain:SetJunXu_GS2(nTongId, nType, nJunXunType, nNum, nCurNo, nDataVer)
  ...
  if nType == self.JUNXU_MEDICINE then
    szMsg = "帮会设置了领土战军需，帮会所有成员可以额外领取<color=red>" .. self.JUNXU_NAME[self.JUNXU_MEDICINE][nMedicineLevel] .. " " .. nNum .. "箱<color>。 "
    pTong.SetDomainJunXunMedicineNum(nNum)
  end
  if nType == self.JUNXU_HELPFUL then
    szMsg = "帮会设置了领土战军需，帮会所有成员可以领取<color=red>" .. self.JUNXU_NAME[self.JUNXU_HELPFUL][nHelpfulLevel] .. " " .. nNum .. "个<color>。"
...
function Domain:UpdateBaZhuZhiYin_GS(szName, bFirst, nCurCount, nMaxCount, nAddCount)
  if not szName or not bFirst or not nCurCount or not nMaxCount or nCurCount < 0 or nMaxCount < 0 or not nAddCount or nAddCount < 0 then
    return 0
  end
  ...
  local szMsg = ""
  if 0 == bFirst then
    szMsg = string.format("目前为止，你已经缴纳霸主之印<color=yellow>%s<color>个，当前缴纳最多的玩家已经缴纳了<color=yellow>%s<color>个。", nCurPlayerCount, nMaxCount)
  elseif 1 == bFirst then
    szMsg = string.format("恭喜！你已经缴纳霸主之印<color=yellow>%s<color>个，是缴纳最多的玩家了，要再接再厉啊！", nCurPlayerCount)
  end
...
function Domain:GetTongAward_GS(tbTongAmount)
  ...
  local szMsg = "由于您所属帮会在缴纳霸主之印的活动中表现优越，增加%s建设资金，以资鼓励。"
  for i = 1, 10 do
