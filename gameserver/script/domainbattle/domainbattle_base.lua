...
function tbBase:StartGame()
  ...
  if tbMap then
    ...
    -- 注册禁止使用复活
    Map:RegisterMapForbidReviveType(self.nMapId, 0, 1, "领土争夺战期间，所有征战地图禁止使用九转续命丸复活！")
  end
  self:AddNpc()
...
function tbBase:OnDeath(pKiller)
  ...
  if Domain.KILLER_SCORE[nKillerLevel] and Domain.KILLER_SCORE[nKillerLevel][nSelfLevel] then
    ...
    self:AddPlayerScore(pPlayer, nScore, math.floor(nScore * nRate), "击退敌方" .. Domain.KILLER_LEVEL[nSelfLevel][2])
...
function tbBase:OnTowerNpcDeath(pNpc, pKiller)
  ...
  if self.tbTowerNpc[nNpcId] then
    szNpcName = self.tbTowerNpc[nNpcId].nNpcNo .. "号龙柱"
  end
  if pPlayer and pPlayer.dwTongId ~= 0 then
    ...
    self:AddPlayerScore(pPlayer, Domain.SCORE_PER_TOWER, Domain.SCORE_PER_TOWER, "攻占了" .. szNpcName)
  ...
  if nNextId ~= 0 then
    szMsg = string.format("<color=green>%s<color>战报：本%s成功攻占了[%s]的<color=green>%s<color>！", szMapName, szNewType, szOldOwnerName, szNpcName)
    self:Msg2TongOrUnion(nNextId, szMsg) -- 广播
  end
  if nOldId ~= 0 and nNextId ~= nOldId then
    szMsg = string.format("<color=red>%s<color>战报：本%s的<color=red>%s<color>被[%s]攻占了！", szMapName, szOldType, szNpcName, szNewOwnerName)
  ...
  Timer:Register(Domain.TOWER_REVIVE_TIME * Env.GAME_FPS, self.EndReviveTimer, self, nNextId, nX, nY, self.tbTowerNpc[nNpcId].nDeathTimes + 1, self.tbTowerNpc[nNpcId].nNpcNo)
...
function tbBase:EndReviveTimer(nNextId, nX, nY, nDeathTimes, nNpcNo)
  ...
  if (KLib.Number2UInt(nNextId) == self.nDefendTong or KLib.Number2UInt(nNextId) == self.nDefendUnion) and nNextId ~= 0 and self.bIsCaptainCity == 1 then
...
function tbBase:Join2Game(pPlayer)
  ...
  if self.nState == Domain.BATTLE_STATE then -- 征战期
    ...
    local nWriteDate = me.GetTask(2093, 84)
    local nNowDate = tonumber(GetLocalDate("%Y%m%d"))
    if Domain.bOpenWritePlayerInfo == 1 and (GetTime() - nServerStart) <= Domain.nCheckPlayerTimeLimit and nNowDate ~= nWriteDate and pPlayer.GetExtMonthPay() < Domain.nMinPay then
      GCExcute({ "Domain:WritePlayerInfo__GC", pPlayer.szAccount, pPlayer.szName, pPlayer.GetPlayerIpAddress() })
      pPlayer.SetTask(2093, 84, nNowDate)
    end
    --领土封停
    if (pPlayer.dwTongId == self.nDefendTong or (pPlayer.dwUnionId == self.nDefendUnion and self.nDefendUnion ~= 0)) and self.bIsCaptainCity == 1 then -- 主城防守BUF
      local pTong = KTong.GetTong(self.nDefendTong)
      if pTong then
    ...
    if nResTime > 0 then
      Dialog:SendBattleMsg(pPlayer, "<color=green>攻下或守护龙柱可获得积分\n按 ~ 键可以查询即时战报<color>")
      Dialog:SetBattleTimer(pPlayer, "<color=green>剩余时间：<color=white>%s<color><color>\n", nResTime)
      Dialog:ShowBattleMsg(pPlayer, 1, 0) --开启界面
    end
    Dialog:SendBlackBoardMsg(pPlayer, "你进入帮会可征战地图，自动转为征战状态") -- 黑条
  ...
  elseif self.nState == Domain.STOP_STATE then -- 休战期
    ...
    -- 禁止仇杀
    pPlayer.ForbidEnmity(1)
    local szMsg = "本领土争夺战已结束，当前地图处于休战期，禁止敌对战斗行为。"
    local pOwner = KTong.GetTong(self.nWinId) or KUnion.GetUnion(self.nWinId)
    if pOwner then
      szMsg = "本领土争夺战以" .. pOwner.GetName() .. "胜利告终，本区域进入休战期！"
    end
  ...
  for nPlayerId, tbInfo in pairs(self.tbPlayer) do
    if tbInfo.bInGame == 1 then
      ...
      if pPlayer then
        ...
        if nMapId == self.nMapId and math.abs(self.tbCenterRange.nX - nX) + math.abs(self.tbCenterRange.nY - nY) <= self.tbCenterRange.nRange then
...
function tbBase:AddBoss()
  if Domain:GetDomainType(self.nDomainId) == "village" and KGblTask.SCGetDbTaskInt(DBTASK_DOMAIN_BATTLE_NO) > 2 then
    return 0
  elseif Domain:GetDomainType(self.nDomainId) ~= "village" and self.nDefendTong ~= 0 then
    return 0
  end
  local szMsg = "流亡军队的将军出现了！速速前往挑战！"
...
function tbBase:AddPlayerScore(pPlayer, nSelfScore, nTeamScore, szMsg)
  ...
  pPlayer.Msg(string.format("你%s获得<color=yellow>%d点<color>领土战功勋！", szMsg, nSelfScore))
  ...
  if tbTeamPlayer then
    for i = 1, nCount do
      -- 同地图同帮会才共享积分
      if tbTeamPlayer[i].dwTongId == nSelfTongId and tbTeamPlayer[i].nMapId == self.nMapId and pPlayer.nId ~= tbTeamPlayer[i].nId then
        self:__AddPlayerScore(tbTeamPlayer[i], nTeamScore)
        tbTeamPlayer[i].Msg(string.format("你的队友%s，你获得共享<color=yellow>%d<color>点领土战功勋", szMsg, nTeamScore))
      end
    end
  end
end
