...
function tbLingPai:CanPosUse(x, y)
  if math.abs(x - self.CALL_BOSS_POS[1]) + math.abs(y - self.CALL_BOSS_POS[2]) <= self.USE_RANDE or math.abs(x - self.CALL_BOSS_POS_NEW[1]) + math.abs(y - self.CALL_BOSS_POS_NEW[2]) <= self.USE_RANDE then
    return 1
  end
  return 0
end
...
function tbLingPai:OnUse()
  ...
  GeneralProcess:StartProcess("召唤中...", 5 * Env.GAME_FPS, { self.EndProcess, self, it.dwId, nMapId, nX, nY }, nil, tbEvent)
  return 0
end
