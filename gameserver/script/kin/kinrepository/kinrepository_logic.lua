...
function KinRepository:CheckRepAuthority(nKinId, nExcutorId, nFigureLevel)
  ...
  -- 3级以下权限不关联玩家权限
  if
    (nFigureLevel == self.AUTHORITY_RETIRE and (nExcutorFigure <= Kin.FIGURE_REGULAR or nExcutorFigure == Kin.FIGURE_RETIRE)) -- 正式的和荣誉成员能操作
    or (nFigureLevel == self.AUTHORITY_FIGURE_REGULAR and nExcutorFigure <= Kin.FIGURE_REGULAR)
  then -- 正式以上成员能操作
    return 1, cKin, cMemberExcutor
  end
  -- 族长肯定有权限
...
function KinRepository:GetExtendMoney(nType, nLevel)
  local nAvg = KJbExchange.GetPrvAvgPrice()
