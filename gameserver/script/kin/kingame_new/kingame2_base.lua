      ...
      if tbPos then
        for _, tb in pairs(tbPos) do
          ...
          if pNpc then
            ...
            table.insert(self.tbTrapNpc[nRoomId][nDirection], pNpc.dwId)
          end
        end
      end
    end
...
function tbBase:DelTrapNpc(nRoomId, nDirection)
  ...
  for _, nNpcId in pairs(self.tbTrapNpc[nRoomId][nDirection or 1]) do
...
function tbBase:AddFireTrapNpc()
  ...
  for i = 1, #KinGame2.TRAP_FIRE_LONG_POS do
    ...
    if pNpc then
      pNpc.AddFightSkill(1475, 1, 1)
      self.tbTrapFireLong[i] = pNpc.dwId
    end
  end
  self:OnCastFire()
...
function tbBase:OnCastFire()
  for i = 1, #KinGame2.TRAP_FRIE_CAST_POS do
    local nNpcId = self.tbTrapFire[i]
    local pNpc = KNpc.GetById(nNpcId)
    local tbPos = KinGame2.TRAP_FRIE_CAST_POS[i]
    if pNpc and tbPos[1] and tbPos[2] then
      pNpc.CastSkill(KinGame2.TRAP_FIRE_SKILL_ID, 25, tbPos[1], tbPos[2])
    end
  end
  for i = 1, #KinGame2.TRAP_FIRE_LONG_CAST_POS do
    local nNpcId = self.tbTrapFireLong[i]
    local pNpc = KNpc.GetById(nNpcId)
