...
function tb2ndRoom:AddStartWaitNpc()
  ...
  local pNpc = KNpc.Add2(KinGame2.WAIT_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPos[1], tbPos[2])
  self.nWaitNpcId = pNpc.dwId
end

function tb2ndRoom:StartAddNpc()
  if self.nWaitNpcId then
    local pNpc = KNpc.GetById(self.nWaitNpcId)
    if pNpc then
      pNpc.Delete()
      self.nWaitNpcId = nil
    end
...
function tb2ndRoom:AddBoss()
  ...
  local pNpc = KNpc.Add2(KinGame2.XOYO_SHUSHENG_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPos[1], tbPos[2])
...
function tb3rdRoom:AddNpc()
  ...
  local pNpc = KNpc.Add2(KinGame2.WUYOU_SHUSHENG_TEMPLATEID, 130, -1, self.tbBase.nMapId, tbPos[1], tbPos[2])
  self.nNpcId = pNpc.dwId
  for i = 1, #KinGame2.SHUSHENG_WARING_PERCENT do
    pNpc.AddLifePObserver(KinGame2.SHUSHENG_WARING_PERCENT[i])
  end
end

function tb3rdRoom:OnNpc_Death()
  if self.nNpcId then
    local pNpc = KNpc.GetById(self.nNpcId)
    if pNpc then
      pNpc.Delete()
    end
  end
...
function tb3rdRoom:OnAddOpenSwitchNpc(nDirection)
  if nDirection == 1 then
    self.nLeftEnemyCount = self.nLeftEnemyCount - 1
    if self.nLeftEnemyCount <= KinGame2.MAX_NUM_ENEMY_ADD_SHOUHUZHE and not self.nLeftId then
      local tbLeftPos = KinGame2.LEFT_SWITCH_ENEMY_POS
      local pNpc = KNpc.Add2(KinGame2.LEFT_SWITCH_ENEMY_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbLeftPos[1], tbLeftPos[2])
      self.nLeftId = pNpc.dwId
      Npc:RegPNpcOnDeath(pNpc, self.OnOpenSwitch, self, 1)
    end
  elseif nDirection == 2 then
    self.nRightEnemyCount = self.nRightEnemyCount - 1
    if self.nRightEnemyCount <= KinGame2.MAX_NUM_ENEMY_ADD_SHOUHUZHE and not self.nRightId then
      local tbRightPos = KinGame2.RIGHT_SWITCH_ENEMY_POS
      local pNpc = KNpc.Add2(KinGame2.RIGHT_SWITCH_ENEMY_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbRightPos[1], tbRightPos[2])
      self.nRightId = pNpc.dwId
      Npc:RegPNpcOnDeath(pNpc, self.OnOpenSwitch, self, 2)
    end
  end
end
...
function tb3rdRoom:OnArrive(nNpcId)
  ...
  if self.nNpcId then
    local pNpc2 = KNpc.GetById(self.nNpcId)
    if pNpc2 then
      pNpc2.ReduceLife(KinGame2.SHUSHENG_REDUCE_BLOOD[pNpc.nTemplateId])
      if pNpc2.nCurLife <= 0 then
        self:OnNpc_Death()
      end
    end
  end
...
function tb3rdRoom:EndRoomTime()
  if not self.nNpcId or not KNpc.GetById(self.nNpcId) or KNpc.GetById(self.nNpcId).nCurLife <= 0 then
    self.nIsSucess = 0
  else
    self.nIsSucess = 1
  end
...
function tb4thRoom:AddBoss()
  ...
  local pNpc = KNpc.Add2(KinGame2.DUGURUOLAN_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbDuguPos[1], tbDuguPos[2])
  self.nDuguId = pNpc.dwId
  pNpc.AddLifePObserver(KinGame2.CHANGE_LIFE_PERCENT)
  pNpc.AddLifePObserver(KinGame2.WARNING_LIFE_PERCENT)

  pNpc = KNpc.Add2(KinGame2.DUOGUOYICHEN_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbDongguoPos[1], tbDongguoPos[2])
  self.nDongguoId = pNpc.dwId
  pNpc.AddLifePObserver(KinGame2.CHANGE_LIFE_PERCENT)
  pNpc.AddLifePObserver(KinGame2.WARNING_LIFE_PERCENT)
end
...
function tb4thRoom:ChangeBoss()
  ...
  local pNpc = KNpc.GetById(self.nDuguId)
  local pNpc2 = KNpc.GetById(self.nDongguoId)
  self.nDuguLifeReduce = pNpc.nMaxLife - pNpc.nCurLife
  self.nDongguoLifeReduce = pNpc2.nMaxLife - pNpc2.nCurLife
  pNpc.Delete()
  pNpc2.Delete()
  local tbDuguPos = KinGame2.DUGURUOLAN_NPC_POS
  local tbDongguoPos = KinGame2.DUOGUOYICHEN_NPC_POS
  pNpc = KNpc.Add2(KinGame2.DUGURUOLAN_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbDongguoPos[1], tbDongguoPos[2])
  pNpc2 = KNpc.Add2(KinGame2.DUOGUOYICHEN_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbDuguPos[1], tbDuguPos[2])
  self.nDuguId = pNpc.dwId
  self.nDongguoId = pNpc2.dwId
  pNpc.ReduceLife(self.nDuguLifeReduce)
  pNpc2.ReduceLife(self.nDongguoLifeReduce)
  pNpc.AddLifePObserver(KinGame2.WARNING_LIFE_PERCENT)
  pNpc2.AddLifePObserver(KinGame2.WARNING_LIFE_PERCENT)
  Npc:RegPNpcOnDeath(pNpc, self.OnDeath_Boss_Dugu, self)
  Npc:RegPNpcOnDeath(pNpc2, self.OnDeath_Boss_Dongguo, self)
end
...
function tb5thRoom:AddStartWaitNpc()
  ...
  local pNpc1 = KNpc.Add2(KinGame2.WAIT_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPosLeft[1], tbPosLeft[2])
  self.nWaitNpcLeftId = pNpc1.dwId
  local pNpc2 = KNpc.Add2(KinGame2.WAIT_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPosRight[1], tbPosRight[2])
  self.nWaitNpcRightId = pNpc2.dwId
end

function tb5thRoom:StartAddNpc()
  if self.nWaitNpcLeftId then
    local pNpc = KNpc.GetById(self.nWaitNpcLeftId)
    if pNpc then
      pNpc.Delete()
      self.nWaitNpcLeftId = nil
    end
  end
  if self.nWaitNpcRightId then
    local pNpc = KNpc.GetById(self.nWaitNpcRightId)
    if pNpc then
      pNpc.Delete()
      self.nWaitNpcRightId = nil
    end
  end
  self.nStartTimer = 0
  self:AddBoss()
...
function tb5thRoom:AddBoss()
  ...
  local pNpc = KNpc.Add2(KinGame2.BAIMAYI_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbBmyPos[1], tbBmyPos[2])
  self.nBaimayiId = pNpc.dwId
  local tbJunjiePos = KinGame2.BAIMAJUNJIE_NPC_POS
  local pNpc2 = KNpc.Add2(KinGame2.BAIMAJUNJIE_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbJunjiePos[1], tbJunjiePos[2])
  self.nBaimajunjieId = pNpc2.dwId
  for i = 1, #KinGame2.ADD_ARMY_PERCENT do
    Npc:RegPNpcLifePercentReduce(pNpc2, KinGame2.ADD_ARMY_PERCENT[i], self.AddArmy, self)
  end
  Npc:RegPNpcOnDeath(pNpc2, self.OnDeath_Boss_Bmjj, self)
  Npc:RegPNpcOnDeath(pNpc, self.OnDeath_Boss_Bmy, self)
end
...
function tb5thRoom:OnCastSkill()
  if self.nShadowCount and self.nShadowCount > 0 then
    if self.nBaimajunjieId then
      local pNpc = KNpc.GetById(self.nBaimajunjieId)
      if pNpc then
        local _, x, y = pNpc.GetWorldPos()
        pNpc.CastSkill(KinGame2.AOE_SKILL_ID_5TH, 10, x * 32, y * 32, 1)
      end
    end
  end
  return 0
...
function tb6thRoom:AddBoss()
  ...
  local pNpc = KNpc.Add2(KinGame2.SIMAYANNAN_NPC_TEMPLATEID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPos[1], tbPos[2])
  self.nBossId = pNpc.dwId
  Npc:RegPNpcLifePercentReduce(pNpc, KinGame2.START_NEW_STEP_PERCENT, self.BossNewStep, self)
  Npc:RegPNpcOnDeath(pNpc, self.OnDeath_Boss, self)
end
...
function tb6thRoom:CastAoe()
  ...
  local tbNpc, nCount = KNpc.GetAroundNpcListByNpc(self.nBossId, 50, KinGame2.FREEZE_NPC_TEMPLATEID)
  if self.nBossId then
    local pNpc = KNpc.GetById(self.nBossId)
    if pNpc and nCount > 0 then
      local _, x, y = pNpc.GetWorldPos()
      pNpc.CastSkill(KinGame2.AOE_SKILL_ID_6TH, 1, x * 32, y * 32, 1)
    end
  end
end
...
function tb6thRoom:AddAttack()
  ...
  if self.nBossId then
    local pNpc = KNpc.GetById(self.nBossId)
    if pNpc then
      pNpc.AddSkillState(KinGame2.ADD_ATTACK_SKILL_ID, 1, 2, KinGame2.ADD_ATTCK_TIME * Env.GAME_FPS, 1, 0, 1)
    end
  end
...
function tb7thRoom:AddBoss()
  local tbPos = KinGame2.XIMENFEIXUE_POS
  local pNpc = KNpc.Add2(KinGame2.XIMENFEIXUE_TEMPLATE_ID, self.tbBase.nMonsterAvgLevel, -1, self.tbBase.nMapId, tbPos[1], tbPos[2])
  self.nBossId = pNpc.dwId
  Npc:RegPNpcLifePercentReduce(pNpc, KinGame2.ADD_SHADOW_PERCENT, self.AddShadow, self)
end

function tb7thRoom:AddShadow()
  if not self.nBossId then
    return 0
  end
  local pNpc = KNpc.GetById(self.nBossId)
  if not pNpc then
  ...
  local nReduceLife = pNpc.nMaxLife - pNpc.nCurLife --新加的boss要减去血量
  pNpc.Delete()
  self.nBossId = nil
  local tbNpcPos = {}
