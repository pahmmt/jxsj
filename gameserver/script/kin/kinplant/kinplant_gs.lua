...
function KinPlant:CanPlantTree(pPlayer, pItem)
  ...
  if nTimeFlag == 0 then
    return 0, "不在种植期，每天的09：00 - 23：00为种植期。"
  end
  --pos
  ...
  if nTreeCountToday >= self.nMaxPlantCount then
    return 0, string.format("我已经种下了%s植物，不能再种了，还是给他人留点地方吧。", self.nMaxPlantCount)
  end

  local nFlagEx = Player:CheckTask(self.TASKGID, self.TASK_DATE, "%Y%m%d", self.TASK_COUNT, self.nMaxPlantCount)
  if nFlagEx == 0 then
    return 0, string.format("今天你已经种下了%s植物，不能再种了，还是给他人留点地方吧。", self.nMaxPlantCount)
  end
...
function KinPlant:GatherSeed(dwNpcId, nPlayerId, nFlag)
  ...
  if not nFlag then
    ...
    GeneralProcess:StartProcess("摘取果实...", 3 * Env.GAME_FPS, { KinPlant.GatherSeed, KinPlant, dwNpcId, nPlayerId, 1 }, nil, tbEvent)
    return
  end
...
function KinPlant:GetAward(dwNpcId, nPlayerId, nType)
  ...
  if nType == 1 then
    ...
    if pPlayer.CountFreeBagCell() < nNeedBag + nFreeCount then
      pPlayer.Msg(string.format("背包空间不足%s格，请清理下再来吧。", nNeedBag + nFreeCount))
      return 0
    end
  ...
  if pNpcEx then
    ...
    if bHasMax then
      ...
      if nHealthGroup >= 5 and cKin and tbTitle and szTreeName then
        local szKinMsg = string.format("[%s]家族成员[%s]的%s%s成熟了，勤劳致富靠双手，快去家族领地看看吧！", cKin.GetName(), szPlayerName, tbTitle[1], szTreeName)
        KKin.Msg2Kin(dwKinId, szKinMsg, 0)
        if nHealthGroup == #self.tbHealthTitile then
          local szWorldMsg = string.format("[%s]家族成员[%s]的丰硕作物成熟了，勤劳致富靠双手，快去家族领地看看吧！", cKin.GetName(), szPlayerName)
...
function KinPlant:TreeDredging(nNpcId, nPlayerId, nFlag)
  ...
  if not nFlag then
    ...
    GeneralProcess:StartProcess("铲除中...", 3 * Env.GAME_FPS, { self.TreeDredging, self, nNpcId, nPlayerId, 1 }, nil, tbEvent)
    return
  end
  local pNpc = KNpc.GetById(nNpcId)
  if pNpc then
    ...
    if tbTemp then
      if tbTemp.szPlayerName == pPlayer.szName then
        ...
        pPlayer.Msg("你铲除掉了一株植物。")
      else
        KKin.Msg2Kin(pPlayer.dwKinId, string.format("%s的作物被[%s]铲除掉了。", pNpc.szName, pPlayer.szName), 0)
      end
    end
  end
...
function KinPlant:CheckDredging(nNpcId, nPlayerId)
  ...
  if nKinFigure <= 0 or nKinFigure > nDredging then
    return 0, "你没有权限挖掉这棵植物。"
  end
  if cKin.GetCaptainLockState() == 1 then
    return 0, "你已被罢免，族长权限被冻结！"
  end
  return 1, nIndex
end
...
function KinPlant:CanGatherSeed(pPlant, pPlayer)
  ...
  if szPlayerName == pPlayer.szName then
    ...
    local nNeedBag = KItem.GetNeedFreeBag(tbFruit[1], tbFruit[2], tbFruit[3], tbFruit[4], nil, nNum)
    if pPlayer.CountFreeBagCell() < nNeedBag + nFreeCount then
      return 0, string.format("背包空间不足%s格，请清理下再来吧。", nNeedBag + nFreeCount)
    end
    return 1
  else
...
function KinPlant:MergeDialog(tbOpt, nNpcId)
  ...
  for i = 1, 3 do
    if self.tbPlantWeekTask[i] and self.tbPlantWeekTask[i][tbTask[i]] then
      nFlag = 0
      table.insert(tbOpt, 1, { self.tbTypeName[i] or "[<color=yellow>家园订单<color>]", self.OnDialog, self, tbTask[i], i })
    end
  end
  if nFlag == 0 then
    return 1
...
function KinPlant:OnDialog(nNum, nType)
  ...
  local tbOpt = { { "领取订单种子", self.GetSeed, self, nNum, nType }, { "上交果实", self.OnDialogEx, self, nNum, nType }, { "我再想想" } }
...
function KinPlant:GetSeed(nNum, nType)
  ...
  if me.CountFreeBagCell() < nNeedBag then
    me.Msg(string.format("背包空间不足%s格，请清理下再来吧。", nNeedBag))
    return 0
  end
  local nTaskInfo = KGblTask.SCGetDbTaskInt(DBTASK_KINPLANT_TASK)
  local nWeakly = math.floor(nTaskInfo / 1000000)
...
function KinPlant:GetSelfPlant()
  local tbPoint = {}
  for i = self.TASK_PLANT_NUM1, self.TASK_PLANT_NUM3 do
    local nPlantPointFlag = me.GetTask(self.TASKGID, i)
    if nPlantPointFlag > 0 and self.tbPlantInfo[me.dwKinId] and self.tbPlantInfo[me.dwKinId][nPlantPointFlag] and self.tbPlantInfo[me.dwKinId][nPlantPointFlag][2] > 0 then
      table.insert(tbPoint, self.tbNpcPoint[nPlantPointFlag])
    end
  end
  return tbPoint
end
...
PlayerEvent:RegisterGlobal("OnLoginOnly", KinPlant.RefreshKinPlant, KinPlant)
...
PlayerEvent:RegisterGlobal("OnLoginOnly", KinPlant.Login_ProcessSubPlayerPlantTask, KinPlant)
