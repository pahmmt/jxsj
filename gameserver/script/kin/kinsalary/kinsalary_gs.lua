...
function Kinsalary:DoAddSalary_GS(nKinId, nMemberId, nType, nSalary)
  ...
  if nLevel > nCurSalaryLevel then
    ...
    KKin.Msg2Kin(nKinId, string.format("本周家族工资已达到%s级，本家族成员领取工资时可获得%s倍加成银锭。", nLevel, self.MEMBER_AWARD[nLevel], 1))
  end
  ...
  -- 频道公告
  self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你在%s中获得了%s点工资。", tbInfo.szName, nSalary))
end
...
function Kinsalary:GetSalary_GS(pPlayer)
  ...
  if pPlayer.IsAccountLock() ~= 0 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "你的账号处于锁定状态，无法领取工资。")
    return 0
  end

  -- 等级限制
  if pPlayer.nLevel < self.MIN_LEVEL then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, string.format("对不起，你的等级不足%s级，无法领取工资。", self.MIN_LEVEL))
    return 0
  end
  ...
  if nDay == 1 and nTime < self.TIME_GETSALARY then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "工资结算中，请晚上19点以后再来领取。")
    return 0
  end
  ...
  if nSalary <= 0 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你上周没有获得任何工资，无法领取。")
    return 0
  end
  ...
  if nCurSession ~= nSysSession then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "系统正在对家族工资进行维护，请稍后再来领取工资。")
    return 0
  end
  ...
  if nGetAward == 1 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你本周已经领取过工资了。")
    return 0
  end
  ...
  local nAssistantRate = self.ASSISTANT_AWARD[nKinSalaryLevel] or 1

  -- 族长只能领一次
  local nCaptain = Kin:CheckSelfRight(nKinId, nMemberId, 1)
  local nAssistant = Kin:CheckSelfRight(nKinId, nMemberId, 2)

  if nCaptain == 1 then
    if pKin.GetSalaryCaptainGet() ~= 1 then
      nSalary = math.floor(nSalary * nCaptainRate)
    else
      nSalary = math.floor(nSalary * nMemberRate)
    end
  elseif nAssistant == 1 then
    if pKin.GetSalaryAssistantGet() ~= 1 then
      nSalary = math.floor(nSalary * nAssistantRate)
    else
      nSalary = math.floor(nSalary * nMemberRate)
    end
  else
  ...
  if nYinding + nSalary > self.MAX_NUMBER then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "您的家族银锭将超出上限，请在银锭商店中消费后再来领取。")
    return 0
  end
  ...
  self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你获得了%s家族银锭", nSalary))
  ...
  -- 频道公告
  pPlayer.SendMsgToFriend(string.format("您的好友[%s]领取了<color=green>%s家族银锭<color>和<color=green>%s个家族元宝<color>", pPlayer.szName, nSalary, nCount))
  Player:SendMsgToKinOrTong(pPlayer, string.format("领取了%s家族银锭和%s个家族元宝", nSalary, nCount))

  -- add jbreturn
  if jbreturn:IsPermitIp(pPlayer) == 1 and jbreturn:GetMonLimit(pPlayer) > 0 then
    pPlayer.SetTask(self.TASK_GID, self.TASK_YINDING, pPlayer.GetTask(self.TASK_GID, self.TASK_YINDING) + nSalary * 5)
  end
...
function Kinsalary:DoGetSalary_GS(nKinId, nMemberId)
  ...
  elseif Kin:CheckSelfRight(nKinId, nMemberId, 2) == 1 then
    pKin.SetSalaryAssistantGet(1)
  end
...
function Kinsalary:GetKinSalaryInfo(pPlayer)
  ...
  local tbInfo = {
    ...
    nSalaryAssistantGet = pKin.GetSalaryAssistantGet(),
  }
...
function c2s:ApplyKinOpenSalary()
  ...
  local nAssistantRate = Kinsalary.ASSISTANT_AWARD[nKinSalaryLevel] or 1
...
function c2s:ApplyKinGetSalary()
  ...
  -- 提示充值和激活
    ...
    local szMsg = "    每月<color=yellow>充值50元<color>以上的玩家可激活<color=yellow>家族工资福利<color>，此项福利在领取家族工资时能让您额外获得大量不绑定的家族元宝，购买宝箱开启到<color=yellow>绑定玄晶及绑定银两<color>"
    local tbOpt = {
      { "<color=yellow>立即充值<color>", c2s.ApplyOpenOnlinePay, c2s },
      { "<color=yellow>激活特权<color>", Player.ActionKinWage, Player },
      { "直接领取", Kinsalary.GetSalary_GS, Kinsalary, me },
      { "我再想想" },
    }
