  ...
  if pPlayer then
    ...
    if self.tbMapList[nTempMapId] then
      for nMapId, varValue in pairs(self.tbMapList[nTempMapId]) do
        if type(varValue) == "table" and self.tbMapList[nTempMapId][nMapId] and self.tbMapList[nTempMapId][nMapId].OnUsed ~= 1 and self.tbMapList[nTempMapId][nMapId].IsServer then
