...
function tbNpc:PlayerClick(nNpcId, szPlayerName, nMapId)
  ...
  GeneralProcess:StartProcess("游戏中...", 5 * Env.GAME_FPS, { self.RecordClickTime, self, nNpcId, szPlayerName, nMapId }, nil, tbEvent)
...
function tbNpc:GetAward(nMapId)
  ...
  for _, pPlayer in pairs(tbCouplePlayer) do
    ...
    if not pItem then
      ...
      pItem = KItem.AddItemInPos(nMapId, nPosX, nPosY, tbAwardItemGDPL[1], tbAwardItemGDPL[2], tbAwardItemGDPL[3], tbAwardItemGDPL[4], 0, 0, 0, nil, nil, 0, 0, pPlayer)
