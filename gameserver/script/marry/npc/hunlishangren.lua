...
function tbNpc:GetOpt()
  ...
  for _, tbItemInfo in pairs(tbItemList) do
    local szOpt = string.format("<color=yellow>%s<color> 售价：<color=yellow>%s<color>情花", tbItemInfo.szName, tbItemInfo.nCost)
...
function tbNpc:OnWishBuy(nIndex, nCount)
  ...
  if nNeedCount > nQingHuaCount then
    local szErrMsg = string.format("购买<color=yellow>%s<color>个<color=yellow>%s<color>，需要<color=yellow>%s<color>朵情花。你现在的数量不足，请确带够来再来吧。", nCount, tbItemInfo.szName, nNeedCount)
    Dialog:Say("")
    return
  end

  local szMsg = string.format("你现在要购买<color=yellow>%s<color>个<color=yellow>%s<color>，需要<color=yellow>%s<color>个朵情花。确定吗？", nCount, tbItemInfo.szName, nNeedCount)
  local tbOpt = {
