...
function tbNpc:GetChatMsg()
  ...
  if szMapClass == "city" then
    szMsg = string.format("今天我家主人<color=yellow>%s<color>和<color=yellow>%s<color>举办<color=yellow>皇家典礼<color>，大家赶快去找江津村老月参加。我在此为大家送上祝福！", szMaleName, szFemaleName)
  elseif szMapClass == "village" then
    szMsg = string.format("今天我家主人<color=yellow>%s<color>和<color=yellow>%s<color>举办<color=yellow>王侯典礼<color>，大家赶快去找江津村老月参加。我在此为大家送上祝福！", szMaleName, szFemaleName)
  else
    return
  end
...
function tbNpc:GetBuffDlg()
  ...
  GeneralProcess:StartProcess("领取祝福...", 5 * Env.GAME_FPS, { self.GetBuff, self, me.szName, tbSkillInfo }, nil, tbEvent)
end
