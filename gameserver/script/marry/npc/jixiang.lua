...
function tbNpc:StartWedding()
  ...
  local szMsg = string.format("良辰已到，<color=yellow>%s<color>与<color=yellow>%s<color>的典礼正式开始。", tbCoupleName[1], tbCoupleName[2])
  him.SendChat(szMsg)

  self:SetCurState(self.STATE_VISITORNPC)

  Dbg:WriteLog("Marry", "结婚系统", string.format("%s跟%s的典礼正式开始", tbCoupleName[1], tbCoupleName[2]), string.format("开启仪式时间：%s", os.date("%Y年%m月%d日%H分", GetTime())))
end
...
function tbNpc:IsCoupleOnStage(nMapId)
  ...
  if 1 == Marry:CheckPlayerOnStage(nMapId, tbCoupleName[1]) and 1 == Marry:CheckPlayerOnStage(nMapId, tbCoupleName[2]) then
    bIsCoupleOnStage = 1
  end
...
function tbNpc:Marry(tbCoupleName, nMapId)
  ...
  local szBroadcastMsg = string.format("【<color=turquoise>%s<color>】与【<color=turquoise>%s<color>】已经结为侠侣，大家祝福他们吧。", tbCoupleName[1], tbCoupleName[2])
...
function tbNpc:ChouJiang(nMapId, nWeddingLevel)
  ...
  if 0 ~= nPlayerCount then
    ...
    if pPlayer then
      ...
      KPlayer.SendMail(pPlayer.szName, "典礼抽奖礼品", string.format("恭喜！<color=yellow>%s<color>与<color=yellow>%s<color>的<color=green>%s<color>举行了抽奖活动，您有幸中了大奖！请注意查收附件。", tbCoupleName[1], tbCoupleName[2], Marry.WEDDING_LEVEL_NAME[nWeddingLevel]), 0, 0, 1, unpack(tbAwardInfo.tbGDPL))
      local szMsg = string.format("<color=yellow>%s<color>在抽奖活动中获得了一个%s，大家恭喜他吧。获奖者请在邮件中查收奖品。", pPlayer.szName, tbAwardInfo.szAwardName)
