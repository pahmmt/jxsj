...
function tbNpc:OnDialog()
  ...
  if tbCoupleName and #tbCoupleName == 2 and (me.szName ~= tbCoupleName[1] and me.szName ~= tbCoupleName[2]) then
...
function tbNpc:OnCostMoney(nOpt, bSure, nNum)
  ...
  if 0 == bSure then
    ...
    if "" ~= szInfo then
      szMsg = szMsg .. string.format("【您的祝福信息将发布<color=yellow>%s<color>，会额外收取您<color=yellow>%s<color>朵情花】。", szInfo, nCost)
    end
    Dialog:Say(szMsg, {
...
function tbNpc:SendNotify(nOpt, nNum)
  local tbCoupleName = Marry:GetWeddingOwnerName(me.nMapId)
  local szNotify = string.format("<color=green>[%s]<color>为[%s]和[%s]的典礼祝福，送上礼金<color=orange>%s<color>两。", me.szName, tbCoupleName[1], tbCoupleName[2], nNum)
  local szTongMsg = string.format("为[%s]和[%s]的典礼祝福，送上礼金<color=orange>%s<color>两。", tbCoupleName[1], tbCoupleName[2], nNum)
...
function tbNpc:CanSendMoney(nSendMoney)
  ...
  elseif nMySendSum + nSendMoney > self.MAX20E then
    szErrMsg = string.format("我只能接受每个人<color=yellow>%s<color>礼金，您已经赠送了<color=yellow>%s<color>礼金，不能再送这么多了，谢谢。", self.MAX20E, nMySendSum)
    return 0, szErrMsg
  end

  local nAllPlayerSendSum = self:GetTotalGift()
  if nAllPlayerSendSum >= 2 * self.MAX20E then
    szErrMsg = string.format("我最多只能代替二位侠侣接受<color=yellow>%s<color>礼金，现在新人的亲朋好友已经赠送这么多了，您的心意我已经知道了，不用再送了", self.MAX20E * 2)
    return 0, szErrMsg
  elseif nAllPlayerSendSum + nSendMoney > self.MAX20E * 2 then
    szErrMsg = string.format("我最多只能代替二位侠侣接受<color=yellow>%s<color>礼金，现在他们的亲朋好友已经赠送了<color=yellow>%s<color>礼金了，不能再送这么多了。", self.MAX20E * 2, nAllPlayerSendSum)
    return 0, szErrMsg
  end
