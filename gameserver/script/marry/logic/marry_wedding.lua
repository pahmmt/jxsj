...
function Marry:AddWeddingResult_GS(nRet, tbInfo, dwItemId, nTime, nDate)
  ...
  if nRet ~= 1 then
    ...
    Dialog:Say(string.format("你们的典礼日期成功设置为：<color=yellow>%s<color>", szDate))
    ...
    local szBroadcastMsg = string.format("【<color=yellow>%s<color>】与【<color=yellow>%s<color>】的%s订于<color=yellow>%s<color>在%s举行，欢迎届时参加。", me.szName, pFemale.szName, Marry.WEDDING_LEVEL_NAME[pFinalItem.nLevel], szDate, Marry.MAP_LEVEL_NAME[nMapLevel])

    me.SendMsgToFriend(szBroadcastMsg)
    me.SendMsgToKinOrTong(0, szBroadcastMsg)
    me.Msg(string.format("你们的典礼日期成功设置为：<color=yellow>%s<color>", szDate))

    local tbFriendList = me.GetRelationList(Player.emKPLAYERRELATION_TYPE_BIDFRIEND)
    for _, szName in pairs(tbFriendList or {}) do
      KPlayer.SendMail(szName, "侠侣典礼邀请", szBroadcastMsg)
    end

    Dbg:WriteLog("Marry", "结婚系统", me.szName, me.szAccount, pFemale.szName, pFemale.szAccount, string.format("申请日期：%s", os.date("%Y年%m月%d日", GetTime())), string.format("预定日期：%s", szDate), string.format("婚礼档次：%s", pFinalItem.nLevel), string.format("地图类型：%s", Marry.MAP_LEVEL_NAME[nMapLevel]))
    -- 恢复本来me
    Setting:RestoreGlobalObj()
...
function Marry:SelectServer(nFrom)
  ...
  for nIndex, nDynMapId in next, self.tbMissionMap, nFrom do
    if nCount <= 0 then
      tbOpt[#tbOpt + 1] = { "下一页", self.SelectServer, self, nLast }
      break
    end
...
function Marry:AttendWedding(nFrom)
  ...
  for nIndex, nDynMapId in next, self.tbMissionMap, nFrom do
    if nCount <= 0 then
      tbOpt[#tbOpt + 1] = { "下一页", self.AttendWedding, self, nLast }
      break
    end
...
function Marry:DoAttendWedding(nDynMapId, szMaleName, szFemaleName, nMapLevel, nSure)
  ...
  if self:GetCurPlayerCount(nDynMapId) >= self:GetMaxPlayerCount(nDynMapId) then
    Dialog:Say("对不起，参加典礼的人数已达到上限，无法进入了。")
    return 0
  end
  ...
  --		local tbOpt =
  --		{
  --			{"确定", self.DoAttendWedding, self, nDynMapId, szMaleName, szFemaleName, nMapLevel, 1},
  --			{"我再想想"},
  --		}
  ...
  if Marry:GetWeddingPlayerLevel(nDynMapId, me.szName) <= 0 then
    ...
    -- 邀请函
      ...
      if nFindItem ~= 1 then
        Dialog:Say("对不起，您没有得到邀请，无法参加他们的典礼。\n您可以向二位侠侣或他们的结义兄弟、闺中密友索要<color=yellow>邀请函<color>。当然，如果您是二位侠侣的<color=yellow>好友<color>或<color=yellow>同一家族成员<color>，可以直接参加。")
        return 0
      end
      Marry:SetWeddingPlayerLevel(nDynMapId, me.szName, 1)
    end
...
function Marry:QueryWedding(nType, varQuery, nFrom)
  ...
  if nType == 1 then
    for nWeddingLevel, tbMap in pairs(self.tbGlobalBuffer) do
      for nDate, tbRow in pairs(tbMap) do
        ...
        local szMsg = string.format("<color=yellow>%s<color> 预定的典礼信息\n\n", varQuery)
        szMsg = szMsg .. string.format("典礼日期：<color=green>%s年%s月%s日<color>\n", nYear, nMonth, nDay)
        szMsg = szMsg .. string.format("典礼等级：<color=green>%s<color>\n", self.WEDDING_LEVEL_NAME[nWeddingLevel])

        if nWeddingLevel <= 2 then
          for nIndex, tbInfo in pairs(tbRow) do
            if varQuery == tbInfo[1] or varQuery == tbInfo[2] then
              szMsg = szMsg .. string.format("典礼场地：<color=green>%s<color>\n", self.MAP_LEVEL_NAME[tbInfo[3]])
              Dialog:Say(szMsg)
              return 0
            end
          end
  ...
  elseif nType == 2 then
    ...
    local tbOpt = { { "<color=green>返回上层<color>", tbNpc.QuerySpedayWedingInfo, tbNpc }, { "我知道了" } }
    local szTitle = string.format("<color=yellow>%s年%s月%s日<color>预定典礼的有：\n\n", nYear, nMonth, nDay)
      ...
      if tbDate then
        ...
        if nWeddingLevel <= 2 then
          ...
          for i = nLast, #tbDate do
            szMsg = szMsg .. string.format("%s%s<color>", szColor, Lib:StrFillR(tbDate[i][1], 16)) .. " - " .. string.format("%s%s<color>\n", szColor, Lib:StrFillL(tbDate[i][2], 16))
            nCount = nCount - 1
            nLast = nLast + 1
            if nCount <= 0 then
              table.insert(tbOpt, 1, { "<color=green>下一页<color>", self.QueryWedding, self, nType, varQuery, nLast })
              break
            end
          end
        elseif not nFrom then
          szMsg = string.format("%s%s<color>", szColor, Lib:StrFillR(tbDate[1], 16)) .. " - " .. string.format("%s%s<color>\n", szColor, Lib:StrFillL(tbDate[2], 16)) .. szMsg
        end
      end
    end
...
function Marry:SetTitle(pMan, pWoman)
  ...
  -- 娘子相公称号
  pMan.AddSpeTitle(string.format("%s的知己", pWoman.szName), GetTime() + 5000 * 60 * 60 * 24, "gold")
  pWoman.AddSpeTitle(string.format("%s的知己", pMan.szName), GetTime() + 5000 * 60 * 60 * 24, "gold")
end
...
function Marry:ClearSpecNpc_GS()
  local tbNpcName = { "皇家管家", "皇家庆典", "皇家新喜", "王侯管家", "王侯庆典", "王侯新喜" }
...
function Marry:GetCurWeekSuperInfo(nDay)
  ...
  for i = 1, 7 do
    ...
    if self.tbGlobalBuffer[4][nTmpDate] then
      szDate = os.date("%Y年%m月%d日", nTime + nTmpDay * 24 * 3600 - tbTime.wday * 24 * 3600)
