...
function tbMiniGame:CheckStep(nStep)
  ...
  if math.mod(nCurStep, 1) ~= 0 then
    ...
    if szCurGame then
      Dialog:Say(string.format("小游戏<color=yellow>%s<color>正在进行当中，等当前游戏结束之后再玩下一个游戏吧。", szCurGame))
    end
    return 0
  end
...
function tbMiniGame:Change2Box(tbBoxInfo)
  ...
  if nOwnCount < tbBoxInfo.nCount then
    local szErrMsg = string.format("你的<color=yellow>%s<color>数量不足，兑换%s至少需要<color=yellow>%s<color>个钱袋。", szName, tbBoxInfo.szName, tbBoxInfo.nCount)
    Dialog:Say(szErrMsg)
    return 0
  end
