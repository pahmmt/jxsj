...
function tbMission:StartGame(szMaleName, szFemaleName, nDynMapId, nMapLevel, nWeddingLevel, nStartTime, nEndTime)
  ...
  Dbg:WriteLog("Marry", "结婚系统", string.format("%s跟%s的典礼开启", szMaleName, szFemaleName), string.format("开启日期：%s", os.date("%Y年%m月%d日%H分", GetTime())), string.format("典礼档次：%s", nWeddingLevel), string.format("场地类型：%s", Marry.MAP_LEVEL_NAME[nMapLevel]))
end
...
function tbMission:OpenRightUI(pPlayer, nRemainFrame)
  ...
  Dialog:SetBattleTimer(pPlayer, "<color=green>典礼剩余时间：%s<color>\n", nRemainFrame)
...
function tbMission:UpdateAllRightUI()
  ...
  local szMsg = string.format("<color=yellow>当前场地人数：%s/%s<color>\n%s\n\n%s", self:GetCurPlayerCount(), self:GetMaxPlayerCount(), Marry.PERFORM_STEP[self.nCurStep], self.szRightMsg)
