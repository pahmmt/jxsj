...
<color=yellow>    如果一生中只能实现一个愿望，那么我愿给我最珍惜人以最真的情意！典礼的礼乐已经奏响，在这烟火灿烂的每一个瞬间，我们的整个世界都将变得绚丽、新奇！<color>
      ...
      szDate,
      Lib:StrFillR(szMaleName, 30),
      Lib:StrFillL(szFemaleName, 30),
      Lib:StrFillR(szMaleKin, 30),
      Lib:StrFillL(szFemaleKin, 30)
    )
    ...
    if tbRow then
      if nWeddingLevel <= 2 then
        for nIndex, tbInfo in pairs(tbRow) do
          tbMsg[nWeddingLevel] = tbMsg[nWeddingLevel] .. string.format("<color=yellow>%s<color>", Lib:StrFillR(tbInfo[1], 16)) .. " 和 " .. string.format("<color=yellow>%s<color>", Lib:StrFillL(tbInfo[2], 16))
          if math.mod(#tbMsg[nWeddingLevel], 2) == 0 then
            tbMsg[nWeddingLevel] = tbMsg[nWeddingLevel] .. "\n"
          end
        end
      else
        tbMsg[nWeddingLevel] = string.format("<color=yellow>%s<color>", Lib:StrFillR(tbRow[1], 16)) .. " 和 " .. string.format("<color=yellow>%s<color>", Lib:StrFillL(tbRow[2], 16))
      end
    end
  end
