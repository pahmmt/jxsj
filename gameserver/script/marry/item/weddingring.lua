...
function tbItem:GetWeddingHorse()
  ...
  if me.CountFreeBagCell() < 1 then
    me.Msg("背包空间不足，请清理出1格背包空间再来领取侠侣坐骑吧。")
    return
  end
...
function tbItem:Come2Couple()
  ...
  -- 在战斗状态下需要nTime秒的延时
  GeneralProcess:StartProcess(string.format("正在传送去[%s]那...", szCoupleName), 5 * Env.GAME_FPS, { self.DoSelectMember, self, nCoupleId, me.nId }, nil, tbEvent)
end
