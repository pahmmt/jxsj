...
function tbItem:OnUse()
  ...
  for i = 1, 9 do
    ...
    if -1 ~= nCount then
      ...
      local szOpt = string.format("<color=%s>%s	剩余%s个<color>", szColor, self.tbSubItemInfo[i].szName, nCount)
...
function tbItem:OnUseTakeOut(nSubItem, nCount)
  ...
  if me.CountFreeBagCell() < nCount then
    Dialog:Say(string.format("您的背包空间不足，请清理出%s个背包空间再来吧。", nCount))
    return 0
  end
...
function tbUselessLibao:FemaleUse(pItem)
  ...
  if nCount_Exchange then
    local szMsg = string.format("由于性别原因，您无法使用该礼包，但是你可以把它兑换成<color=yellow>%s<color>朵情花，你确定要兑换吗？", nCount_Exchange)
    local tbOpt = {
...
function tbUselessLibao:Change2Qinghua(pItem, nChangeCount)
  if me.CountFreeBagCell() < 1 then
    Dialog:Say("背包空间不足，请清理出1个包裹空间再来试试吧。")
    return 0
  end

  me.AddStackItem(Marry.ITEM_QINGHUA_ID[1], Marry.ITEM_QINGHUA_ID[2], Marry.ITEM_QINGHUA_ID[3], Marry.ITEM_QINGHUA_ID[4], { bForceBind = 1 }, nChangeCount)
