...
function Baibaoxiang:CheckAllowGetAward(pPlayer, nType)
  ...
  if nType == 1 then
    ...
    if nAwardType == 1 or nAwardType == 5 then
      if pPlayer.CountFreeBagCell() < nAwardNum + nSpaceGC then
        Dialog:SendBlackBoardMsg(pPlayer, "你背包满了，放不下，留" .. tostring(nAwardNum + nSpaceGC) .. "格空间再来吧。")
        return 0
      end
    ...
    elseif nAwardType == 3 then
      if nAward * nAwardNum + pPlayer.nCashMoney > pPlayer.GetMaxCarryMoney() then
        Dialog:SendBlackBoardMsg(pPlayer, "对不起，领取后，您身上的银两将会超出上限，请整理后再来领取。")
        return 0
      end
    end
    ...
    if pPlayer.CountFreeBagCell() < nSpaceGC then
      Dialog:SendBlackBoardMsg(pPlayer, "你背包满了，放不下，留" .. tostring(nSpaceGC) .. "格空间再来吧。")
      return 0
    end
  ...
  elseif nType == 2 then
    ...
    if pPlayer.CountFreeBagCell() < nSpace + nSpaceGC then
      local szMsg = string.format("你背包满了，放不下，留%s格空间再来吧。", nSpace + nSpaceGC)
...
function Baibaoxiang:GetAward(pPlayer, nType)
  ...
  if nType == 1 then
    ...
    if nAwardType == 1 then
      ...
      if nAwardLevel >= 4 then
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. nAwardNum .. "<color>个" .. nAward .. "级玄晶！")
        Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. nAwardNum .. "个" .. nAward .. "级玄晶！", 0)
      end
    ...
    elseif nAwardType == 2 then
      ...
      -- channel
      if nAwardLevel >= 4 then
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. (nAward * nAwardNum) .. "<color>点精活！")
        Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. (nAward * nAwardNum) .. "点精活！", 0)
      end

      -- need channel msg
      pPlayer.Msg(string.format("您获得了<color=yellow>%s<color>点精力", nAward * nAwardNum))
      pPlayer.Msg(string.format("您获得了<color=yellow>%s<color>点活力", nAward * nAwardNum))
    ...
    elseif nAwardType == 3 then
      ...
      -- channel
      if nAwardLevel >= 4 then
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. (nAward * nAwardNum) .. "<color>银两！")
        Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. (nAward * nAwardNum) .. "银两！", 0)
      end
    ...
    elseif nAwardType == 4 then
      ...
      -- channel
      if nAwardLevel >= 4 then
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. (nAward * nAwardNum) .. string.format("<color>绑定%s！", IVER_g_szCoinName))
        Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. (nAward * nAwardNum) .. string.format("绑定%s！", IVER_g_szCoinName), 0)
      end
    ...
    elseif nAwardType == 5 then
      ...
      pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. (nAward * nAwardNum) .. "<color>个箱子！")
      Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. (nAward * nAwardNum) .. "个箱子！", 0)
  ...
  elseif nType == 2 then
    ...
    if nAwardLevel >= 4 then
      pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]在百宝箱中获得了<color=yellow>" .. (nAward * nAwardNum) .. "<color>个金贝壳！")
      Player:SendMsgToKinOrTong(pPlayer, "在百宝箱中获得了" .. (nAward * nAwardNum) .. "个金贝壳！", 0)
    end
...
function Baibaoxiang:GetCoin_GS(nPlayerID, nCoin)
  ...
  -- channel
  local szMsg = string.format("玩家：<color=green>%s<color>在百宝箱的彩金池里获得了%s枚金贝壳！！！", pPlayer.szName, nCoin)
  KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, szMsg)

  pPlayer.SendMsgToFriend("您的好友[<color=green>" .. pPlayer.szName .. "<color>]在百宝箱的彩金池里获得了<color=yellow>" .. nCoin .. "<color>枚金贝壳！")

  Player:SendMsgToKinOrTong(pPlayer, "在百宝箱的彩金池里获得了" .. nCoin .. "枚金贝壳！", 0)
...
function Baibaoxiang:UpdateHelpTable(pPlayer, nCoin)
  ...
  local szTxt = "<color=cyan>" .. szDate .. "\n" .. "<color=yellow>" .. pPlayer.szName .. " <color=green>在百宝箱的彩池中获得" .. "<color=yellow> " .. nCoin .. " <color=green>" .. "枚金贝壳！<color>"
...
function Baibaoxiang:OnChangeBack(nType, tbItem)
  ...
  if nType == 1 then
    ...
    me.Msg(string.format("您获得了<color=yellow>%s<color>点精力", nExCount * 225))

    me.ChangeCurGatherPoint(nExCount * 200) -- huoli
    me.Msg(string.format("您获得了<color=yellow>%s<color>点活力", nExCount * 200))
  elseif nType == 2 then
    me.AddStackItem(18, 1, 205, 1, nil, nExCount)
    me.ChangeCurGatherPoint(nExCount * 100) -- huoli
    me.Msg(string.format("您获得了<color=yellow>%s<color>点活力", nExCount * 100))
  elseif nType == 3 then
    me.AddStackItem(18, 1, 205, 1, nil, nExCount * 200)
    me.ChangeCurGatherPoint(nExCount * 20000) -- huoli
    me.Msg(string.format("您获得了<color=yellow>%s<color>点活力", nExCount * 20000))
  end
end
