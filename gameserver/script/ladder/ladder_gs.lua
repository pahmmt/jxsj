...
function Ladder:TranslateResult(tbPlayerList, nLadderType, nMode)
  ...
  if (nClassLadderType == 0 and nClassType == Ladder.LADDER_CLASS_WLLS and nType ~= 3) or (nClassLadderType == 0 and nClassType == Ladder.LADDER_CLASS_WLDH) then
    if nMode == 1 and tbPlayerList.tbLadder then
      for _, tbPlayer in pairs(tbPlayerList.tbLadder) do
        if string.sub(tbPlayer.szContext, 1, 14) ~= "<color=yellow>" then
          ...
          local sztext = "  " .. Lib:StrFillL("战队成员", 17) .. "门派" .. "\n\n<color=yellow>"
  ...
  elseif nClassLadderType == 0 and nClassType == Ladder.LADDER_CLASS_LADDER and nType == self.LADDER_TYPE_LADDER_ACTION and nNum == self.LADDER_TYPE_LADDER_ACTION_XOYOGAME then -- 逍遥收集榜
  ...
  elseif nClassLadderType == 0 and nClassType == Ladder.LADDER_CLASS_LADDER and nType == self.LADDER_TYPE_LADDER_ACTION and nNum == self.LADDER_TYPE_LADDER_ACTION_LADDER1 then -- 寒武遗迹
    if tbPlayerList.tbLadder then
      for _, tbPlayer in pairs(tbPlayerList.tbLadder) do
        if nMode == 1 then
          ...
          local szContext, szTxt1 = string.format("分数：%s 击杀玩家数：%s\n点击列表模式可以查询自己和他人的排名情况", nGrade, nKillCount), string.format("%s分", nGrade)
          tbPlayer.szContext = szContext
          tbPlayer.szTxt1 = szTxt1
        elseif nMode == 2 then
...
function Ladder:OnApplyList(nLadderType, nPage)
  ...
  if (self.LADDER_CLASS_WLLS ~= nClass or self.LADDER_TYPE_WLLS_HONOR == nType) and not (self.LADDER_CLASS_LADDER == nClass and self.LADDER_TYPE_LADDER_KINREPUTE == nType) and not (self.LADDER_CLASS_LADDER == nClass and self.LADDER_TYPE_LADDER_EVENTPLANT == nType and self.LADDER_TYPE_LADDER_EVENTPLANT_CURTEAM == nNum) then
...
function Ladder:OnApplySearchResult(nLadderType, szName, nSearchType)
  ...
  if nRank <= 0 then
    Dialog:Say(string.format("找不到 %s", szDesc))
    return
  end
...
function Ladder:GetExContext(nClass, nType)
  ...
  szTxt = "等级    排名最低值    荣誉最低值\n" .. szTxt
  return szTxt
end

function Ladder:OnApplyAdvSearch(nClass, nType, nSmall, szLadderName)
  local szMsg = string.format("%s排名查询\n", szLadderName)
  szMsg = szMsg .. self:GetExContext(nClass, nType)
  local tbOpt = {
    { "查询名次", self.SearchPlayerByRank, self, me, nClass, nType, nSmall },
    { "查询玩家名", self.SearchPlayerByName, self, me, nClass, nType, nSmall },
    { "我只是来看看的" },
  }
  if (self.LADDER_CLASS_LADDER == nClass and self.LADDER_TYPE_LADDER_KINREPUTE == nType) or (self.LADDER_CLASS_LADDER == nClass and self.LADDER_TYPE_LADDER_EVENTPLANT == nType and self.LADDER_TYPE_LADDER_EVENTPLANT_CURTEAM == nNum) then
    tbOpt = {
      { "查询名次", self.SearchKinReputeByRank, self, me, nClass, nType, nSmall },
      { "查询家族名", self.SearchKinReputebyName, self, me, nClass, nType, nSmall },
      { "我只是来看看的" },
    }
  end
  if self.LADDER_CLASS_WLLS == nClass and (self.LADDER_TYPE_WLLS_CUR_PRIMAY == nType or self.LADDER_TYPE_WLLS_CUR_ADV == nType) then
    table.insert(tbOpt, 3, { "查询战队名", self.SearchWllsLadderTeamByTeamName, self, me, nClass, nType, nSmall })
  end
...
function Ladder:SearchPlayerByRank(pPlayer, nClass, nType, nSmall, nFlag, nRank)
  ...
  if not nFlag then
    Dialog:AskNumber(string.format("请输入%s：", szType), 10000, self.SearchPlayerByRank, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  ...
  if type(nRank) ~= "number" then
    Dialog:Say(string.format("您的%s非法数字。", szType))
    return 0
  end

  if nRank <= 0 then
    Dialog:Say(string.format("您的%s必须大于0。", szType))
    return 0
  end

  local nLadderType = self:GetType(0, nClass, nType, nSmall)
  if self:SearchLadderByRank(nLadderType, nRank - 1) == 0 then
    Dialog:Say(string.format("对不起您所找的玩家不存在排名中。"))
    return 0
  end
end
...
function Ladder:SearchKinReputeByRank(pPlayer, nClass, nType, nSmall, nFlag, nRank)
  local szType = "家族名次"
  if not nFlag then
    Dialog:AskNumber(string.format("请输入%s：", szType), 100000, self.SearchKinReputeByRank, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  nRank = tonumber(nRank)
  --名字合法性检查
  ...
  local nLadderType = self:GetType(0, nClass, nType, nSmall)
  if self:SearchLadderByRank(nLadderType, nRank - 1) == 0 then
    Dialog:Say(string.format("对不起您所找的家族不存在排名中。"))
    return 0
  end
end
...
function Ladder:SearchWllsLadderTeamByTeamName(pPlayer, nClass, nType, nSmall, nFlag, szText)
  ...
  if not nFlag then
    Dialog:AskString(string.format("请输入%s：", szType), 16, self.SearchWllsLadderTeamByTeamName, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  --名字合法性检查
...
function Ladder:SearchPlayerByName(pPlayer, nClass, nType, nSmall, nFlag, szText)
  ...
  if not nFlag then
    Dialog:AskString(string.format("请输入%s：", szType), 16, self.SearchPlayerByName, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  --名字合法性检查
  local nLen = GetNameShowLen(szText)
...
function Ladder:SearchKinReputebyName(pPlayer, nClass, nType, nSmall, nFlag, szText)
  local szType = "家族名"
  if not nFlag then
    Dialog:AskString(string.format("请输入%s：", szType), 16, self.SearchKinReputebyName, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  --名字合法性检查
  local nLen = GetNameShowLen(szText)
...
function Ladder:SearchEPlatReputebyName(pPlayer, nClass, nType, nSmall, nFlag, szText)
  local szType = "家族名"
  if not nFlag then
    Dialog:AskString(string.format("请输入%s：", szType), 16, self.SearchEPlatReputebyName, self, pPlayer, nClass, nType, nSmall, 1)
    return
  end
  --名字合法性检查
  local nLen = GetNameShowLen(szText)
