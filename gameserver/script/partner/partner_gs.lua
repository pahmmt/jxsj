...
function Partner:Upgrade(nPartnerIndex)
  ...
  if me.IsAccountLock() == 1 then
    me.Msg("您的账号处于锁定状态，不可以升级同伴！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以升级同伴！请先解除锁定。")
  ...
  if nNewLevel % self.LEVELTOUPSKILL == 0 then
    if nTalentLevel < self.TALENT_MAX and self:CanUpgradeSkill(pPartner) == 1 then
      local szMsg = string.format("你当前同伴的领悟度为<color=yellow>%d<color>，如果现在升级则有<color=yellow>%d%%<color>的概率将一项技能提升一级，确定现在升级吗？", nTalentLevel, nTalentLevel)
      local tbOpt = {
        { "是", self.SetLevelUp, self, me.nId, nPartnerIndex, 1 }, -- 要升技能等级
        { "否" },
      }
...
function Partner:SetLevelUp(nPlayerId, nPartnerIndex, bUpSkill)
  ...
  if nNewLevel == 120 then
    ...
    nHours = nHours % 24
    local szLog = string.format("同伴Log:%s的同伴%s升到120级, 历时%d天%d时%d分%d秒", pPlayer.szName, pPartner.szName, nDays, nHours, nMinutes, nSeconds)
    Dbg:WriteLog(szLog)
  end
  ...
  pPlayer.Msg(string.format("您的同伴%s等级提升至%d", pPartner.szName, nNewLevel))
...
function Partner:CallPartner(nPartnerIndex)
  if me.nActivePartner == nPartnerIndex then
    ...
    if pPartner.GetValue(self.emKPARTNERATTRIBTYPE_FRIENDSHIP) < self.FRIENDSHIP_SHUYUAN then
      local szMsg = string.format("您和您的同伴%s关系日渐疏远，同伴不愿出战。", pPartner.szName)
      me.Msg(szMsg)
    ...
    if me.nFightState == 1 then
      ...
      GeneralProcess:StartProcess("出手相助中...", self.CALLPROCESSTIME * Env.GAME_FPS, { self.CallSucc, self, me.nId, nPartnerIndex }, nil, tbEvent)
    else
...
function Partner:CallSucc(nPlayerId, nPartnerIndex)
  ...
  elseif nCurLevel < self.MAXLEVEL and Partner.tbLevelSetting[nCurLevel] and nCurExp >= Partner.tbLevelSetting[nCurLevel].nExp then
...
function Partner:DissolvePartner(nPartnerIndex)
  -- 账号处于锁定状态，不能解散同伴
  if me.IsAccountLock() == 1 then
    me.Msg("您的账号处于锁定状态，不可以解散同伴！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以解散同伴！请先解除锁定。")
    Account:OpenLockWindow(me)
    return 0
  end
  if Account:Account2CheckIsUse(me, 6) == 0 then
  ...
  local szMsg = string.format("解除关系后将不能再找回该同伴，该同伴所增加的潜能点、财富荣誉值、同伴技能都将失效。确定要和<color=red>%s<color>解除同伴关系？", pPartner.szName)
  local tbOpt = {
    { "确定。", self.DissolveConfirm, self, me.nId, nPartnerIndex },
    { "我再想想。" },
  }
  Dialog:Say(szMsg, tbOpt)
...
function Partner:CanDissolvePartner(pPartner)
  ...
  if nStarLevel >= self.DELLIMITSTARLEVEL * 2 or pPartner.nSkillCount >= self.DELLIMITSKILLCOUNT then
    ...
    if nApplyTime == 0 then
      szMsg = string.format("%0.1f星级以上或%d技能以上的同伴必须在龙五太爷处申请才能删除。", self.DELLIMITSTARLEVEL, self.DELLIMITSKILLCOUNT)
      return 0, szMsg
    end

    local nDiff = GetTime() - nApplyTime
    if nDiff <= Partner.DEL_USABLE_MINTIME then
      szMsg = string.format("您的同伴正在删除申请中，还有%0.1f小时才可以删除%0.1f星以上或%d技能以上的同伴", (Partner.DEL_USABLE_MINTIME - nDiff) / 3600, self.DELLIMITSTARLEVEL, self.DELLIMITSKILLCOUNT)
    elseif nDiff >= Partner.DEL_USABLE_MAXTIME then
      szMsg = "您提交的删除申请已经过期，请重新申请！"
    end
  end
...
function Partner:ApplyDelPartner(nPlayerId)
  ...
  if pPlayer.IsAccountLock() == 1 then
    pPlayer.Msg("您的账号处于锁定状态，不可以申请删除同伴！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以申请删除同伴！请先解除锁定。")
  ...
  pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, "申请删除同伴")
  pPlayer.Msg("您成功申请了删除同伴！")
end
...
function Partner:CancelDelPartner(nPlayerId)
  ...
  pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, "撤销同伴删除申请")
  pPlayer.Msg("您已经撤销了同伴删除的申请！")
end
...
function Partner:DeletePartnerLog(nPlayerId, nPartnerIndex, nWay)
  ...
  szLog = szLog .. string.format(" %d,%d,%d,%d,%d,%d,%d,%d,%d;", nTempId, nLevel, nPTempId, nPPoint, pPartner.GetAttrib(0), pPartner.GetAttrib(1), pPartner.GetAttrib(2), pPartner.GetAttrib(3), nPartnerValue)
...
function Partner:SendGift(nPartnerIndex)
  if me.IsAccountLock() == 1 then
    me.Msg("您的账号处于锁定状态，不能给同伴赠送礼物！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不能给同伴赠送礼物！请先解除锁定。")
    Account:OpenLockWindow(me)
    return 0
  end
  if Account:Account2CheckIsUse(me, 6) == 0 then
  ...
  if self:CanUpgradeSkill(pPartner) == 0 then
    me.Msg(string.format("您的同伴%s已经将所有技能都升满了，不需要再提高领悟度了！", pPartner.szName))
    return
  end
  ...
  if nTalentLevel >= self.TALENT_MAX then
    me.Msg(string.format("您的同伴%s已经心领神会，无需再提高领悟度。", pPartner.szName))
    return
  end
...
function Partner:OnSendGiftOk(nPartnerIndex, tbItemObj)
  ...
  if nNewLevel > self.TALENT_MAX or bTooMuch == 1 then
    ...
    Dialog:Say("<color=yellow>礼物太多了，会浪费，确定全部赠送？<color>", {
      { "赠送（会浪费）", self.DelGift, self, nPartnerIndex, pPartner.GetValue(self.emKPARTNERATTRIBTYPE_TEMPID), tbItemObj, nTalentAdded },
      { "取消" },
    })
    return 0
  end

  local szMsg = string.format("现在放入的礼物，会使同伴的领悟度提升到<color=yellow>%d<color>，确定把这些都送给同伴吗？", nNewLevel)
  local tbOpt = {
    { "确定", self.DelGift, self, nPartnerIndex, pPartner.GetValue(self.emKPARTNERATTRIBTYPE_TEMPID), tbItemObj, nTalentAdded },
    { "我再想想" },
  }
  Dialog:Say(szMsg, tbOpt)
end
...
function Partner:DelGift(nPartnerIndex, nPartnerTempId, tbItemObj, nTalentAdded)
  ...
  Dbg:WriteLog("Spreader:OnItemConsumed", "推广员消耗记录：", Spreader:ExtractZoneGroup(), me.szAccount, string.format("[%s]赠送礼物给同伴%s，添加消耗记录[%s]", me.szName, pPartner.szName, tostring(nConsumeValue)))

  Dbg:WriteLog("同伴Log:", me.szName, "通过给予界面，增加同伴领悟度（总价值量）:", nTalentPointAdded)

  if nTalentPointAdded <= 0 then
    me.Msg("您没有放入任何有效物品，不会增加同伴的领悟度！")
    return 0
  end
...
function Partner:MoonStoneOkFun(tbParams, tbBoxItem)
  ...
  if me.CanAddItemIntoBag({
    nGenre = tbParams.nGenre,
    nDetail = tbParams.nDetail,
    nParticular = tbParams.nParticular,
    nLevel = tbParams.nLevel,
    nSeries = -1,
    bBind = bBind,
    nCount = nTargetCount,
  }) ~= 1 then
  ...
  local szMsg = string.format("%s用%d个月影之石兑换%d个%s", me.szName, nDropCount - nRemainToDel, nCount, KItem.GetNameById(tbParams.nGenre, tbParams.nDetail, tbParams.nParticular, tbParams.nLevel))
...
function Partner:ConvertToZhenYuan(nPartnerIndex)
  ...
  if self:CanConvertToZhenYuan(pPartner) ~= 1 or pPartner.GetValue(self.emKPARTNERATTRIBTYPE_LEVEL) ~= 1 then
    me.Msg("<color=yellow>同伴必须为1级，且为可凝聚真元的同伴！<color>（只有抓捕同伴时其头像框为蓝色的同伴方可凝聚，在F9界面中，此种同伴的形象栏右上角的图标会显示为蓝色）")
  ...
  if nCost > me.nCashMoney then
    Dialog:Say(string.format("凝聚该同伴需要银两<color=yellow>%s<color>，背包银两不足！", Item:FormatMoney(nCost)))
    return
  end

  local szMsg = string.format("转化为真元之后，同伴将会消失，同时凝聚该同伴需要消耗银两<color=yellow>%s<color>，你确定要转化真元吗？", Item:FormatMoney(nCost))
  local tbOpt = {
    { "确定", Partner.OnSureConvert, Partner, me.nId, nPartnerIndex, 0 },
    { "我再想想" },
  }
...
function Partner:ResetPartnerEquipParam(pItem)
  if not pItem or pItem.IsPartnerEquip() ~= 1 then
    return
  end

  local nValue = pItem.GetGenInfo(1)
  nValue = Lib:SetBits(nValue, 0, 0, 1)
  pItem.SetGenInfo(1, nValue)
end
...
function Partner:OnSureConvert(nPlayerId, nPartnerIndex, nStep)
  ...
  if nStep == 0 then -- 启动进度条
    ...
    GeneralProcess:StartProcess("真元凝聚中...", self.CONVERTPROCESSTIME * Env.GAME_FPS, { self.OnSureConvert, self, nPlayerId, nPartnerIndex, 1 }, { pPlayer.CallClientScript, { "Partner:PlayerConvertAnimate", 0 } }, tbEvent)
  ...
  elseif nStep == 1 then -- 转化真元：添加道具，删除同伴
    ...
    if not pItem then
      local szLog = string.format("%s凝聚真元失败[银两扣除成功，同伴删除成功，真元添加失败]！扣除银两%d两，删除同伴（%d,%d）。", pPlayer.szName, nCost, nPartTempId, nSkillCount)
      Dbg:WriteLog("[partnerconvert]", szLog)
      return
    end
  end
