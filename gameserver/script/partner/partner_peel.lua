...
function Partner:ApplyPeelPartner(nPlayerId)
  ...
  if pPlayer.IsAccountLock() == 1 then
    pPlayer.Msg("您的账号处于锁定状态，不可以申请重生同伴！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以申请重生同伴！请先解除锁定。")
...
function Partner:PeelPartner(pPartner)
  ...
  if me.CountFreeBagCell() < nItemCount then
    me.Msg("包裹空位不够，请清理一下再来！")
    self:SendClientMsg("包裹空位不够，请清理一下再来！")
    return 0
  end
  ...
  for nLevel, nCount in pairs(tbRetItem) do
    ...
    local nAddCount = me.AddStackItem(self.tbPartnerJinghua.nGenre, self.tbPartnerJinghua.nDetail, self.tbPartnerJinghua.nParticular, nLevel, nil, nCount, Player.emKITEMLOG_TYPE_PEEL_PARTNER)
  ...
  me.Msg(string.format("你的同伴%s的等级和所有技能等级已经变成1。", pPartner.szName))
  self:SendClientMsg(string.format("你的同伴%s的等级和所有技能等级已经变成1。", pPartner.szName))
...
function Partner:CalPeelRetItem(pPartner)
  ...
  for _, nLevel in pairs(tbLevel) do
    ...
    local szIndex = string.format("%s_%s_%s_%s", self.tbPartnerJinghua.nGenre, self.tbPartnerJinghua.nDetail, self.tbPartnerJinghua.nParticular, nLevel)
...
function Partner:ApplyUnBindPartEq(nPlayerId)
  ...
  if pPlayer.IsAccountLock() == 1 then
    pPlayer.Msg("您的账号处于锁定状态，不可以申请解绑同伴装备！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以申请同伴装备解绑！请先解除锁定。")
    Account:OpenLockWindow(pPlayer)
    return 0
  end
  if Account:Account2CheckIsUse(pPlayer, 6) == 0 then
  ...
  pPlayer.AddSkillState(self.BINDPARTEQ_SKILLIED, 1, 2, self.BIND_PARTNERQUIP_MAXTIME * Env.GAME_FPS, 1, 0, 1)
...
function Partner:PostUnBind(nCount)
  ...
  me.Msg(string.format("您成功解绑了%s件同伴装备。", nCount))
