...
function tbBlessCard:ViewBlessToPlayer(nItemId)
  ...
  if not szList or #szList <= 0 then
    Dialog:Say("你还没有向任何好友拜过年，快去向好友拜年送祝福吧！", { "我知道了" })
    return 0
  end
...
function tbBlessCard:ChangeBlessCard(nItemId)
  ...
  if me.DelItem(pItem, Player.emKLOSEITEM_USE) ~= 1 then
    ...
    if nGiveCount ~= nCount then
      ...
      --家族好友公告
      local szFMsg = string.format("您的好友[<color=yellow>%s<color>]收到了5位好友的诚挚祝福，换取到一份丰厚的奖励和一张%s，真是可喜可贺！", me.szName, KItem.GetNameById(unpack(tbGdpl)))
...
function tbMatch:MakeMatch(nItemId)
  ...
  GeneralProcess:StartProcess("加工中...", 5 * Env.GAME_FPS, { self.DoMake, self, nItemId }, nil, tbEvent)
end
  ...
  if nExtPrizeIndex > 0 and self.tbExtPrize[nExtPrizeIndex] then
    if not IpStatistics:IsStudioRole(me) then
      ...
      if MathRandom(nRand) <= nRate then
        ...
        local szFMsg = string.format("您的好友[<color=yellow>%s<color>]打开%s获得一个%s,真是可喜可贺呀！", me.szName, pItem.szName, KItem.GetNameById(unpack(tbGdpl)))
