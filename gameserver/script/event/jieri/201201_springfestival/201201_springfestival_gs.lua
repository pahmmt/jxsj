...
function SpringFestival:OnPlayerChangeFubi()
  ...
  for _, tbItem in pairs(tbFind) do
    if tbItem.pItem then
      local nRet = tbItem.pItem.Regenerate(tbCanUseGdpl[1], tbCanUseGdpl[2], tbCanUseGdpl[3], tbCanUseGdpl[4], tbItem.pItem.nSeries, tbItem.pItem.nEnhTimes, tbItem.pItem.nLucky, tbItem.pItem.GetGenInfo(), 0, tbItem.pItem.dwRandSeed, 0)
      if nRet ~= 1 then
...
function SpringFestival:OnInputWordCard(tbItemObj, bSure)
  ...
  for _, pItem in pairs(tbItemObj) do
    ...
    if
      pItem[1]
      and nIsLevelHasGet ~= pItem[1].nLevel --是否已经交过，以防万一
      and self:CheckWordCardCanDel(pItem[1].SzGDPL()) == 1 --防止放入其它物品
      and not tbCanDelItem[pItem[1].SzGDPL()]
    then --以防万一，放入重复的字卡
      nCount = nCount + 1
...
function SpringFestival:BuyHopeCardByCoin()
  ...
  local tbOpt = {}
  tbOpt[#tbOpt + 1] = { "确定购买", self.SureBuyHopeCard, self }
  tbOpt[#tbOpt + 1] = { "我再想想" }
  Dialog:Say(szMsg, tbOpt)
end
