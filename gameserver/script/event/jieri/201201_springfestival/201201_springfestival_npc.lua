...
function tbJubaopen:CheckCanGetIngot()
  ...
  if (nTime < SpringFestival.nBeginGetIngotTimeDay or nTime >= SpringFestival.nEndGetIngotTimeDay) and (nTime < SpringFestival.nBeginGetIngotTimeNight or nTime >= SpringFestival.nEndGetIngotTimeNight) then
  ...
  if nTime >= SpringFestival.nBeginGetIngotTimeDay and nTime <= SpringFestival.nEndGetIngotTimeDay and nLastGetTime >= SpringFestival.nBeginGetIngotTimeDay and nLastGetTime <= SpringFestival.nEndGetIngotTimeDay then
    return 0, "每个时间段一人只能摸一次宝物，请等待下一个阶段再来吧！\n    活动期间每天12:00-14:00、20:00-22:00<color>，聚宝盆内将会刷新各式宝物。"
  end
  if nTime >= SpringFestival.nBeginGetIngotTimeNight and nTime <= SpringFestival.nEndGetIngotTimeNight and nLastGetTime >= SpringFestival.nBeginGetIngotTimeNight and nLastGetTime <= SpringFestival.nEndGetIngotTimeNight then
    return 0, "每个时间段一人只能摸一次宝物，请等待下一个阶段再来吧！\n    活动期间每天12:00-14:00、20:00-22:00<color>，聚宝盆内将会刷新各式宝物。"
  end
...
function tbJubaopen:GetIngot(nNpcId)
  ...
  if not pItem then
    ...
    local szMsg = string.format("你获得了一个<color=green>%s<color>，快去寻找拥有匹配宝物的有缘人吧！", pItem.szName)
...
function tbLantern:DoFireLantern(nNpcId)
  ...
  if #tbFind > 0 then
    if me.DelItem(tbFind[1].pItem, Player.emKLOSEITEM_USE) ~= 1 then
      ...
      me.AddSkillState(SpringFestival.nFireLanternBuffId, 1, 2, SpringFestival.nFireLanternBuffTime, 1, 0, 1) --加个状态
      ...
      --好友，家族公告
      local szFMsg = string.format("您的好友[<color=yellow>%s<color>]点燃了新春彩灯，还幸运的召唤出了一个会施仙术的灯笼精灵，真是太神奇了！", me.szName)
...
function tbHopeTree:Change(tbGdpl, nDateTime)
  ...
  if nConsumeCount == 0 then
    ...
    if not pItem then
      ...
      local szFMsg = string.format("您的好友[<color=yellow>%s<color>]打开了祈愿树送予的幸运奖券，获得了%s的抽奖资格。", me.szName, szDate)
