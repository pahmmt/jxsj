...
function tbMeijiujie20110501:GetIndexSeq(nCount, nMaxCount, nCollectionCount)
  if nCount <= 0 or nMaxCount <= 0 or nCollectionCount <= 0 or nCount > nMaxCount or nCount > nCollectionCount then
...
function tbMeijiujie20110501:Duijiang()
  ...
  if nCellNeeded > 0 and me.CountFreeBagCell() < nCellNeeded then
    Dialog:Say("背包空间不足，请整理出" .. tostring(nCellNeeded) .. "格空间再来兑换奖励。")
    return
  end
...
function tbMeijiujie20110501:GetLibao()
  ...
  if me.CountFreeBagCell() < 1 then
    Dialog:Say("背包空间不足，请整理出1格空间再来领取五一劳动者宝箱。")
    return
  end
