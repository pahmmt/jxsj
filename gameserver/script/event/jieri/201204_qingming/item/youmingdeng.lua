...
function tbItem:MakeShuHun(nPlayerId, nItemId, nSure, nCount)
  ...
  if nSure == 0 then
    local szMsg = string.format("你确定使用<color=red>%d<color>点精力<color=red>%d<color>点活力将<color=yellow>%d<color>个幽冥灯合成<color=yellow>%d<color>个赎魂灯么?", nCount * tbQingMing2012.nCostMKP, nCount * tbQingMing2012.nCostGTP, nCount * tbQingMing2012.nNeededCount, nCount)
    local tbOpt = {
