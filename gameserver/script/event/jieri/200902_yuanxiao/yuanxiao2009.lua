...
function tbYuanXiao09:CheckHaoLi(nIndex)
  ...
  if (me.nPrestige < 200) and (me.GetExtMonthPay() < 15) then --江湖威望不低于200或者当月充值不低于15元
    return 0, "您不满足获得礼物的条件，<color=red>需要本月充值达到15元，或者江湖威望达到200<color>就可以来领奖了。", { { "我知道了（离开）" } }
  end

  if me.nLevel < 69 then
    return 0, "您的等级不够，<color=yellow>69级<color>以后再来您礼物吧。", { { "我知道了（离开）" } }
  end
  ...
  if me.CountFreeBagCell() < nRequire then
    return 0, "领取礼物需要<color=red> " .. tostring(nRequire) .. " <color>格背包空间，整理一下包包再来吧。", { { "我知道了（离开）" } }
  end

  return 1, "这是我送给您的礼物，是过去一年您作出努力的奖励，同时希望您来年更加努力奋斗。", { { "谢谢您！（离开）" } }
end
...
function tbYuanXiao09:CheckHaoYouZhuFu()
  --[[
	...
	--]]
  if (me.nPrestige < 200) and (me.GetExtMonthPay() < 15) then --江湖威望不低于200或者当月充值不低于15元
    return 0, "您不满足送出祝福的条件，<color=red>需要本月充值达到15元，或者江湖威望达到200<color>就可以来送出祝福了。", { { "我知道了（离开）" } }
  end

  if me.nLevel < 69 then
    return 0, "您的等级不够，<color=red>69级<color>以后再来祝福吧。", { { "我知道了（离开）" } }
  end
  ...
  if (not tbPlayerId) or not tbTeamMemberList or #tbTeamMemberList ~= 2 or Lib:CountTB(tbPlayerId) ~= 2 then
    return 0, "哪位是要祝福你的好友呢？只有与<color=red>要送出祝福方单独组队并让他在附近<color>，才能获得他给你的祝福。", { { "我知道了（离开）" } }
  end
  ...
  if 1 ~= me.IsFriendRelation(pFriend.szName) or me.GetFriendFavorLevel(pFriend.szName) < 3 then --DONE:亲密度修改为３
    return 0, "您与队友<color=red>不是好友或亲密度未达3级<color> ", { { "我知道了（离开）" } }
  end

  --当天是否已经送出过祝福
  local nDate = tonumber(GetLocalDate("%Y%m%d"))
  if me.GetTask(self.TASKGID, self.TASK_ZHUFU_TIME) >= nDate then
    return 0, "今天你<color=red>已经获得过祝福了<color>，明天再来吧。", { { "我知道了（离开）" } }
  end
  ...
  if nCount >= self.MAX_ZHUFU_COUNT then
    return 0, "您已经<color=red>总共获得了10次祝福<color>，没有机会再获得祝福了。", { { "我知道了（离开）" } }
  end
  ...
  if nFlag == 1 then
    return 0, "你已经<color=red>获得过这位好友的祝福了<color>，重复祝福是没有意义的。", { { "我知道了（离开）" } }
  end

  if me.CountFreeBagCell() < 1 then
    return 0, "获得祝福需要背包空间<color=red>1格<color>背包空间，整理一下包包再来吧。", { { "我知道了（离开）" } }
  end

  --所有的条件满足，送出祝福
  return 1, "你获得你的好友<color=red>" .. pFriend.szName .. "<color>送出的新春祝福，在新的一年里，他的祝福会常伴你左右。这是你获得的祝福奖励", { { "谢谢您（离开）" } }, pFriend
end
