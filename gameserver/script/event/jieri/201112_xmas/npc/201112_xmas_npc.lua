...
function tbSanta:CheckCanGetSock()
  ...
  if (nTime < Xmas2011.nNpcBeginWalkTimeDay or nTime > Xmas2011.nNpcEndWalkTimeDay) and (nTime < Xmas2011.nNpcBeginWalkTimeNight or nTime > Xmas2011.nNpcEndWalkTimeNight) then
  ...
  if nTime >= Xmas2011.nNpcBeginWalkTimeDay and nTime <= Xmas2011.nNpcEndWalkTimeDay and nLastGetTime >= Xmas2011.nNpcBeginWalkTimeDay and nLastGetTime <= Xmas2011.nNpcEndWalkTimeDay then
    return 0, "每阶段每人只能领取一次袜子，请下阶段再来领取！"
  end
  if nTime >= Xmas2011.nNpcBeginWalkTimeNight and nTime <= Xmas2011.nNpcEndWalkTimeNight and nLastGetTime >= Xmas2011.nNpcBeginWalkTimeNight and nLastGetTime <= Xmas2011.nNpcEndWalkTimeNight then
    return 0, "每阶段每人只能领取一次袜子，请下阶段再来领取！"
  end
  ...
  if me.CountFreeBagCell() < 1 then
    return 0, "需要<color=yellow>1格<color>背包空间，整理下再来！"
  end
  return 1
end
...
function tbSanta:GiveSock()
  ...
  if not pItem then
    ...
    StatLog:WriteStatLog("stat_info", "shengdanjie_2012", "get_socks", me.nId, 1)
    --记录领取的次数和时间
    local nGetCount = me.GetTask(Xmas2011.nTaskGroupId, Xmas2011.nHasGetSockCountTaskId)
  ...
  if not pFinSnowBoy then
    ...
    for _, szName in pairs(tbPlayerList) do
      ...
      if pPlayer then
        ...
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]和他的队友堆成了圣诞雪人！获得了宝箱【冰雪之心】！大家掌声鼓励！")
      end
    end
...
function tbSnowMan:OnInputSnowBall(nNpcId, tbItemObj, bSure)
  ...
  if bSure and bSure == 1 then
    if me.CountFreeBagCell() < 1 then
      Dialog:Say("请保证留出<color=yellow>1格<color>背包空间！")
      return 0
    end
    local nDelCount = 0
    for _, pItem in pairs(tbItemObj) do
...
function tbSnowMan:GiveReturnBack(nCount)
  ...
  local szFMsg = "您的好友[<color=yellow>" .. me.szName .. "<color>]给雪人滚滚捐赠了" .. nReturn .. "颗莹白的雪花团子，获得了宝箱【雪人的报恩】，雪人滚滚脸红了！"
