...
function tbPrizeSock:OpenStar(nId)
  ...
  if pStar then
    ...
    if me.DelItem(pItem, Player.emKLOSEITEM_USE) ~= 1 then
      ...
      local tbAward = Lib._CalcAward:RandomAward(3, 3, 2, 75000, Lib:_GetXuanReduce(nDay), { 8, 2, 0 })
      local nRet, szName, nType, nNum = Lib._CalcAward:RandomItem(me, tbAward)
      local nNowDate = tonumber(GetLocalDate("%Y%m%d"))
      if me.GetTask(2216, 20) ~= nNowDate then
        local pItemEx = me.AddItemEx(18, 1, 1880, 1, nil, 100, 1)
        if pItemEx then
          me.SetItemTimeout(pItemEx, os.date("%Y/%m/%d/23/59/59", GetTime()))
          me.SetTask(2216, 20, nNowDate)
          StatLog:WriteStatLog("stat_info", "shengdanjie_2012", "socks_award_spe", me.nId, "18_1_1880_1, 1")
        end
      end
      if nRet == 1 then
        local szLog = ""
        if nType == 1 then
          szLog = string.format("18_1_114_%s, 1", nNum)
        elseif nType == 2 then
          szLog = string.format("bindmoney, %s", nNum)
        end
        StatLog:WriteStatLog("stat_info", "shengdanjie_2012", "socks_award", me.nId, szLog)
      end
    end
  else
...
function tbNormalSnowBall:MakeSnowBall(nCount, nItemId)
  GeneralProcess:StartProcess("加工雪球中...", 5 * Env.GAME_FPS, { self.DoMake, self, nItemId, nCount }, nil, tbEvent)
end
...
function tbSnowBase:OnUse()
  ...
  GeneralProcess:StartProcess("放置雪人冰座中...", 5 * Env.GAME_FPS, { self.DropSnowBase, self, it.dwId }, nil, tbEvent)
end
...
function tbStonePiece:MakeStoneBox(nCount, nItemId)
  GeneralProcess:StartProcess("制作宝箱中...", 5 * Env.GAME_FPS, { self.DoMakeBox, self, nItemId, nCount }, nil, tbEvent)
end
