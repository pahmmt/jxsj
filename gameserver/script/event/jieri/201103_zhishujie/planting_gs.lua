...
function tbZhiShu2011:GetAward(dwNpcId, nPlayerId, nType, nDetail)
  ...
  elseif tbAward[1] == 3 then
    if me.CountFreeBagCell() < tbAward[2][6] then
      Dialog:SendBlackBoardMsg(pPlayer, string.format("您的背包空间不足，需要%s格背包空间。", tbAward[2][6]))
      return 0
    end
...
function tbZhiShu2011:CanGatherSeedforOther(dwNpcId, nPlayerId)
  ...
  --不是帮会，家族，好友
  if (not tbTemp.tbZhiShu2011.dwKingId or tbTemp.tbZhiShu2011.dwKingId ~= pPlayer.GetKinMember() or pPlayer.GetKinMember() == 0) and (not tbTemp.tbZhiShu2011.dwTongId or tbTemp.tbZhiShu2011.dwTongId ~= pPlayer.dwTongId or pPlayer.dwTongId == 0) and (not tbTemp.tbZhiShu2011.szName or pPlayer.IsFriendRelation(tbTemp.tbZhiShu2011.szName) ~= 1) then
    return 0
  end
  ...
  for i = 1, #tbTeamMemberList do
    ...
    if pPlayerEx then
      ...
      if pPlayerEx.CountFreeBagCell() < 1 then
        return 0, string.format("你们队伍的<color=yellow>%s<color>包裹空间不足1格。", pPlayerEx.szName)
      end
...
function tbZhiShu2011:MakeStepAction(nPlayerId, dwNpcId, nFlag)
  ...
  if not nFlag then
    ...
    GeneralProcess:StartProcess("培育希望种子...", 5 * Env.GAME_FPS, { SpecialEvent.tbZhiShu2011.MakeStepAction, SpecialEvent.tbZhiShu2011, nPlayerId, dwNpcId, 1 }, nil, tbEvent)
  else
