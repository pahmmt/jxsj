...
function SpringFrestival:ServerStartFunc()
  ...
  if nData >= self.HuaDengOpenTime and nData <= (self.HuaDengCloseTime + 1) then --活动期间内启动服务器
    ...
    if (nData == self.HuaDengOpenTime and nTime < SpringFrestival.HuaDengOpenTime_C) or (nData == (self.HuaDengCloseTime + 1) and nTime > SpringFrestival.HuaDengOpenTime_C) then --9号12点前和15号12点后，服务器启动不加载花灯
      return
    end
