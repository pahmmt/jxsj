...
function tbItem:OnSelect(szPlayerName, dwItemId, nSure)
  ...
  if not nSure then
    ...
    local szMsg = (nFriend == 1) and string.format("你确定赠送给好友<color=yellow>%s<color>么？\n（赠送给好友将累计亲密度点数，2月15日-2月21日可领取总点数排行榜奖励）", szPlayerName) or string.format("%s<color=yellow>不是你的好友<color>，你确定赠送吗？\n（赠送给好友将累计亲密度点数，2月15日-2月21日可领取总点数排行榜奖励）", szPlayerName)
    local tbOpt = {
      { "确定", self.OnSelect, self, szPlayerName, dwItemId, 1 },
      { "我再想想" },
    }
    Dialog:Say(szMsg, tbOpt)
    return 0
