...
function tbQingren_2011:QueryBuffer(nFrom)
  ...
  for i = nLast, #self.tbBuffer do
    szMsg = szMsg .. string.format("<color=cyan>%s<color=yellow>%s%s<color>\n", Lib:StrFillL(i .. ".", 4), Lib:StrFillL(self.tbBuffer[i].szPlayerName, 18), Lib:StrFillL(self.tbBuffer[i].nPoint, 6))
    nCount = nCount - 1
    nLast = nLast + 1
    if nCount <= 0 and nLast < #self.tbBuffer then
      table.insert(tbOpt, 1, { "下一页", self.QueryBuffer, self, nLast })
      break
    end
  end
  Dialog:Say(szMsg, tbOpt)
