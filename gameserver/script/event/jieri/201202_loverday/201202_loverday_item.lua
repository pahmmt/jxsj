      ...
      for _, pPlayer in pairs(tbMember) do
        if pPlayer then
          ...
          if nGiveCount ~= nCount then
            ...
            local szFMsg = string.format("丘比特之箭，将您的好友[<color=yellow>%s<color>]和队友甜蜜锁定，情缘红烛绽放爱的花火！", pPlayer.szName)
    ...
    for _, tbInfo in ipairs(self.tbExtPrize) do
      ...
      if nRand <= nRateT then
        ...
        if pExt then
          ...
          local szFMsg = string.format("您的好友[<color=yellow>%s<color>]打开%s获得一个%s,真是可喜可贺呀！", me.szName, pItem.szName, pExt.szName)
