...
function LoverDay2012:CheckCanGetGetRing()
  ...
  if (nTime < self.nLoveMatchBeginTimeDay or nTime >= self.nLoveMatchEndTimeDay) and (nTime < self.nLoveMatchBeginTimeNight or nTime >= self.nLoveMatchEndTimeNight) then
  ...
  if nTime >= self.nLoveMatchBeginTimeDay and nTime <= self.nLoveMatchEndTimeDay and nLastGetTime >= self.nLoveMatchBeginTimeDay and nLastGetTime <= self.nLoveMatchEndTimeDay then
  ...
  if nTime >= self.nLoveMatchBeginTimeNight and nTime <= self.nLoveMatchEndTimeNight and nLastGetTime >= self.nLoveMatchBeginTimeNight and nLastGetTime <= self.nLoveMatchEndTimeNight then
...
function LoverDay2012:GetRing()
  ...
  if not pItem then
    ...
    local szMsg = string.format("你获得了一个<color=green>%s<color>！", pItem.szName)
