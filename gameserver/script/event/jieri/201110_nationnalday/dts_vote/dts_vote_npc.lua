...
function tbDtsVote:VoteTickets1(szName)
  Dialog:Say(string.format("您确定要给侠士<color=yellow>%s<color>祝福？", szName), { { "确定", self.VoteTicketsEx, self, szName }, { "我再想想" } })
  return
end
...
function tbDtsVote:QueryAwardInfo()
  Dialog:Say(string.format(
    [[目前奖池拥有的绑金数量<color=yellow>%s<color>，原石宝箱数<color=yellow>%s<color>个。
...
function tbDtsVote:GetVoteAward()
  ...
  if self.tbAwardList[me.szName][3] > 0 and me.CountFreeBagCell() < self.tbAwardList[me.szName][3] then
    Dialog:Say(string.format("背包空间不足%s格，请清理下再来吧。", self.tbAwardList[me.szName][3]))
    return 0
  end
