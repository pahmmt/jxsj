...
function tbNpc:OnDialog()
  ...
  local tbOpt = { { "放入家族团圆粽", self.RItem, self, him.dwId }, { "我再想想" } }
...
function tbNpc:Fire(dwId, nFlag)
  ...
  if not nFlag then
    Dialog:Say("你是否要点燃火苗？\n<color=red>一旦点燃后所有家族成员将不能再投入粽子<color>", { { "是的", self.Fire, self, dwId, 1 }, { "我再想想" } })
    return
  end
...
function tbNpc2:OnDialog()
  ...
  if not tbTemp.tbDragonB2012.nTimerId1 then
    Dialog:Say("<color=yellow>" .. cKin.GetName() .. "<color>家族香喷喷的团圆粽出锅啦！！大家快来尝尝！！ ", { { "尝一个粽子", self.OnUse, self, him.dwId }, { "我再想想" } })
    return
  end
