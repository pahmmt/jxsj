...
function tbMaterial:OnUse()
  if me.CountFreeBagCell() < 1 then
    Dialog:SendBlackBoardMsg(me, "您的背包空间不足1格。")
    return
  end
...
function tbItem:OnUse()
  ...
  if nMemberCount > 1 then
    ...
    for _, nId in ipairs(tbMemberId) do
      ...
      if pPlayer then
        ...
        if pPlayer.CountFreeBagCell() < 1 then
          Dialog:SendBlackBoardMsg(me, string.format("玩家<color=yellow>%s<color>背包空间不足1格。", pPlayer.szName))
          return
        end
    ...
    local nMaxBindMoney = tbDragonBoatFestival2012:GetMaxBandMoney(tbAward)
    if me.CountFreeBagCell() < 1 then
      Dialog:SendBlackBoardMsg(me, "对不起，您背包空间不足1格。")
      return
    end
...
function tbBox:OnUse()
  ...
  if me.CountFreeBagCell() < 3 then
    Dialog:SendBlackBoardMsg(me, "对不起，您背包空间不足3格。")
    return
  end
  if me.GetBindMoney() + nMaxBindMoney > me.GetMaxCarryMoney() then
  ...
  if pNpc then
    ...
    me.SendMsgToFriend("您的好友[" .. me.szName .. "]在城中摆出了一盆菖蒲花，也带来了一份端午的祈福。")
  end
  --额外玉如意
  if MathRandom(10000) <= 1000 and IpStatistics:CheckStudioRole(me) == 0 then
    ...
    me.SendMsgToFriend("您的好友[" .. me.szName .. "]在[端午节活动]中获得了<color=green>玉如意<color>。")
  ...
  if MathRandom(100) <= nRate then
    ...
    if pItem then
      ...
      me.SendMsgToFriend("您的好友[" .. me.szName .. "]在[端午节活动]中获得了<color=green>经验跟宠【葫小芦】<color>。")
