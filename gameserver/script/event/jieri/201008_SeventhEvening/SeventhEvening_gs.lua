...
function SeventhEvening:QueryXialv(nFrom)
  ...
  for i = nLast, #self.tbXialvBuffer do
    szMsg = szMsg .. string.format("<color=cyan>%s<color=yellow>%s%s\n%s%s<color>\n\n", Lib:StrFillL(i .. ".", 3), Lib:StrFillL(self.tbXialvBuffer[i].szMaleName, 20), Lib:StrFillL(self.tbXialvBuffer[i].nPoint, 4), Lib:StrFillL(" ", 3), Lib:StrFillL(self.tbXialvBuffer[i].szFemaleName, 16))
    nCount = nCount - 1
    nLast = nLast + 1
    if nCount <= 0 and nLast < #self.tbXialvBuffer then
      table.insert(tbOpt, 1, { "下一页", self.QueryXialv, self, nLast })
      break
    end
  end
