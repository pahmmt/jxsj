...
function tbEuropean:Join()
  ...
  if nMySession > 0 and self.tbGlobalBuffer[nMySession] then
    ...
    if nCount > 0 then
      if tbMySession.nResult > 0 and tbMySession.nResult <= 3 then
        local szMsg = string.format(
          [[<newdialog>    欢迎参加2012欧洲杯有奖竞猜，快来为您的主队加油，并赢取丰厚大奖吧！
              <color=green>%s<color>
    ...
    您已经投了<color=yellow>%s<color>注给<color=yellow>%s<color>。]],
          tbMySession.szTime,
          Lib:StrFillC(tbMySession.tbTeam[1], 8),
          Lib:StrFillC(tbMySession.tbTeam[3], 8),
          self.IMAGE_PATH[tbMySession.tbTeam[1]] or "",
          self.IMAGE_PATH[tbMySession.tbTeam[3]] or "",
          tbMySession.tbTeam[tbMySession.nResult],
          tbMySession.tbAward[tbMySession.nResult],
          nCount,
          tbMySession.tbTeam[nResult]
        )
        local nAward = 0
    ...
    您已经投了<color=yellow>%s<color>注给<color=yellow>%s<color>。<color=green>请等待比赛结束当天14点之前更新结果。<color>]],
          ...
          self.IMAGE_PATH[tbMySession.tbTeam[3]] or "",
          Lib:StrFillC(tbMySession.tbTeam[1], 8),
          tbMySession.tbAward[1],
          Lib:StrFillC(tbMySession.tbTeam[2], 8),
          tbMySession.tbAward[2],
          Lib:StrFillC(tbMySession.tbTeam[3], 8),
          tbMySession.tbAward[3],
          nCount,
          tbMySession.tbTeam[nResult]
        )
        Dialog:Say(szMsg)
  ...
  local szMsg = string.format(
    ...
    Lib:StrFillC(tbSession.tbTeam[1], 8),
    Lib:StrFillC(tbSession.tbTeam[3], 8),
    self.IMAGE_PATH[tbSession.tbTeam[1]] or "",
    self.IMAGE_PATH[tbSession.tbTeam[3]] or "",
    Lib:StrFillC(tbSession.tbTeam[1], 8),
    tbSession.tbAward[1],
    Lib:StrFillC(tbSession.tbTeam[2], 8),
    tbSession.tbAward[2],
    Lib:StrFillC(tbSession.tbTeam[3], 8),
    tbSession.tbAward[3]
  )
