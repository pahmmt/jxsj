...
function tbNpc:CheckCanChange(tbItemObj)
  ...
  for i = 1, #tbItem do
    for j = i + 1, #tbItem do
      if string.format("%s%s%s%s", tbItem[i][1], tbItem[i][2], tbItem[i][3], tbItem[i][4]) == string.format("%s%s%s%s", tbItem[j][1], tbItem[j][2], tbItem[j][3], tbItem[j][4]) then
        nHasSame = 1
      end
    end
  end
