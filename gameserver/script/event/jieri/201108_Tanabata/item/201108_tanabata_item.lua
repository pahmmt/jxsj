...
function tbMaleItem:CheckCanUse()
  ...
  for _, pNpc in ipairs(tbNpcList) do
    if pNpc.nKind == 3 or pNpc.nTemplateId == Tanabata201108.nTongxinzhuTemplateId or pNpc.nTemplateId == Tanabata201108.nHongzhuTemplateId then
...
function tbMaleItem:OnUse()
  if self:CheckCanUse() == 1 then
    local tbMember = me.GetTeamMemberList()
    GeneralProcess:StartProcess("点燃中...", 5 * Env.GAME_FPS, { self.DoFire, self, it.dwId, tbMember }, nil, tbEvent)
  end
end
...
function tbFemaleItem:CheckCanUse()
  ...
  for _, pNpc in ipairs(tbNpcList) do
    if pNpc.nTemplateId == Tanabata201108.nHongzhuTemplateId or pNpc.nTemplateId == Tanabata201108.nTongxinzhuTemplateId then
      ...
      if
        szFemaleName == me.szName
        and tbMember[1]
        and tbMember[2] --如果蜡烛点完换队伍，则不让使用
        and szMaleName == (tbMember[1].szName ~= me.szName and tbMember[1].szName or tbMember[2].szName)
      then
        pFire = pNpc
        break
      end
...
function tbFemaleItem:OnUse()
  ...
  if bCanUse == 1 then
    GeneralProcess:StartProcess("许愿中...", 5 * Env.GAME_FPS, { self.DoHope, self, it.dwId, pFire, bCouple }, nil, tbEvent)
  end
end
