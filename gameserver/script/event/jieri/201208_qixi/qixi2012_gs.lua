...
function tbQiXi2012:CheckCanAcceptSeed(pPlayer)
  ...
  if nFreeCount < nNeedCount then
    return -2, string.format("背包空间不足,需要<color=yellow>%s个<color>背包空间。", nNeedCount)
  end
  return 1
end
...
function tbQiXi2012:SeedPlant2Rose(dwNpcId, nPlayerId, nFlag)
  ...
  if not nFlag then
    ...
    GeneralProcess:StartProcess("浇灌中", 5 * Env.GAME_FPS, { self.SeedPlant2Rose, self, dwNpcId, nPlayerId, 1 }, nil, tbEvent)
    return
  end
  ...
  if self.OPENSUOXINYU_RANDPET[nLevel] and self.OPENSUOXINYU_RANDPET[nLevel] > 0 then
    ...
    if nRand <= self.OPENSUOXINYU_RANDPET[nLevel] then
      ...
      if pItem then
        ...
        pPlayer.SendMsgToFriend("您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]打开情缘宝物锁心玉获得了【葫小芦·跟宠】。")
