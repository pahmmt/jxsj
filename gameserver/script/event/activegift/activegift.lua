...
function ActiveGift:AddActive(nGrade, nId)
  ...
  if nLastGrade < self.nMaxActiveGrade then
    ...
    me.Msg("获得活跃度<color=yellow>" .. nTotalGrade - nLastGrade .. "<color>") --完成提示
    --log
    ...
    if nTotalGrade >= self.nMaxActiveGrade then
      me.Msg("您当天的活跃度已经到达最高。") --完成提示
    end
  else
    me.Msg("您当天的活跃度已经到达最高不能再累计了。") --完成提示
    return
  end
...
function ActiveGift:GetActiveAward(nId)
  ...
  if self.tbAwardName[nId] then
    me.SendMsgToFriend(string.format("您的好友[<color=yellow>%s<color>]获得1个<color=yellow>%s<color>，当前活跃度<color=yellow>%s<color>点，点击小地图活跃度图标（快捷键J）可以查看自己活跃度。", me.szName, self.tbAwardName[nId], me.GetTask(self.nTaskGroupId, self.nActiveGrade)))
  end
  Dbg:WriteLog("ActiveGift", "获得一个活跃度奖励" .. nId)
end
...
function ActiveGift:GetMonthAward(nId)
  ...
  Dbg:WriteLog("ActiveGift", "获得一个累计天数奖励" .. nId)
end
