...
function tbNpc:OnDialog(nFlag)
  ...
  local tbOpt = { { "离开", self.BuyItem, self }, { "我还想欣赏此番美景" } }
  Dialog:Say(szMsg, tbOpt)
  return
end

function tbNpc:BuyItem(nFlag)
  local szMsg = "正所谓山上容易下山难，要安全离开这里需要老身的神仙绳（15000金币,现打折只需<color=yellow>5000金币<color>），您是否要购买并使用？"
  local tbOpt = { { "购买", self.BuyItem, self, 1 }, { "我再想想" } }
  if not nFlag then
    Dialog:Say(szMsg, tbOpt)
    return
  end
  ...
  me.SendMsgToFriend("您的好友[" .. me.szName .. "]下了灵山顶，开始了崭新的剑侠世界之旅，请周知。")
