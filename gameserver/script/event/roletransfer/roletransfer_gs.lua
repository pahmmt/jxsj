...
function tbRoleTransfer:OnDialog()
  ...
  local szMsg = "两人组队，可以在我这里进行角色转移操作，请确认您要进行转移角色操作吗？\n\n庆新剑侠世界公测，<color=yellow>15000金币<color>出售的角色转移道具<color=yellow>角色转移证<color>和<color=yellow>神仙绳<color>，现均打折只需要<color=yellow>5000金币<color>即可！"
  local tbOpt = {
...
function tbRoleTransfer:ApplyRoleTransfer(nFlag)
  ...
  if not nFlag then
    Dialog:Say("您确认要进行转移吗？\n\n<color=red>  注：转移时将取出钱庄存入的金币以及撤销金币交易所贩卖和收购的金币，转移期间请勿进行金币所交易和钱庄金币存入操作。<color>", { { "确认", self.ApplyRoleTransfer, self, 1 }, { "我再想想" } })
    return
  end
  ...
  me.SendMsgToFriend("您的好友[" .. me.szName .. "]成功申请角色转移。")
...
function tbRoleTransfer:CheckCanTransfer()
  ...
  if Account:GetIntValue(me.szAccount, "OldPlayerBack.ActiveValue") == SpecialEvent.tbOldPlayerBack.nActiveBatch and nNowDate <= SpecialEvent.tbOldPlayerBack.nCloseDate[2] then
    return 0, "您的账号处于老玩家回归中，在此活动期间不能申请转移角色。"
  end
  if SpecialEvent.tbOldPlayerBack.tbOldPlayerInfo[1] and SpecialEvent.tbOldPlayerBack.tbOldPlayerInfo[1][me.szAccount] and SpecialEvent.tbOldPlayerBack.tbOldPlayerInfo[1][me.szAccount][7] == 1 and nNowDate <= SpecialEvent.tbOldPlayerBack.nCloseDate[2] then
    return 0, "您的账号处于老玩家转新服回归中，在此活动期间不能申请转移角色。"
  end
  --组队，必须队长申请
...
function tbRoleTransfer:BuyItem(nFlag)
  if not nFlag then
    Dialog:Say("您没有角色转移资格证<color=yellow>（15000金币,现打折只需5000金币）<color>，请确认购买吗？\n\n<color=yellow>点击确认则直接扣除5000金币进行购买，请再三确认。<color>", { { "购买角色转移资格证", self.BuyItem, self, 1 }, { "取消" } })
    return
  end
...
function tbRoleTransfer:OnPlayerLogIn()
  ...
  for _, tb in pairs(tbFind) do
    ...
    if szTransferAccount ~= "" and szTransferAccount ~= me.szAccount then
      ...
      me.SendMsgToFriend("您的好友[" .. me.szName .. "]角色转移成功。")
...
PlayerEvent:RegisterGlobal("OnLoginOnly", SpecialEvent.tbRoleTransfer.OnLogin_CheckTransferFail, SpecialEvent.tbRoleTransfer)
------------------------------------- 角色转移失败相关处理 --------------------------------------------

PlayerEvent:RegisterGlobal("OnLoginOnly", SpecialEvent.tbRoleTransfer.OnPlayerLogIn, SpecialEvent.tbRoleTransfer)
