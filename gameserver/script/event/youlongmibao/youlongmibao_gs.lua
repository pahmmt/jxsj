...
function Youlongmibao:CheckGameStart(pPlayer)
  ...
  if self:CheckGetAward(pPlayer) == 1 then
    Dialog:SendBlackBoardMsg(pPlayer, "你有奖励尚未领取，不能继续挑战。")
    return 0
  end
  ...
  if self.tbPlayerList[pPlayer.nId] then
    ...
    if nTimes >= self.MAX_TIMES then
      Dialog:SendBlackBoardMsg(pPlayer, "你已经进行了4次游戏，无法再继续了，你可选择重新比武。")
      return 0
    end
  end
...
function Youlongmibao:ShowAward(pPlayer)
  ...
  if bOK ~= 1 then
    Dialog:SendBlackBoardMsg(pPlayer, "你没有奖励可领取。")
    return 0
  end
...
function Youlongmibao:GetAward(pPlayer, nType)
  ...
  if pPlayer.CountFreeBagCell() < 1 then
    Dialog:SendBlackBoardMsg(pPlayer, "你背包满了，放不下，留1格空间再来吧。")
    return 0
  end
  ...
  if nType == 1 then
    ...
    if pItem then
      ...
      if tonumber(tbItem.SystemMsg) and tonumber(tbItem.SystemMsg) == 1 then
        local szMsg = "[<color=yellow>" .. pPlayer.szName .. "<color>]进入游龙密室比武获得了<color=yellow>" .. pItem.szName .. "<color>。"
        KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg)
      end
      if tonumber(tbItem.TongMsg) and tonumber(tbItem.TongMsg) == 1 then
        local szMsg = "<color>]进入游龙密室比武获得了<color=yellow>" .. pItem.szName .. "<color>。"
        Player:SendMsgToKinOrTong(pPlayer, szMsg, 1)
      end
      if tonumber(tbItem.FriendMsg) and tonumber(tbItem.FriendMsg) == 1 then
        local szMsg = "您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]进入游龙密室比武获得了<color=yellow>" .. pItem.szName .. "<color>。"
  ...
  elseif nType == 2 then
    ...
    -- 频道公告
    if tonumber(tbItem.SystemMsg) and tonumber(tbItem.SystemMsg) == 1 then
      local szMsg = "[<color=yellow>" .. pPlayer.szName .. "<color>]进入游龙密室比武获得了<color=yellow>" .. nChangeCoin .. "<color>枚古币。"
      KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg)
    end
    if tonumber(tbItem.TongMsg) and tonumber(tbItem.TongMsg) == 1 then
      local szMsg = "<color>]进入游龙密室比武获得了<color=yellow>" .. nChangeCoin .. "<color>枚古币。"
      Player:SendMsgToKinOrTong(pPlayer, szMsg, 1)
    end
    if tonumber(tbItem.FriendMsg) and tonumber(tbItem.FriendMsg) == 1 then
      local szMsg = "您的好友[<color=yellow>" .. pPlayer.szName .. "<color>]进入游龙密室比武获得了<color=yellow>" .. nChangeCoin .. "<color>枚古币。"
      pPlayer.SendMsgToFriend(szMsg)
    end

    -- 帮助锦囊
    if tonumber(tbItem.Level) >= 5 then
      self:UpdateHelpTable(pPlayer, string.format("%s枚古币", nChangeCoin))
    end
  ...
  -- 非法类型
    ...
    pPlayer.Msg("领奖类型不正确，请删除插件或者第三方工具后重试。")
    return 0
  end
...
function Youlongmibao:UseZhanShu(pPlayer, nUseBindType, G, D, P, L)
  ...
  if nLastBindType and nLastBindType ~= nUseBindType then
    pPlayer.Msg("由于您上次使用的战书和本次使用的战书类型不同，所以必须重新比武。")
...
function Youlongmibao:Continue(pPlayer)
  ...
  if KGblTask.SCGetDbTaskInt(DBTASD_EVENT_YOULONGGESWITCH) == 1 then
    ...
    if nFlag == 0 then
      Dialog:SendBlackBoardMsg(pPlayer, "今天打的够多了，你不累我都累了，还是明天再来吧！")
  ...
  if nCanYoulongCount <= 0 then
    Dialog:SendBlackBoardMsg(pPlayer, string.format("你累积使用游龙战书的次数已用完，请明日再来。"))
    return 0
  end

  if (nDailyAttendTimes > self.NO_TIME_MAX_NUM) and (GetTime() - nInterval < self.MAX_INTERVAL) then
    Dialog:SendBlackBoardMsg(pPlayer, "您太累了，先休息一会儿吧。")
    return 0
  end
  ...
  if nFind <= 0 then
    Dialog:SendBlackBoardMsg(pPlayer, "你身上已经没有战书了，必须拥有战书才能继续比武。")
    return 0
  end
...
function Youlongmibao:Restart(pPlayer)
  -- 有奖励未领
  if self:CheckGetAward(pPlayer) == 1 then
    Dialog:SendBlackBoardMsg(pPlayer, "你有奖励尚未领取，不能重新开始挑战。")
    return 0
  end
...
function Youlongmibao:StartFight(pPlayer)
  ...
  Dialog:SendBlackBoardMsg(pPlayer, string.format("你现在将要进行第%s次挑战。", nTimes))
...
function Youlongmibao:OnChallenge(tbItem)
  ...
  if me.CountFreeBagCell() < (math.ceil(nExCount / 100)) then
    Dialog:Say(string.format("你的背包空间不足，请整理%s格背包空间。", math.ceil(nExCount / 100)))
    return 0
  end
...
function Youlongmibao:OnShengwang(nType, tbItem)
  ...
  if nExCount ~= 1 or nLingCount ~= 1 or nFlag > 0 then
    Dialog:Say("请放入游龙阁声望令及游龙阁声望交换凭证各一个, 同样的声望道具不允许交换。")
    return 0
  end
...
function Youlongmibao:OnChangeCoin(tbItem, nSure)
  ...
  if not nSure then
    local szMsg = string.format("你打算将放入的声望令牌兑换为<color=yellow>%s游龙古币<color>吗？", nValue)
    local tbOpt = {
      { "<color=yellow>确定<color>", self.OnChangeCoin, self, tbItem, 1 },
      { "我再想想" },
    }
...
function Youlongmibao:UpdateHelpTable(pPlayer, szItemName)
  ...
  local szTxt = string.format("<color=cyan>%s\n<color=yellow>%s <color=green>进入游龙密室比武获得了<color=yellow> %s<color>", szDate, pPlayer.szName, szItemName)
...
function Youlongmibao:OnPlayerGetAward(nType)
  ...
  if not tbVaildType[nType] then
    me.Msg("领奖类型不正确，请删除插件或者第三方工具后重试。")
    return
  end
