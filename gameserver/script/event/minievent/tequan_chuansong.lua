...
function tbTequanChuansong:OnUse()
  ...
  table.insert(tbOpt, #tbOpt + 1, { "我再考虑考虑" })
...
function tbTequanChuansong:OnTransItem(tbPosTb, szFrom)
  ...
  for szName, tbPos in next, tbPosTb, szFrom do
    ...
    if nCount <= 0 then
      tbOpt[#tbOpt] = { "下一页", self.OnTransItem, self, tbPosTb, tbOpt[#tbOpt - 1][1] }
      break
    end
...
function tbTequanChuansong:OnShowMapList(tbPosTb, nPage, szFrom)
  ...
  if nPage > 1 then
    tbOpt[#tbOpt + 1] = { "上一页", self.OnShowMapList, self, tbMapList, nPage - 1, szFrom }
  end
  ...
  if nEnd < #tbMapList then
    tbOpt[#tbOpt + 1] = { "下一页", self.OnShowMapList, self, tbMapList, nPage + 1, szFrom }
  end
