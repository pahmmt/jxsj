...
function PlayerLevelUpGift:LoadAwardFile()
  ...
  for nId, tbRow in ipairs(tbData) do
    if nId > 1 then
      ...
      local nValue = tonumber(tbRow.Value) or 0
      if nAwardId > 0 and nClass > 0 and nLevel > 0 then
        ...
        tbOneClassAward.nValue = tbOneClassAward.nValue + nValue
        nTotalValue = nTotalValue + nValue
      end
    end
  end
  self.tbAwardInfo = tbAwardInfo
  self.nAwardTotalValue = nTotalValue
end
...
function PlayerLevelUpGift:GetAwardTotalValue()
  return self.nAwardTotalValue or 0
end
