...
if not MODULE_GAMECLIENT then
  ...
  function HoleSolution:OnSelectMoneyType(szTitle, nType)
    local tbMoneyType = { --nType    szType       nNum
  ...
  function HoleSolution:__CompensateByMoney(nType, nCount)
    ...
    local szMsg = ""
    szMsg = szMsg .. string.format("你确定要扣除<color=yellow>%d%s<color>吗？你将补偿价值量<color=yellow>%d<color>,", nCount, szType, nCompensateValue)
  ...
  function HoleSolution:__DegradeEquip(tbDelEquip, nArrearage)
    ...
    for _, tbEquip in pairs(tbDelEquip) do
      if tbEquip[1].nEnhTimes >= 0 then
        ...
        if tbEquip[1].Regenerate(tbEquip[1].nGenre, tbEquip[1].nDetail, tbEquip[1].nParticular, tbEquip[1].nLevel, tbEquip[1].nSeries, nEnhTimes, tbEquip[1].nLucky, tbEquip[1].GetGenInfo(), 0, tbEquip[1].dwRandSeed, 0) == 1 then --不管之前该装备是不是被改造过，只要到这里就会改造属性都会被剥离
      ...
      for nLevel, nNum in pairs(tbBuChang) do
        for i = 1, nNum do
          ...
          if not pItem then --and nLevel >= 5 then
            KPlayer.SendMail(me.szName, "补偿", "", 0, 0, 1, 18, 1, 114, nLevel)
          end
        end
      end
