...
function NewEPlatForm:GameStart()
  ...
  local szMsg = string.format("家族趣味竞技活动：[%s]开始报名了，请到新手村[晏若雪]处报名", tbMacthLevelCfg.szName)
...
function NewEPlatForm:GamePkStart()
  self.GameState = 2
  local tbMacthLevelCfg = self:GetMacthTypeCfg(self:GetMacthType())
  local szMsg = string.format("趣味活动：[%s]开始了，请参赛选手做好比赛准备。", tbMacthLevelCfg.szName)
  KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg)
  Dialog:GlobalMsg2SubWorld_GS(szMsg)
  self.nMatchTaskId = 0
  self:OpenMission()
