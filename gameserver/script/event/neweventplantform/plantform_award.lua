...
function NewEPlatForm:GetPlayerAward_Month(nFlag)
  ...
  if nRet == 1 then
    if not nFlag then
      Dialog:Say(string.format("您上个月家族趣味竞技总积分<color=yellow>%s分<color>，家族上个月总积分<color=yellow>%s分<color>，可以领取5个家族趣味竞技宝箱，您确定领取？", nSelfGrade, nKinGrade), { { "确定", self.GetPlayerAward_Month, self, 1 }, { "我再想想" } })
      return
    end
  elseif nRet == 0 then
...
function NewEPlatForm:StarCondition()
  ...
  if nIndex > #self.tbCardAward then
    me.Msg("本轮所有奖励已经领取完毕，不可以再次领取。")
    me.CallClientScript({ "UiManager:OpenWindow", "UI_INFOBOARD", "本轮所有奖励已经领取完毕，不可以再次领取。" })
    return 0
  end
  ...
  if tbCostItem then
    ...
    if nCount < tbCostItem[1][5] then
      local tbOpt = { { "胜利果（小）", self.BuyItem, self, 1 }, { "我再想想" } }
      if nIndex == 3 then
        szName = "胜利果（大）"
        tbOpt[1] = { "胜利果（大）", self.BuyItem, self, 2 }
      end
      Dialog:Say("本轮需要<color=yellow>" .. szName .. tbCostItem[1][5] .. "个<color>，您的道具数量不足，是否购买？\n\n<color=green>胜利果（小）：<color><color=yellow>100金币 / 个<color>\n<color=green>胜利果（大）：<color><color=yellow>300金币 / 个", tbOpt)
      return 0
    end
  end
end
...
function NewEPlatForm:BuyItem(nIndex, nFlag)
  ...
  if me.nCoin < tbBuyItem[nIndex][2] then
    Dialog:Say("您的金币不足<color=yellow>" .. tbBuyItem[nIndex][2] .. "<color>！", { { "我知道啦" } })
    return
  end
  if me.CountFreeBagCell() < 1 then
  ...
  if not nFlag then
    Dialog:Say(string.format("您是否愿意花费<color=yellow>%s<color>金币购买一个<color=yellow>%s<color>?", tbBuyItem[nIndex][2], tbName[nIndex]), { { "是的，我愿意", self.BuyItem, self, nIndex, 1 }, { "我再想想" } })
    return
  end
