...
function NewEPlatForm:ClearReadyMap() end
...
function NewEPlatForm:GiveWeleeAwardToPlayer(pPlayer, nRank)
  ...
  local szMatchName = "趣味活动"
  if tbMacth then
    szMatchName = szMatchName .. "：" .. tbMacth.szName
  end
  if nRank > 0 and nRank <= 3 then
    local szMsg = string.format("在刚刚结束的%s活动中获得第<color=yellow>%d<color>名！", szMatchName, nRank)
...
PlayerEvent:RegisterGlobal("OnLoginOnly", NewEPlatForm.OnLogin, NewEPlatForm)
