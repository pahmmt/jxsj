...
function tbChangeLive:Init()
  ...
  for _, tbRow in ipairs(tbInput) do
    ...
    tbMap[tbRow["jsAccDatabase"] .. "_" .. string.upper(tbRow["jsAccount"])][tbRow["jxServerGroup"] .. "_" .. tbRow["jxAccount"]] = { tonumber(tbRow["nAccValG"]), tonumber(tbRow["nAccValS"]) }
  end
...
function tbChangeLive:GetAward(nAccValG, nAccValS, szJxAccDB, szJxAccount)
  ...
  -- make log
  Dbg:WriteLog("SpecialEvent.ChangeLive", "剑网转剑世", me.szAccount, me.szName, "绑定财富：" .. tonumber(nAccValG), "不绑定财富：" .. tonumber(nAccValS), "补偿财富：" .. tonumber(nExtraValG))
...
function tbChangeLive:TransValue(nAccValG, nAccValS)
  ...
  -- STR to INT
  return math.floor(tonumber(nMKP)), math.floor(tonumber(nGTP)), math.floor(tonumber(nMoneyG)), math.floor(tonumber(nMoneyS)), math.floor(tonumber(nCoinG))
end
