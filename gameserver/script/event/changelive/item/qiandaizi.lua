...
function tbQianDaiziItem:OnUse()
  -- 奖励空了，则干掉钱袋子物件
  if me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_COIN_G) <= 0 and me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_MONEY_G) <= 0 and me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_MONEY_S) <= 0 then
  ...
  -- 读玩家身上任务标记
  local szMsg = string.format("钱总是不够用。不过您的钱袋子，目前还有些钱：\n\n" .. "<color=yellow>\t绑定金币：\t%d\n<color>" .. "<color=yellow>\t绑定银两：\t%d\n<color>" .. "<color=yellow>\t银两：\t%d\n<color>", me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_COIN_G), me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_MONEY_G), me.GetTask(SpecialEvent.ChangeLive.TASKGID, SpecialEvent.ChangeLive.TASK_CHANGELIVE_MONEY_S))
