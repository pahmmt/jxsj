...
function EventKind:OnUse()
  ...
  if nDelay > 0 then
    ...
    GeneralProcess:StartProcess("使用中...", nDelay * Env.GAME_FPS, { self.OnUse_Delay, self, me.nId, it.dwId, nUseDel, 1 }, nil, tbEvent)
    return 0
  end
...
function EventKind:InitGenInfo()
  ...
  if #tbItemLiveTimeId <= 0 then
    ...
    for nParam, szParam in ipairs(tbItemLiveTimeId) do
      local tbItemName = EventManager.tbFun:SplitStr(szParam)
      local szGenIdName = tbItemName[1]
      if szGenIdName == string.format("%s,%s,%s,%s", it.nGenre, it.nDetail, it.nParticular, it.nLevel) and tbItemName[2] and tbItemName[2] ~= "" then
        if tonumber(tbItemName[2]) ~= nil then
          if tonumber(tbItemName[2]) > 0 then
            it.SetTimeOut(0, (GetTime() + tonumber(tbItemName[2]) * 60))
          end
...
function EventKind:GetTip()
  ...
  if #tbItemLiveTimeId <= 0 then
    for nParam, szParam in ipairs(tbItemLiveTime) do
      local tbItemName = EventManager.tbFun:SplitStr(szParam)
      local szGenIdName = tbItemName[1]
      if szGenIdName == string.format("%s,%s,%s,%s", it.nGenre, it.nDetail, it.nParticular, it.nLevel) and tbItemName[5] and tbItemName[5] ~= "" then
...
function EventKind:OnDialog(nCheck, nDelayCheck)
  ...
  if nDelayCheck == 0 then
    ...
    if nDelay > 0 then
      ...
      GeneralProcess:StartProcess("请等待...", nDelay * Env.GAME_FPS, { self.OnDialog, self, nCheck, 1 }, nil, tbEvent)
      return 0
    end
  end
