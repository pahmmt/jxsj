...
function Event:Init(tbEvent)
  ...
  Event.tbEventParts = Event.tbEventParts or {}
...
if MODULE_GC_SERVER then
  ...
  function Event:OnTimeDropNpcGC() end
end
-----------GAMEGCSERVER 函数END-----
...
if MODULE_GAMESERVER then
  ...
  function Event:ExeTimer(nEventId, nPartId, tbFun, bGCFlag)
    if not EventManager.EventManager.tbEvent[nEventId] or not EventManager.EventManager.tbEvent[nEventId].tbEventPart or not EventManager.EventManager.tbEvent[nEventId].tbEventPart[nPartId] then
      return 0
    end
