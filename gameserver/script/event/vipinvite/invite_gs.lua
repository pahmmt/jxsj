...
function tbVipInvite:RequireOtherUpLevel(nFlag)
  ...
  if not nFlag then
    Dialog:Say(szMsg, { { "确定", self.RequireOtherUpLevel, self, 1 }, { "我再想想" } })
    return
  end
...
function tbVipInvite:CanInvite(pPlayer, nItemId)
  ...
  if not nHonorRank or nHonorRank <= 0 or nHonorRank > self.nWealthOrder then
    return false, "只有财富荣誉排名前60的的玩家才有邀请资格。"
  end
...
tbVipInvite.tbNotEnoughCoinDialog = {
  "您的金币不足。",
  { { "我要充值", c2s.ApplyOpenOnlinePay, c2s }, { "我知道了" } },
}
function tbVipInvite:DoLevelUp(nDestLevel, nFactionId, bConfirmed, nCoin)
  ...
  if not bConfirmed then
    ...
    Dialog:Say(szMsg, { { "确定", self.DoLevelUp, self, nDestLevel, nFactionId, true, nCoin }, { "我再想想" } })
    return
  end
...
function tbVipInvite:DoBuyEquip()
  ...
  if me.CountFreeBagCell() < 1 then
    Dialog:Say("背包空间不足，请整理出1格空间来再来购买。")
    return
  end
...
PlayerEvent:RegisterGlobal("OnLoginOnly", SpecialEvent.tbVipInvite.SetCanLevelUp_OnLogin, SpecialEvent.tbVipInvite)
