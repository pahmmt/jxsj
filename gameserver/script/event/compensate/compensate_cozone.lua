...
function CompensateCozone:_OpenFreshSubPlayerCompenFlag()
  if self.nLoginRegisterId and self.nLoginRegisterId > 0 then
    PlayerEvent:UnRegister("OnLoginOnly", self.nLoginRegisterId)
    self.nLoginRegisterId = 0
  end
  self.nLoginRegisterId = PlayerEvent:RegisterGlobal("OnLoginOnly", SpecialEvent.CompensateCozone.OnLogin, SpecialEvent.CompensateCozone)
end
