...
function tbDuanWu2011:CheckCanMake(pPlayer, nNum)
  ...
  if pPlayer.CountFreeBagCell() < 1 then
    szErrMsg = "需要<color=yellow>1格<color>背包空间，整理下再来！"
    return 0, szErrMsg
  end
...
function tbDuanWu2011:MakeDumplingDlg(nCount, nSure)
  ...
  if not nSure then
    ...
    local tbOpt = {
      { "确定", self.MakeDumplingDlg, self, nCount, 1 },
      { "我再想想" },
    }
  ...
  GeneralProcess:StartProcess("肉粽加工中", 5 * Env.GAME_FPS, { self.MakeDumpling, self, me.nId, nCount }, nil, tbEvent)
end
