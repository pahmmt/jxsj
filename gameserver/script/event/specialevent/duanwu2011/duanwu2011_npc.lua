...
function tbDuanWu2011:ViewTodayKinMedalsRank()
  ...
  if #self.tbTodayRank > 7 then
    table.insert(tbOpt, { "下一页", tbDuanWu2011.ViewTodayKinMedalsRankNextPage, tbDuanWu2011 })
  end
...
function tbDuanWu2011:ViewYestoryMedalsRank()
  ...
  if #self.tbYestodayRank > 5 then
    table.insert(tbOpt, { "下一页", tbDuanWu2011.ViewYestoryMedalsRankNextPage, tbDuanWu2011 })
  end
  table.insert(tbOpt, { "结束对话" })
  Dialog:Say(szMsg, tbOpt)
end
...
function tbDuanWu2011:CheckViewYestodayMedalsRank()
  ...
  if nDate < self.RANK_OPEN_DAY then
    return -1, "今天是活动第一天，还没有昨日积分可以查看哦。"
  end
...
function tbDuanWu2011:CheckGetMedalAward()
  ...
  if nRet ~= 1 then
    return 0, "您的家族端午忠魂积分没有排在前十名里，没有家族的奖励可以领取。"
  end
  if self.tbAwardRecord[nKinId] then
    return 0, "你们的家族已经领过奖励，今天再接再厉吧！"
  end
  return 1
end
...
function tbDuanWu2011:ChangeDuanWuBelt()
  Dialog:OpenGift("把你的一级炼化白银腰带+忠魂密令（中级）或者一级炼化黄金腰带+忠魂密令（高级）给我，我会为你打造一件更完美的腰带，强化等级不会改变哦!新腰带的强化等级不变，新增加的乃是秘籍技能加1的属性，性价比很高哦!", nil, { tbDuanWu2011.OnChangeDuanWuBelt, tbDuanWu2011 })
end
...
function tbDuanWu2011:OnChangeDuanWuBelt(tbItem)
  ...
  if #tbItem ~= 2 then
    Dialog:Say("给予的物品数量不对，你只要放入你的腰带和忠魂秘令便可以了。")
    return 0
  end
  local tbItemKey = {}
  local nCount = 0
  ...
  elseif self.tbEquipChangeList[tbItemKey[2]] then
    szError = "放入的忠魂密令不对，白银腰带需要放入忠魂秘令（中级），黄金腰带需要忠魂秘令（高级）"
  ...
  local nRet = pEquip.Regenerate(tonumber(tbTargetEquip[1]), tonumber(tbTargetEquip[2]), tonumber(tbTargetEquip[3]), tonumber(tbTargetEquip[4]), pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, nil, 0, pEquip.dwRandSeed, pEquip.nStrengthen)
  if nRet ~= 1 then
    Dbg:WriteLog("tbDuanWu2011", "refine_equip_error")
    return 0
  end
  pEquip.Bind(1)
  me.Msg("恭喜你，升级成功！你的腰带已经成功升级为<color=gold>" .. pEquip.szName .. "<color>")
  return 1
end
