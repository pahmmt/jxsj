...
function tbVnChristmas:OnDialog_Buy(bBuy)
  ...
  if nFlag <= 0 then
    Dialog:Say("我想你还没有资格购买！", { { "我知道了" } })
    return 0
  elseif nFlag >= 2 then
    Dialog:Say("你都买过了，还来找我干什么？", { { "我知道了" } })
    return 0
  end
  if not bBuy then
...
function tbDurk:OnUse()
  ...
  if nFlag == 1 and nNowDate >= SpecialEvent.tbVnChristmas.nChristmasStartTime and nNowDate <= SpecialEvent.tbVnChristmas.nChristmasEndTime and nRate <= self.nRateLimit then
