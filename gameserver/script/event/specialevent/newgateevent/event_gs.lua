...
function tbNewGateEvent:GetAward(nFlag)
  ...
  if not nFlag then
    ...
    if self.nMoney + me.GetBindMoney() > me.GetMaxCarryMoney() then
      Dialog:Say("您的身上的绑定银两即将达到上限，请清理一下身上的绑定银两。", { "我知道了" })
      return 0
    end
...
function tbNewGateEvent:GetStudentBindMoney()
  ...
  if me.GetExtMonthPay() < 100 then
    Dialog:Say("您充值不足100元，不能领取。", { "我知道了" })
    return 0
  end
  if me.CountFreeBagCell() < 2 then
    Dialog:Say("您的背包空间不足，需要2格背包空间。", { "我知道了" })
...
function tbNewGateEvent:GetStudentAwardEx(nType)
  if nType == 1 then
    ...
    if nAwardLevel + 5 > 80 then
      Dialog:Say("您已经没有奖励领取啦", { "我知道了" })
      return 0
    end
    if me.nLevel < nAwardLevel + 5 then
      Dialog:Say(string.format("您等级不足%s级", nAwardLevel + 5), { "我知道了" })
      return 0
    end
  ...
  if nType ~= 3 then
    ...
    if pPlayer then
      if pPlayer.CountFreeBagCell() < 1 then
        Dialog:Say("队友的背包空间不足，需要1格背包空间。", { "我知道了" })
        return 0
      end
      ...
      if pItem then
        ...
        me.SetTask(self.TASK_GROUPID, self.TASK_GRADE + nType - 1, 1)
        pPlayer.Msg("恭喜您获得幸运礼包，该礼包需要交易给您的好友[" .. me.szName .. "]。")
      end
    else
      Dialog:Say("队友不在身边。", { "我知道了" })
      return 0
    end
  end
  me.Msg("恭喜您获得奖励。")
...
function tbNewGateEvent:CheckGetAwardKin(nType)
  if nType == 1 then
    ...
    if me.CountFreeBagCell() < nNeedBag then
      return 0, string.format("您的背包空间不足，需要%s格背包空间。", nNeedBag)
    end
...
function tbNewGateEvent:OnLogin()
  ...
  if me.FindSpeTitle(self.szTitleName) == 0 and nFlag == 1 then
    ...
    Player:SendMsgToKinOrTong(me, "您是江湖新秀获得特殊称号，更有惊喜礼物，详情去礼官处查询。", 1)
    me.Msg("您是江湖新秀获得特殊称号，更有惊喜礼物，详情去礼官处查询。")
  end
end

PlayerEvent:RegisterGlobal("OnLoginOnly", tbNewGateEvent.OnLogin, tbNewGateEvent)
