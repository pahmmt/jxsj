...
function tbNpc:CheckCanZhuFu()
  ...
  local nFlag = Player:CheckTask(ZhouNianQing2011.TASKGID, ZhouNianQing2011.TASK_ZHUFU_DATE, "%Y%m%d", ZhouNianQing2011.TASK_ZHUFUCOUNT, 3)
  if nFlag == 0 then
    return 0, "每天只能祝福3次。"
  end
...
function tbNpc:ConfirmSel(bSendAnnounce, nMsgIndex)
  ...
  -- 记录任务变量
  me.SetTask(ZhouNianQing2011.TASKGID, ZhouNianQing2011.TASK_ZHUFUCOUNT, me.GetTask(ZhouNianQing2011.TASKGID, ZhouNianQing2011.TASK_ZHUFUCOUNT) + 1)
  ...
  if nCount ~= 0 then
    ...
    if tbBonus then -- 中奖了
      local pItem = me.AddItemEx(tbBonus[1], tbBonus[2], tbBonus[3], tbBonus[4], { bForceBind = 1 }, Player.emKITEMLOG_TYPE_JOINEVENT)
