...
function tbOldPlayerBack:OnDialog_New()
  ...
  local tbOpt = {
    { "激活资格", self.ActivatePlayer, self, 1 },
...
function tbOldPlayerBack:Get2NewGateAwardEx(nNum, nFlag, nFlagCoin, nFlagMoney, nType)
  ...
  if nFlag == 0 then
    ...
    local tbOpt = {
      { string.format("<color=%s>领取绑定金币<color>", szCoinColor), self.Get2NewGateAwardEx, self, nNum, 1, nFlagCoin, nFlagMoney, 4 },
  ...
  if nType then
    if me.nLevel < self.tbLevel[nNum][1] then
      Dialog:Say("提升到对应等级再来领奖吧！", { "我知道了" })
      return
    end
    if nType == 4 then
      if nFlagCoin == 0 then
        Dialog:Say("您已经领取过了！", { "我知道了" })
        return
      end
    elseif nType == 5 then
...
function tbOldPlayerBack:GetOldPlayerBack()
  ...
  if bIsActivate == 0 then
    Dialog:Say("您还没有激活资格呢，先去激活资格吧。", { "我知道了" })
    return 0
  end
  local szAccount = string.lower(me.szAccount)
...
function tbOldPlayerBack:GetOldPlayerBackEx(nBindCoin, nBindMoney, nTimecoe, nTimes)
  ...
  if me.CountFreeBagCell() < 2 then
    Dialog:Say("您的背包空间不足，需要2格背包空间。", { "我知道了" })
    return 0
  end
  local nGetBindCoin = math.floor(nBindCoin * self.nReBackCoinRate * nTimes)
  local nConsumeCoin = math.floor(nBindCoin * (1 - self.nReBackCoinRate) * nTimes)
  ...
  --强化领取
  me.AddSkillState(892, 1, 2, 7 * 24 * 3600 * 18, 1, 0, 1)
...
function tbOldPlayerBack:GetOldPlayerBackAwardItemEx(nGen, nDetal, bOK)
  ...
  if nDetal then
    ...
    if not bOK then
      Dialog:Say("你确定选择领取<color=yellow>" .. tbAward[1] .. "<color>", { { "确定", self.GetOldPlayerBackAwardItemEx, self, nGen, nDetal, 1 }, { "我再想想" } })
      return
    end
  end
  if not tbAward then
  ...
  if me.GetTask(unpack(self.tbAward2OldTaskID[nGen])) == 1 then
    Dialog:Say("你已经领取过了。", { "我知道了" })
    return 0
  end
  if tbAward[2] == 2 then
    ...
    local nNeedCount = KItem.GetNeedFreeBag(tbAward[3][1], tbAward[3][2], tbAward[3][3], tbAward[3][4], { bForceBind = 1, bTimeOut = bTimeOut }, tbAward[3][5])
    if me.CountFreeBagCell() < nNeedCount then
      Dialog:Say(string.format("您的背包空间不足，需要%s格背包空间。", nNeedCount), { "我知道了" })
      return 0
    end
  end
  if tbAward[2] == 2 then
    ...
    if not tbAward[4] then
      me.AddStackItem(tbAward[3][1], tbAward[3][2], tbAward[3][3], tbAward[3][4], { bForceBind = 1 }, tbAward[3][5])
    else
      for i = 1, nCount do
  ...
  me.SetTask(self.tbAward2OldTaskID[nGen][1], self.tbAward2OldTaskID[nGen][2], 1)
end
...
function tbOldPlayerBack:ActivateKinAward()
  ...
  if nFlag == 1 then
    Dialog:Say("你已经激活过家族奖励了。", { "我知道了" })
    return
  end
  for i = self.TASK_TASKID_EVENT_XOYO, self.TASK_TASKID_EVENT_XIAKE do
    if me.GetTask(self.TASK_GID, i) < 2 then
      Dialog:Say("你还是先完成任务再来激活吧。", { "我知道了" })
      return
    end
  end
  local dwKinId = me.GetKinMember()
  if dwKinId == 0 then
    Dialog:Say("你还没有家族呢，加了家族才能激活家族奖励。", { "我知道了" })
    return
  end
  if me.nKinFigure > 3 or me.nKinFigure == 0 then
    Dialog:Say("只有正式成员才可以激活家族奖励。", { "我知道了" })
    return
  end
...
function tbOldPlayerBack:GetKinAward(nFlag)
  ...
  if not cKin then
    Dialog:Say("你还没有家族恐怕无法领取奖励吧。", { "我知道了" })
    return 0
  end
  ...
  if (nFigure ~= 1 and nGetCount >= self.nKinAwardMaxCount) or (nFigure == 1 and nGetCount >= self.nKinAwardMaxCount * 2) then
    Dialog:Say("你已经获得足够多了，机会还是留给其他人吧。", { "我知道了" })
    return 0
  end
  ...
  if not nFlag then
    ...
    Dialog:Say(string.format("您现在有%s次家族奖励可以领取。", nCouldGetCount), { { "领取奖励", self.GetKinAward, self, 1 }, "我知道了" })
    return
  end
  ...
  if nType == 1 then
    if self.tbOldPlayerInfo[nType][szAccount] and self.tbOldPlayerInfo[nType][szAccount][7] and self.tbOldPlayerInfo[nType][szAccount][7] == 0 then
      if me.GetRoleCreateDate() <= self.nCreatDateLimit then
        Dialog:Say(string.format("只有%s之后建立的角色才可以激活转新服资格。", os.date("%Y-%m-%d", Lib:GetDate2Time(self.nCreatDateLimit))), { "我知道了" })
        return 0
      end
      self.tbOldPlayerInfo[nType][szAccount][7] = 1
      me.SetTask(self.TASK_GID, self.TASK_TASKID_ACTIVATE_NEW, 1)
      Dialog:Say("恭喜您激活了转新服资格。", { "我知道了" })
      Dialog:SendBlackBoardMsg(me, "恭喜您激活了转新服资格。")
    elseif not self.tbOldPlayerInfo[nType][szAccount] then
      Dialog:Say("你还没有资格激活转新服资格。", { "我知道了" })
      return
    else
      Dialog:Say("该账号下已经有角色激活过转新服的资格了。", { "我知道了" })
      return
    end
  elseif nType == 2 then
    ...
    if tbOldPlayerInfo and tbOldPlayerInfo[5] and tbOldPlayerInfo[5] == 0 then
      if Account:GetIntValue(me.szAccount, "OldPlayerBack.ActiveValue") == self.nActiveBatch then
        Dialog:Say("该账号已经激活过了。", { "我知道了" })
        return
      end
      if me.GetRoleCreateDate() >= self.nRoleCreateDate then
        Dialog:Say(string.format("您的这个角色不是江湖前辈，请使用符合资格的角色来激活吧。\n<color=red>建立角色时间必须在%s之前<color>", os.date("%Y-%m-%d", Lib:GetDate2Time(self.nRoleCreateDate))), { "我知道了" })
        return
      end
      ...
      Dialog:Say("恭喜您激活了江湖前辈资格。", { "我知道了" })
      Dialog:SendBlackBoardMsg(me, "恭喜您激活了江湖前辈资格。")
    elseif not tbOldPlayerInfo then
      Dialog:Say("你还没有资格激活江湖前辈资格。", { "我知道了" })
      return
    else
      Dialog:Say("该账号下已经有角色激活过江湖前辈的资格了。", { "我知道了" })
      return
    end
  end
...
function tbOldPlayerBack:ReadOldPlayerBack_GS()
