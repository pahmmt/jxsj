...
tbOldPlayerBack.tbLevel = { { 1, 10 }, { 30, 10 }, { 50, 10 }, { 69, 10 }, { 79, 10 }, { 89, 10 }, { 99, 10 }, { 109, 30 } } --等级段
tbOldPlayerBack.nAwardState = 11111111 --领取的标志，分别表示上面等级段领取情况
...
tbOldPlayerBack.nCloseDate = { 20130228, 20121120 } --结束时间
tbOldPlayerBack.nRoleCreateDate = 20120901 --老玩家创建角色必须在这之前
tbOldPlayerBack.nCoinNeed = 150 --玩家消耗或者充值最小值必须大于500
tbOldPlayerBack.nKinAwardMaxCount = 10 --最多可获得多少次家族奖励

tbOldPlayerBack.GTASK_BUFF = 13 --buff批次 每次+2,（1作为活动批次，2作为清buff批次）
...
--tbOldPlayerBack.TASK_TASKID_OLD_GUMULIBAO	= 158;	-- 古墓礼包领取(废弃，不可重用原先礼包给的道具有问题，重新统一再给一份，变量换到159)
tbOldPlayerBack.TASK_TASKID_OLD_GUMULIBAO = 159

tbOldPlayerBack.TASK_TASKID_TELL = 121 --上线通知发邮件

tbOldPlayerBack.nCreatDateLimit = 20121011 --转新服限制建号时间

--额外奖励物品奖励
tbOldPlayerBack.tbAward2Old = {
  [1] = { "特殊称号", 1, { 6, 55, 1, 1 } },
  [2] = { "曼佗罗花", 2, { 18, 1, 1782, 1, 200 } },
  [3] = { "古墓情缘礼包", 2, { 18, 1, 1836, 1, 1 }, 30 * 24 * 3600 },
  --[2] = {"西域龙魂包", 2, {21,9,11,1}},
...
-- 额外奖励对应记录的任务变量表
tbOldPlayerBack.tbAward2OldTaskID = {
  [1] = { tbOldPlayerBack.TASK_GID, tbOldPlayerBack.TASK_TASKID_ITEM_OLD_TITLE },
  [2] = { tbOldPlayerBack.TASK_GID, tbOldPlayerBack.TASK_TASKID_ITEM_OLD_BAG },
  [3] = { tbOldPlayerBack.TASK_GID, tbOldPlayerBack.TASK_TASKID_OLD_GUMULIBAO },
}
...
tbOldPlayerBack.tbEquit2New = {
  ["gate1026"] = { { 18, 1, 1206, 1 }, 80 },
  ["gate1027"] = { { 18, 1, 1206, 1 }, 80 },
  ["gate1122"] = { { 18, 1, 1206, 1 }, 80 },
}
