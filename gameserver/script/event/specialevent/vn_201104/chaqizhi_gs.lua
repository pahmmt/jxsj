...
function tbChaQi2011:GatherSeed(dwNpcId, nPlayerId)
  ...
  if pPlayer.CountFreeBagCell() < 1 then
    Setting:SetGlobalObj(pPlayer)
    Dialog:Say("背包空间不足<color=yellow>1格<color>！")
    Setting:RestoreGlobalObj()
    return 0
  end
  ...
  if pPlayer and pPlayer.nMapId == nMapId and self:CheckInExpAround(pPlayer, x, y) == 1 then
    ...
    if tbMemberList then
      for _, pMember in pairs(tbMemberList) do
        if pMember.nId ~= pPlayer.nId and pMember.nMapId == nMapId and self.tbPlantInfo[GetServerId()] and self.tbPlantInfo[GetServerId()][pMember.szName] and self:CheckInExpAround(pMember, x, y) == 1 then -- 同一地图才能获得组队经验
          nTimes = nTimes + 1
        end
      end
    end
