...
function tbItemCreatKin:OnUse()
  ...
  if nKinId <= 0 then
    Dialog:Say("您还没有家族，可以通过该卡片不需要银两及人员要求建立家族，您需要建立家族吗？", { { "建立家族", self.CreatKin, self, it.dwId, me.nId }, { "我再想想" } })
    return 0
  else
    ...
    Dialog:Say("您已经有家族了，可以使用该道具将家族提升为<color=yellow>金牌家族<color>，是否确认？", { { "金牌标志", self.KinLogo, self, it.dwId, me.nId }, { "我再想想" } })
  end
end
...
function tbInviteCard:OnUse(nParam, nPage, tbPlayer)
  ...
  if nPage == 1 and #tbPlayer > self.nPageCount then
    table.insert(tbOpt, #tbOpt - 1, { "下一页", self.OnUse, self, nParam, nPage + 1, tbPlayer })
  elseif nPage > 1 and #tbPlayer - nPage * self.nPageCount < self.nPageCount then
    table.insert(tbOpt, #tbOpt - 1, { "上一页", self.OnUse, self, nParam, nPage - 1, tbPlayer })
  elseif nPage > 1 and #tbPlayer - nPage * self.nPageCount > self.nPageCount then
    table.insert(tbOpt, #tbOpt - 1, { "上一页", self.OnUse, self, nParam, nPage - 1, tbPlayer })
    table.insert(tbOpt, #tbOpt - 1, { "下一页", self.OnUse, self, nParam, nPage + 1, tbPlayer })
  end
...
function tbInviteCard:ViewPlayer(nPlayerId, pNewPlayer)
  ...
  Dialog:Say(szMsg, { { "开始密聊", self.Chat, self, pPlayer, pNewPlayer }, { "我再想想" } })
