...
function tbNpc:GetGobalMatchAward(nSure)
  ...
  if nSure == 0 then
    Dialog:Say(string.format("恭喜您在这次跨服巾帼英雄PK赛中获得<color=yellow>%s<color>，领取奖励吗？", BeautyHero.AWARD_VOTER[nGlobalRankAward - 1].szName), { { "我要领取", self.GetGobalMatchAward, self, 1 }, { "我再想想" } })
...
function tbNpc:GetGobalRestAward(nSure)
  ...
  if nSure == 0 then
    Dialog:Say(string.format("恭喜您在这次跨服巾帼英雄PK赛参与活动获得<color=yellow>%d<color>绑金的奖励，确定领取吗？", nCoin), { { "我要领取", self.GetGobalRestAward, self, 1 }, { "我再想想" } })
    return
  end
...
function tbNpc:GetGobalAwardRepair(nSure)
  ...
  if nSure == 0 then
    Dialog:Say(string.format("很遗憾，由于本服是新开服务器，没有跨服资格，现对武林群芳谱排名前十的玩家给出补偿, 确定领取吗？"), { { "我要领取", self.GetGobalAwardRepair, self, 1 }, { "我再想想" } })
    return
  end
