...
function tbNpc:AttendGirlLogo()
  if me.nSex ~= 1 then
    ...
    Dialog:Say(szMsg, { { "去官网看看", self.GoToURL, self }, { "我再想想" } })
    return 0
  end
  if SpecialEvent.Girl_Vote:IsHaveGirl(me.szName) == 1 then
    ...
    Dialog:Say(szMsg, { { "前往官网补充个人资料", self.GoToURL, self }, { "随便看看（离开）" } })
    return 0
  end
  Dialog:Say("你还<color=red>未报名<color>，请先报名才能参加", { { "我要参加", self.State1SignUp, self }, { "我再想想" } })
  return 0
end
...
function tbNpc:GetAwardDay(nFlag)
  ...
  me.AddSkillState(2578, 1, 2, nTime * 18, 1, 0, 1)
  me.Msg("恭喜你获得人气宝贝称号。")
...
function tbNpc:State1SignUp(nSure)
  ...
  Dialog:Say(szDialogMsg, { { "去官网看看", self.GoToURL, self }, { "我再想想" } })
  return 0
end
