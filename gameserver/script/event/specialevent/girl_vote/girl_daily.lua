...
if MODULE_GAMESERVER then
  ...
  --美女认证
  function tbGirl:OnLogin_GirlTitle()
    if me.nSex ~= Env.SEX_FEMALE then
      return 0
    end
  ...
  PlayerEvent:RegisterGlobal("OnLogin", SpecialEvent.Girl_Vote.OnLogin_GirlTitle, SpecialEvent.Girl_Vote)
end
...
if MODULE_GC_SERVER then
  ...
  function tbGirl:WriteFile_Daily()
    ...
    for szName, tbInfo in pairs(tbBuff) do
      local szOut = string.format("%s\t%s\t%s\t%s\t%s\t%s\n", szGateWay, szName, tbInfo[1], tbInfo[2], os.date("%Y-%m-%d %H:%M:%S", tbInfo[3]), tbInfo[4])
