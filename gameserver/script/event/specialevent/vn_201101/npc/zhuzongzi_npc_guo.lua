...
function tbZongZi:DialogAddMuChai(nPlayerId, nNpcId, nCheck)
  ...
  GeneralProcess:StartProcess("加木柴中", 3 * Env.GAME_FPS, { self.DialogAddMuChai, self, nPlayerId, nNpcId, 1 }, nil, tbEvent)
end
...
function tbZongZi:DialogGetZongZi(nPlayerId, nNpcId, nCheck)
  ...
  if pPlayer.CountFreeBagCell() < 1 then
    Dialog:Say("您的背包空间不足，请清理出<color=yellow>1格<color>背包空间。")
    return 0
  end
  local pNpc = KNpc.GetById(nNpcId)
  if not Npc then
  ...
  GeneralProcess:StartProcess("收获中", 3 * Env.GAME_FPS, { self.DialogGetZongZi, self, nPlayerId, nNpcId, 1 }, nil, tbEvent)
end
