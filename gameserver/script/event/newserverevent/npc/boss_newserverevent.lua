...
function tbBoss:OnDeath()
  ...
  if pNpc then
    ...
    if cKin then
      pNpc.szName = ""
      pNpc.SetTitle(string.format("<color=yellow>%s<color>家族的战利宝箱", cKin.GetName()))
      pNpc.Sync()
    end
  ...
  if me.dwKinId ~= nKinId then
    ...
    if nHasGetCount >= NewServerEvent.nMaxOpenBoxCount then
      ...
      if him.GetTempTable("SpecialEvent").tbHasGetPlayer and him.GetTempTable("SpecialEvent").tbHasGetPlayer[me.nId] == 1 then
        ...
        local tbOpt = {}
        tbOpt[#tbOpt + 1] = { "我要领取", self.GivePrize, self, him.dwId }
        tbOpt[#tbOpt + 1] = { "我再想想" }
...
function tbBox:GivePrize(nNpcId)
  ...
  if me.CountFreeBagCell() < 1 then
    local szMsg = "背包空间不足，请保证留出<color=yellow>1格<color>背包空间！。"
    Dialog:Say(szMsg)
    me.Msg(szMsg)
    return 0
  end
...
function tbNpcEx:OnDialog()
  ...
  Dialog:Say(szMsg, { { "我要领取", self.OnDialogEx, self, him.dwId }, { "我再想想" } })
  return
end
...
function tbNpcEx:OnDialogEx(nId)
  ...
  local nCount = me.GetTask(SpecialEvent.NewServerEvent.nTaskId, SpecialEvent.NewServerEvent.TASK_GETITEM)
  ...
  if pNpc.GetTempTable("Npc").tbGetAwardList[me.szName] then
    Dialog:Say(string.format("您本次奖励已经领取过了，今天还剩余%s次领奖机会。", SpecialEvent.NewServerEvent.nMaxGetItemDay - nCount))
    return 0
  end
  ...
  me.AddExp(me.GetBaseAwardExp() * SpecialEvent.NewServerEvent.nExpAward)
  nCount = nCount + 1
