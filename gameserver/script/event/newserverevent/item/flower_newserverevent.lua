...
function tbFlowerLeaves:OnUse()
  ...
  local szMsg = string.format("您可以消耗精力、活力各<color=yellow>%s<color>点，将它制作成一束<color=yellow>%s<color>。\n\n确定制作么？", NewServerEvent.nMakeFlowerJinghuo, szName)
  local tbOpt = {
    { "确定制作", self.MakeFlower, self, it.dwId },
    { "我再想想" },
  }
...
function tbFlowerLeaves:MakeFlower(nItemId)
  ...
  GeneralProcess:StartProcess("制作中...", 5 * Env.GAME_FPS, { self.DoMake, self, pItem.dwId }, nil, tbEvent)
end
...
function tbFlower:CheckCanUse()
  ...
  for _, pNpc in ipairs(tbNpcList) do
    if pNpc.nKind == 3 or pNpc.nTemplateId == NewServerEvent.nFireTemplateId then
