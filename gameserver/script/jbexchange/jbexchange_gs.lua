...
function JbExchange:AddBill(nPrice, nCount, nType)
  ...
  if not nPrice or not nCount or not nType or 0 == Lib:IsInteger(nPrice) or 0 == Lib:IsInteger(nCount) or 0 == Lib:IsInteger(nType) then
    return
  end
...
if MODULE_GAMESERVER then -- GS专用
  -- 注册事件回调
  PlayerEvent:RegisterGlobal("OnLoginOnly", JbExchange.OnLogin, JbExchange)
end
