...
function tbMission:OnJoin(nGroupId)
  ...
  if self.nPlayState == 0 then
    Dialog:SendBlackBoardMsg(me, "战斗尚未打响，请在此准备！")
  elseif self.nPlayState == 1 then
    Dialog:SendBlackBoardMsg(me, "战斗打响！请前去阻止那些对铁浮城垂涎三尺的敌人吧！")
  end
...
function tbMission:InitGame(nTime, nMaxGroup)
  ...
  for i = 1, nMaxGroup do
    ...
    self.tbMisCfg.tbChannel[i] = { string.format("铁浮城%s频道", i), 20, tbIcon[1], tbIcon[2] }
  end
...
function tbMission:TimerSyncReportData()
  ...
  for _, pPlayer in pairs(tbPlayerList) do
    ...
    if tbPlayer then
      ...
      tbInfo.tbPlayerScore.tbKiller = {
        nCount = tbPlayer[3],
        nScore = tbPlayer[2] - tbPlayer[8] * Newland.OCCUPY_POLE_POINT - tbPlayer[7] * Newland.PROTECT_POINT - tbPlayer[9] * Newland.PLAYER_THRONE_POINT,
      }
...
function tbMission:OnDeath(pKillerNpc)
  ...
  if pKillerPlayer then
    ...
    if pItem and pItem.nLevel >= Newland.MANTLE_LEVEL then
      ...
      -- 披风路线log
      Dbg:WriteLog("Newland", "跨服城战", string.format("击杀者：%s，披风等级：%s，门派：%s，路线：%s", pKillerPlayer.szName, nMantleLevel, pKillerPlayer.nFaction, pKillerPlayer.nRouteId), string.format("被杀者：%s，披风等级：%s，门派：%s，路线：%s", me.szName, pItem.nLevel, me.nFaction, me.nRouteId))

      StatLog:WriteStatLog("stat_info", "newland", "kill", me.nId, me.GetHonorLevel(), pItem.nLevel, me.nFaction, me.nRouteId, pKillerPlayer.szName, pKillerPlayer.GetHonorLevel(), nMantleLevel, pKillerPlayer.nFaction, pKillerPlayer.nRouteId, me.nMapId)
    end
  end
...
function tbMission:OpenRightUI(pPlayer, szTitle, nRemainFrame)
  ...
  local szMsg = string.format("\n<color=green>个人积分：<color=yellow>%s\n<color=green>个人排名：<color=yellow>%s\n<color=green>帮会积分：<color=yellow>%s<color>\n<color=green>帮会排名：<color=yellow>%s<color>\n", 0, 0, 0, 0)
