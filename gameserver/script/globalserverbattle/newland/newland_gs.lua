...
function Newland:OnCaptainSignup_GS()
  ...
  if self:CheckTongCaptain(me) ~= 1 then
    Dialog:Say("对不起，您不是帮会首领，无法代表帮会报名。")
    return 0
  end

  -- 个人是否报过名
  if me.GetTask(self.TASK_GID, self.TASK_SIGNUP) == 1 then
    Dialog:Say("对不起，您已经报过名了。")
    return 0
  end
  ...
  if self:GetSignupCount() >= self.MAX_GROUP then
    Dialog:Say("对不起，报名成功的帮会数量已经达到上限。")
    return 0
  end
...
function Newland:OnMemberSignup_GS(nIndex)
  ...
  local pTong = KTong.GetTong(me.dwTongId)
  if not pTong then
    Dialog:Say("对不起，您还没有加入帮会，无法确认参加跨服城战。")
    return 0
  end
  ...
  if not tbSignup then
    Dialog:Say("对不起，您所在的帮会没有报名，无法确认参加跨服城战。")
    return 0
  end
...
function Newland:StartGame_GS()
  if self.tbMissionGame then
    ...
    self.tbMissionGame:BroadCastMission("战斗打响！请前去阻止那些对铁浮城垂涎三尺的敌人吧！", self.BOTTOM_BLACK_MSG)
  end
...
function Newland:EndGame_GS(nWinGroup)
  if self.tbMissionGame then
    ...
    local szMsg = string.format("<color=yellow>[%s]<color>帮会的<color=green>[%s]<color>成为新的铁浮城城主！", szGroupName, szCaptainName)
...
function Newland:ShowPlayerResult(szPlayerName, nPoint, nSort, nBoxCount, nGroupSort, nGroupPoint)
  ...
  if pPlayer then
    if nPoint >= self.PLAYER_POINT_LIMIT then
      pPlayer.Msg(string.format("恭喜！您在这次城战中获得了<color=yellow>%s积分<color>，在帮会中排<color=yellow>%s名<color>，将获得<color=yellow>%s铁浮城荣耀点<color>（可在铁浮城远征大将处换购卓越战功箱）", nPoint, nSort, nBoxCount))
    else
      pPlayer.Msg(string.format("很遗憾，您在这次城战中获得了<color=yellow>%s积分<color>，未达到<color=yellow>500积分<color>，无法获得铁浮城荣耀点（可在铁浮城远征大将处换购卓越战功箱）", nPoint))
    end
  end
end
...
function Newland:TimerSyncDate_GS()
  if self.tbMissionGame then
    ...
    table.sort(self.tbSortGroup, function(a, b)
      return a.nPoint > b.nPoint
    end)
  ...
  for nNpcDwId, tbInfo in pairs(self.tbPole) do
    ...
    for _, pPlayer in pairs(tbPlayerList or {}) do
      ...
      if nGroupIndex == tbInfo.nOwnerGroup then
        ...
        pPlayer.Msg(string.format("您守卫龙柱有功，获得<color=yellow>%s点<color>个人积分！", self.PROTECT_POINT))
      end
    end
...
function Newland:OnOccupyPole(pPlayer, nNpcDwId)
  ...
  if pPlayer then
    ...
    pPlayer.Msg(string.format("您攻下龙柱，获得<color=yellow>%s点<color>个人积分！", self.OCCUPY_POLE_POINT))
  end
...
function Newland:OnKillPlayer(pKiller, pDied)
  ...
  pKiller.Msg(string.format("您杀敌有功！获得<color=yellow>%s点<color>个人积分！", nPoint))
end
...
function Newland:OnLoseThrone(szPlayerName, nGroupIndex)
  ...
  local szMsg = string.format("<color=yellow>[%s]<color>帮会的<color=green>[%s]<color>失去了王座！", szGroupName, szPlayerName or "未知")
...
function Newland:AddThronePoint()
  ...
  if szPlayerName then
    ...
    if pPlayer then
      ...
      pPlayer.Msg(string.format("您坐在王座上，获得<color=yellow>%s点<color>个人积分！", self.PLAYER_THRONE_POINT))
      ...
      for _, pTmpPlayer in pairs(tbPlayerList) do
        ...
        if nGroupIndex == self.tbThrone.nOwnerGroup and pTmpPlayer.szName ~= szPlayerName then
          ...
          pTmpPlayer.Msg(string.format("您护卫王座有功，获得<color=yellow>%s点<color>个人积分！", self.PROTECT_POINT))
        end
      end
    end
  end
...
function Newland:CheckSellBox(pPlayer)
  ...
  if self:CheckCastleOwner(pPlayer.szName) ~= 1 then
    Dialog:Say("对不起，您不是城主，无法购买城主宝箱。")
    return 0
  end

  if self.tbCastleBuffer.nSellBox <= 0 then
    Dialog:Say("对不起，您没有城主宝箱可以购买！")
    return 0
  end
...
function Newland:BuyCastleBox_GS(nCount, nSure)
  ...
  if not nSure then
    local szMsg = string.format("此次共购买<color=yellow>%s个辉煌战功箱<color>，共花费<color=yellow>%s两<color>跨服绑银，是否确定购买？", nCount, Item:FormatMoney(nCostMoney))
    local tbOpt = {
      { "确定", self.BuyCastleBox_GS, self, nCount, 1 },
      { "我再想想" },
    }
  ...
  if nCurrentMoney < nCostMoney then
    Dialog:Say("<color=yellow>对不起，您的跨服绑银不足，无法兑换奖励。<color><enter><enter><color=green>按Ctrl+G打开奇珍阁，购买跨服活动专用绑银，可增加您的跨服绑银。<color>")
    return 0
  end
...
function Newland:BuyCastleBoxFailed_GS(szPlayerName, nCount)
  ...
  pPlayer.AddWaitGetItemNum(-1)
  pPlayer.Msg("对不起，您没有城主宝箱可以购买！")
end

-- 判断个人奖励
function Newland:CheckSingleAward(pPlayer)
  if self:GetPeriod() ~= self.PERIOD_WAR_REST then
    Dialog:Say("对不起，现在无法购买宝箱。")
    return 0
  end
  ...
  if nLeftBox <= 0 then
    Dialog:Say(string.format("您剩余的铁浮城荣誉点为：<color=yellow>%s<color>，无法继续购买宝箱。", nPoint))
    return 0
  end
...
function Newland:GetSingleAward_GS(nCount, nSure)
  ...
  if not nSure then
    local szMsg = string.format("此次共购买<color=yellow>%s个卓越战功箱<color>，共花费<color=yellow>%s两<color>跨服绑银，是否确定购买？", nCount, Item:FormatMoney(nCostMoney))
    local tbOpt = {
      { "确定", self.GetSingleAward_GS, self, nCount, 1 },
      { "我再想想" },
    }
    Dialog:Say(szMsg, tbOpt)
    return 0
...
function Newland:_ShowPlayerLocalTask()
  ...
  me.Msg("参加届数：" .. me.GetTask(self.TASK_GID, self.TASK_SESSION))
  me.Msg("是否报名：" .. me.GetTask(self.TASK_GID, self.TASK_SIGNUP))
  me.Msg("领取箱子：" .. me.GetTask(self.TASK_GID, self.TASK_WAR_BOX))
  me.Msg("领取经验：" .. me.GetTask(self.TASK_GID, self.TASK_WAR_EXP))
end
...
function Newland:OnCallAtom(pNpc, nGroupIndex, nMapId, pPlayer)
  ...
  local szGroupName = self:GetGroupNameByIndex(nGroupIndex)
  local szMsg = string.format("<color=yellow>[%s]<color>帮会的<color=green>[%s]<color>使用了%s！", szGroupName, pPlayer.szName, pNpc.szName)
  self:BroadCast_GS(szMsg, self.SYSTEM_CHANNEL_MSG)
end
...
function Newland:OnPlayerLogin()
  if self:CheckCastleOwner(me.szName) == 1 and GetTime() - me.GetTask(self.TASK_GID, self.TASK_INTERVAL) > 3600 * Env.GAME_FPS then
    local szMsg = string.format("只见得天际降下一道金光，紫气旖旎东来，闻得晴空中炸起一声闷雷，铁浮城主<color=yellow>%s<color>上线了。", me.szName)
...
if Newland.nEventLoginId then
  PlayerEvent:UnRegisterGlobal("OnLoginOnly", Newland.nEventLoginId)
end
Newland.nEventLoginId = PlayerEvent:RegisterGlobal("OnLoginOnly", Newland.OnPlayerLogin, Newland)
