...
function tbMission:OnDeath(pKillerNpc)
  ...
  if pKillerPlayer then
    ...
    if pItem and pItem.nLevel >= Xkland.MANTLE_LEVEL then
      ...
      -- 披风路线log
      Dbg:WriteLog("Xkland", "跨服城战", string.format("击杀者：%s，披风等级：%s，门派：%s，路线：%s", pKillerPlayer.szName, nMantleLevel, pKillerPlayer.nFaction, pKillerPlayer.nRouteId), string.format("被杀者：%s，披风等级：%s，门派：%s，路线：%s", me.szName, pItem.nLevel, me.nFaction, me.nRouteId))
    end
  end
