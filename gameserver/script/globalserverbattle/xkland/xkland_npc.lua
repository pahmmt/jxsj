...
function tbNpc:SetFreeRevival()
  ...
  local szMsg = string.format(
    ...
    nCastleMoney,
    nRevivalMoney,
    Xkland.MANTLE_TYPE[7],
    tbFreeRevival[7] or 0,
    Xkland.MANTLE_TYPE[8],
    tbFreeRevival[8] or 0,
    Xkland.MANTLE_TYPE[9],
    tbFreeRevival[9] or 0,
    Xkland.MANTLE_TYPE[10],
    tbFreeRevival[10] or 0
  )
...
function tbNpc:ExchangePartnerEq(nStep, nLevel, tbItemObj, tbAddItemInfo)
  ...
  local szMsg, tbOpt = "", {}
  if nStep == 1 then
    szMsg = "打开卓越战功箱和辉煌战功箱有机会获得不同装备的装备碎片，你要用装备碎片换取哪个级别的同伴装备？"
    tbOpt = {
      -- 响应操作，进入第二步
  ...
  elseif nStep == 2 then
    ...
    -- 玩家点击确定按钮进入第三步
    szMsg = "打开卓越战功箱和辉煌战功箱有机会获得不同装备的装备碎片，"
    szMsg = szMsg .. string.format("<color=green>您可以换取%s之刃、%s战衣、%s护身符。", szLevel, szLevel, szLevel)
  ...
  elseif nStep == 4 then
    ...
    elseif nToDelCount < 50 then -- 扣除了不足50个碎片，玩家有亏损，记个LOG
...
function tbNpc:GetPartnerEquipExchangeInfo(tbItemObj, nLevel)
  ...
  -- 如果到这里，说明可以交换
  if szMsg == "" then
    szMsg = string.format("你确定要用50个%s换取<color=red>%s<color>", KItem.GetNameById(18, 1, nParticular, 1), KItem.GetNameById(unpack(self.tbExchangeInfo[nParticular])))
    tbOpt = {
