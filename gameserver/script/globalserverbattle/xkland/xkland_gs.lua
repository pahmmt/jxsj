...
function Xkland:OnCompetitiveBidding_GS(nCoin)
  ...
  -- 城主帮会不允许竞标
  if szTongName ~= "无" and szTongName == self.tbLocalCastleBuffer.szTongName then
...
function Xkland:OnSetMemberAward(nGroupIndex, tbAward)
  ...
  if not tbAward or not tbAward.nAwardCount or type(tbAward.nAwardCount) ~= "number" or not tbAward.nMultiple or type(tbAward.nMultiple) ~= "number" or not tbAward.nForceSend or type(tbAward.nForceSend) ~= "number" or not tbAward.nExtraBox or type(tbAward.nExtraBox) ~= "number" then
  ...
  -- log
  Dbg:WriteLog("Xkland", "跨服城战", me.szAccount, me.szName, string.format("本服领袖设置奖励，军团编号：%s, 人数：%s，倍数：%s，强制发放：%s，额外城主箱子：%s", nGroupIndex, tbAward.nAwardCount, tbAward.nMultiple, tbAward.nForceSend, tbAward.nExtraBox))
end
...
function Xkland:TimerSyncDate_GS()
  if self.tbMissionGame then
    ...
    table.sort(self.tbSortGroup, function(a, b)
      return a.nThronePoint > b.nThronePoint
    end)
