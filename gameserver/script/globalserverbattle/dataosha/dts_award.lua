...
function DaTaoSha:GetAwardForMe(nSure)
  ...
  if nSure == 0 then
    Dialog:Say(szMsg, { { "我要领取", self.GetAwardForMe, self, 1 }, { "我再想想" } })
    return
  end

  if nPlayerCount < nNeedCount then
    Dialog:Say(string.format("您的背包空间不足,至少需要%d格背包空间。", nNeedCount))
    return
  end
  ...
  me.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, string.format("[寒武遗迹]1阶段%s次，2阶段%s次，3阶段%s次，冠军%s次，冠军额外奖励%s次, 领取%s瓷瓶，%s宝瓶，%s雪魂令", tbAwardList[1], tbAwardList[2], tbAwardList[3], tbAwardList[4], tbAwardList[5], tbAwardItem[1], tbAwardItem[2], tbAwardItem[3]))
...
function DaTaoSha:GetGlobalAwardForMe(nSure)
  ...
  if nSure == 0 then
    Dialog:Say(string.format("恭喜您在本次大逃杀活动中获得%d的排名，确定领取奖励吗？", nRank), { { "我要领取", self.GetGlobalAwardForMe, self, 1 }, { "我再想想" } })
    return
  end
