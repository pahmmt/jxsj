...
tbqianghuajuanzhou.szInfo = "交易给您的队友！"
...
function tbqianghuajuanzhou:OnQiangHua(nType, nId)
  ...
  if DaTaoSha:GetPlayerMission(me).nLevel == 1 then
    if pEquip.nEnhTimes < 14 then
      ...
      local szMsg = string.format("你的%s已经成功强化到+%s", pEquip.szName, pEquip.nEnhTimes)
      me.Msg(szMsg)
    else
      local szMsg = string.format("你的%s已经不能强化了！", pEquip.szName)
      me.Msg(szMsg)
    end
  else
    if pEquip.nEnhTimes < 14 then
