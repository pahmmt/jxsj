...
function tbItem:OnUseEx(nItemId)
  ...
  local szMsg = "请选择一个职业来进行这次残酷之旅吧！\n<color=yellow>每名队员只能在下面的五行里选择其中之一，且各个队员选择不能相同，在本次活动中无法更改。<color>"
...
function tbItem:OnSelectSeries(nItemId, nSeries)
  ...
  if not tbSeries[nSeries] then
    local tbOpt = {
      { "选择其他五行", self.OnUseEx, self, nItemId },
      { "结束对话" },
    }
    local szMsg = "该五行属性已经被小队队员选取，请选择其他五行属性。"
...
function tbItem:SelectSeries(nItemId, nSeries)
  ...
  local szMsg = string.format("您分配到了%s,请选择路线", tbFaction.szFactionName)
  local tbOpt = {
