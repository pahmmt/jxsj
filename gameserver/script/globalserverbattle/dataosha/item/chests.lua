...
function tbNpc:OnDialog()
  ...
  GeneralProcess:StartProcess("打开宝箱中...", self.DELAY_TIME * Env.GAME_FPS, { self.OnOpen, self, him.dwId }, nil, tbEvent)
end
  ...
  for _, nPlayerId in pairs(tbPlayerIdList) do
    ...
    if pPlayer then
      if pPlayer.CountFreeBagCell() < 3 then
        local szMsg = string.format("玩家%s的包裹不足，不能打开箱子！", pPlayer.szName)
