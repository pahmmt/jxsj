...
function MissionBase:OnLeave(nGroupId, nState)
  ...
  me.Msg("您和您的队友在寒武遗迹的激战中没能坚持到最后。可与<color=yellow>卖火柴小女孩<color>对话领取奖励！")
  Dialog:SendBlackBoardMsg(me, "您和您的队友在寒武遗迹的激战中没能坚持到最后。请再接再厉。")
...
function MissionBase:OnGameOverCamp(nGroupId)
  ...
  local szMsg2 = "比赛结束，恭喜你们获得了比赛的胜利"
  self:BroadcastBlackBoardMsg(szMsg2)
  local szMsg3 = "恭喜您和您的队友获得本次活动胜利，现在请去<color=yellow>卖火柴小女孩<color>处领取奖励！"
  self:BroadcastMsg(0, szMsg3, "队伍")
  self:SendBattleMsg2All("<color=green>比赛结束，请等待%s<enter>", DaTaoSha.TIME_GAMEOVER)
  self:Rest()
  self:CreateTimer(Env.GAME_FPS * 60 * DaTaoSha.TIME_GAMEOVER, self.EndGame, self)
...
function MissionBase:OnGameOverNoCamp()
  ...
  self.nIsCamp = 0
  self.nIsGamOver = 1
  self:BroadcastBlackBoardMsg("第三阶段结束，没有决出最后的获胜队伍！")
  local szMsg = "第三阶段结束，没有决出最后的获胜队伍，您可与<color=yellow>卖火柴小女孩<color>对话领取奖励！"
  self:BroadcastMsg(0, szMsg, "队伍")
  self:SendBattleMsg2All("<color=green>比赛结束， 等待%s<enter>", DaTaoSha.TIME_GAMEOVER)
  self:Rest()
  self:CreateTimer(Env.GAME_FPS * 60 * DaTaoSha.TIME_GAMEOVER, self.EndGame, self)
  return 0
end
...
function MissionBase:GetReady()
  ...
  self:AddMissionItem()
  self:BroadcastBlackBoardMsg("您有3分钟准备时间，右键点击包裹中门派令牌选择五行职业。")
  self:SendBattleMsg2All("<color=green>准备阶段。<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_RELAX)
...
function MissionBase:Round1Start()
  ...
  self.nRound = 1
  self:BroadcastBlackBoardMsg("第一阶段开始，请与队友配合，在重重危急中生存下来吧。")
  self:SendBattleMsg2All("<color=green>第一阶段进行中<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_ROUND1)
  self:UpdateBattleMsgEx(0)
...
function MissionBase:Round1Relax()
  ...
  self.nUiMsgState = self.nUiMsgState + 1
  self:Rest()
  self:BroadcastBlackBoardMsg("现在进入休息阶段，<color=yellow>出生点处有物资商人<color>提供各种宝物！")
  self:SendBattleMsg2All("<color=green>第一阶段结束，休息一下<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_RELAX)
  self:UpdateBattleMsgEx(0)
  self:ClearNpc(self.tbNpcId)
  self:CallMerchant(1)
end
...
function MissionBase:Round2Start()
  ...
  self:NewField(self.nRound)
  self:BroadcastBlackBoardMsg("第二阶段开始，请与队友配合，在重重危急中生存下来吧。")
  self:SendBattleMsg2All("<color=green>第二阶段进行中<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_ROUND2)
  self:UpdateBattleMsgEx(0)
  self:StartPk()
...
function MissionBase:Round2Relax()
  ...
  self.nUiMsgState = self.nUiMsgState + 1
  self:BroadcastBlackBoardMsg("现在进入休息阶段，<color=yellow>出生点处有物资商人<color>提供各种宝物！")
  self:SendBattleMsg2All("<color=green>第二阶段结束，休息一下<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_RELAX)
  self:UpdateBattleMsgEx(0)
  self:Rest()
  self:ClearNpc(self.tbNpcId)
  self:CallMerchant(2)
...
function MissionBase:Round3Start()
  ...
  self:ClearNpc(self.tbMerchantId)
  self:NewField(self.nRound)
  self:BroadcastBlackBoardMsg("第三阶段开始，请与队友配合，在重重危急中生存下来吧。")
  self:SendBattleMsg2All("<color=green>第三阶段进行中<enter>剩余时间:<color=white>%s<color><enter>", DaTaoSha.TIME_ROUND3)
  self:UpdateBattleMsgEx(0)
  self:StartPk()
  self:WriteLogInfo(2)
end
...
function MissionBase:CallChests()
  ...
  self.nUiMsgState = 1
  self:BroadcastBlackBoardMsg("打开宝箱，所有队员都可以获得一些道具。", 0)
    ...
    if #self:GetPlayerIdList(nId) > 0 then
      ...
      if pNpc then
        ...
        if self.tbGroups[nId].n > 0 then
          ...
          if pPlayer then
            pNpc.szName = pPlayer.szName .. "队伍的物资箱"
          end
        end
...
function MissionBase:SetPlayerState()
  local tbPlayer = self:GetPlayerList()
  for _, pPlayer in pairs(tbPlayer) do
    pPlayer.GetNpc().SetTrickName("神秘人物")
    --只允许近聊
...
function MissionBase:AddMiji(pPlayer, nFactionId, nRoute)
  ...
  --古墓派马牌技能
  if nFactionId == 13 then
    for i = 2894, 2899 do
      pPlayer.AddFightSkill(i, 50)
    end
  end
end
...
function MissionBase:OnDeath(pKillerNpc)
  ...
  self.tbGroups[nGroupId].nLifeCount = self.tbGroups[nGroupId].nLifeCount - 1
  local szMsg = string.format("<color=yellow>%s<color>被击为重伤！", me.szName)
  self:BroadcastMsg(nGroupId, szMsg, "队伍")
  if self.tbGroups[nGroupId].nLifeCount <= 0 then
    local tbPlayer = self:GetPlayerList(nGroupId)
    local nValue = 0
    local nCount = 0
    ...
    if self.nRound <= 2 then
      ...
      for _, pPlayer in pairs(tbPlayer) do
        if DaTaoSha:IsPlayerDeath(pPlayer) == 0 then
          ...
          if nAddCount == 0 then
            pPlayer.Msg("您的背包已满,不能增加寒武符石!")
          end
        end
      end
      KTeam.Msg2Team(pKillerPlayer.nTeamId, string.format("<color=yellow>%s<color>通过击败神秘人物使每个队员获得了%s枚寒武符石,自己获得%s枚寒武符石，但安全区队友无法获得！", pKillerPlayer.szName, DaTaoSha.MIS_KILL_PLAYER_EARN, DaTaoSha.MIS_KILL_PLAYER_EARN_S))
    end
  end
end
...
function MissionBase:OnKillNpc()
  ...
  KTeam.Msg2Team(me.nTeamId, "<color=yellow>" .. me.szName .. "<color>通过击败寒武遗迹机关人使每个队员获得了" .. DaTaoSha.MIS_KILL_NPC_EARN .. "枚寒武符石，但安全区队友无法获得！")
end
...
function MissionBase:RandomFaction(pPlayer, nGroupId, nSeries)
  ...
  for i = 1, 3 do -- 一个系最多三个门派
    ...
    if (DaTaoSha.FACTION[nSeries][i].nSexLimit == -1 or DaTaoSha.FACTION[nSeries][i].nSexLimit == pPlayer.nSex) and (not DaTaoSha.FACTION[nSeries][i].nGateLimit or DaTaoSha.tbGateLimit[Transfer:GetMyGateway(pPlayer)]) then
  ...
  --给队伍提示
  local szMsg = string.format("玩家<color=yellow>%s<color>选择<color=yellow>%s<color>随机到<color=yellow>%s<color>门派。", pPlayer.szName, DaTaoSha.FACTION[nSeries].szSeriesName, DaTaoSha.FACTION[nSeries][tbFaction[nRandom]].szFactionName)
  self:BroadcastMsg(nGroupId, szMsg, "队伍")
  pPlayer.Msg(szMsg)
...
function MissionBase:SelectRoute(pPlayer, nRoute)
  ...
  --给队伍提示
  local szMsg = string.format("玩家<color=yellow>%s<color>选择了路线：<color=yellow>%s<color>", pPlayer.szName, DaTaoSha.FACTION[nSeries][nFaction][nRoute])
  self:BroadcastMsg(nGroupId, szMsg, "队伍")
  return 1
end
...
function MissionBase:SetPlayerDeath(pPlayer)
  local szMsg = "您已尽力，若队友在本阶段存活，您亦可继续挑战下一阶段。"
  pPlayer.Msg(szMsg)
  Dialog:SendBlackBoardMsg(pPlayer, szMsg)
