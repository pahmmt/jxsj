...
function tbNpc:OnBuy120House(nSureFlag)
  ...
  local szMsg = "在雕像树立的这段时间内，你的成就得到了广大民众的认可，累计突破了1500点崇敬度。" .. "特此，授予你120级的特殊坐骑，并扣除你1500点崇敬度。" .. "之后，只要你的崇敬度继续累积到1500点，仍然可以来我处领取120级的特殊坐骑。"
  Dialog:Say(szMsg)
end
...
function tbNpc:AboutStatInfo(nType)
  Dialog:Say(self.tbAboutStatuary[nType], { { "返回上层", self.OnAboutStatInfo, self }, { "结束对话" } })
end
...
function tbNpc:OnAboutGbWllsRepute()
  ...
  local tbOpt = {
    ...
    { "<color=yellow>辉煌战功箱资格兑换卓越战功箱资格<color>", self.OnGiftChip, self },
    { "我来看看" },
  }
...
function tbNpc:OnGiftChip()
  ...
  if nRemainCount <= 0 then
    Dialog:Say("您的兑换辉煌战功宝箱的次数不足，不能兑换！", {
      { "返回上一页", self.OnAboutGbWllsRepute, self },
      { "我来看看" },
    })
    return 0
  end

  Dialog:AskNumber("请输入兑换数量：", nRemainCount, self.OnChangeHuihuang2Zhuoyue, self, 0)
end

function tbNpc:OnChangeHuihuang2Zhuoyue(nFlag, nCount)
  local nRemainCount = me.GetTask(GbWlls.TASKID_GROUP, GbWlls.TASKID_CHENGZHUBOX_NUM)
  if nRemainCount < nCount or nRemainCount <= 0 then
    Dialog:Say("您的兑换辉煌战功宝箱的次数不足，不能兑换！", {
      { "返回上一页", self.OnAboutGbWllsRepute, self },
      { "我来看看" },
    })
  ...
  if not nFlag or nFlag ~= 1 then
    Dialog:Say(string.format("你确定用%s个<color=yellow>辉煌战功箱兑换资格<color>兑换成%s个<color=yellow>卓越战功箱兑换资格<color>吗？注意：兑换成功后卓越战功箱将不得兑换回来。", nCount, nCount * 25), {
      { "确定", self.OnChangeHuihuang2Zhuoyue, self, 1, nCount },
      { "返回上一页", self.OnAboutGbWllsRepute, self },
      { "再考虑考虑" },
    })
    return 0
  ...
  nRemainCount = nRemainCount - nCount
  if nRemainCount < 0 then
    nRemainCount = 0
  end
  me.SetTask(GbWlls.TASKID_GROUP, GbWlls.TASKID_CHENGZHUBOX_NUM, nRemainCount)
  local nZhuoyueRemain = me.GetTask(GbWlls.TASKID_GROUP, GbWlls.TASKID_CHENGZHANBOX_NUM)
  nZhuoyueRemain = nZhuoyueRemain + nCount * 25
  me.SetTask(GbWlls.TASKID_GROUP, GbWlls.TASKID_CHENGZHANBOX_NUM, nZhuoyueRemain)
  Dbg:WriteLogEx(Dbg.LOG_INFO, "GbWlls", "OnChangeHuihuang2Zhuoyue", me.szName, nCount)
  Dialog:Say("兑换成功", {
    { "返回上一页", self.OnAboutGbWllsRepute, self },
    { "我来看看" },
  })
...
function tbNpc:OnOpenGiftOk(nChangeType, tbItemObj)
  ...
  if me.CountFreeBagCell() < nNeedFreeBag then
    me.Msg(string.format("您的背包空间不足%s格，请清理后再来兑换。", nNeedFreeBag))
    return 0
  end
