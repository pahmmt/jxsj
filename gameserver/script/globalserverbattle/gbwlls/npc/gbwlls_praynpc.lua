...
%s
    ...
    szMemberMsg,
    Lib:Transfer4LenDigit2CnNum(nSession),
    szServerName,
    szMacthName,
    nTotal,
    szRate,
...
function tbNpc:OnGetStarServerAward(nSureFlag)
  ...
  local szTime = string.format("%02d/%02d/%02d/%02d/%02d/%02d", tbTime.year, tbTime.month, tbTime.day, 23, 59, 59)
...
function tbNpc:AboutInfo(nIndex)
  Dialog:Say(string.format(self.tbAbout[nIndex]), { { "返回上层", self.About, self }, { "结束对话" } })
end
