...
function GbWlls:GiveLuckCardAward(pPlayer, nNowTime)
  ...
  if nNowDay == nWinDay and nNowDay > 0 then
    ...
    local szKinTongMsg = string.format("为今天参加跨服比赛的有缘人<color=green>%s<color>加油成功，获得祝福奖励<color=green>跨服联赛宝箱<color>。", szGuessName)
    if pPlayer.dwTongId > 0 then
      Player:SendMsgToKinOrTong(pPlayer, szKinTongMsg, 1)
    elseif pPlayer.dwKinId > 0 then
      Player:SendMsgToKinOrTong(pPlayer, szKinTongMsg, 0)
    end
  else
