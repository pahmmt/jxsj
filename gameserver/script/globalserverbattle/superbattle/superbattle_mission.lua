...
function tbMission:OnJoin(nGroupId)
  ...
  if self.nWarState <= SuperBattle.WAR_INIT then
    SuperBattle:SendMessage(me, SuperBattle.MSG_BOTTOM, "战斗尚未打响，请在此准备！")
  else
    SuperBattle:SendMessage(me, SuperBattle.MSG_BOTTOM, "战斗打响！奋勇杀敌吧！")
  end
...
function tbMission:InitGame(nMapId)
  ...
  self.szRightTitle = "<color=white>剩余准备时间：%s<color>"
  ...
  self.tbMisCfg.tbChannel = {
    [1] = { "宋方频道", 20, tbSongIcon[1], tbSongIcon[2] },
    [2] = { "金方频道", 20, tbJinIcon[1], tbJinIcon[2] },
  }
...
function tbMission:State_CampFight()
  ...
  self.szRightTitle = "<color=white>剩余营地争夺时间：%s<color>"
  ...
  local szMsg = "采石矶战场已经开启，现在进入营地争夺阶段。"
...
function tbMission:State_Admiral()
  ...
  self.szRightTitle = "<color=white>剩余将军保卫时间：%s<color>"
  ...
  local szMsg = "双方将领身先士卒，请护卫他们一同杀敌吧。"
  self:BroadCastMission(SuperBattle.MSG_BOTTOM, szMsg)
  self:BroadCastMission(SuperBattle.MSG_CHANNEL, szMsg)
end
...
function tbMission:State_Marshal()
  ...
  self.szRightTitle = "<color=white>剩余元帅时间：%s<color>"
  ...
  local szMsg = "双方元帅亲自出马了！请跟随他们展开最后的决战！"
  self:BroadCastMission(SuperBattle.MSG_BOTTOM, szMsg)
  self:BroadCastMission(SuperBattle.MSG_CHANNEL, szMsg)
end
...
function tbMission:State_EndGame()
  ...
  -- 结束通告
  local szMsg = "在本次采石矶大战中，"
  if nWinner == 0 then
    szMsg = string.format("%s双方战成平局。", szMsg)
  else
    szMsg = string.format("%s%s取得胜利！", SuperBattle:GetCampName(nWinner), szMsg)
  end
...
function tbMission:OnDeath(pKillerNpc)
  ...
  if pKiller then
    ...
    -- 披风路线log
    Dbg:WriteLog("SuperBattle", "跨服宋金", string.format("击杀者：%s，披风等级：%s，门派：%s，路线：%s", pKiller.szName, SuperBattle:GetMantleLevel(pKiller), pKiller.nFaction, pKiller.nRouteId), string.format("被杀者：%s，披风等级：%s，门派：%s，路线：%s", me.szName, nMantleLevel, me.nFaction, me.nRouteId))

    SuperBattle:StatLog("kill", me.nId, SuperBattle:GetSession(), me.GetHonorLevel(), nMantleLevel, me.nFaction, me.nRouteId, pKiller.szName, pKiller.GetHonorLevel(), SuperBattle:GetMantleLevel(pKiller), pKiller.nFaction, pKiller.nRouteId)
    --		end
  end
...
function tbMission:TimerSyncReportData()
  ...
  for i, tbInfo in ipairs(SuperBattle.tbSortPlayer) do
    ...
    local szFaction = pPlayer and Player:GetFactionRouteName(pPlayer.nFaction, pPlayer.nRouteId) or "未知"
    tbPlayerList[i] = {
