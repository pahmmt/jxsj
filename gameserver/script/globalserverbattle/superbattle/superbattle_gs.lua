...
function SuperBattle:StopGame_GS(nWinner)
  ...
  table.sort(self.tbSortPlayer, function(a, b)
    return a.nPoint > b.nPoint
  end)
  ...
  for i, tbInfo in ipairs(self.tbSortPlayer) do
    ...
    local szMsg = string.format("您在本次战场中获得了<color=yellow>%s点<color>积分，排名：<color=yellow>%s<color>，排行点数：<color=yellow>%s<color>，获得经验：<color=yellow>%s分钟<color>基准经验，获得威望：<color=yellow>%s<color>。", tbInfo.nPoint, i, nRst, nExp, nRepute)
...
function SuperBattle:CheckPower(pPlayer)
  ...
  if math.abs(nFightPower - nCurPower) > self.MAX_OFFSET then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "与本服战斗力差距过大")
    return 0
  end
  return 1
end
...
function SuperBattle:CheckSignup_GS(pPlayer)
  ...
  if self:CheckIsSignup() ~= 1 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "跨服宋金还没有开始报名。")
    return 0
  end
  ...
  if pPlayer.nLevel < 100 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你的等级不足，请历练一番再来吧。")
    return 0
  end

  -- 门派限制
  if pPlayer.nFaction <= 0 then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你还没有加入门派，学会武功，如何上阵杀敌。")
    return 0
  end
  ...
  if not pItem or pItem.nLevel < SuperBattle.MANTLE_LEVEL then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你的披风还不够闪亮，贸然前往太危险了。")
    return 0
  end

  -- 参加的场次
  if self:GetAttendCount(pPlayer) >= self.MAX_ATT_DAY then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, string.format("对不起，你今天已经参加满<color=yellow>%s<color>场了，请明天再来吧。", self.MAX_ATT_DAY))
    return 0
  end

  -- 报名间隔
  if GetTime() - pPlayer.GetTask(self.TASK_GID, self.TASK_INTERAL) < self.INTERAL_TIME then
    self:SendMessage(pPlayer, self.MSG_MIDDLE, "对不起，你两次报名的间隔太短了，请等下再来吧。")
    return 0
  end
  ...
  if self:CheckSignup_GS(pPlayer) == 1 then
    ...
    if nSignOk >= 3 and EventManager.IVER_bOpenTiFu ~= 1 then
      if not nSure then
        ...
        local szMsg = string.format("你今天报名成功的次数为<color=yellow>%s次<color>，再次报名需要消耗<color=yellow>%s点<color>江湖威望，确定要报名么？", nSignOk, self.REPUTE_COST)
        local tbOpt = {
          { "<color=yellow>确定<color>", self.SignupBattle_GS, self, pPlayer, 1 },
          { "我再想想" },
        }
...
function SuperBattle:SignupBattleFailed_GS(szPlayerName, nType)
  ...
  if pPlayer then
    local szMsg = ""
    if nType == 1 then
      szMsg = "你所报名的跨服宋金已经开战了，请尽快前往英雄岛参战吧。"
    elseif nType == 2 then
      szMsg = "你已经报过名了，请等候排队开启。"
    end
    self:SendMessage(pPlayer, self.MSG_BOTTOM, szMsg)
    self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
  end
...
function SuperBattle:EnterBattleFailed_GS(szPlayerName)
  local pPlayer = KPlayer.GetPlayerByName(szPlayerName)
  if pPlayer then
    local szMsg = "对不起，你还没有报名，或战场尚未开启，请排队等候。"
    self:SendMessage(pPlayer, self.MSG_BOTTOM, szMsg)
    self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
  end
end
...
function SuperBattle:CancelSignupFailed_GS(szPlayerName, nType)
  ...
  if pPlayer then
    if nType == 1 then
      local szMsg = "你所报名的跨服宋金已经开战了，请尽快前往英雄岛参战吧。"
      self:SendMessage(pPlayer, self.MSG_BOTTOM, szMsg)
      self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
    end
  end
...
function SuperBattle:StartGameSuccess_GS(szPlayerName)
  ...
  if pPlayer then
    ...
    self:SendMessage(pPlayer, self.MSG_BOTTOM, szMsg)
    self:SendMessage(pPlayer, self.MSG_CHANNEL, szMsg)
    pPlayer.AddSkillState(SuperBattle.IN_BUFFER_ID, 1, 2, 45 * 60 * Env.GAME_FPS, 1, 0, 1)
...
function SuperBattle:SelectStateResult_GS(szPlayerName, nType, nOnlyApplyState)
  ...
  if pPlayer then
    ...
    if nOnlyApplyState == 1 then
      ...
      pPlayer.CallClientScript({ "Ui:ServerCall", "UI_NEWCALENDARVIEW", "OnGlobalSongJinUpdate", nOpen, nAttend, nQueue, nType, nTime })
    else
      if nType == 1 then
...
function SuperBattle:GetCampName(nCamp)
  return self.CAMP_NAME[nCamp] or "未知"
end
...
function SuperBattle:OnKillPlayer(pKiller, pDied)
  ...
  self:SendMessage(pKiller, self.MSG_CHANNEL, string.format("您杀敌有功！获得<color=yellow>%s点<color>个人积分！", nPoint))
  ...
  if pKiller.nTeamId then
    ...
    if tbMemberId and nMemberCount > 1 then
      ...
      local szMsg = string.format("您的队友杀敌有功！您获得<color=yellow>%s点<color>共享积分！", nTmpPoint)
  ...
  if self:GetPlayerTypeData(pKiller, "nCurSeriesKill") >= self.BASE_SERIES_KILL then
    ...
    self:SendMessage(pKiller, self.MSG_CHANNEL, string.format("您连续击杀敌人！额外获得<color=yellow>%s点<color>个人积分！", nExtPoint))
  end
end
      ...
      for nNpcDwId, tbInfo in pairs(self.tbPole) do
        ...
        for _, pPlayer in pairs(tbPlayerList or {}) do
          ...
          if tbPlayerData and tbPlayerData.nCamp == tbInfo.nOwner then
            ...
            self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你护卫营地有功，获得了%s点积分。", self.POLE_PROTECT_POINT))
          end
        end
        -- 军团积分
      ...
      for nNpcDwId, tbInfo in pairs(self.tbAdmiral) do
        ...
        for _, pPlayer in pairs(tbPlayerList or {}) do
          ...
          if tbPlayerData and tbPlayerData.nCamp == tbInfo.nCamp then
            ...
            self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你护卫将军有功，获得了%s点积分。", self.ADMIRAL_PROTECT_POINT))
          end
        end
        -- 军团积分
      ...
      for nNpcDwId, tbInfo in pairs(self.tbMarshal) do
        ...
        for _, pPlayer in pairs(tbPlayerList or {}) do
          ...
          if tbPlayerData and tbPlayerData.nCamp == tbInfo.nCamp then
            ...
            self:SendMessage(pPlayer, self.MSG_CHANNEL, string.format("你护卫元帅有功，获得了%s点积分。", self.MARSHAL_PROTECT_POINT))
          end
        end
      end
    end
...
function SuperBattle:AddPole(nNpcId, nMapId, nMapX, nMapY, nOwner, nIndex)
  ...
  if pNpc then
    ...
    pNpc.szName = string.format("%s战旗", self.CAMP_NAME[nOwner] or "")
  end
end
...
function SuperBattle:OccupyPole(pPlayer, nNpcDwId)
  ...
  if tbPole.nOwner == 0 then
    ...
    local pNpc = KNpc.GetById(nNpcDwId)
    if pNpc then
      local szMsg = string.format("%s<color=yellow>%s<color>成功占领了%s！", self:GetCampName(tbPlayerData.nCamp), pPlayer.szName, pNpc.szName)
  ...
  elseif tbPole.nOwner == tbPlayerData.nCamp then
    ...
    local szMsg = string.format("%s<color=yellow>%s<color>向%s发起攻击！", self:GetCampName(tbPlayerData.nCamp), pPlayer.szName, self.POLE_POS[tbPole.nIndex].szName)
    self.tbMissionGame:BroadCastMission(self.MSG_BOTTOM, szMsg)
    self.tbMissionGame:BroadCastMission(self.MSG_CHANNEL, szMsg)
  end
end
...
function SuperBattle:OnBabyDeath(pPlayer, pNpc)
  ...
  if tbPole.nBaby <= 0 then
    local pNpc = KNpc.GetById(nNpcDwId)
    if pNpc then
      local szMsg = string.format("%s<color=yellow>%s<color>成功占领了%s！", self:GetCampName(tbPole.nNext), pPlayer and pPlayer.szName or "神秘人", pNpc.szName)
      self.tbMissionGame:BroadCastMission(self.MSG_BOTTOM, szMsg)
      self.tbMissionGame:BroadCastMission(self.MSG_CHANNEL, szMsg)
      pNpc.Delete()
    end
    ...
    if pPlayer then
      ...
      for _, pTmpPlayer in pairs(tbPlayerList or {}) do
        ...
        if nTmpCamp == nCamp then
          ...
          self:SendMessage(pTmpPlayer, self.MSG_CHANNEL, string.format("您占领营地有功！获得<color=yellow>%s点<color>个人积分！", nPoint))
...
function SuperBattle:UpdateNpcState()
  for nNpcDwId, tbInfo in pairs(self.tbPole) do
    ...
    if pNpc then
      ...
      tbInfo.nPic = nPic
      tbInfo.szName = string.format("%s战旗", self.CAMP_NAME[tbInfo.nOwner] or "")
    end
  end
  for nNpcDwId, tbInfo in pairs(self.tbAdmiral) do
    local pNpc = KNpc.GetById(nNpcDwId)
    if pNpc then
      ...
      tbInfo.nPic = nPic
      tbInfo.szName = string.format("%s将军", self.CAMP_NAME[tbInfo.nCamp] or "")
    end
  end
  for nNpcDwId, tbInfo in pairs(self.tbMarshal) do
    local pNpc = KNpc.GetById(nNpcDwId)
    if pNpc then
      ...
      tbInfo.nPic = nPic
      tbInfo.szName = string.format("%s元帅", self.CAMP_NAME[tbInfo.nCamp] or "")
    end
  end
end
...
function SuperBattle:OpenGameWait(pPlayer)
  ...
  local szMsg = string.format("<color=green>当前排队人数：<color=yellow>%s/%s<color>", 0, self.MIN_QUEUE)
...
function SuperBattle:ShowGameWait(pPlayer, nQueueLen)
  local szMsg = string.format("<color=green>当前排队人数：<color=yellow>%s/%s<color>", nQueueLen, self.MIN_QUEUE)
  Dialog:SendBattleMsg(pPlayer, szMsg, 1)
  Dialog:ShowBattleMsg(pPlayer, 1, 0)
end
...
function c2s:ApplySuperBattleTrans()
  if SuperBattle:CheckIsGlobal() ~= 1 then
    ...
    if szMapClass ~= "village" and szMapClass ~= "city" then
      local szMsg = "只能在城市和新手村进行传送，请自行返回后再来。"
