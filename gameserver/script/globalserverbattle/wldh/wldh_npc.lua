...
function tbNpc:QueryOtherMatch(nGameType, nType, nFlag, szText)
  ...
  if nLen < 4 or nLen > 16 then
    Dialog:Say(string.format("您的%s的字数达不到要求。", szType), { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
    return 0
  end

  --是否允许的单词范围
  if KUnify.IsNameWordPass(szText) ~= 1 then
    Dialog:Say(string.format("您的%s含有非法字符。", szType), { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
    return 0
  end

  --是否包含敏感字串
  if IsNamePass(szText) ~= 1 then
    Dialog:Say(string.format("您的%s含有非法的敏感字符。", szType), { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
    return 0
  end

  if nType == 2 then
    if not League:FindLeague(nLGType, szText) then
      Dialog:Say("您查询的武林大会战队不存在。", { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
      return 0
    end
    --显示战队情况
  ...
  if nType == 1 then
    ...
    if not szLeagueName then
      Dialog:Say("您查找的玩家不在武林大会战队中.", { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
      return 0
    end
...
function tbNpc:QueryLeague(nType, szFindName)
  ...
  if not szLeagueName then
    ...
    if not szLeagueName then
      Dialog:Say("官员：您还没有战队！", { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
      return 0
    end
  end
...
%s
  ...
  Dialog:Say(szMemberMsg, { { "返回上层", self.QueryMatch, self, nType }, { "结束对话" } })
end
