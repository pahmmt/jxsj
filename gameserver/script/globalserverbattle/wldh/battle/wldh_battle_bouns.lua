...
function tbBattle:ProcessSeriesBouns(tbKillerBattleInfo, tbDeathBattleInfo)
  ...
  if 5 >= (nPLRank - nMeRank) then
    ...
    if math.fmod(nSeriesKill, 3) == 0 then
      ...
      tbKillerBattleInfo.pPlayer.Msg(string.format("%s方%s %s连续击退敌人%d名，获得%d积分的连斩奖励。", Wldh.Battle.NAME_CAMP[tbKillerBattleInfo.tbCamp.nCampId], Wldh.Battle.NAME_RANK[tbKillerBattleInfo.nRank], tbKillerBattleInfo.pPlayer.szName, tbKillerBattleInfo.nSeriesKill, self.SERIESKILLBOUNS))
    end
