...
function tbNpc:ViewGroup()
  ...
  for i = 1, #Wldh.Battle.tbGroupIndex do
    ...
    szMsg = szMsg .. string.format("赛场%d：", i) .. "<color=yellow>" .. szLeagueNameSong .. "<color>" .. " vs " .. "<color=yellow>" .. szLeagueNameJin .. "<color>\n"
  end
...
function tbNpc:TransToMap()
  ...
  if tbGroup then
    szMsg = string.format("你的战队今日将在<color=yellow>%d号<color>赛场<color=yellow>%s方<color>比赛。要传送过去么？", tbGroup[1], Wldh.Battle.NAME_CAMP[tbGroup[2]])
...
function tbNpc:QueryLeague(szFindName)
  ...
  if not szLeagueName then
    ...
    if not szLeagueName then
      Dialog:Say("官员：您还没有战队！", { { "返回上层", self.QueryResult, self }, { "结束对话" } })
      return 0
    end
  end
...
%s
  ...
  Dialog:Say(szMemberMsg, { { "返回上层", self.QueryResult, self }, { "结束对话" } })
end
...
function tbNpc:QueryOther(nType, nFlag, szText)
  ...
  if nLen < 4 or nLen > 16 then
    Dialog:Say(string.format("您的%s的字数达不到要求。", szType), { { "返回上层", self.QueryResult, self }, { "结束对话" } })
    return 0
  end

  --是否允许的单词范围
  if KUnify.IsNameWordPass(szText) ~= 1 then
    Dialog:Say(string.format("您的%s含有非法字符。", szType), { { "返回上层", self.QueryResult, self }, { "结束对话" } })
    return 0
  end

  --是否包含敏感字串
  if IsNamePass(szText) ~= 1 then
    Dialog:Say(string.format("您的%s含有非法的敏感字符。", szType), { { "返回上层", self.QueryResult, self }, { "结束对话" } })
    return 0
  end

  if nType == 2 then
    if not League:FindLeague(Wldh.Battle.MATCH_TYPE, szText) then
      Dialog:Say("您查询的武林大会战队不存在。", { { "返回上层", self.QueryResult, self }, { "结束对话" } })
      return 0
    end
  ...
  if nType == 1 then
    ...
    if not szLeagueName then
      Dialog:Say("您查找的玩家不在武林大会战队中。", { { "返回上层", self.QueryResult, self }, { "结束对话" } })
      return 0
    end
