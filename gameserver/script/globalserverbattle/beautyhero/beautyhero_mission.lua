...
function Mission:OnDeath(pKillerNpc)
  ...
  if self:FindAttendPlayer(pKillerPlayer.nId) == 1 and self.nState == BeautyHero.MELEE then -- 混战模式下加分处理
    ...
    tbPlayer[me.nId].nTimerId = Timer:Register(BeautyHero.RETURN_TO_MELEE_TIME[nDeath] * Env.GAME_FPS, self.ReturnToMelee, self, me.nId)
  ...
  if self.nState == BeautyHero.ELIMINATION then
    Timer:Register(BeautyHero.END_DELAY * Env.GAME_FPS, self.AutoRevivePlayer, self, me.nId)
...
function Mission:TimerStart(szFunction)
  ...
  self.nTimerId = Timer:Register(tbTimer[2], self.TimerStart, self, tbTimer[3]) -- 开启新的定时
...
function Mission:UpdateMapPlayerInfo(nPlayerId)
  ...
  if self.nState == BeautyHero.SIGN_UP then
    szTimeFmt = "<color=green>报名剩余时间：<color>"
  ...
  elseif self.nState == BeautyHero.READY_ELIMINATION or self.nState == BeautyHero.MATCH_REST then
    ...
    if BeautyHero.BOX_NUM[nEliminationCount + 1][1] > 2 then
      szN = BeautyHero.BOX_NUM[nEliminationCount + 1][1] .. "强"
    else
      szN = "冠军"
    end
...
function Mission:AssignPlayerToMelee()
  ...
  -- 混战保护时间
  self.nFightTimerId = Timer:Register(BeautyHero.MELEE_PROTECT_TIME * Env.GAME_FPS, self.ChangeFight, self)
  return 1
end
...
function Mission:RestartMelee()
  ...
  -- 混战保护时间
  self.nFightTimerId = Timer:Register(BeautyHero.MELEE_RESTART_PROTECT * Env.GAME_FPS, self.ChangeFight, self)
end
...
function Mission:KickPlayerFromArena(nPlayerId)
  ...
  if nRet ~= 1 then -- 结束该混战区域活动
    ...
    Timer:Register(BeautyHero.END_DELAY * Env.GAME_FPS, self.CloseArena, self, nArenaId)
  end
end
...
function Mission:AssignPlayerToElimination()
  ...
  -- 进入保护时间
  Timer:Register(BeautyHero.ELIMI_PROTECT_TIME * Env.GAME_FPS, self.ChangeFight, self)
end
...
function Mission:EndElimination()
  ...
  local szMsg = ""
  if nIndex > 2 then
    szMsg = string.format("%s强比赛结束，%s强选手已产生，请晋级选手做好准备，比赛将在5分钟后进行", tonumber(nIndex), tonumber(math.ceil(nIndex / 2)))
    self:MsgToMapPlayer(szMsg)
  end
  if self.tbArena then
    for i in pairs(self.tbArena) do
...
-- 空函数~啥都不做
function Mission:EndChampionAward() end
