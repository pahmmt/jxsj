...
function tbNpc:SelectBeauty(nMapId, nCurPage)
  ...
  if nCurEndCount < #tbTmp then
    table.insert(tbOpt, { "下一页", self.SelectBeauty, self, nMapId, nCurPage + 1 })
  end
...
function tbNpc:GetVoteAward(nMapId, bSure)
  ...
  if not bSure or bSure ~= 1 then
    if not GLOBAL_AGENT then
      Dialog:Say(szMsg, { { "我要领取", self.GetVoteAward, self, nMapId, 1 }, { "我再想想" } })
    else
...
function tbNpc:GetMatchAward(nMapId, bSure)
  ...
  if not bSure or bSure ~= 1 then
    if not GLOBAL_AGENT then
      --szMsg = szMsg.."确定领取奖励吗？";
      Dialog:Say(szMsg, { { "我要领取", self.GetMatchAward, self, nMapId, 1 }, { "我再想想" } })
    else
      --szMsg = szMsg.."您可以回本服丁丁处领取这份奖励。";
      Dialog:Say(szMsg)
    end
