...
function BeautyHero:LeaveMap(nMapId, bConfirm)
  ...
  local szMsg = "你确定要离开比赛场吗？离开的话将失去比赛资格。"
  --if GLOBAL_AGENT then
  --	szMsg = szMsg.."如果确定的话，您将回到英雄岛";
  Dialog:Say("你确定要离开比赛场吗？离开的话将失去比赛资格。", {
    { "确定", BeautyHero.LeaveMap, BeautyHero, nMapId, 1 },
    { "我再考虑一下" },
  })
end
...
function BeautyHero:CancelSignUp(nMapId, bConfirm)
  ...
  Dialog:Say("你确定要取消报名？", {
    { "确定", BeautyHero.CancelSignUp, BeautyHero, nMapId, 1 },
    { "我再考虑一下" },
  })
end
