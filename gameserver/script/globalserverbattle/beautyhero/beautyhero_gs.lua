...
function BeautyHero:AwardChampionStart(nSeries, nMapId, nWinnerId)
  local pFlagNpc = KNpc.Add2(self.FLAG_NPC_TAMPLATE_ID, 10, -1, nMapId, self.FLAG_X, self.FLAG_Y)
  local tbTemp = pFlagNpc.GetTempTable("BeautyHero")
  tbTemp.tbFactionData = {}
  tbTemp.tbFactionData.nWinnerId = nWinnerId
  tbTemp.tbFactionData.nFlagTimerId = Timer:Register(self.FLAG_EXIST_TIME * Env.GAME_FPS, self.ExcuteAwardChampion, self, pFlagNpc.dwId, 0)
end
...
function BeautyHero:ExcuteAwardChampion(nNpcId, nPlayerId)
  ...
  if nPlayerId ~= 0 and tbTemp.tbFactionData and tbTemp.tbFactionData.nWinnerId and (tbTemp.tbFactionData.nWinnerId == nPlayerId) then
