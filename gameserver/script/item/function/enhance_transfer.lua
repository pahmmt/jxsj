...
Item.TRANSFER_EQUIP_CLASS = { -- 武器
  [emWeapon] = { Item.EQUIP_MELEE_WEAPON, Item.EQUIP_RANGE_WEAPON },
  -- 防具
  [emAromr] = { Item.EQUIP_ARMOR, Item.EQUIP_BOOTS, Item.EQUIP_BELT, Item.EQUIP_HELM, Item.EQUIP_CUFF },
  -- 饰品
  [emJewelry] = { Item.EQUIP_RING, Item.EQUIP_NECKLACE, Item.EQUIP_AMULET, Item.EQUIP_PENDANT },
}
...
function Item:EnhanceTransfer(pEquip, tbTransferItem, nMoneyType, nParam)
  ...
  --客服log------------
  local szPlayerLog = string.format("玩家: %s，尝试将装备{%s_%d}强化转移至装备{%s_%d}，扣除道具成功{%s}", me.szName, szSrcEquipName, nSrcEquipOldEnh, szDestEquipName, nDestEquipOldEnh, szDelItemLog)
  ...
  -- 清除一次性强化传承状态
  if bHasTransferItem == 1 and (nTransferDiscount == 1 and nOtherRebate == 0) then
    me.RemoveSkillState(2263)
  end
  ...
  if nOldRet == 1 and nNewRet == 1 then
    ...
    if nOldEnhTimes >= 12 then
      ...
      me.SendMsgToFriend("您的好友[" .. me.szName .. "]通过强化转移将" .. szTypeName .. "强化到+" .. szOldEnhanceLevel .. "。")
      Player:SendMsgToKinOrTong(me, "通过强化转移将" .. szTypeName .. "强化到" .. szOldEnhanceLevel .. "。", 0)
    end

    --客服log------------
    local szPlayerLog = string.format("玩家: %s，成功将装备{%s_%d}强化转移至装备{%s_%d}，结果：原装备变成{%s_%d}，目的装备变为{%s_%d}", me.szName, szSrcEquipName, nSrcEquipOldEnh, szDestEquipName, nDestEquipOldEnh, szSrcEquipName, nSrcEquipNewEnh, szDestEquipName, nDestEquipNewEnh)
    --me.PlayerLog(Log.emKITEMLOG_TYPE_USE, szPlayerLog);
    me.ItemLog(pEquip, 0, Log.emKITEMLOG_TYPE_USE, szPlayerLog)
  end
...
function Item:WeaponPeel(pEquip, nParam)
  ...
  if me.CountFreeBagCell() < 1 then
    ...
    Dialog:SendBlackBoardMsg(me, "您的背包空间不足，需要1格背包空间。")
    return 0
  end
