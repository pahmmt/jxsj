    ...
    if tbRefineSetting.m_tbRefine and tbRefineSetting.m_tbRefine[nRefineId] then
      if not nIndex then
        for i, tbProdEquip in pairs(tbRefineSetting.m_tbRefine[nRefineId]) do
          if (tbProdEquip.nSex < 0 or tbProdEquip.nSex == me.nSex) and (tbProdEquip.nFaction < 0 or tbProdEquip.nFaction == me.nFaction) and (tbProdEquip.nAttRoute < 0 or tbProdEquip.nAttRoute == me.GetAttRoute()) and (CompareNumberTable(tbProdEquip.tbEquip, { pEquip.nGenre, pEquip.nDetail, pEquip.nParticular, pEquip.nLevel }) == 1) then
...
function Item:CalcItemCountInTable(tbItemSet, nGenre, nDetail, nPar, nLevel)
  ...
  for _, pItem in pairs(tbItemSet) do
    if pItem.nGenre == nGenre and pItem.nDetail == nDetail and pItem.nParticular == nPar and pItem.nLevel == nLevel then
...
function Item:DoRefine(tbRefineItem, nMoneyTeyp, nIndex)
  ...
  if not tbProduce or #tbProduce ~= 1 or (pEquip.IsExEquip() == 1 and not tbProduce[1].tbEquip[5]) then -- 没产物？产物不只1个？
    me.Msg("您提供的材料不能炼化出新的装备")
    return 0
  end
  ...
  local nRet = pEquip.Regenerate(tbProduce[1].tbEquip[1], tbProduce[1].tbEquip[2], tbProduce[1].tbEquip[3], tbProduce[1].tbEquip[4], pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, nil, 0, pEquip.dwRandSeed, pEquip.nStrengthen)
  if nRet ~= 1 then
