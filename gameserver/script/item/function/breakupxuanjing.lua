...
function tbGift:OnOK(tbParam)
  ...
  while pItem do
    if pItem.szClass ~= self.ITEM_CLASS or pItem.nLevel < self.NEEDLEVEL_MIN or pItem.nLevel > self.NEEDLEVEL_MAX or pItem.IsBind() ~= 1 then
...
function tbGift:OnSwitch(pPickItem, pDropItem, nX, nY)
  ...
  if pDropItem.szClass ~= self.ITEM_CLASS or pDropItem.nLevel < self.NEEDLEVEL_MIN or pDropItem.nLevel > self.NEEDLEVEL_MAX or pDropItem.IsBind() ~= 1 then
