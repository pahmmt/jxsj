...
function Item:LoadSwitchSetting(szFile)
  ...
  for _, tbData in pairs(tbFile) do
    ...
    local szFeature1 = string.format("%s_%s_%s_%s_%s_%s_%s_%s", tbData.kind5, tbData.kind6, self:__AdjustStr(tbData.SuiteID, self.DEFAULT_SETTING), self:__AdjustStr(tbData.bVN, self.DEFAULT_SETTING), self:__AdjustStr(tbData.b2, self.DEFAULT_SETTING), tbData.Category, self:__AdjustStr(tbData.kind4, self.DEFAULT_SETTING), D)

    -- 其它特征（二级特征），内功系/外功系/格斗系_布/皮/甲_五行
    local szFeature2 = string.format("%s_%s_%s", self:__AdjustStr(tbData.kind3, self.DEFAULT_SETTING), self:__AdjustStr(tbData.kind2, self.DEFAULT_SETTING), tbData.kind7)
...
function Item:GetEquipFeatures(szGDPL, nFaction, nRouteId, szVsType, nEquipPos)
  ...
  if nEquipPos == 3 then
    ...
    if tbFeature1[6] ~= tostring(nWeaponCat) then
      szFeature1 = string.format("%s_%s_%s_%s_%s_%s_%s_%s", tbFeature1[1], tbFeature1[2], tbFeature1[3], tbFeature1[4], tbFeature1[5], tostring(nWeaponCat), tbFeature1[7], tonumber(nDstDetail))
    end
  end
...
function Item:IsOtherFeatureMatch(szSrc, szDest)
  ...
  if szSrc1 ~= self.DEFAULT_SETTING and szDest1 ~= self.DEFAULT_SETTING and szSrc1 ~= szDest1 then
    return 0
  end
  ...
  if szSrcAppearance ~= self.DEFAULT_SETTING and szDestAppearance ~= self.DEFAULT_SETTING and szSrcAppearance ~= szDestAppearance then
    return 0
  end
...
if MODULE_GAMESERVER then
  ...
  function Item:ApplySwitchOK(tbSelectInfo)
    ...
    local tbOpt = {
      { "确定", self.SureSwitch, self, me.nId, tbSelectInfo },
      { "取消" },
    }
  ...
  function Item:SureSwitch(nPlayerId, tbSelectInfo)
    ...
    for i = Item.EQUIPPOS_HEAD, Item.EQUIPPOS_PENDANT do
      ...
      if pEquip then
        ...
        local szCurLogMsg = string.format("原装备：%s(%d_%d_%d_%d_%d_%d_%d_%d_%d_%d)", pEquip.szName, pEquip.nGenre, pEquip.nDetail, pEquip.nParticular, pEquip.nLevel, pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, 0, pEquip.dwRandSeed, pEquip.nStrengthen)
        ...
        if tbSelectInfo.tbSwitchInfo[i + 1] == 1 then
          ...
          if tbGDPL then
            ...
            if nRet ~= 1 then
              ...
              -- log格式:装备名字(g_d_p_l_五行_强化次数_幸运值_版本号_随机种子_改造等级)
              szCurLogMsg = szCurLogMsg .. string.format("，新装备：%s(%d_%d_%d_%d_%d_%d_%d_%d_%d_%d)", pEquip.szName, pEquip.nGenre, pEquip.nDetail, pEquip.nParticular, pEquip.nLevel, pEquip.nSeries, pEquip.nEnhTimes, pEquip.nLucky, 0, pEquip.dwRandSeed, pEquip.nStrengthen)
            end
          end
        else
  ...
  function Item:IsSelectFactionRoute(nFaction, nRoute)
    if nFaction <= 0 or nFaction > Env.FACTION_NUM then
      return 0
    end
  ...
  function Item:CheckInfoFormat(tbSelectInfo)
    ...
    if tbSelectInfo.nFaction <= 0 or tbSelectInfo.nFaction > Env.FACTION_NUM or tbSelectInfo.nRoute <= 0 or tbSelectInfo.nRoute > 2 then
      return 0
    end
