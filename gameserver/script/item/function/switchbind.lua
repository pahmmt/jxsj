...
Item.tbEquipSwitchBind = {
  [Item.SWITCHBIND_PARTNEREQUIP] = {
    ...
    [Item.SWITCHBIND_UNBIND] = {
      ...
      checktime = {
        Partner.TASK_BIND_PARTNEREQ_GROUPID,
        Partner.TASK_BIND_PARTNEREQ_SUBID,
        Partner.BIND_PARTNERQUIP_MINTIME,
        Partner.BIND_PARTNERQUIP_MAXTIME,
      },
  ...
  [Item.SWITCHBIND_ZHENYUAN] = {
    ...
    [Item.SWITCHBIND_UNBIND] = {
      nCount = 5,
      checktime = {
        Item.tbZhenYuan.TASK_GID_UNBIND,
        Item.tbZhenYuan.TASK_SUBID_UNBIND,
        Item.tbZhenYuan.UNBIND_MIN_TIME,
        Item.tbZhenYuan.UNBIND_MAX_TIME,
      },
  ...
  [Item.SWITCHBIND_STONE] = {
    ...
    [Item.SWITCHBIND_UNBIND] = {
      nCount = 5,
      checktime = {
        Item.tbStone.TASK_GID_UNBIND,
        Item.tbStone.TASK_SUBID_UNBIND,
        Item.tbStone.UNBIND_MIN_TIME,
        Item.tbStone.UNBIND_MAX_TIME,
      },
...
function Item:SwitchBindGift_Trigger(nPlayerId, nOpType, nEquipType)
  ...
  if tbCheckTime then
    ...
    if nApplyTime == 0 or GetTime() - nApplyTime < tbCheckTime[3] or GetTime() - nApplyTime > tbCheckTime[4] then
      me.Msg("尚未到可操作时间，请稍等。")
      return
    end
  end

  local szMsg = string.format("放入你想要%s的%s吧，最多可放入%d个。", self.tbOperateStr[nOpType][1], self.tbEquipSwitchBind[nEquipType].szTip, self.tbEquipSwitchBind[nEquipType][nOpType].nCount)
  Dialog:OpenGift(szMsg, { "Item:SwitchBindGift_Check", nOpType, nEquipType }, { self.SwitchBindGift_OK, self, nOpType, nEquipType })
end
...
function Item:SwitchBindGift_Check(tbGiftSelf, pPickItem, pDropItem, nX, nY, nOpType, nEquipType)
  if pDropItem then
    ...
    if nCount > self.tbEquipSwitchBind[nEquipType][nOpType].nCount - 1 then
      me.Msg(string.format("一次最多只能%s%d件%s的%s！", self.tbOperateStr[nOpType][1], self.tbEquipSwitchBind[nEquipType][nOpType].nCount, self.tbOperateStr[nOpType][2], self.tbEquipSwitchBind[nEquipType].szTip))
      return 0
    end

    local szCheckFun = self.tbEquipSwitchBind[nEquipType].checkfun
    if szCheckFun and pDropItem[szCheckFun]() == 0 or pDropItem.IsBind() ~= self.tbOperateStr[nOpType][3] then
      me.Msg(string.format("只能放入%s的%s！", self.tbOperateStr[nOpType][2], self.tbEquipSwitchBind[nEquipType].szTip))
      return 0
    end
  end
...
function Item:SwitchBindGift_OK(nOpType, nEquipType, tbItemObj)
  if Lib:CountTB(tbItemObj) > self.tbEquipSwitchBind[nEquipType][nOpType].nCount or Lib:CountTB(tbItemObj) <= 0 then
    return
  end
...
function Item:SwitchBindLog(pItem, nOpType)
  if pItem.IsZhenYuan() == 1 and nOpType == self.SWITCHBIND_UNBIND then
    Dbg:WriteLog("真元解绑", me.szName, string.format("真元：%s, 价值量：%d，真元星级：%d_%d_%d_%d", pItem.szName, Item.tbZhenYuan:GetZhenYuanValue(pItem), Item.tbZhenYuan:GetAttribPotential1(pItem), Item.tbZhenYuan:GetAttribPotential2(pItem), Item.tbZhenYuan:GetAttribPotential3(pItem), Item.tbZhenYuan:GetAttribPotential4(pItem)))
  end
end
