...
function tbZhenYuan:Tip_LevelAndExp()
  return string.format("真元等级：%d [%d/%d]\n\n", Item.tbZhenYuan:GetLevel(it), Item.tbZhenYuan:GetCurExp(it), Item.tbZhenYuan:GetNeedExp(Item.tbZhenYuan:GetLevel(it)))
end
...
function tbZhenYuan:InitName()
  ...
  if self:GetEquiped(it) == 1 then
    szName = "【护体真元】" .. Item.tbZhenYuanSetting.tbPartnerToZhenYuan[nParterId].szPartnerName
  else
...
function tbZhenYuan:OnZhenYuanDrop()
  ...
  if MODULE_GAMESERVER then
    ...
    if self:GetEquiped(it) ~= 1 then
      ...
      for i = 1, self.ATTRIB_COUNT do
        -- 加0.5做四舍五入运算
        tbAttrib[i] = self["GetAttrib" .. i .. "Range"](self, it) + math.floor(self:GetAttribMapValue(tbBaseAttrib[i].szName, tbAttribEnhanced[i]) + 0.5)
      end
      Dbg:WriteLog("真元系统", me.szName, "真元护体", string.format("%s_%s_%d_%1.0f_[%d,%d,%d,%d,%d,%d,%d,%d]", it.szName, it.szGUID, self:GetLevel(it), self:GetZhenYuanValue(it), self:GetAttribPotential1(it), tbAttrib[1], self:GetAttribPotential2(it), tbAttrib[2], self:GetAttribPotential3(it), tbAttrib[3], self:GetAttribPotential4(it), tbAttrib[4]))
    end
...
function tbZhenYuan:CalcRefineInfo(pDestItem, pSrcItem, nAttribId, bPreView)
  ...
  if nDestStarLevel >= self.ATTRIB_MAXSTARLEVEL then
    return 0, "该真元指定资质已满，请选择其它属性炼化吧！"
  elseif nDestStarLevel > nStrStarLevel then
    return 0, "用来炼化的真元对应的属性资质的星级不能低于被炼化的真元属性资质的星级"
  end
...
function tbZhenYuan:LiLianCheck(tbGiftSelf, pPickItem, pDropItem, nX, nY)
  ...
  local szContent = string.format("请放入未满级的<color=green>1~%d<color>个真元。<enter><enter>注意：1个真元升1级所需的历练经验，可使下列所有真元均提升1级！所以<color=green>推荐一次放入%d个<color>", self.LILIAN_MAX_COUNT, self.LILIAN_MAX_COUNT)
  if pDropItem then
    if pDropItem.IsZhenYuan() == 0 then
      UiManager:OpenWindow(Ui.UI_INFOBOARD, "只能放入真元！")
      me.Msg("只能放入真元！")
      return 0
    end

    if self:GetLevel(pDropItem) >= self.MAXLEVEL then
      UiManager:OpenWindow(Ui.UI_INFOBOARD, "只能放入未满级真元！")
      me.Msg("只能放入未满级真元！")
      return 0
    end

    if nCount >= self.LILIAN_MAX_COUNT then
      local szMsg = string.format("每次最多只能给%d个真元同时添加经验！", self.LILIAN_MAX_COUNT)
      me.Msg(szMsg)
    ...
    szContent = string.format("请放入未满级的<color=green>1~%d<color>个真元。<enter><enter>每个真元将获得%d分钟的历练经验！", self.LILIAN_MAX_COUNT, nLiLianUsed)
  end

  if pPickItem then
    nCount = nCount - 1
    if nCount <= 0 then
      szContent = string.format("请放入未满级的<color=green>1~%d<color>个真元。<enter><enter>注意：1个真元升1级所需的历练经验，可使下列所有真元均提升1级！所以<color=green>推荐一次放入%d个<color>", self.LILIAN_MAX_COUNT, self.LILIAN_MAX_COUNT)
    else
      local nLiLianTime = me.GetTask(self.EXPSTORE_TASK_MAIN, self.EXPSTORE_TASK_SUB)
      local nLiLianUsed = 0
      local pItem = tbGiftSelf:First()
