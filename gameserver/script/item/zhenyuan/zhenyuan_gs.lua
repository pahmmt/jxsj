...
if MODULE_GAMESERVER then
  ...
  for key, _ in pairs(tbZhenYuan.tbParam) do
    local funSet = function(_self, ...)
      tbZhenYuan.SetParam(_self, key, unpack(arg))
    end
    local funGen = function(_self, ...)
  ...
  function tbZhenYuan:EatExpBook(pItem, tbExpBook)
    ...
    local szMsg = string.format("您给%s使用了%d本同伴经验书，提升至%d级！", pItem.szName, nCount, nNewLevel)
    me.Msg(szMsg)

    -- 等级变化，价值量发生了变化，要求修改排行榜数据
    -- 全局服不应用GUID榜
    if nOldValue ~= self:GetZhenYuanValue(pItem) and IsGlobalServer() == false and (self:GetParam1(pItem) ~= 1 or self:GetEquiped(pItem) == 1) then
    ...
    if nNewLevel / 5 ~= nOrgLevel / 5 then
      ...
      for i = 1, self.ATTRIB_COUNT do
        -- 加0.5做四舍五入运算
        tbAttrib[i] = self["GetAttrib" .. i .. "Range"](self, pItem) + math.floor(self:GetAttribMapValue(tbBaseAttrib[i].szName, tbAttribEnhanced[i]) + 0.5)
      end

      StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanshengji", me.nId, string.format("%s_%s_%d_%1.0f_[%d,%d,%d,%d,%d,%d,%d,%d]", pItem.szName, pItem.szGUID, nNewLevel, self:GetZhenYuanValue(pItem), self:GetAttribPotential1(pItem), tbAttrib[1], self:GetAttribPotential2(pItem), tbAttrib[2], self:GetAttribPotential3(pItem), tbAttrib[3], self:GetAttribPotential4(pItem), tbAttrib[4]))
    end

    StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanjingyan", me.nId, string.format("%s,%s,%d,%d", pItem.szName, pItem.szGUID, self.EXPWAY_EXPBOOK, #tbExpBook * self.EXPTIMES_XIULIAN))
  ...
  function tbZhenYuan:AddExp(pItem, nExpTime, nExpWay, pPlayer)
    ...
    if pItem then
      ...
      local szMsg = string.format("%s增加了%d点经验", pItem.szName, nSumExp)

      if nLevelUp > 0 then
        self:AddLevel(pItem, nLevelUp, pPlayer)
        szMsg = szMsg .. string.format("，等级提升至%d", self:GetLevel(pItem))
      end

      StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanjingyan", pPlayer.nId, string.format("%s,%s,%d,%d", pItem.szName, pItem.szGUID, nExpWay, nExpTime))
    ...
    if nExpWay ~= 0 then
      ...
      if nOrg == self.EXPSTORE_MAX then
        szMsg = "你的累积历练经验已达上限，将不能继续累积！"
      else
        szMsg = string.format("本次获得历练经验%s分钟。", nExpTime, nUseTime, nExpTime - nUseTime)
      end
  ...
  function tbZhenYuan:AddLevel(pItem, nLevelUp, pPlayer)
    ...
    if nNewLevel / 5 ~= nOrgLevel / 5 and nLevelUp ~= 0 then
      ...
      StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanshengji", pPlayer.nId, string.format("%s,%s,%d,%1.0f,%d,%d,%d,%d,%d,%d,%d,%d", pItem.szName, pItem.szGUID, nNewLevel, self:GetZhenYuanValue(pItem), self:GetAttribPotential1(pItem), tbAttrib[1], self:GetAttribPotential2(pItem), tbAttrib[2], self:GetAttribPotential3(pItem), tbAttrib[3], self:GetAttribPotential4(pItem), tbAttrib[4]))
    end
  ...
  function tbZhenYuan:Generate(varPartner, nParam1)
    ...
    StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanhuode", me.nId, string.format("%s,%s,%d,%1.0f,%d,%d,%d,%d,%d,%d,%d,%d", pItem.szName, pItem.szGUID, 1, self:GetZhenYuanValue(pItem), self:GetAttribPotential1(pItem), tbAttrib[1], self:GetAttribPotential2(pItem), tbAttrib[2], self:GetAttribPotential3(pItem), tbAttrib[3], self:GetAttribPotential4(pItem), tbAttrib[4]))
  ...
  function tbZhenYuan:Refine(pDestItem, pSrcItem, nAttriId, bSure)
    ...
    if bSure == 0 then
      --[[
		...
		if nLeftLevel == self.REFINE_MAXLEVELUP and nBalanceValue > 0 or
			...
			local tbOpt = 
			{
				{"确定", self.Refine, self, pDestItem, pSrcItem, nAttriId, 1},
				{"取消"},
			}
    ...
    -- 重新生成，计算相关属性
    pDestItem.Regenerate(pDestItem.nGenre, pDestItem.nDetail, pDestItem.nParticular, pDestItem.nLevel, pDestItem.nSeries, pDestItem.nEnhTimes, pDestItem.nLucky, pDestItem.GetGenInfo(), pDestItem.nVersion, pDestItem.dwRandSeed, pDestItem.nStrengthen)
    ...
    if nStarLevelAdded > 0 then
      me.Msg(string.format("恭喜！%s炼化成功！%s资质提升了%0.1f星！该真元等级降为%d级", pDestItem.szName, self:GetAttribTipName(pDestItem.GetBaseAttrib()[nDestAttribIndex].szName), nStarLevelAdded / 2, nRandom))
      me.Msg(string.format("%s已经被炼化入主真元中！", szSrcName))

      StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanlianhua", me.nId, string.format("%s,%s,%d,%d,%s,%s,%d,%d", pDestItem.szName, pDestItem.szGUID, nAttriId, nOrgStarLevel, szSrcName, szSrcGuid, nSrcAttribStar, nSrcLevel))
      ...
      for i = 1, self.ATTRIB_COUNT do
        -- 加0.5做四舍五入运算
        tbAttrib[i] = self["GetAttrib" .. i .. "Range"](self, pDestItem) + math.floor(self:GetAttribMapValue(tbBaseAttrib[i].szName, tbAttribEnhanced[i]) + 0.5)
      end

      StatLog:WriteStatLog("stat_info", "zhenyuan", "zhenyuanlianhuaresult", me.nId, string.format("%s,%s,%d,%1.0f,%d,%d,%d,%d,%d,%d,%d,%d", pDestItem.szName, pDestItem.szGUID, self:GetLevel(pDestItem), self:GetZhenYuanValue(pDestItem), self:GetAttribPotential1(pDestItem), tbAttrib[1], self:GetAttribPotential2(pDestItem), tbAttrib[2], self:GetAttribPotential3(pDestItem), tbAttrib[3], self:GetAttribPotential4(pDestItem), tbAttrib[4]))
      ...
      Dbg:WriteLog("真元系统", me.szName, "真元炼化护体情况", string.format("炼化前[pDestItem(%s_%s):%d, pSrcItem(%s_%s):%d],炼化后[pDestItem(%s_%s):%d]", pDestItem.szName, pDestItem.szGUID, bDeseEquiped, szSrcName, szSrcGuid, bSrcEquiped, pDestItem.szName, pDestItem.szGUID, self:GetEquiped(pDestItem)))
  ...
  function tbZhenYuan:OnClientUseLilianExp(dwId, nType)
    local szMsg = string.format("请放入未满级的<color=green>1~%d<color>个真元。<enter><enter>注意：1个真元升1级所需的历练经验，可使下列所有真元均提升1级！所以<color=green>推荐一次放入%d个<color>", self.LILIAN_MAX_COUNT, self.LILIAN_MAX_COUNT)
  ...
  if tbZhenYuan.nStartId then
    PlayerEvent:UnRegisterGlobal("OnLoginOnly", tbZhenYuan.nStartId)
    tbZhenYuan.nStartId = nil
  end
  tbZhenYuan.nStartId = PlayerEvent:RegisterGlobal("OnLoginOnly", Item.tbZhenYuan.OnLogin, Item.tbZhenYuan)
  ...
  function tbZhenYuan:PlayerApplyUnBind(nPlayerId)
    ...
    if pPlayer.IsAccountLock() == 1 then
      pPlayer.Msg("您的账号处于锁定状态，不可以进行该操作，请先解除锁定！")
      return
    end
    ...
    if nApplyTime == 0 or GetTime() - nApplyTime < 0 or GetTime() - nApplyTime > self.UNBIND_MAX_TIME then
      ...
      pPlayer.AddSkillState(self.UNBIND_BUFF_SKILLID, 1, 2, self.UNBIND_MAX_TIME * Env.GAME_FPS, 1, 0, 1)
      Dbg:WriteLog("UnBindZhenYuan", "角色名：" .. pPlayer.szName, "账号名：" .. pPlayer.szAccount, "提交了真元解绑申请")
      pPlayer.Msg("您成功提交了真元解绑申请！")
      pPlayer.PlayerLog(Log.emKPLAYERLOG_TYPE_JOINSPORT, "申请真元解绑")
      pPlayer.CallClientScript({ "Item.tbZhenYuan:UpdateHelpPage" })
    else -- 是取消申请
      local szMsg = ""
      if GetTime() - nApplyTime < self.UNBIND_MIN_TIME then
        szMsg = string.format("您已经提交了解绑申请，还有%0.1f小时申请生效。是否要撤销申请？", (self.UNBIND_MIN_TIME - GetTime() + nApplyTime) / 3600)
      else
        szMsg = "您提交的解绑申请已经开始生效，可以解绑真元了，是否要现在撤销？"
      end

      local tbOpt = {
        { "确定", Item.tbZhenYuan.CancelUnBind, Item.tbZhenYuan, nPlayerId },
        { "我再想想" },
      }
      Dialog:Say(szMsg, tbOpt)
    end
  ...
  function tbZhenYuan:CancelUnBind()
    ...
    Dbg:WriteLog("UnBindZhenYuan", "角色名：" .. pPlayer.szName, "账号名：" .. pPlayer.szAccount, "取消了真元解绑申请")
    pPlayer.Msg("您已经撤消了的真元解绑申请！")
  ...
  function tbZhenYuan:PostUnBind(nCount)
    ...
    Dbg:WriteLog("UnBindZhenYuan", "角色名：" .. me.szName, "账号名：" .. me.szAccount, "成功解绑了" .. nCount .. "个真元")
    me.Msg(string.format("您成功解绑了%s个真元。", nCount))
