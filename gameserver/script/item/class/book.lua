...
function tbBook:GetTip(nState) -- 获取秘籍Tip
  ...
  if tbSetting then
    if it.nLevel == 3 then
      szTip = szTip .. string.format("\n修为：%d/%d", nKarma, tbSetting.m_tbHighLevelKarma[nLevel])
    else
      szTip = szTip .. string.format("\n修为：%d/%d", nKarma, tbSetting.m_tbLevelKarma[nLevel])
    end
  end
  szTip = szTip .. "<color>"
  ...
  for i, nSkill in ipairs(tbSkill) do
    if nSkill > 0 then
      ...
      local tbMsg = { "<color=cyan>1级<color>" }
      ...
      if nMaxLevel > 1 then
        tbMsg = { "\n\n<color=cyan>" .. nMaxLevel .. "级<color>[最高级]" }
