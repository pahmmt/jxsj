...
function tbRefund:OnUse()
  if self:CanUse() ~= 1 then
    --me.Msg("无权使用！"); -- 静默
    Dbg:WriteLog("fanhuan", me.szAccount, me.szName, me.GetPlayerIpAddress(), "以[" .. it.nPrice .. "]" .. IVER_g_szCoinName .. "购买[" .. self.DbgInfo.ItemName[it.nParticular] .. self.DbgInfo.Level[it.nLevel] .. "],但无权使用(物品已扣除)")
    return 1
  end
