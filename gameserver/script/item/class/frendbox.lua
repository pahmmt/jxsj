...
function tbItem:OnUse()
  ...
  if nTempId1 ~= nTempId2 then
    tbOpt = { { "选择女同伴", self.SelectTemp, self, nTempId1, it.dwId }, { "选择男同伴", self.SelectTemp, self, nTempId2, it.dwId }, { "取消" } }
  else
    tbOpt = { { "选取同伴", self.SelectTemp, self, nTempId1, it.dwId }, { "取消" } }
  end
...
function tbItem:SelectTemp(nNpcTempId, nItemId)
  ...
  local tbOpt = {
    ...
    { "力量40%，身法30%，内功30%", self.SelectSeries, self, nNpcTempId, nItemId, 10 },
    { "身法30%，外功20%，内功50%", self.SelectSeries, self, nNpcTempId, nItemId, 11 },
    { "取消" },
  }
  Dialog:Say(szInfo, tbOpt)
  return 0
