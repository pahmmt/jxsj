...
function tbItem:SureUse(nItemId, nFlag)
  ...
  if me.nExtRepState ~= pItem.nLevel - 1 then
    Dialog:Say(string.format("该道具只能为您增加第%s页储物箱，请确保您的储物箱前%s页已经开放。", pItem.nLevel + 1, pItem.nLevel))
    return 1
  end
  if not nFlag then
    local tbOpt = {
    ...
    Dialog:Say(string.format("使用本道具将会增加第%s页储物箱扩展箱分页，但一种类型的储物箱扩展分页道具，<color=red>每个角色只能使用一次<color>，您确定要使用吗？", pItem.nLevel + 1), tbOpt)
    return 1
  end
