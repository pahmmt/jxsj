...
function tbItem:OnUse()
  ...
  elseif nFigure == self.PLAYER_FIGURE_FRESHMAN then
    local szMsg = "    拜师的好处：\n    当今世界处处充满了危机，对于一位新人，最好能找一位资历深厚的前辈，并且拜他为师。这样，不仅可以在你遇到困难的时候有个强而有力的后援，还可以通过向前辈学习，" .. "进一步了解这个世界。在你做任务的时候师傅可以帮你完成，只要2人组队，师傅杀的任务怪你也会计数。另外在你出师之后，你们依然保有师徒名分，更可以在师傅升级后领取师傅精进奖励。"
    Dialog:Say(szMsg)
  end
...
function tbItem:AddBuff4Student()
  ...
  if tbPlayerList then
    for _, pPlayer in ipairs(tbPlayerList) do
      if pPlayer.nLevel < self.nMax_ChuanGong_Level and pPlayer.nLevel < me.nLevel and me.szName == pPlayer.GetTrainingTeacher() then
...
function tbItem:IsStudentAround()
  ...
  if tbPlayerList then
    for _, pPlayer in ipairs(tbPlayerList) do
      if pPlayer.nLevel < self.nMax_ChuanGong_Level and pPlayer.nLevel < me.nLevel and 1 == me.IsTeacherRelation(pPlayer.szName, 1) then
        bIsStudentAround = 1
        break
      end
    end
