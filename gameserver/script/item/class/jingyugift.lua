...
if MODULE_GAMESERVER then
  ...
  function NewPlayerGiftEx:AddAward(nPlayerId, nItemId)
    ...
    if pItemEx then
      ...
      local szMsg = string.format("%s打开%s获得一个%s,真是鸿运当头呀！", pPlayer.szName, szItemName, self.tbHonor[nIndex][2])
      local szKinMsg = string.format("打开%s获得一个%s,真是鸿运当头呀！", szItemName, self.tbHonor[nIndex][2])
      KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, szMsg)
      if pPlayer.dwTongId and pPlayer.dwTongId > 0 then
        Player:SendMsgToKinOrTong(pPlayer, szKinMsg, 1)
      else
