    ...
    for nitem = 1, #self.tbItemList[nkind] do
      ...
      if tbItem and tbItem.nProbability == 0 then
        if self:GetItem(me, tbItem, tbTask, nMustGet) == 0 then
          Dbg:WriteLog("随机获得物品失败", me.szName, string.format("类别：%s，物品序号：%s", nkind, nitem))
          return 1
        else
...
function tbRandomItem:GetItem(pPlayer, tbitem, tbTask, nMustGet)
  ...
  if tbitem.nCoin ~= 0 then
    ...
    if nAddCoin == 1 then
      ...
      Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s绑定%s", tbitem.nCoin, IVER_g_szCoinName))
    else
      Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("绑定%s达到上限,随机获得了%s绑定%s失败", IVER_g_szCoinName, tbitem.nCoin, IVER_g_szCoinName))
    end
  end
  if tbitem.nGenre ~= 0 and tbitem.nDetailType ~= 0 and tbitem.nParticularType ~= 0 then
    ...
    if tbItemInfo.bTimeOut ~= 1 then
      ...
      if nAddCount > 0 then
        local szAnnouce = string.format("恭喜您获得了<color=yellow>%s<color>", szItemName)
        pPlayer.Msg(szAnnouce)
        Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得物品%s", szItemName))
      else
      ...
      for i = 1, nCount do
        ...
        if pItem then
          ...
          pPlayer.Msg(szAnnouce)
          Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得物品%s", pItem.szName))
  ...
  if tbitem.nExp ~= 0 then
    ...
    pPlayer.Msg(szAnnouce)
    Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s经验", tbitem.nExp))
  end
  ...
  if tbitem.nBaseExp ~= 0 then
    ...
    pPlayer.Msg(szAnnouce)
    Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s经验", me.GetBaseAwardExp() * tbitem.nBaseExp))
  end
  ...
  if tbitem.nMKP ~= 0 then
    ...
    pPlayer.Msg(szAnnouce)
    Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s精力", tbitem.nMKP))
  end
  ...
  if tbitem.nGTP ~= 0 then
    ...
    pPlayer.Msg(szAnnouce)
    Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s活力", tbitem.nGTP))
  end
  ...
  if tbitem.varLuaScriptFun and tbitem.varLuaScriptFun ~= "" then
    ...
    elseif varResult and type(varResult) == "table" then
      ...
      if pAddItem then
        -- 产出了石头数据埋点 todo zjq 这里的日志可以去掉，随机宝石不使用这种方式
    ...
    if bSucc == 1 then
      Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s，执行了脚本函数，成功", tbitem.varLuaScriptFun))
    else
      Dbg:WriteLog("随机获得物品", pPlayer.szName, string.format("随机获得了%s，执行了脚本函数，失败", tbitem.varLuaScriptFun))
    end
  end
...
function tbRandomItem:GetItemList()
  ...
  for nLine = 2, nLineCount do
    ...
    if nProbability >= 0 then
      tbClassItemList[nClassParamID].nMaxProp = tbClassItemList[nClassParamID].nMaxProp + nProbability
    end
  end
