...
local tbItem = Item:GetClass("horse_box_base")

tbItem.tbHorse = {
  --{天数，{id，名字}--道具1，{id，名字}--道具2}
  [1] = { 40, { { 1, 12, 27, 4 }, "[坐骑]吉祥虎" }, { { 1, 12, 51, 4 }, "[坐骑]沙漠之风" } },
}
...
function tbItem:OnUse()
  ...
  local tbOpt = {
    { self.tbHorse[nCount][2][2], self.AddMask, self, 2, it.dwId, nType, nTime, nCount },
    { self.tbHorse[nCount][3][2], self.AddMask, self, 3, it.dwId, nType, nTime, nCount },
    { "关闭" },
  }
