...
function tbItem:OnSelectPartner(nIndex, dwId)
  ...
  if Partner:GetSelfStartCount(pPartner) >= self.nLimitStarLevel * 2 then -- 配置表中的星级是实际星级的2倍
    ...
    if nDiffTime < Partner.PEEL_USABLE_MINTIME then
      local szMsg = string.format("您的同伴正在重生申请中，还有%0.1f小时才可以重生%0.1f星以上的同伴", (Partner.PEEL_USABLE_MINTIME - nDiffTime) / 3600, Partner.PEELLIMITSTARLEVEL)
      me.Msg(szMsg)
      me.CallClientScript({ "UiManager:OpenWindow", "UI_INFOBOARD", szMsg })
      return
  ...
  for nLevel = 1, 4 do
    local szItemName = KItem.GetNameById(Partner.tbPartnerJinghua.nGenre, Partner.tbPartnerJinghua.nDetail, Partner.tbPartnerJinghua.nParticular, nLevel)
    szMsg = szMsg .. string.format("<color=yellow>%d个%s<color>\n", tbRetItem[nLevel] or 0, szItemName)
  end

  local tbOpt = {
    { "确定", self.OnConfirmPeel, self, nIndex, dwId },
    { "我再看看" },
  }
...
function tbItem:OnConfirmPeel(nIndex, dwId)
  ...
  szMsg = szMsg .. string.format("%d, %d, %d, %d, %d, %d, %d, %d, %d", nPartnerId, nLevel, nPotentialId, nPotentialRemained, pPartner.GetAttrib(0), pPartner.GetAttrib(1), pPartner.GetAttrib(2), pPartner.GetAttrib(3), nPartnerValue)
