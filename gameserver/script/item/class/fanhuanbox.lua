...
tbFanHuan.tbFanHuanList = {
  ...
  tbChooseList = {
    ...
    { "6000绑金返利", { 18, 1, 1870, 1 }, 1, 1, 30 * 24 * 60 * 60, "绑定金币返还券（6000点）" },
    { "600万绑银返利", { 18, 1, 1352, 3 }, 6, 1, 30 * 24 * 60 * 60, "绑定银两返还券（100万点）" },
  },
}
...
function tbFanHuan:OnUse()
  ...
  local szMsg = string.format("恭喜你即将获得<color=yellow>奇珍阁6000金币道具购买<color>的超值返利券，请选择你需要的返利类型：", tbOneFanHuanBox.szName)

  for i, tbItem in pairs(tbOneFanHuanBox.tbChooseList) do
    local tbOneItem = tbItem[2]
    local szTip = "<item=" .. tbOneItem[1] .. "," .. tbOneItem[2] .. "," .. tbOneItem[3] .. "," .. tbOneItem[4] .. ">"
...
function tbFanHuan:OnGetItem(dwId, nIndex, nCheck)
  ...
  local szFanHuanMsg = "打开玲珑宝盒获得奇珍阁商品购买<color=yellow>6000绑金超值返利券<color>。"
  if 2 == nIndex then
    szFanHuanMsg = "打开玲珑宝盒获得奇珍阁商品购买<color=yellow>600万绑银超值返利券<color>。"
  end

  if me.dwTongId and me.dwTongId > 0 then
    Player:SendMsgToKinOrTong(me, szFanHuanMsg, 1)
  else
    Player:SendMsgToKinOrTong(me, szFanHuanMsg, 0)
  end
