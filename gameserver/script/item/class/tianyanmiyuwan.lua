...
function tbItem:OnUse(nParam)
  ...
  if nRet == 1 then
    ...
    if me.GetTeamId() then
      KTeam.Msg2Team(me.nTeamId, string.format("队友<color=yellow>%s<color>抓取了同伴<color=yellow>%s<color>。", me.szName, szNpcName))
    end
  end
  return 0
end
...
function tbItem:CheckUsable(nParam)
  ...
  elseif MODULE_GAMESERVER then
    ...
    if nTimes >= nCanUseCount and nCanUseCount > 0 then
      me.Msg(string.format("你每天只能使用%s个镶边帛帖。", nCanUseCount))
      return 0
    end
  end
