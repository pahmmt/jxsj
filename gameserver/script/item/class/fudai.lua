...
function tbFuDaiItem:SureUse(nItemId, nFlag)
  ...
  if pPlayer.nLevel >= 50 then
    if pPlayer.GetTask(self.TASK_GROUP_ID, self.TASK_GETEXTRACOUNT_FLAG) < nCoZoneTime and nCoZoneTime + 10 * 24 * 60 * 60 > nNowTime then
  ...
  if nUseCount >= nMaxUse and nFlag ~= 2 then
    if nExCount > 0 then
      local szMsg = string.format("你今天的福袋开启次数已经用完，但你还有<color=yellow>%s<color>次赠送的开启福袋机会，是否要使用赠送机会开启？", nExCount)
      local tbOpt = {
        { "我确定要使用", self.SureUse, self, nItemId, 2 },
        { "我再考虑考虑" },
      }
    ...
    if not nFlag then
      if nUseCount == nMaxUse then
        ...
        if bCoZoneAdd and IVER_g_nTwVersion == 0 then
          szZoneMsg = "<color=gold>%s、合服后7天是优惠期，等级达到50级的玩家每天还可再多开5个。<color><enter>"
        end
        if nVip == 0 then
          local szMsgPay = ""
          local nIndex = 1
          local szExMsg = ""
          if EventManager.IVER_bOpenFuFaiVip == 1 then
            szMsgPay = string.format("<color=gold>%s、%s%s满%s,当月剩余天数每天都可获得额外多开10个福袋的奖励。<color><enter>", nIndex, IVER_g_szPayMonth, IVER_g_szPayName, IVER_g_szPayLevel1)
            nIndex = nIndex + 1
            szExMsg = "<color=yellow>怎样可以每天多开几个福袋？<color><enter>"
          end
          if szZoneMsg and szZoneMsg ~= "" then
            szMsgPay = szMsgPay .. string.format(szZoneMsg, nIndex)
            szExMsg = "<color=yellow>怎样可以每天多开几个福袋？<color><enter>"
          end
          szMsg = string.format("这是您使用的第<color=yellow>%s个<color>福袋，已不能获得丰厚奖励，继续使用将<color=yellow>只会获得100绑定银两<color>。<enter>您还有<color=yellow>10次<color>获得<color=yellow>高额绑金绑银<color>的机会尚未使用，充值<color=yellow>15元<color>即可激活。（该福利激活后至当月月底每日均可享受）", nUseCount + 1)
        else
          szMsg = string.format("这是您使用的第<color=yellow>%s个<color>福袋，已不能获得丰厚奖励，建议留到明天再使用。继续使用将只会获得100绑定银两，您是否继续使用？", nUseCount + 1)
        end
        ...
        if nVip == 0 and EventManager.IVER_bOpenFuFaiVip == 1 then
          table.insert(tbOpt, 1, { "马上充值15元激活该福利", self.OnOpenOnlinePay, self })
          table.insert(tbOpt, 1, { "不激活，获得100绑定银两", self.SureUse, self, nItemId, 1 })
        else
          table.insert(tbOpt, 1, { "继续打开福袋获得100绑定银两", self.SureUse, self, nItemId, 1 })
        end
        Dialog:Say(szMsg, tbOpt)
        return 0
      end
    ...
    pPlayer.Msg(string.format("这是您今天使用的第<color=yellow>%s个<color>黄金福袋，恭喜您获得<color=yellow>100<color>绑定银两。每天只有前<color=yellow>%s个<color>黄金福袋可以获得丰厚奖励，祝您游戏愉快。", nUseCount + 1, nMaxUse))
...
function tbFuDaiItem:GetItem(pPlayer, tbitem, nFlag)
  ...
  local szFriendMsg = ""
  local szMsg = "您打开黄金福袋获得了"
  if tbitem.nAnnouce == 1 then
    szMsg = string.format("%s打开黄金福袋获得了", pPlayer.szName)
  end
  if tbitem.nBaseExp ~= 0 then
    local nAddExp = pPlayer.GetBaseAwardExp() * tbitem.nBaseExp
    szMsg = string.format("%s<color=yellow>%s<color>点经验，", szMsg, nAddExp)
    pPlayer.AddExp(nAddExp)
  end

  if tbitem.nMoney ~= 0 then
    szMsg = string.format("%s<color=yellow>%s<color>银两，", szMsg, tbitem.nMoney)
    szFriendMsg = string.format("<color=yellow>%s<color>银两", tbitem.nMoney)
  ...
  if tbitem.nBindMoney ~= 0 then
    szMsg = string.format("%s<color=yellow>%s<color>绑定银两，", szMsg, tbitem.nBindMoney)
    szFriendMsg = string.format("<color=yellow>%s<color>绑定银两", tbitem.nBindMoney)
  ...
  if tbitem.nCoin ~= 0 then
    ...
    szMsg = string.format("%s<color=yellow>%s<color>绑定%s，", szMsg, tbitem.nCoin, IVER_g_szCoinName)
    szFriendMsg = string.format("<color=yellow>%s<color>绑定%s", tbitem.nCoin, IVER_g_szCoinName)
    KStatLog.ModifyAdd("bindcoin", "[产出]黄金福袋开出", "总量", tbitem.nCoin)
  end
  if tbitem.nAnnouce == 1 then
    szMsg = string.format("%s真是鸿运当头呀！", szMsg)
  else
    szMsg = string.format("%s运气不错呀！", szMsg)
  end
  if tbitem.nAnnouce == 1 then
    KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg)
    --公告
    if szFriendMsg ~= "" then
      pPlayer.SendMsgToFriend("您的好友[" .. pPlayer.szName .. "]打开黄金福袋获得了" .. szFriendMsg .. "。")
      Player:SendMsgToKinOrTong(pPlayer, "打开黄金福袋获得了" .. szFriendMsg .. "。", 1)
    end
  end

  if nFlag == 2 then
    pPlayer.SetTask(self.TASK_GROUP_ID, self.TASK_COUNT_EX, pPlayer.GetTask(self.TASK_GROUP_ID, self.TASK_COUNT_EX) - 1)
    pPlayer.Msg(string.format("你使用了福袋开启赠送机会，你还剩余<color=yellow>%s<color>次开启福袋赠送机会", pPlayer.GetTask(self.TASK_GROUP_ID, self.TASK_COUNT_EX)))
  end
  ...
  local szMsg2 = string.format("<color=yellow>%s<color>您今天共开启了<color=yellow>%s个<color>黄金福袋，%s每天开启前<color=yellow>%s个<color>均有丰厚的奖励！", szMsg, nCount, szMsgPay, nTotalCount)
