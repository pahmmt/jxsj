...
--精气散ID,对应增加精力值
tbItem.tbUseItem = {
  [1] = { TASK_GROUPID = 2024, TASK_ID1 = 1, TASK_ID2 = 2, TASK_IDEX = 20, nUseMax = 5, nAddPoint = 500, szTypeName = "小精气散" },
  [2] = { TASK_GROUPID = 2024, TASK_ID1 = 5, TASK_ID2 = 6, nUseMax = 5, nAddPoint = 1000, szTypeName = "中精气散" },
  [3] = { TASK_GROUPID = 2024, TASK_ID1 = 13, TASK_ID2 = 14, nUseMax = -1, nAddPoint = 1500, szTypeName = "大精气散" },
}
...
function tbItem:OnUse()
  ...
  if me.GetTask(tbAddMKP.TASK_GROUPID, tbAddMKP.TASK_ID2) >= nOrgUseMax and nOrgUseMax >= 0 then
    if tbAddMKP.TASK_IDEX and me.GetTask(tbAddMKP.TASK_GROUPID, tbAddMKP.TASK_IDEX) > 0 then
      ...
      me.Msg(string.format("每个玩家每天只能使用<color=yellow>%s个<color>%s。", nOrgUseMax, tbAddMKP.szTypeName))
      return 0
    end
  end
  ...
  local szMsg = string.format("使用<color=yellow>%s<color>，获得<color=yellow>%s点<color>精力。\n", tbAddMKP.szTypeName, tbAddMKP.nAddPoint)
  me.Msg(szMsg)
  szMsg = ""
