...
function tbXiang:OnUse()
  ...
  local tbOpt = {
    { "取出药", self.OnUseGet, self, it.dwId },
    { "放回药", self.OnUsePut, self, it.dwId },
    { "我再想想" },
  }

  Dialog:Say("你打开了药箱，想要做什么呢？", tbOpt)
  return 0
end
...
function tbXiang:OnUseGet(nItemId)
  ...
  local nFreeItemCount = me.CalcFreeItemCountInBags(nGenre, nDetail, nParticular, nLevel, nSeries, KItem.IsItemBindByBindType(tbBaseProp.nBindType))
  ...
  GeneralProcess:StartProcess("开启" .. pItem.szName .. "...", 10 * Env.GAME_FPS, { self.OnUseAskCount, self, nMaxTakeOutCount }, nil, tbBreakEvent)
  return 0
end

function tbXiang:OnUseAskCount(nMaxTakeOutCount)
  Dialog:AskNumber("请输入取出的数量：", nMaxTakeOutCount, self.OnUseTakeOut, self)
end
...
function tbXiang:OnUseTakeOut(nTakeOutCount)
  ...
  local nMaxTakeOutCount = pItem.GetGenInfo(1)
  local nFreeItemCount = me.CalcFreeItemCountInBags(pItem.GetExtParam(PARAM_IDX_GENRE), pItem.GetExtParam(PARAM_IDX_DETAILTYPE), pItem.GetExtParam(PARAM_IDX_PARTICULAR), pItem.GetExtParam(PARAM_IDX_LEVEL), pItem.GetExtParam(PARAM_IDX_SERIES), nElemBind)

  if tbTimeOut then
    if me.CountFreeBagCell() < nTakeOutCount then
      me.Msg(string.format("你的背包不足，需要%s格背包空间。", nTakeOutCount))
      return 0
    end
  end
  if nFreeItemCount < nMaxTakeOutCount then
  ...
  for i = 1, nTakeOutCount do
    ...
    if tbTimeOut then
      tbInfo.bTimeOut = 1
      local szTime = string.format("%02d%02d%02d%02d%02d", tbTimeOut[1], tbTimeOut[2], tbTimeOut[3], tbTimeOut[4], tbTimeOut[5])
  ...
  if szItemName ~= "" then
    me.Msg("您获得了" .. tostring(nTakeOutCount) .. "个" .. szItemName .. "!", "")
  end
end
...
function tbXiang:OnCheckUsePutDel(nItemId, nDelCount)
  ...
  if not nXiangCount or nXiangCount >= self:GetMaxItemCount(pItem) then
    me.Msg("这个药箱已满，无法再放入药品了。")
    return 0
  end

  local nBind = pItem.IsBind()
  local tbFind = me.FindItemInBags(pItem.GetExtParam(PARAM_IDX_GENRE), pItem.GetExtParam(PARAM_IDX_DETAILTYPE), pItem.GetExtParam(PARAM_IDX_PARTICULAR), pItem.GetExtParam(PARAM_IDX_LEVEL), pItem.GetExtParam(PARAM_IDX_SERIES))
  local nBagCount = 0
  for _, tbItem in pairs(tbFind) do
    ...
    if pItem1.IsBind() == nBind and nType1 == nType and ((nTime > 0 and math.abs(nTime1 - nTime) <= 60) or nTime == nTime1) then
...
function tbXiang:OnUsePut(nItemId)
  ...
  Dialog:AskNumber("请输入放回的数量：", nMaxPutCount, self.OnUsePutExec, self, nItemId)
end
