...
function tbItem:OnUse()
  ...
  for i = 0, me.nPartnerCount - 1 do
    ...
    if pPartner.GetValue(Partner.emKPARTNERATTRIBTYPE_LEVEL) < Partner.MAXLEVEL and pPartner.GetValue(Partner.emKPARTNERATTRIBTYPE_EXP) < Partner:GetMaxStoreExp(pPartner) then
...
function tbItem:OnSelectPartner(nItemId, nPartnerIndex)
  ...
  local szMsg = string.format("您的同伴%s增加了%d点经验。您今天共计给同伴使用了<color=yellow>%d<color>本同伴经验书。您每天能给同伴使用<color=yellow>%s<color>本，前<color=yellow>%s<color>本可获得额外经验。", pPartner.szName, nAddedExp, nTimes + 1, nCount, self.nCountOtherExp)
  me.Msg(szMsg)
