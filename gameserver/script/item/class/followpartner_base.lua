...
function tbItem:OnUse()
  ...
  if nValid == 1 and nType == nMyType and nNpcId == nMyNpcId and nState == 1 then
    ...
    table.insert(tbOpt, 1, { "收起宠物", self.CallBackPartner, self, it.dwId, nSkillId, nSkillLevel })
  end
  if nValid ~= 1 then
    table.insert(tbOpt, 1, { "召唤宠物", self.CallPartner, self, it.dwId, nNpcId, nType, nChangeColor, nSkillId, nSkillLevel })
    nState = 2
  elseif nMyType <= 0 then
    table.insert(tbOpt, 1, { "召唤宠物", self.CallPartner, self, it.dwId, nNpcId, nType, nChangeColor, nSkillId, nSkillLevel })
  end
  if tbType[5] > 0 and it.GetGenInfo(1) >= tbType[6] and it.GetGenInfo(3) ~= nDate then
    table.insert(tbOpt, 1, { "<color=green>领取奖励<color>", self.GetAward, self, it.dwId, tbType })
  end
  local szMsg = ""
  if tbType[5] ~= 2 then
  ...
  if it.GetGenInfo(3) == nDate then
    szMsg = szMsg .. "\n\n<color=green>今天已经领取过奖励了<color>"
  end
  if nState <= 0 then
    szMsg = "已经有宠物召唤出来了，如果需要召唤请先收回宠物。"
  end
...
function tbItem:GetAward(nItemId, tbType)
  ...
  elseif tbType[5] == 5 and tbType[17] ~= "" then
    ...
    if me.CountFreeBagCell() < nNeedBag then
      Dialog:Say(string.format("您的背包空间不足%s格。", nNeedBag))
      return
    end
    if not tbGDPL[6] then
...
function tbItem:CallPartner(nItemId, nNpcId, nType, nChangeColor, nSkillId, nSkillLevel, bProcesss)
  ...
  if me.nFightState == 1 then
    if not bProcesss then
      ...
      GeneralProcess:StartProcess("召唤宠物...", 5 * Env.GAME_FPS, { self.CallPartner, self, nItemId, nNpcId, nType, nChangeColor, nSkillId, nSkillLevel, 1 }, nil, tbEvent)
      return
    end
  end
  ...
  if pPartner then
    ...
    if nSkillId > 0 and nSkillLevel > 0 then
      me.AddSkillState(nSkillId, nSkillLevel, 2, nTime * 18, 1, 0, 1)
    end
  end
end
