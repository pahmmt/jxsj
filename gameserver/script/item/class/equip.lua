...
-- 潜能名称列表
tbEquip.POTENTIAL_NAME = { "力量", "身法", "外功", "内功" }
...
function tbEquip:DialogChange(pEquip)
  ...
  table.insert(tbOpt, { "不需要转换" })
  Dialog:Say("当前装备不可使用，但可以转化为以下合适门派的相同装备，<color=green>装备的一切属性都将等价转换，不会有任何损失。<color>", tbOpt)
end
...
function tbEquip:CalcEnhanceAttrib(nState) -- 计算装备强化/玄晶拆卸后的魔法属性值
  ...
  if (it.nEnhTimes >= Item:CalcMaxEnhanceTimes(it)) or (nNewEnhanceTimes and nNewEnhanceTimes > Item:CalcMaxEnhanceTimes(it)) then
    return
  end
  ...
  if nState == Item.TIPS_TRANSFER then
    pTemp = KItem.CreateTempItem(it.nGenre, it.nDetail, it.nParticular, it.nLevel, it.nSeries, (Item:GetEquipType(it) ~= nType or nNewEnhanceTimes <= it.nEnhTimes) and it.nEnhTimes or nNewEnhanceTimes, it.nLucky, it.GetGenInfo(), 0, it.dwRandSeed, it.nIndex, (Item:GetEquipType(it) ~= nType or nNewEnhanceTimes <= it.nEnhTimes) and it.nStrengthen or nNewStrengthen, it.nCount, it.nCurDur, it.nMaxDur, it.GetRandomInfo(), tbTaskData)
  else
    pTemp = KItem.CreateTempItem(it.nGenre, it.nDetail, it.nParticular, it.nLevel, it.nSeries, it.nEnhTimes + 1, it.nLucky, it.GetGenInfo(), 0, it.dwRandSeed, it.nIndex, it.nStrengthen, it.nCount, it.nCurDur, it.nMaxDur, it.GetRandomInfo(), tbTaskData)
  end
  ...
  if it.IsWhite() ~= 1 then
    ...
    if (nState == Item.TIPS_ENHANCE) and nEnhNameColor then -- 装备强化预览
      ...
      if (nOpen == 0 and nEnhTimes <= Item:CalcMaxEnhanceTimes(it)) or (nOpen == 1 and nEnhTimes < Item.nEnhTimesLimitOpen) then
        szTip = szTip .. string.format(" <color=gold>→<color> <color=0x%x>+%d<color>", nEnhNameColor, nEnhTimes)
      end
    elseif nState == Item.TIPS_STRENGTHEN and Item:CheckStrengthenEquip(it) == 1 then
      szTip = szTip .. string.format(" <color=gold>→<color> <color=0x%x>+%d 改<color>", it.nNameColor, it.nEnhTimes)
    elseif nState == Item.TIPS_TRANSFER and nEnhNameColor and nNewEnhanceTimes and nNewStrengthen and nType then
      if nNewEnhanceTimes <= Item:CalcMaxEnhanceTimes(it) and (nNewEnhanceTimes > it.nEnhTimes or nNewStrengthen > it.nStrengthen) and nType == Item:GetEquipType(it) then
        --print(nState,nNewEnhanceTimes,nNewStrengthen,nType)
        szTip = szTip .. string.format(" <color=gold>→<color> <color=0x%x>+%d%s<color>", nEnhNameColor, nNewEnhanceTimes, nNewStrengthen > 0 and " 改" or "")
      end
    end
  end
...
function tbEquip:Tip_Warning() -- 获得Tip字符串：对于复制装备的警告信息
  if it.IsDuped() == 1 then
    return "<color=red>！！此物品为复制品，将会被系统没收！！\n谨防上当，请勿交易<color>\n"
  end
...
function tbEquip:Tip_Durability()
  ...
  szTip = szTip .. "耐 久 度：" .. math.ceil(it.nCurDur / 10)
  if bColor == 1 then
    szTip = szTip .. "<color>"
  end
...
function tbEquip:Tip_FixSeries()
  local TIP_SERISE = {
    "<color=yellow>推荐金系<color>",
    "<color=green>推荐木系<color>",
    "<color=blue>推荐水系<color>",
    "<color=red>推荐火系<color>",
    "<color=gray>推荐土系<color>",
  }
  local szTip = "（"
  ...
  if (nState == Item.TIPS_PREVIEW) or (nState == Item.TIPS_GOODS) then -- 属性预览状态
    ...
    if it.nEquipPos == Item.EQUIPPOS_WEAPON then
      ...
      if tbMA.weapondamagemin_v.tbValue then
        szTip = szTip .. string.format("\n外功攻击力增加：%s到%s点", tbMA.weapondamagemin_v.tbValue[1], tbMA.weapondamagemax_v.tbValue[1])
        szTip = szTip .. string.format("\n内功攻击力增加：%s到%s点", tbMA.weaponmagicmin_v.tbValue[1], tbMA.weaponmagicmax_v.tbValue[1])
      end
    end
  end
    ...
    if (not nPos1) or not nPos2 then -- 不参与五行激活的装备
      ...
      if nTotal > 0 then -- 存在暗属性
        if nSeries < 0 then -- 五行不确定的情况
          szTip = szTip .. string.format("\n\n<color=blue>五行激活（0/%d）", nTotal)
          szTip = szTip .. string.format("<color=gray>%s（？） %s（？） 角色（？） <color>", Item.EQUIPPOS_NAME[nPos1], Item.EQUIPPOS_NAME[nPos2]) -- 总是灰的
          szTip = szTip .. "<color>"
          ...
          if me.nSeries ~= nAccSeries then
            szTip = szTip .. string.format("<color=gray>角色（%s）<color>", szAccSeries)
          else
            szTip = szTip .. string.format("<color=white>角色（%s）<color>", szAccSeries)
          end
  ...
  if nState == Item.TIPS_PREVIEW then -- 属性预览状态，显示魔法属性范围
    ...
    if tbBaseProp then
      ...
      if nCount > 0 then -- 具有强化属性
        ...
        szTip = string.format("%s\n<color=blue>强化（%d/%d）<color>", szTip, it.nEnhTimes, nCouldEnhanceTimes)
        for i, tbMA in ipairs(tbEnhMA) do
            ...
            if nOpen == 0 or (nOpen == 1 and tbMA.nTimes < Item.nEnhTimesLimitOpen) then
              if tbMASS[i].bActive ~= 1 then -- 未激活显示为灰色
                if nScriptVersion ~= 1 then
                  szTip = szTip .. "\n<color=gray>" .. Lib:StrFillL(string.format("（强化+%d）", tbMA.nTimes), 12) .. szDesc .. "<color>"
                elseif tbMA.nTimes <= Item:CalcMaxEnhanceTimes(it) then
                  szTip = szTip .. "\n<color=gray>" .. Lib:StrFillL(string.format("(+ %d)", tbMA.nTimes), 12) .. szDesc .. "<color>"
                end
              else
                szTip = szTip .. "\n" .. Lib:StrFillL(string.format("（强化+%d）", tbMA.nTimes), 12) .. szDesc
              end
            end
          end
        end
          ...
          if (tbMA.bActive ~= 1) and (tbMA.bVisible == 1) then
            if nOpen == 0 or (nOpen == 1 and tbMA.nTimes < Item.nEnhTimesLimitOpen) then
              if tbEnhEnhMASS and (tbEnhEnhMASS[i].bActive == 1) then
                szTip = szTip .. "\n<color=gold>" .. Lib:StrFillL(string.format("（强化+%d）", tbMA.nTimes), 12) .. szDesc .. "<color>"
              else
                if nScriptVersion ~= 1 then
                  szTip = szTip .. "\n<color=gray>" .. Lib:StrFillL(string.format("（强化+%d）", tbMA.nTimes), 12) .. szDesc .. "<color>"
                elseif tbMA.nTimes <= Item:CalcMaxEnhanceTimes(it) then
...
function tbEquip:Tip_StrAttrib(nState)
  ...
  --Vn--
  if (nState ~= Item.TIPS_TRANSFER or (nNewStrengthen and nNewStrengthen == 0) or Item:GetEquipType(it) ~= nType) and (it.IsWhite() == 1 or it.nEnhTimes < 14 or it.nEnhTimes > 15) then
    return "" -- 白色装备不显示强化激活属性
  end
  ...
  for i = 1, #tbMASS do
    ...
    if (szDesc ~= "") and (tbMA.bVisible == 1) then
      ...
      if (nState == Item.TIPS_STRENGTHEN and Item:CheckStrengthenEquip(it) == 1) or (nNewStrengthen and nNewStrengthen > 0 and Item:GetEquipType(it) == nType) then
        ...
        szDesc = "\n" .. szColor .. Lib:StrFillL(string.format("（强化+%d 改）", tbMA.nTimes), 12) .. szDesc .. "<color>"
      elseif (tbMA.bActive == 1) and (tbMA.bVisible == 1) then
        szDesc = "\n" .. Lib:StrFillL(string.format("（强化+%d 改）", tbMA.nTimes), 12) .. szDesc
      else
        if nScriptVersion ~= 1 then
          szDesc = "\n<color=gray>" .. Lib:StrFillL(string.format("（强化+%d 改）", tbMA.nTimes), 12) .. szDesc .. "<color>"
        elseif tbMA.nTimes <= Item:CalcMaxEnhanceTimes(it) then
          szDesc = "\n<color=gray>" .. Lib:StrFillL(string.format("(+ %d 改)", tbMA.nTimes), 12) .. szDesc .. "<color>"
        end
      end
    end
  ...
  for i = 0, #tbMass do
    ...
    if tbMagic.szName ~= "" then
      ...
      if tbMagic.szName == "disguise_part_base" then
        szMagicDesc = string.format("\n可以改变%s的外观", Item.szResPart[tbMagic.tbValue[2]])
      else
...
function tbEquip:Tip_RepairInfo(nState) -- 获得Tip字符串：修理信息
  if nState == Item.TIPS_CREPAIR then
    ...
    if nMoney >= 0 then
      local szMoney = "免费"
      if nMoney > 0 then
        szMoney = Item:FormatMoney(nMoney) .. "两"
      end
      return "\n<color=yellow>普修费用：" .. szMoney .. "<color>"
    end
...
function tbEquip:GetHoleDesc(nHoleType, nValue)
  ...
  if not tbStone then
    ...
    szDesc = szDesc .. "<color=gray>未镶嵌宝石" .. "（0/" .. nHoleLevel .. "）<color>"
  else
    ...
    for i, tbSingleAttrib in pairs(tbAttrib) do
      if (tbSingleAttrib.szName == "lifemax_permillage") or (tbSingleAttrib.szName == "manamax_permillage") or (tbSingleAttrib.szName == "lifemax_v") or (tbSingleAttrib.szName == "manamax_v") then --这个写法太挫了
        if tbSingleAttrib.szName == "lifemax_permillage" or tbSingleAttrib.szName == "manamax_permillage" then --血量千分比
          local szAdd = tbSingleAttrib.tbValue[1] >= 0 and "增加" or "减少"
          szDesc = szDesc .. "生命和内力最大值：" .. szAdd .. (tbSingleAttrib.tbValue[1] / 10) .. "%" .. "（" .. tbGDPL[4] .. "/" .. nHoleLevel .. "）"
          return szDesc .. "\n"
        elseif tbSingleAttrib.szName == "lifemax_v" or tbSingleAttrib.szName == "manamax_v" then --血量点数
          local szAdd = tbSingleAttrib.tbValue[1] >= 0 and "增加" or "减少"
          szDesc = szDesc .. "生命和内力最大值：" .. szAdd .. tbSingleAttrib.tbValue[1] .. "点" .. "（" .. tbGDPL[4] .. "/" .. nHoleLevel .. "）"
          return szDesc .. "\n"
        end
      else
        local szLvDesc = tbSingleAttrib.szName == "skilllevel_added" and "" or "（" .. tbGDPL[4] .. "/" .. nHoleLevel .. "）"
        szDesc = szDesc .. FightSkill:GetMagicDesc(tbSingleAttrib.szName, tbSingleAttrib.tbValue, nil, 1, nil, 0) .. szLvDesc
      end
    end
  end
  return szDesc .. "\n"
