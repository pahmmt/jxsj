...
tbItemSongjinZhaoshu.tbCamp = { "<color=orange>战场报名点_宋(%s%s)<color>", "<color=purple>战场报名点_金(%s%s)<color>" } -- 玩家的选项
tbItemSongjinZhaoshu.tbBattleSeq = { "一", "二", "三" }
...
function tbItemSongjinZhaoshu:SelectCamp(nItemId, nLevelId, nWithOutItem)
  ...
  tbOpt[#tbOpt + 1] = { "我再考虑一下" } -- 退出
  local szMsgFmt = "宋金大战一触即发，英雄豪杰当披坚执锐，逐鹿沙场。\n"
  local nFlag = 0
  ...
  if NewBattle:OnlyBattleOpen() > 0 then
    for i = 1, 3 do
      ...
      if n == 1 then
        szMsgFmt = szMsgFmt .. string.format("<color=yellow>宋金战场之冰火天堑%s<color>已开放。\n", Lib:Transfer4LenDigit2CnNum(i))
        szMsgFmt = szMsgFmt .. string.format("<color=orange>宋军%d人<color>，<color=purple>金军%d人<color>。\n", nS, nJ)
      end
    end
    szMsgFmt = szMsgFmt .. "通过下面<color=green>任意<color>报名点均可进入。\n"
  else
    for i = 1, nOpenCount do
      ...
      if nSongnCampNum < 0 or not Battle.szLastMapName then -- 未开始报名时
        szMsgFmt = szMsgFmt .. string.format("当前<color=yellow>%s%s<color>报名尚未开始。\n", szBattleMapName, self.tbBattleSeq[i])
      else -- 报名已开始
        szMsgFmt = szMsgFmt .. string.format("当前<color=yellow>%s%s<color>报名已经开始。<color=orange>宋军人数为：%d<color>；<color=purple>金军人数为：%d<color>。\n", szBattleMapName, self.tbBattleSeq[i], nSongnCampNum, nJinCampNum)
        nFlag = 1
      end
    end
  end
  ...
  if nFlag == 1 then
    szMsgFmt = szMsgFmt .. "如双方人数相差太多，人数较多的一方会暂时无法报名。"
  end
  szMsgFmt = szMsgFmt .. "你需要到哪个报名点？"
...
function tbItemSongjinZhaoshu:TransPlayer(nItemId, nMapId, nWithOutItem, bNewBattle)
  ...
  GeneralProcess:StartProcess("正在传送去战场报名点...", self.nTime * Env.GAME_FPS, { self.TransPlayerSucessFul, self, nItemId, nMapId, pPlayer.nId, nWithOutItem, bNewBattle }, nil, tbEvent) -- 在战斗状态下需要nTime秒的延时
end
