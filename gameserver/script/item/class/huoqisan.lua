...
tbItem.tbUseItem = {
  --等级 =
  [1] = { TASK_GROUPID = 2024, TASK_ID1 = 3, TASK_ID2 = 4, TASK_IDEX = 21, nUseMax = 5, nAddPoint = 500, szTypeName = "小活气散" },
  [2] = { TASK_GROUPID = 2024, TASK_ID1 = 7, TASK_ID2 = 8, nUseMax = 5, nAddPoint = 1000, szTypeName = "中活气散" },
  [3] = { TASK_GROUPID = 2024, TASK_ID1 = 15, TASK_ID2 = 16, nUseMax = -1, nAddPoint = 1500, szTypeName = "大活气散" },
}
...
function tbItem:OnUse()
  ...
  if me.GetTask(tbAddGTP.TASK_GROUPID, tbAddGTP.TASK_ID2) >= nOrgUseMax and nOrgUseMax >= 0 then
    if tbAddGTP.TASK_IDEX and me.GetTask(tbAddGTP.TASK_GROUPID, tbAddGTP.TASK_IDEX) > 0 then
      ...
      me.Msg(string.format("每个玩家每天只能使用<color=yellow>%s个<color>%s。", nOrgUseMax, tbAddGTP.szTypeName))
      return 0
    end
  end
  ...
  local szMsg = string.format("使用<color=yellow>%s<color>，获得<color=yellow>%s点<color>活力。\n", tbAddGTP.szTypeName, tbAddGTP.nAddPoint)
  me.Msg(szMsg)
  szMsg = ""
  for nId, tbItem in ipairs(self.tbUseItem) do
    ...
    if nOrgMax < 0 then
      szMsg = szMsg .. string.format("今天还可以使用<color=yellow>%s<color>无限个\n", tbItem.szTypeName)
    else
      --local szCount = "零"
      ...
      local szExMsg = ""
      if nExCount > 0 then
        szExMsg = string.format("(额外<color=yellow>%s<color>个)", nExCount)
      end
      local nUseCount = nOrgMax - nMinCount
      if nUseCount < 0 then
        nUseCount = 0
      end
      szMsg = szMsg .. string.format("今天还可以使用<color=yellow>%s%s<color>个%s\n", tbItem.szTypeName, nUseCount, szExMsg)
    end
  end
  me.Msg(szMsg)
