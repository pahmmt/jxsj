...
function tbStone:Upgrade(dwGemId, dwId, nHolePos)
  ...
  if pOperate.IsEquip() == 1 then
    ...
    if nRet == -1 then
      me.Msg("该装备没有打孔，不能进行宝石升级操作！")
      return 2
    elseif nRet == 0 then
      me.Msg("该装备指定孔位置上没有镶嵌宝石，不能进行宝石升级操作！")
      return 2
    end
    ...
    if pGemStone.Delete(me, 150) ~= 1 then
      Dbg:WriteLog("Stone", "Upgrade", me.szAccount, me.szName, string.format("删除原石(%d_%d_%d_%d)失败！", unpack(tbGemGDPL)))
      return 0
    end

    if pOperate.EnchaseStone(tbGDPL[1], tbGDPL[2], tbGDPL[3], tbGDPL[4] + 1, nHolePos) ~= 1 then
      Dbg:WriteLog("Stone", "Upgrade", me.szAccount, me.szName, string.format("删除原石(%d_%d_%d_%d)成功，装备%s(%d_%d_%d_%d)第%d个孔内宝石升级失败！", tbGemGDPL[1], tbGemGDPL[2], tbGemGDPL[3], tbGemGDPL[4], pOperate.szName, tbOperateGDPL[1], tbOperateGDPL[2], tbOperateGDPL[3], tbOperateGDPL[4], nHolePos))
      return 0
    end
  ...
  elseif pOperate.GetStoneType() == Item.STONE_PRODUCT then
    ...
    if pOperate.Delete(me, 150) ~= 1 then
      Dbg:WriteLog("Stone", "Upgrade", me.szAccount, me.szName, string.format("删除原石(%d_%d_%d_%d)成功，删除宝石(%d_%d_%d_%d)失败！", tbGemGDPL[1], tbGemGDPL[2], tbGemGDPL[3], tbGemGDPL[4], unpack(tbOperateGDPL)))
      return 0
    end

    local pItem = me.AddItemEx(tbOperateGDPL[1], tbOperateGDPL[2], tbOperateGDPL[3], tbGemGDPL[4], { bForceBind = bForceBind })
    if not pItem then
      Dbg:WriteLog("Stone", "Upgrade", me.szAccount, me.szName, string.format("删除原石(%d_%d_%d_%d)成功，删除宝石(%d_%d_%d_%d)成功，添加新宝石失败！", tbGemGDPL[1], tbGemGDPL[2], tbGemGDPL[3], tbGemGDPL[4], unpack(tbOperateGDPL)))
      return 0
    end
...
function tbStone:Exchange(nSelColor, nSelInfo, dwStoneId1, dwStoneId2, dwStoneId3)
  ...
  -- 判断两次就够了
  if self:IsExchangeFeatureMatch(pStone1, pStone2) ~= 1 or self:IsExchangeFeatureMatch(pStone2, pStone3) ~= 1 then
    me.Msg("请放入特征相同的原石！")
    return 0
  end
  ...
  local szInfo = string.format("%d_%d_%d_%d,%d_%d_%d_%d,%d_%d_%d_%d,", pStone1.nGenre, pStone1.nDetail, pStone1.nParticular, pStone1.nLevel, pStone2.nGenre, pStone2.nDetail, pStone2.nParticular, pStone2.nLevel, pStone3.nGenre, pStone3.nDetail, pStone3.nParticular, pStone3.nLevel)

  -- 删除原来的原石
  if me.DelItem(pStone1) ~= 1 then
    Dbg:WriteLog("Stone", "Exchange", me.szAccount, me.szName, string.format("删除原石1(%d_%d_%d_%d)失败！", tbOrgGem1[1], tbOrgGem1[2], tbOrgGem1[3], tbOrgGem1[4]))
    return 0
  end

  if me.DelItem(pStone2) ~= 1 then
    Dbg:WriteLog("Stone", "Exchange", me.szAccount, me.szName, string.format("删除原石1(%d_%d_%d_%d)成功，删除原石2(%d_%d_%d_%d)失败！", tbOrgGem1[1], tbOrgGem1[2], tbOrgGem1[3], tbOrgGem1[4], tbOrgGem2[1], tbOrgGem2[2], tbOrgGem2[3], tbOrgGem2[4]))
    return 0
  end

  if me.DelItem(pStone3) ~= 1 then
    Dbg:WriteLog("Stone", "Exchange", me.szAccount, me.szName, string.format("删除原石1(%d_%d_%d_%d)成功，删除原石2(%d_%d_%d_%d)成功, 删除原石3(%d_%d_%d_%d)失败！", tbOrgGem1[1], tbOrgGem1[2], tbOrgGem1[3], tbOrgGem1[4], tbOrgGem2[1], tbOrgGem2[2], tbOrgGem2[3], tbOrgGem2[4], tbOrgGem3[1], tbOrgGem3[2], tbOrgGem3[3], tbOrgGem3[4]))
    return 0
  end
  ...
  if not pItem then
    Dbg:WriteLog("Stone", "Exchange", me.szAccount, me.szName, string.format("删除原石1(%d_%d_%d_%d)，原石2(%d_%d_%d_%d)，原石3(%d_%d_%d_%d)成功，添加新宝石(%d_%d_%d_%d)失败！", tbOrgGem1[1], tbOrgGem1[2], tbOrgGem1[3], tbOrgGem1[4], tbOrgGem2[1], tbOrgGem2[2], tbOrgGem2[3], tbOrgGem2[4], tbOrgGem3[1], tbOrgGem3[2], tbOrgGem3[3], tbOrgGem3[4], tbSelGDPL[1], tbSelGDPL[2], tbSelGDPL[3], tbSelGDPL[4]))
    return 0
  end
...
function tbStone:BreakUp(dwId)
  ...
  if me.DelItem(pStone, 150) ~= 1 then
    Dbg:WriteLog("Stone", "BreakUp", me.szAccount, me.szName, string.format("删除原石(%d_%d_%d_%d)失败！", unpack(tbGemGDPL)))
    return 0
  end
  ...
  if nMoney < self.BREAKUP_COST_MONEY then
    if me.CostBindMoney(nMoney, Player.emKPAY_BREAKUPSTONE) ~= 1 then
      Dbg:WriteLog("Stone", "BreakUp", me.szAccount, me.szName, string.format("扣除%s绑银失败", nMoney))
      return 0
    end
    if me.CostMoney(self.BREAKUP_COST_MONEY - nMoney, Player.emKPAY_BREAKUPSTONE) ~= 1 then -- 扣除费用
      Dbg:WriteLog("Stone", "BreakUp", me.szAccount, me.szName, string.format("扣除%s银两失败", self.BREAKUP_COST_MONEY - nMoney))
      return 0
    end
  else
    if me.CostBindMoney(self.BREAKUP_COST_MONEY, Player.emKPAY_BREAKUPSTONE) ~= 1 then
      Dbg:WriteLog("Stone", "BreakUp", me.szAccount, me.szName, string.format("扣除%s绑银失败", self.BREAKUP_COST_MONEY))
      return 0
    end
  end
...
function tbStone:ApplyUnBindStone(nPlayerId)
  ...
  if pPlayer.IsAccountLock() == 1 then
    pPlayer.Msg("您的账号处于锁定状态，不可以申请解绑宝石！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以申请解绑宝石！请先解除锁定。")
  ...
  pPlayer.AddSkillState(self.UNBIND_STONE_SKILLID, 1, 2, self.UNBIND_MAX_TIME * Env.GAME_FPS, 1, 0, 1)
...
function tbStone:ApplyBreakUpStone(nPlayerId)
  ...
  if pPlayer.IsAccountLock() == 1 then
    pPlayer.Msg("您的账号处于锁定状态，不可以申请高级原石拆解！请先解除锁定。")
    Partner:SendClientMsg("您的账号处于锁定状态，不可以申请高级原石拆解！请先解除锁定。")
    Account:OpenLockWindow(pPlayer)
    return 0
  end
  ...
  pPlayer.AddSkillState(self.BREAKUP_STONE_SKILLID, 1, 2, self.BREAKUP_MAX_TIME * Env.GAME_FPS, 1, 0, 1)
...
function tbStone:__RandItemGetStone2(nMode)
  local tbGDPL = self:RandomStone2(nMode)
  if not tbGDPL then
    return 0
  end
...
function tbStone:BrodcastMsg(szRes, pStone)
  ...
  local szMsg = "在" .. szRes .. "中获得了<color=yellow>" .. pStone.szName .. "<color>，真是鸿运当头！"
  ...
  if nMsgLevel == 4 then
    KDialog.NewsMsg(1, Env.NEWSMSG_NORMAL, "恭喜" .. me.szName .. szMsg)
  elseif nMsgLevel >= 5 then
    Dialog:GlobalNewsMsg("恭喜" .. me.szName .. szMsg)
  end
end
