...
function tbStone:CalcValueInfo()
  ...
  if it.nDetail == 1 then
    ...
    elseif it.nParticular == 37 then
      szTransIcon = "\\image\\effect\\other\\new_jin1.spr"
    elseif it.nParticular >= 38 then --如果做后续宝石需要修改
      szTransIcon = "\\image\\effect\\other\\new_jin1.spr"
    end
  end
...
function tbStone:Tip_Info(nTipState)
  ...
  local szStoneType = it.GetStoneType() == 1 and "宝石" or "原石"

  local nSpecial = tbStoneAttrib.nSpecial
  local szSpecial = nSpecial == 0 and "普通" or "特殊"

  local bExchange = Item.tbStone:GetStoneExchangeFeature(it)
  local szExchange = (bExchange == 1) and "可兑换其他原石\n" or ""
  ...
  for _, tbSingleAttrib in pairs(tbActualAttrib) do
    if tbSingleAttrib.szName == "skilllevel_added" then
      if me.nRouteId == 0 then
        ...
        szMsg = "当前门派路线<color=gold>" .. Item.tbStone.tbSkillStoneDesc[tbSingleAttrib.tbValue[1]][1] .. "<color>技能：增加1级\n"
        if Item.tbStone.tbSkillStoneDesc[tbSingleAttrib.tbValue[1]][2] then
          szMsg = szMsg .. "或针古墓路线<color=gold>" .. Item.tbStone.tbSkillStoneDesc[tbSingleAttrib.tbValue[1]][2] .. "<color>：增加1级\n"
        end
      end
    end
    szTip = szTip .. FightSkill:GetMagicDesc(tbSingleAttrib.szName, tbSingleAttrib.tbValue, nil, 1) .. "\n"
  end
  szTip = "<color=greenyellow>" .. szTip .. szMsg .. "<color>"
  return szTip
end
...
function tbStone:Tip_UpgradeLimit(nTipState)
  ...
  if it.nDetail == 2 then
    szStCanUp = szStCanUp .. "\n用于对<color=gold>" .. Item.tbStone.tbStonePreName[it.nLevel - 1] .. Item.tbStone.tbStoneName[it.nParticular] .. "（" .. (it.nLevel - 1) .. "级）<color>进行升级\n"
    if it.nLevel >= 3 then
      szStCanUp = szStCanUp .. "\n可以拆解为3个<color=gold>" .. Item.tbStone.tbStonePreName[it.nLevel - 1] .. Item.tbStone.tbStoneName[it.nParticular] .. "·原石（" .. (it.nLevel - 1) .. "级）<color>\n"
    end
  end
...
function tbStone:Tip_EquipQuaLimit(nTipState)
  ...
  if Item.tbEquipHoleLevel[7] <= it.nLevel then -- 质量7表示黄金装备
    szTip = "只能镶嵌在<color=gold>史诗<color>及以上品质的装备上"
  elseif Item.tbEquipHoleLevel[6] <= it.nLevel then -- 质量6表示白银装备
    szTip = "只能镶嵌在<color=gold>卓越<color>及以上品质的装备上"
  end
...
function tbStone:GetStoneExchangeFeature(pStone)
  if not pStone or type(pStone) ~= "userdata" or pStone.GetStoneType() ~= Item.STONE_GEM then
    return 0
  end
...
function tbStone:CanUpgrade(varStone, varGem)
  ...
  if tbStoneInfo[2] ~= Item.STONE_PRODUCT or tbGemInfo[2] ~= Item.STONE_GEM then
    return 0, "只能用与该宝石属性相同，比该宝石等级高1级的原石，才能进行升级。"
  end
...
function tbStone:GetBreakUpList(varStone)
  ...
  if tbGDPL[1] ~= Item.STONEITEM or tbGDPL[2] ~= Item.STONE_GEM or tbGDPL[4] < self.STONE_BREAKUP_LEVEL_LIMIT then
    return
  end
...
function tbStone:CanBreakUp(varStone, pPlayer)
  ...
  if pStone.nLevel >= self.STONE_BREAKUP_LEVEL_APPLY then
    ...
    if nState == 0 then
      pPlayer.Msg(self.STONE_BREAKUP_LEVEL_APPLY .. "级以上的原石拆解需要到宝石工匠那里申请！")
      return 0
    elseif nState == 1 then
      pPlayer.Msg("尚未到可操作时间，请耐心等待！")
      return 0
    end
  end
  ...
  if nMoney < self.BREAKUP_COST_MONEY then
    pPlayer.Msg("您的银两不够，拆解原石需要花费" .. Item:FormatMoney(self.BREAKUP_COST_MONEY) .. "银两（优先扣除绑定银两）。")
    return 0
  end

  if pPlayer.CountFreeBagCell() < tbRes.nCount then
    pPlayer.Msg("背包空间不足，请至少空余" .. tbRes.nCount .. "个空间！")
    return 0
  end
...
function tbStone:GetFightPower(varStone)
  ...
  elseif tbStoneProp and tbStoneProp.nStyle == 4 then
